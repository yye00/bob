# Adversarial analysis — bob for HPC

This document captures the honest critique of the "bob generates
...
evidence for bob. Examined for HPC relevance, it tested:
...
code is written.** If you write a spec detailed enough for bob
...
| using-bob-memory | The four pools don't carve up HPC knowledge |
...
contribution, not bob-as-it-stands. Bob is the substrate; the
...
agentic build orchestrator", not "bob is good for HPC."
...
If the claim narrowed to *"bob ports CPU codes to GPU using
...
Bob is not entering an empty market. The differentiator can't
...
half breaks bob's core synchronous loop assumption.
...
- The orchestration loop is synchronous — needs an event-driven persistent state machine that outlives a `bob run` invocation
- `BOB_FEATURE_TIMEOUT_SECONDS=3600` (1-hour wall) is wrong by an order of magnitude or more
