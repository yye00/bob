...
> agentic build orchestrator (bob), enables AI-assisted
...
- The integration is the contribution (not bob alone, not
...
- Same spec given to plain Aider or Cursor without the bob
  skill bundle / registry
- Measure: time to "completed" (some attempts will not complete),
  correctness, performance

### A "bob without HPC skills" baseline

- Same spec given to bob with the existing software-engineering
  skill bundle (no HPC additions)
- Measure: same as above

### The proposed system

- bob + HPC skill bundle + seeded registry + Tier 2 verification
...
- **bob-without-HPC-skills produces "compiles and runs" but at
...
| First demo build complete | bob produces a 2D Euler solver that passes Sod's tube + MMS within 2× perf of AMReX, validated end-to-end without human intervention beyond the spec |
...
HPC-specific skill+registry+verification stack on top of bob,
