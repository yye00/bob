...
The current bob3 verification layer assumes:

- The build runs locally
- Tests are pytest
- Pass/fail is binary
- Wall clock per criterion is bounded by `BOB3_CRITERION_EXEC_TIMEOUT`
  (default 600 s)
...
| L1 — single GPU | One CUDA / HIP GPU | + `compute_sanitizer_clean`, `nsight_metrics`, `convergence_order_at_least`, `conservation_error`, `deterministic_output` | Full bob3-for-HPC validation on a single accelerator |
| L2 — cluster | Slurm / PBS access | + scaling criteria, multi-node correctness | Tier 3 (deferred) |

The verifier reads `BOB3_HW_TIER` (or auto-detects) and skips
...
Cache lives in `<workspace>/.bob3/verification_cache/`. Hit rate
