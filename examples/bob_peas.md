...
> F-R9-031 addendum #3 (RCCL F004/T3, 2026-07-08 — the counterpart to #2, and a near-miss FALSE-KILL): an in-progress EXTERNAL BUILD is indistinguishable from a mode-C hang on EVERY process/cputime/log signal, so the watchdog MUST recognize it via BUILD-ARTIFACT churn or it will SIGKILL a healthy 10–20min compile. Observed: T3 resumed its WIP and drove a full RCCL build (linked a 273MB librccl.so.1.0, then rebuilt rccl-tests), yet at poll time: (a) `pgrep amdclang|hipcc` sampled 0 — recursive `make` spawns SHORT-LIVED compiler procs (one per file, <1s), so a point sample almost always catches zero mid-build; (b) the subagent cputime was FROZEN because it delegates to `bash→make` and blocks in ep_poll on the child; (c) bob run cputime frozen (waiting on subagent); (d) build.log stale >600s (no tokens stream during compile); (e) the source-write check correctly EXCLUDES build/ (artifact-settling) so compile output didn't register as alive either. ALL alive-signals read false during a legitimate build. Fix: add an ACTIVE-COMPILE liveness check that fires BEFORE the cputime-freeze test — a build artifact (`.o`/`.o.d`/`.so`/`.a`) written under any `*/build/*` dir within the last ~2min ⇒ alive/skip. It is STRICTLY additive (only prevents kills, never causes one) so it cannot mask a real hang (a hung subagent produces no fresh objects). KEY DISTINCTION vs addendum #2: subagent-authored SOURCE-write mtime is NOT a valid liveness veto (the hung agent wrote its last file then froze), but EXTERNAL build-process output (objects under build/, and `pgrep -x make`/cc1plus/ninja/ld.lld) IS — liveness veto must key on external-process evidence, not on files the possibly-since-hung subagent itself wrote.
