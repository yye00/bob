...
When a feature has refinement_attempts >= 2 AND readiness_score < 0.80 AND no
readiness improvement since the last attempt, mark it ``pending_decomposition``
instead of re-executing.  Decomposition produces sub-features; the parent
re-enters ``ready`` only once each sub-feature passes its own gate.

Integration with bob76.evaluator is wired below so the evaluator can verify
this gate's output using the same logic as the sweep orchestrator.
"""
...
import bob76.evaluator as _evaluator  # noqa: F401 — integration wiring
...
def mark_pending_decomposition(
    ...
    *,
    ...
    Sets the feature status to ``pending_decomposition`` so a decomposer
    can split it into smaller sub-features.  When ``db_update`` is provided
    it is called as ``db_update(feature.id, status="pending_decomposition")``
...
__all__ = ["should_decompose", "mark_pending_decomposition"]
