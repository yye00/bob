...
def apply_error_severity_rewrites(
    ...
    if auto_repair:
        for raw_finding in findings:
            # Accept both dicts and namedtuple/dataclass objects (e.g. bob3.linter SmellFinding)
            if hasattr(raw_finding, "_asdict"):
                finding: dict[str, Any] = raw_finding._asdict()
            elif hasattr(raw_finding, "__dict__") and not isinstance(raw_finding, dict):
...
apply_auto_repair = apply_error_severity_rewrites
"""Alias for :func:`apply_error_severity_rewrites` (AC-mandated public name)."""
