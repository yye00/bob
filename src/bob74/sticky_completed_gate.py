...
Public API:
    :func:`check_completion_sticky` — gate check integrating with bob.evaluator
"""
...
def check_completion_sticky(
    ...
    Integrates with bob.evaluator to prevent evaluator FAIL or
...
def evaluator_guard(
    ...
    ``parent_completed`` flag from the bob database.  Called by the
    bob.evaluator before applying a FAIL or cascade-rollback verdict so that
    completed work cannot be un-completed by a re-evaluation.

    Falls back to False (allow demotion) if the bob DB is unavailable.
