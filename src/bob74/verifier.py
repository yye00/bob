"""bob74.verifier — Pattern 9 shell-script integration AC handler (F-R7-594).
...
def demote_shell_script_integration(
    ...
    When an ``integration:`` AC body is a path to an existing, executable
    ``.sh`` or ``.bash`` file, demote the AC to PASS-with-warning.
...
__all__ = ["demote_shell_script_integration"]
