"""Boundary/error coverage detection for bob74 AC sets.
...
The bug this fixes: the original _ensure_boundary_and_error_coverage used naive
substring matching over ALL criteria (including structural lines). A feature whose
slug contained a coverage token — e.g. "failing" (matches "fail"),
"length-capped" (matches "limit") — false-tripped has_error/has_boundary, so the
injector SKIPPED adding the AC, yet the composite scorer (which uses \\b
word-boundary regexes and only counts prose ACs, not structural
File-exists/Function-defined lines) still saw 0 coverage → composite 0.0 for
32/118 features.
"""
...
def detect_coverage_with_word_boundaries(
    ...
    """Return (has_boundary, has_error) using word-boundary regexes on prose ACs only.

    Filters out structural ACs (File exists, Function defined, pytest, etc.)
    before matching, preventing slug tokens from false-tripping coverage detection.
...
def ensure_boundary_and_error_coverage(
    ...
    This is equivalent to detect_coverage_with_word_boundaries and is the
    primary public API for this module.
    ...
    """
    return detect_coverage_with_word_boundaries(criteria)
