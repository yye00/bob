"""EARS-style behavior acceptance criteria — public top-level API.
...
# Full structured parse: subject verb object when condition.
...
def parse_behavior(ac: str) -> BehaviorCriterion | None:
    ...
    Examples::
        ...
        >>> parse_behavior("behavior: parser returns BehaviorAC when AC matches grammar")
        BehaviorCriterion(subject='parser', verb='returns', object_='BehaviorAC',
                          condition='AC matches grammar')

        >>> parse_behavior("pytest: tests/test_foo.py")  # not a behavior AC
        None
    """
