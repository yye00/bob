...
This module is the public entry point; computation is delegated to
``bob3.spec_quality.quality_score`` which owns the four sub-scorers.
"""
...
def generate_remediation_report(
    ...
    failure and how to fix them when score < 0.85. Returns None when passing.
...
def calculate_spec_quality_score(
    ...
    - ac_coverage_score: fraction of public API surfaces mentioned in the
      description that have a corresponding AC
    ...
    ValueError
        When *name* is None or not a string.
    TypeError
        When *name* is None (delegated from compute_score).
    """
    if name is None:
        raise ValueError("feature name must not be None; provide a non-empty string.")
    if not isinstance(name, str):
    ...
    if not isinstance(acceptance_criteria, (list, str)):
        raise ValueError(
            f"acceptance_criteria must be a list or string, got {type(acceptance_criteria).__name__!r}."
        )
