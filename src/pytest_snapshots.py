"""Deterministic pytest snapshot configuration — enforce --maxfail=0 at snapshot boundary.
...
enforce_maxfail_for_snapshots(argv) -> list[str]
...
enforce_maxfail_for_xdist(argv) -> list[str]
    ...
    flags (-n / --numprocesses) are absent the argv is returned unchanged
    (no need to guard against early-halt). When xdist flags are present,
    --maxfail=0 is injected and any existing --maxfail is stripped, so
    snapshot completeness is guaranteed regardless of worker count.
"""
...
enforce_maxfail_for_snapshots = enforce_maxfail_zero
