...
def reset_completion_stamp(feature_id: str) -> None:
    ...
    This delegates to ``bob.orchestrator.sticky_completed.clear_stamp`` when
    available, falling back to a direct DB update via ``bob.db``.
    ...
    try:
        from bob.orchestrator.sticky_completed import clear_stamp
