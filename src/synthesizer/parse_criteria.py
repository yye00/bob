"""synthesizer.parse_criteria — parse LLM AC responses and guarantee coverage.

Addresses two root causes of synthesized=0/118 across prior generations:

(1) The LLM frequently returns a list of OBJECTS instead of flat strings, e.g.
    [{"id":1,"criterion":"...","description":"..."}]. str(dict) yields a
    Python-repr string that is NOT a verifiable AC (scores ~0).
    parse_criteria_response (and its alias extract_criteria_from_response)
    handles both flat strings and objects.

(2) The LLM almost never includes boundary-condition or error-path ACs.
    ...
    inject_missing_boundary_error_acs (and its alias inject_boundary_error_criteria)
    deterministically adds missing coverage.

Public API:
  parse_criteria_response(response_text) -> list[str] | None
  inject_missing_boundary_error_acs(criteria, title="") -> list[str]
  extract_criteria_from_response(response_text) -> list[str] | None  (alias)
  inject_boundary_error_criteria(criteria, title="") -> list[str]    (alias)
"""
...
def parse_criteria_response(response_text: str) -> list[str] | None:
    ...
    failure (malformed JSON, empty array, no JSON found).

    Args:
        response_text: raw text from the LLM synthesizer response.

    Returns:
        A list of non-empty AC strings, or None if parsing fails.
    """
...
# Alias for backward compatibility with code that uses the older name.
extract_criteria_from_response = parse_criteria_response
...
def inject_missing_boundary_error_acs(
    ...
    Args:
        criteria: list of AC strings (must be a list of strings).
    ...
    # Build a probe string from behavior ACs only (skip structural prefixes).
    probe_parts: list[str] = []
    for c in criteria:
        if not c.strip():
            continue
        if _STRUCTURAL_PREFIX_RE.match(c):
            # Extract description after " — " or " - " delimiter if present.
            for sep in (" — ", " - ", "—", "–"):
                if sep in c:
...
# Alias for backward compatibility with code that uses the older name.
inject_boundary_error_criteria = inject_missing_boundary_error_acs
