...
def python_based_convergence_checker_non_empty_name_based_comparison(
    db0: Union[str, Path],
    db1: Union[str, Path],
    db2: Union[str, Path],
) -> tuple[bool, set[str]]:
    ...
    Fixes two root causes of false-positive convergence:
      ...
      2. Name-based comparison: compares feature.name (not feature.id/UUID),
         so fresh ``bob3 init`` runs that mint new UUIDs do not break detection.
    ...
    # Build symmetric difference across all three sets
    all_names = c0 | c1 | c2
    common = c0 & c1 & c2
