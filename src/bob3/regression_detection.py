...
a causal link.  Observed live in bob3 version 12 round 9: F-15f5b3b8
...
Integration
-----------
Both functions are re-exported from ``bob3.orchestrator`` (see the update to
``bob3/orchestrator/__init__.py``).  Call-sites should prefer:

    from bob3.orchestrator import detect_regression_with_evidence, has_causal_link
"""
...
# Alias required by AC: "Function defined: bob3.regression_detection.has_ownership_evidence"
    ...
    *,
    ...
    Args:
        ...
        _create_regression_event_fn: Callable that creates a RegressionEvent
            record.  Defaults to ``bob3.db.create_regression_event``.
