...
# ``bob3.memory_mcp`` server processes. Each sub-agent spawn produces its
...
def _registry_path() -> Path:
    ...
    override = os.environ.get("BOB3_MCP_REGISTRY_PATH")
    if override:
...
def _is_memory_mcp_pid(pid: int) -> bool:
    """Heuristic check that ``pid`` is a bob3.memory_mcp process.

    Reads ``/proc/<pid>/cmdline`` and looks for the ``bob3.memory_mcp``
    ...
    return "bob3.memory_mcp" in cmdline


def _iter_memory_mcp_pids() -> list[int]:
    """Return all currently-running ``bob3.memory_mcp`` PIDs by scanning /proc.
...
def sweep_orphans() -> list[int]:
    """Reap every ``bob3.memory_mcp`` process whose parent is gone.

    Walks ``/proc`` for live ``bob3.memory_mcp`` PIDs, reads the
