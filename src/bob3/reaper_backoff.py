...
Provides the top-level bob3.reaper_backoff module required by the feature ACs:
  - Function defined: bob3.reaper_backoff.calculate_backoff_delay
  - Function defined: bob3.reaper_backoff.should_refuse_redispatch
...
IMPORT ISOLATION: This module is imported by bob3.orchestrator.__init__, which
is itself imported by bob3.orchestrator.reap_backoff. Therefore this module
must NOT import from bob3.orchestrator at module load time. All logic is
implemented locally; heavier bob3.reaper objects are lazy-loaded via __getattr__.
"""
...
def should_refuse_redispatch(feature: "Feature", now: datetime | None = None) -> bool:
    ...
    bob3.orchestrator. Escalation is performed via a lazy import of bob3.db.
    ...
    if reap_count >= _ESCALATION_THRESHOLD:
        import bob3.db as _db  # noqa: PLC0415 — lazy to avoid circular import
        _db.update_feature(
            feature.id,
            status="needs_human",
...
def __getattr__(name: str):
    """Lazy re-exports from bob3.reaper to avoid circular imports at load time."""
    if name in ("BackoffDecision", "handle_exponential_backoff"):
        import bob3.reaper as _reaper  # noqa: PLC0415
