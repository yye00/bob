...
def check_pending_decomposition(
    ...
    *,
    ...
    """Return True when the feature is stuck and should enter pending_decomposition.

    Alias for should_trigger_decomposition with the AC-mandated name.
    ...
    Raises ValueError if feature.refinement_attempts is negative (data corruption).
    """
    return should_trigger_decomposition(
        feature, previous_readiness_score=previous_readiness_score
    )


def decompose_stuck_feature(
    feature: "Feature",
    *,
    previous_readiness_score: float | None = None,
    db_update: Any | None = None,
    ...
    When ``db_update`` is provided it is called as
    ``db_update(feature.id, status="pending_decomposition")`` to persist the
    transition.

    Raises ValueError if feature.refinement_attempts is negative (data corruption).
    """
    if not check_pending_decomposition(
        feature, previous_readiness_score=previous_readiness_score
    ):
        return None
