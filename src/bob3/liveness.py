...
def probe_process_running() -> bool:
    ...
    which scans /proc for live processes whose cmdline matches the
    ``bob[0-9]+ run`` pattern, covering all gen-N binary aliases (e.g. bob14,
    bob92) in addition to the legacy ``bob3 run`` form.

    This fixes the defect where the operator/watchdog used ``pgrep bob3 run``
    ...
    AC: Function defined: bob3.liveness.probe_process_running
    """
    return is_orchestrator_alive()
...
def is_db_active(db_path: pathlib.Path | None = None) -> bool:
    ...
    AC: Function defined: bob3.liveness.is_db_active
    """
    return verify_db_activity(db_path=db_path)
...
# probe MUST match `bob[0-9]+` regex AND honor `.bob3.lock` holder PID)
# ---------------------------------------------------------------------------
...
def matches_process_regex() -> bool:
    ...
    which scans /proc for live processes whose cmdline matches the
    ``bob[0-9]+ run`` pattern, covering all gen-N binary aliases (e.g. bob14,
    bob92, bob93) in addition to the legacy ``bob3 run`` form.
    ...
    AC: Function defined: bob3.liveness.matches_process_regex
    """
    return is_orchestrator_alive()
...
def safe_lock_removal(
    ...
    AC: Function defined: bob3.liveness.safe_lock_removal
    """
