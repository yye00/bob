...
def sweep_orphan_subagents():
    """Module-level proxy so patch('bob3.final_reaper.sweep_orphan_subagents') works in tests."""
    return _get_subagent_reaper().sweep_orphan_subagents()


def find_subagent_pid_for_feature(feature_id):
    """Module-level proxy so patch('bob3.final_reaper.find_subagent_pid_for_feature') works in tests."""
