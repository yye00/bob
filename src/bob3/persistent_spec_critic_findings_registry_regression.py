"""Persistent spec-critic findings registry with regression detection (F-R7-450).

Keyed by (spec_hash, slot_id, defect_type). On re-run with the same defect
type at the same slot, the finding is flagged as REGRESSION and severity is
escalated. Halt-gate fires when critic_repeat_rate > 0.30 over 3 runs.
...
def persistent_spec_critic_findings_registry_regression(
    spec_hash: str,
    slot_id: str,
    defect_type: str,
    *,
