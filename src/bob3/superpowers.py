...
from bob3.security_checks import run_security_checks
from bob3.verification.per_feature_test_scope import scope_pytest_to_feature  # noqa: F401
...
def _file_security_warnings(warnings_list: list) -> None:
    ...
    Guarded by ``BOB3_SECURITY_FILE_FINDINGS`` env var (default ``0`` to
    avoid polluting bob3's own ``reviews/findings.yaml`` during tests
    and self-development). Production orchestrator runs should set
    ``BOB3_SECURITY_FILE_FINDINGS=1``.
    """
    if os.environ.get("BOB3_SECURITY_FILE_FINDINGS", "0").lower() not in {"1", "true", "yes"}:
        logger.debug("security_scan: filing disabled (BOB3_SECURITY_FILE_FINDINGS=0)")
        return
...
def reload_prompt_sources() -> list[str]:
    ...
    bob3.models) land immediately without requiring an orchestrator restart.
