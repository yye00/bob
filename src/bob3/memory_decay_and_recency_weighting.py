"""Memory decay and recency weighting for bob3.
...
- "context" (high decay): short half-life, ~half of BOB3_LESSON_HALF_LIFE_DAYS
...
def get_decay_config() -> MemoryDecayConfig:
    ...
    Reads BOB3_LESSON_HALF_LIFE_DAYS (default 30). Raises ValueError for
    non-positive or non-numeric values so misconfiguration surfaces loudly.
    """
    raw = os.environ.get("BOB3_LESSON_HALF_LIFE_DAYS")
    if raw is None:
        ...
        try:
            ...
            raise ValueError(
                f"BOB3_LESSON_HALF_LIFE_DAYS={raw!r} is not a valid number."
            )
        if half_life <= 0:
            raise ValueError(
                f"BOB3_LESSON_HALF_LIFE_DAYS must be positive, got {half_life}."
            )
