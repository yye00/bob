...
from bob3.cost_cap import enforce_per_attempt_cost_cap  # noqa: F401 — re-export
...
def enforce_cost_cap(
    *,
    ...
    for ``bob3.per_attempt_cost_cap.enforce_cost_cap``.
...
def send_sigterm_on_cost_exceeded(
    *,
    ...
    Delegates to ``bob3.orchestrator.per_attempt_cost_cap.terminate_subagent_on_cost_cap``.
    Satisfies the AC requirement for
    ``bob3.per_attempt_cost_cap.send_sigterm_on_cost_exceeded``.
    """
    terminate_subagent_on_cost_cap(
        feature_id=feature_id,
        pid=pid,
