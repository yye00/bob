...
The function delegates to bob3.orchestrator.project_metadata_check helpers
and re-exports ProjectMetadataCheckResult from bob3.run_loop so callers can
...
from bob3.run_loop import ProjectMetadataCheckResult, verify_project_metadata as _run_loop_verify
...
def verify_project_metadata(
    ...
    ``bob3 init``, so projects.name may still reflect the parent generation
    ...
    Args:
        ...
        db_path: Path to the bob3.db database. Defaults to BOB3_DATABASE_PATH
