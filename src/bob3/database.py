...
The bob3 database implementation lives in the ``bob3.db`` package
(``bob3/db/__init__.py``). This module is a thin, named re-export so callers and
ACs that reference ``bob3.database`` resolve to the same API — in particular
``create_agent_run``, which MUST write its agent_run row to the project's own
database (resolved from ``BOB3_DATABASE_PATH``) rather than a cwd-relative DB.
"""

from __future__ import annotations

from bob3 import db as _db
from bob3.db import *  # noqa: F401,F403 - re-export the full db API surface

# Explicitly re-export the most commonly referenced entry points so static
# importers (and the ``Function defined: bob3.database.<name>`` ACs) resolve.
