...
Used by `bob3 sanitize <spec>` to rewrite a YAML where the human
...
from bob3.boundary_error_detector import detect_coverage_with_boundaries  # noqa: F401
...
def _load_compute():
    ...
    the gen root is on sys.path — true when bob3 runs from cwd=<gen>, but NOT
    ...
    file: <gen>/src/bob3/spec_synthesizer.py → <gen>) to sys.path and retry.
    ...
    gen_root = _pl.Path(__file__).resolve().parents[2]  # <gen>/src/bob3 → <gen>
...
def resilient_import_scorer():
    ...
    - On ModuleNotFoundError, derive the gen root from this file's __file__
      path (<gen>/src/bob3/spec_synthesizer.py → parents[2]) and insert it
    ...
    def _try_import():
        try:
            from bob3.spec_quality.quality_score import compute_score as _gate_compute  # noqa: F401
            return True
        except ImportError:
            return False
    ...
    if not _try_import():
        ...
        if not _try_import():
            raise ImportError(
                ...
                "Check that bob3.spec_quality.quality_score exists in the workspace."
            )
...
def import_spec_quality_scorer() -> Callable[..., Any]:
    ...
    2. On ``ModuleNotFoundError``, derive the gen root from this file's
       ``__file__`` path (``<gen>/src/bob3/spec_synthesizer.py → parents[2]``),
...
```json
["File exists: src/bob3/foo.py", "Function defined: bob3.foo.bar", "pytest: tests/test_foo.py"]
```

Example output (wiring feature):
```json
["File exists: src/bob3/foo.py", "Function defined: bob3.foo.bar", "pytest: tests/test_foo.py", "integration: bob3.orchestrator"]
```
"""
...
async def synthesize_for_feature(
    *,
    ...
    # outlasts a burst. Env-tunable BOB3_SYNTH_MAX_ATTEMPTS (default 40 → ~40min
    ...
    try:
        _MAX_ATTEMPTS = max(1, int(_os.environ.get("BOB3_SYNTH_MAX_ATTEMPTS", "40")))
    except ValueError:
        _MAX_ATTEMPTS = 40
...
def _repair_unreachable_integration_targets(
    ...
    routinely invents plausible-but-wrong module names (``bob3.weekend_watchdog``
    when the real module is ``bob3.spawn_watchdog``; ``bob72.orchestrator`` when
    it is ``bob3.orchestrator``), so 46/128 features scored reachability 0.0 and
    ...
    """
    try:
        from bob3.spec_quality.integration_reachability import check_spec
    except Exception:
        return criteria
...
def _sanitize_bad_acs(criteria: list[str]) -> list[str]:
    ...
    # ANCESTOR lineage names. Derive it from this file's path: <gen>/src/bob3/...
    try:
    ...
    for ac in criteria:
        ...
        def _strip_ancestor(m):
            ...
            name = tok[:-1]   # "bob17"
            # Keep the canonical base package 'bob3' AND the current gen; strip
            # only OTHER ancestor gen names (bob12, bob17, ...).
            if name == "bob3" or name == _cur_gen:
                return tok
            return ""
...
def emit_file_exists_acs(criteria: list[str], description: str) -> list[str]:
    ...
    *SYNTHESIZER under-coverage*: when a description explicitly names a concrete
    source path (e.g. ``src/bob3/brownfield/survey.py``) but synthesis derived a
...
def _ensure_described_files_covered(criteria: list[str], description: str) -> list[str]:
    ...
    names e.g. src/bob3/brownfield/survey.py but synthesis derived a different
...
def _derive_canonical_slug(title: str) -> str | None:
    ...
    # ``bob3.<slug>.<symbol>`` un-importable, which would make the
...
def _default_integration_target(title: str, description: str) -> str:
    ...
    CLI-hint matches route to ``bob3.cli``; orchestration hints route to
    ``bob3.orchestrator.run_loop``. (Earlier revisions had both branches
    ...
    if any(hint in blob for hint in _ORCHESTRATION_HINTS):
        return "bob3.orchestrator.run_loop"
    return "bob3.orchestrator.run_loop"


def _qualify_integration_target(target: str) -> str:
    """Ensure an integration target is module-qualified under ``bob3.*``.
    ...
    trivial reward-hack surface. Prepending ``bob3.`` forces the implementer
    to wire into a real module path.
    """
    target = target.strip()
    if not target:
        return target
    if target.startswith("bob3.") or target == "bob3":
        return target
    return f"bob3.{target}"
...
def _build_fallback_criteria(
    ...
    Guarantees (F-R6-305 + post-review hardening):
      ...
      * Integration targets are always ``bob3.``-qualified.
    ...
    criteria: list[str] = [
        f"File exists: src/bob3/{slug}.py",
        ...
        f"Function defined: bob3.{slug}.{slug}",
    ]
...
async def sanitize_spec_file(
    ...
    *,
    ...
    # Concurrency is env-tunable (BOB3_SYNTH_CONCURRENCY) so the operator can
    ...
    import os as _os
    try:
        _conc = int(_os.environ.get("BOB3_SYNTH_CONCURRENCY", str(concurrency)))
    except ValueError:
        _conc = concurrency
...
def score_gate_threshold_from_env() -> float:
    """Read the score-gate threshold from BOB3_SPEC_QUALITY_THRESHOLD env var.

    Returns 0.85 by default. Clamps to [0.0, 1.0]. Returns default on
    invalid (non-float) values.
    """
    raw = os.environ.get("BOB3_SPEC_QUALITY_THRESHOLD")
    if not raw:
...
# F-af78c082: public alias matching AC "Function defined: bob3.spec_synthesizer.should_emit_function_ac"
should_emit_function_ac = should_emit_function_defined_ac

# F-04ffd352: public alias matching AC "Function defined: bob3.spec_synthesizer.should_emit_exact_function_ac"
...
def emit_function_defined_ac(module: str, symbol: str, description: str) -> str | None:
    ...
    ----------
    module:
        Dotted module path, e.g. ``bob3.reaper``.
    symbol:
...
async def sanitize_spec_file_with_gate_loop(
    ...
    *max_retries* times until the composite spec-quality score meets *threshold*
    (default: ``BOB3_SPEC_QUALITY_THRESHOLD`` env var, else 0.85).
...
# ---------------------------------------------------------------------------
# synthesize_with_score_gate_loop — AC: "Function defined: bob3.spec_synthesizer.synthesize_with_score_gate_loop"
...
# ---------------------------------------------------------------------------
# Public alias — AC: "Function defined: bob3.spec_synthesizer.synthesize_acceptance_criteria"
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Public alias — AC: "Function defined: bob3.spec_synthesizer.synthesize_with_retry"
...
# exponential backoff, BOB3_SYNTH_MAX_ATTEMPTS env-tunable); synthesize_with_retry
...
#: only falls through to deterministic fallback after BOB3_SYNTH_MAX_ATTEMPTS
#: (default 40) are exhausted.  Each retry is logged so bursts are observable.
synthesize_with_retry = synthesize_for_feature

# ---------------------------------------------------------------------------
# Public alias — AC: "Function defined: bob3.spec_synthesizer.score_gate_retry_loop"
...
# ---------------------------------------------------------------------------
# retry_with_backoff — AC: "Function defined: bob3.spec_synthesizer.retry_with_backoff"
...
async def retry_with_backoff(
    ...
    *jitter_seed*.
    ...
    max_attempts:
        ...
        ``BOB3_SYNTH_MAX_ATTEMPTS`` environment variable (default 40).
    base_delay:
    ...
    if max_attempts is None:
        try:
            max_attempts = max(1, int(_os.environ.get("BOB3_SYNTH_MAX_ATTEMPTS", "40")))
        except ValueError:
            max_attempts = 40
