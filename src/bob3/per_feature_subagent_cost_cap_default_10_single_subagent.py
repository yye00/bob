...
``bob3 run --max-cost`` flag caps TOTAL run cost but NOT per-feature attempts.
...
Default cap: $10 USD.  Override via ``BOB3_PER_ATTEMPT_COST_CAP`` (clamped
to [0.5, 100]).

The lower-level signal delivery, audit-log writing, and attempt charging are
handled by ``bob3.orchestrator.per_attempt_cost_cap`` and surfaced through
``bob3.cost_cap.enforce_per_attempt_cap``.
"""
...
def per_feature_subagent_cost_cap_default_10_single_subagent(
    *,
    ...
    :func:`bob3.cost_cap.enforce_per_attempt_cap`, which in turn delegates to
    ``bob3.orchestrator.per_attempt_cost_cap`` for signal delivery and audit
    logging.

    The cap defaults to $10 USD and is controlled by the environment variable
    ``BOB3_PER_ATTEMPT_COST_CAP`` (clamped to [0.5, 100]).
