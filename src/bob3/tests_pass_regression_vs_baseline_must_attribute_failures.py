...
gate.  It delegates to ``bob3.regression_attribution.attribute_failures_to_owning_feature``
which filters failing tests so only those attributable to the
currently-verifying feature count as gate failures.

Public API
----------
``tests_pass_regression_vs_baseline_must_attribute_failures``
...
from bob3.regression_attribution import attribute_failures_to_owning_feature
...
def tests_pass_regression_vs_baseline_must_attribute_failures(
