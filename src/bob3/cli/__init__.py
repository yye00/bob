...
from bob3.progress_events import get_progress_path
...
import bob3.cli.init as _cli_init_module  # noqa: F401 — integration wiring
import bob3.cli.run as _cli_run_module  # noqa: F401 — env_preflight integration wiring
from bob3.cli.spec_trace import spec_trace as _spec_trace_handler
...
def init(project_path, name, spec, brownfield):
    ...
    try:
        ...
        if existing:
            ...
            # F-R7-619: set max_cost_usd from BOB3_MAX_COST_USD (else effectively
            ...
            import os as _os
            _raw = _os.environ.get("BOB3_MAX_COST_USD", "")
            try:
    ...
    if brownfield:
        ...
        from bob3.brownfield.survey import build_survey
...
def plan(spec_file, create, round_num):
    ...
    if create and features:
        try:
            from bob3.spec_synthesizer import find_placeholder_features
            ...
            if find_placeholder_features(spec_check):
                ...
                if _pid:
                    ...
                    console.print(
                        ...
                        "(run bob3 init first). Features will be created "
    ...
    if create and features:
        try:
            from bob3.cli.plan import run_composite_score_gate
...
@click.option("--no-update-db", is_flag=True, help="Skip updating feature rows in bob3.db.")
@click.option(
...
def sanitize(spec_file, dry_run, no_update_db, concurrency):
    ...
    `bob3.db` (matched by feature name) unless --no-update-db.

    Run this after `bob3 init` and before `bob3 run --all` when the spec
    ...
    if not row:
        console.print(
            "[red]Error: no project found in bob3.db. "
            "Run 'bob3 init <project_path>' first.[/red]"
        )
        raise SystemExit(1)
    project_id = row[0]
    ...
    if not dry_run and not no_update_db and report.get("written"):
        ...
        if updates:
            ...
            try:
                ...
                conn.commit()
                console.print(
                    f"[green]Updated {updated} feature row(s) in bob3.db.[/green]"
                )
            finally:
                conn.close()
...
@click.option(
    ...
    help=(
        ...
        "Overrides the BOB3_ABLATION_MODE environment variable."
    ),
)
@click.option(
...
def run(feature, run_all, max_cost, fresh, no_mcp, force_unlock, ablation_mode, max_concurrent_features):
    ...
    if ablation_mode is not None:
        os.environ["BOB3_ABLATION_MODE"] = ablation_mode.upper()

    from bob3.ablation import get_ablation_mode as _get_ablation_mode
    ...
    # matching bob[0-9]+ run is detected in /proc, even if .bob3.lock is
    ...
    from bob3.orchestrator.liveness_probe import is_orchestrator_alive as _is_alive
    if _is_alive():
        console.print(
...
def _show_project_status(console, conn, verbose):
    ...
    from bob3.models import resolve_max_cost_usd as _resolve_max_cost_usd
...
def skill_activation_report_cmd():
    ...
    Reads skill_activation_logged events from .bob3/progress.jsonl and
...
def bundle_cmd(run_id, output_dir):
    ...
    from bob3.reproducibility_bundle_single_command_tarball_export import export_bundle
...
@click.option(
    ...
    "db_path",
    default=None,
    help="Path to bob3.db (defaults to BOB3_DATABASE_PATH env var or bob3.db).",
)
def spec_trace_cmd(target: str, db_path: str | None) -> None:
    ...
    """
    from bob3.spec_quality.provenance import trace_ac
...
def plan_diff_cmd(feature_id: str, workspace: str | None) -> None:
    ...
    list against the current acceptance_criteria stored in bob3.db.
    Exits 0 when there is no drift; exits 1 when the plan is stale.
    """
    import json as _json

    from bob3.orchestrator.plan_gate import compute_plan_vs_spec_drift, load_plan
...
def spec_findings_cmd(since: str | None, findings_file: str | None) -> None:
    ...
    if since is not None:
        ...
        # Load all findings
        from bob3.spec_quality.spec_findings_registry import _findings_path, _load_yaml
...
@click.option("--db", "db_path", default=None, help="Path for survey.db (default: <workspace>/.bob3/survey.db).")
def survey_cmd(workspace_path: str, refresh: bool, db_path: str | None) -> None:
    ...
    symbols + edges in SQLite (.bob3/survey.db), computes PageRank, and
    ...
    console.print(f"[green]Done.[/green] Survey stored at {survey_db or workspace / '.bob3' / 'survey.db'}")
...
@click.option("--name", "spec_name", default=None, help="Top-level spec name to write into the YAML (needed by bob3 init).")
def extract_from_peas_cmd(peas_file: str, out_file: str | None, workspace: str, threshold: float, stubs_only: bool, spec_name: str | None) -> None:
    ...
    import pathlib as _pathlib
    import yaml as _yaml
    from bob3.extract_from_peas import run_pipeline, parse_peas_markdown, emit_stub_features
...
def explain_gate_block_cmd(feature_id_prefix: str, as_json: bool, workspace: str | None) -> None:
    ...
    Loads the feature row from bob3.db, re-runs spec quality scoring on its
    ...
    from bob3.enhanced_verification import explain_gate_block
...
def lint_specs_cmd(spec_file: str, as_json: bool) -> None:
    ...
    from bob3.spec_quality.ambiguity_linter import lint_spec, SpecLintReport
...
def quality_gate_mutmut(
    ...
    from bob3.mutmut_verifier import verify_mutation_score
...
def verify_mutmut_cmd(
    ...
    from bob3.mutmut_gate import run_mutation_tests
