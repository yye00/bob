"""bob3.mutation_verifier — mutmut 3.x post-impl quality gate interface.
...
run_mutation_testing(feature_id, src_files, test_dir, workspace,
    ...
    Run the mutmut post-impl quality gate. Returns None when the gate is
    skipped (pytest failed or empty feature_id), a gate-result dict on
    success, or a dict with skipped=True when mutmut is unavailable.
"""
...
def run_mutation_testing(
    ...
    *,
    ...
    are persisted to runs/<feature_id>/mutation_report.json so the next
    implementer attempt can see which mutations the tests cannot distinguish
