"""bob3.spec_findings_writer — atomic write and corruption recovery for spec_findings.yaml.
...
concurrent writers) leave a malformed YAML that kills bob3 boot with
...
- Reader side: load_with_corruption_recovery() catches yaml.YAMLError, quarantines
  ...
  {} so bob3 boot can continue.  Empty findings is recoverable; boot-loop crash is not.
"""
...
def write_atomic_yaml(data: dict[str, Any], path: Path | str) -> None:
    """AC-required alias: bob3.spec_findings_writer.write_atomic_yaml.
...
def write_spec_findings_atomic(data: dict[str, Any], path: Path | str) -> None:
    """Alias for write_atomic — required by AC (bob3.spec_findings_writer.write_spec_findings_atomic)."""
    write_atomic(data, path)
...
def write_findings_atomically(data: dict[str, Any], path: Path | str) -> None:
    ...
    AC-required name: bob3.spec_findings_writer.write_findings_atomically.
    Delegates to write_atomic — same POSIX atomicity guarantees.
    """
    write_atomic(data, path)
...
def load_with_corruption_recovery(path: Path | str) -> dict[str, Any]:
    ...
    empty dict is returned.  This lets bob3 boot proceed rather than crash-looping.
...
def handle_scanner_error(path: Path | str) -> dict[str, Any]:
    ...
    file to ``<path>.corrupt.<unix_ts>``, and returns {} so bob3 can continue
...
def load_with_corruption_handler(path: Path | str) -> dict[str, Any]:
    """AC-required alias: bob3.spec_findings_writer.load_with_corruption_handler.
...
def quarantine_corrupted(path: Path | str) -> dict[str, Any]:
    ...
    AC: bob3.spec_findings_writer.quarantine_corrupted
    """
    return quarantine_corrupt_findings(path)
...
def quarantine_corrupted_findings(path: Path | str) -> dict[str, Any]:
    ...
    AC: bob3.spec_findings_writer.quarantine_corrupted_findings
    """
    return quarantine_corrupt_findings(path)


def handle_corrupted_findings(path: Path | str) -> dict[str, Any]:
    """AC-required handler: bob3.spec_findings_writer.handle_corrupted_findings.

    Logs a structured spec_findings_corrupt event, quarantines the corrupt file
    to <path>.corrupt.<unix_ts>, and returns {} so bob3 boot continues.
