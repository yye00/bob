"""bob3.synthesizer_retry — retry primitives for transient upstream API 400/empty-response.
...
This module provides two public functions:
  ...
  synthesize_with_retry           — wraps bob3.synthesizer.synthesize_for_feature in the
...
def _max_attempts_from_env() -> int:
    """Read BOB3_SYNTH_MAX_ATTEMPTS from env; fall back to 40 on invalid values."""
    try:
...
async def synthesize_with_retry(
    *,
    ...
    Wraps ``bob3.synthesizer.synthesize_for_feature`` in the
    :func:`retry_with_exponential_backoff` loop. A synthesis pass that encounters an
    upstream 400/empty-response burst will retry up to BOB3_SYNTH_MAX_ATTEMPTS times
    ...
    """
    from bob3.synthesizer import synthesize_for_feature as _synthesize
...
# AC alias: "Function defined: bob3.synthesizer_retry.retry_with_backoff"
