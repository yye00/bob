...
BOB3_SWEEP_PARALLELISM workers), enforces a total cost budget
(BOB3_SWEEP_BUDGET_USD), and writes resumable checkpoints to
.bob3/sweep_checkpoint.json so an interrupted sweep can restart
...
_DEFAULT_CHECKPOINT_PATH = Path(".bob3") / "sweep_checkpoint.json"
...
def _resolve_sweep_parallelism() -> int:
    """Return worker count from BOB3_SWEEP_PARALLELISM, falling back to 4."""
    raw = os.environ.get("BOB3_SWEEP_PARALLELISM", "")
    try:
        value = int(raw)
        if value > 0:
            return value
...
def _resolve_sweep_budget() -> float:
    """Return budget from BOB3_SWEEP_BUDGET_USD, falling back to 100.0."""
    raw = os.environ.get("BOB3_SWEEP_BUDGET_USD", "")
    try:
        value = float(raw)
        return value
    except (ValueError, TypeError):
