...
bob3 orchestration loop. Wraps the underlying timeout machinery from
:mod:`bob3.timeout` and exposes a stable, importable contract so callers
(orchestrator, tests) do not reach into run_loop internals.

The authoritative env var is ``BOB3_FEATURE_TIMEOUT_SECONDS`` (default 1800 s).
...
from bob3.timeout import (  # noqa: F401 — re-exported for callers
    FeatureTimeoutError,
...
async def enforce_feature_timeout(
    ...
    *,
    ...
    (explicit ``timeout_seconds`` or ``BOB3_FEATURE_TIMEOUT_SECONDS``).
    ...
    Args:
        ...
        timeout_seconds: Override timeout in seconds. When ``None`` (default),
            reads ``BOB3_FEATURE_TIMEOUT_SECONDS`` from the environment,
