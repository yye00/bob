"""Length-capped module slug derivation — slug_capper public interface.
...
Root cause: spec_synthesizer._derive_canonical_slug joined ALL title tokens
with underscores without any length cap. enhanced_verification raised
"[Errno 36]" on every verification pass, looping 17 times with no exit.

Fix: cap_slug_at_boundary accumulates whole tokens until the next token
would exceed 60 characters, then stops. An extremely long single token is
hard-truncated to the cap. The same capped slug is used for both the
File-exists path and the Function-defined module path so one implementation
file satisfies both ACs.

Integration: bob3.spec_synthesizer._derive_canonical_slug already applies
this cap inline; this module exposes it as an importable public function.
"""

from __future__ import annotations

from bob3.spec_synthesizer import _derive_canonical_slug as _ss_derive_canonical_slug
