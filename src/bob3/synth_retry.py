"""bob3.synth_retry — retry primitives for transient upstream API failures.
...
This module exposes two public functions:
  ...
  retry_with_backoff   — generic async retry loop with exponential backoff,
                         env-tunable via BOB3_SYNTH_MAX_ATTEMPTS (default 40).

  synthesize_with_retry — thin wrapper around bob3.spec_synthesizer.synthesize_for_feature
...
Both functions are integrated into bob3.orchestrator for the integration AC.
"""
...
def _max_attempts_from_env() -> int:
    """Read BOB3_SYNTH_MAX_ATTEMPTS from env, falling back to 40 on invalid values."""
    try:
        return max(1, int(os.environ.get("BOB3_SYNTH_MAX_ATTEMPTS", str(_DEFAULT_MAX_ATTEMPTS))))
...
async def retry_with_backoff(
    ...
    **fn_kwargs: Any,
    ...
    Args:
        ...
        max_attempts: override attempt count; defaults to BOB3_SYNTH_MAX_ATTEMPTS env var.
...
async def synthesize_with_retry(
    *,
    ...
    Wraps ``bob3.spec_synthesizer.synthesize_for_feature`` in the
    :func:`retry_with_backoff` loop.  A synthesis pass that encounters an
    upstream 400/empty-response burst will retry up to BOB3_SYNTH_MAX_ATTEMPTS
    ...
    Args:
        ...
        max_attempts: override retry count; defaults to BOB3_SYNTH_MAX_ATTEMPTS.
    """
    from bob3.spec_synthesizer import synthesize_for_feature as _synthesize
