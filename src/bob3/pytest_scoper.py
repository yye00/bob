...
2. Walk the *importer* graph up to ``max_depth`` levels (modules that
   ``from bob3.foo import …`` or ``import bob3.foo``).
...
def _is_critical_path(rel_path: str) -> bool:
    ...
    # Directory match — any file under src/bob3/orchestrator/...
...
def _module_for_source(rel_path: str) -> str | None:
    """Convert ``src/bob3/foo.py`` → ``bob3.foo``.

    Returns ``None`` for paths outside ``src/bob3/`` or non-Python files.
    """
    p = Path(rel_path)
    parts = p.parts
    if len(parts) < 3 or parts[0] != "src" or parts[1] != "bob3":
        return None
    if p.suffix != ".py":
        return None
...
def _all_source_files(src_root: Path) -> list[Path]:
    """Enumerate every ``.py`` file under ``src/bob3/`` (recursive)."""
...
def _importers_of(
    ...
    """Return modules under ``src/bob3/`` whose source mentions ``module``.

    We string-match both ``from bob3.foo`` and ``import bob3.foo`` so that
    aliasing (``import bob3.foo as f``) and submodule imports
    (``from bob3.foo.bar import …``) are both detected. The patterns are
    word-bounded on the right so ``bob3.foo`` does not match ``bob3.foobar``.
    """
...
def _tests_mentioning(
    ...
    source. This catches both ``from bob3.foo import …`` and
    ``import bob3.foo`` plus indirect references (string node-ids,
...
def scope_tests_for_diff(
    ...
    Args:
        changed_files: Repo-relative paths (e.g. ``"src/bob3/foo.py"``)
    ...
    Returns:
        ...
        * ``repo_root`` does not contain ``src/bob3/`` or ``tests/``.
    """
    repo_root = Path(repo_root)
    src_root = repo_root / "src" / "bob3"
    ...
    # Map changed source files → seed modules. Files outside src/bob3/
    ...
    for criterion in acceptance_criteria:
        ...
        if lower.startswith("pytest:"):
            ...
            if p.name.startswith("test_") and p.suffix == ".py":
                # test_foo_bar.py → try bob3.foo_bar as seed
                mod_guess = "bob3." + p.stem[len("test_"):]
        ...
        # Function defined: bob3.foo.bar  or  bob3.foo.ClassName
