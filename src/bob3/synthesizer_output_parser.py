"""bob3.synthesizer_output_parser — parse LLM output and guarantee boundary/error-path AC coverage.

Two root causes of synthesized=0/N across prior generations:
...
(1) parse_criteria_response only handled flat JSON arrays of strings. The LLM
    ...
    This module extracts criterion text from recognized object keys.
...
(2) The LLM almost never includes boundary-condition or error-path ACs. The
    ...
    regardless of other sub-metrics. inject_missing_boundary_and_error_acs
    deterministically injects one boundary and one error-path pytest: AC when
    they are absent, mirroring the scorer's token patterns exactly.

Public API:
  parse_criteria_response(response_text) -> list[str] | None
  inject_missing_boundary_and_error_acs(criteria, title="") -> list[str]
"""
...
def _extract_from_object(obj: object) -> str:
...
def parse_criteria_response(response_text: str) -> list[str] | None:
    """Parse synthesizer LLM response into list[str]; returns None on failure.

    Handles both flat JSON arrays of strings AND lists of objects — the latter
    is the consistent root cause of synthesized=0/N across prior generations.
    When the LLM returns [{"id":1,"criterion":"...","description":"..."}], this
    function extracts the criterion text from recognized object keys rather than
    calling str(dict), which would produce a Python-repr string that is not a
    machine-verifiable AC.

    Args:
        response_text: raw LLM response string.

    Returns:
        list of non-empty criterion strings, or None if parsing fails.
    """
    ...
    if json_str is None:
        # Fall back to finding an inline JSON array.
    ...
    items = [c for c in (_extract_from_object(x) for x in parsed) if c]
    return items or None


def inject_missing_boundary_and_error_acs(
    criteria: list[str],
    title: str = "",
) -> list[str]:
    ...
    if not isinstance(criteria, list):
        raise TypeError(
            f"inject_missing_boundary_and_error_acs: criteria must be a list, "
            f"got {type(criteria).__name__!r}"
        )
    for item in criteria:
        if not isinstance(item, str):
            raise ValueError(
                f"inject_missing_boundary_and_error_acs: all criteria must be strings, "
