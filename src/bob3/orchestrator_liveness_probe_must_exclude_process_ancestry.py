...
blocking bob3 version 17 first-boot for 5 minutes until manual probe-patch
was applied.
...
Both predicates are exported by ``bob3.orchestrator.probe_ancestry`` so they
...
def orchestrator_liveness_probe_must_exclude_process_ancestry() -> bool:
    ...
    Wraps ``is_orchestrator_alive`` from ``bob3.orchestrator.liveness_probe``
