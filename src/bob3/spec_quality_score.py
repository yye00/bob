...
``bob3.composite_spec_quality_score_8_sub_metrics_geometric_mean_0``
and ``bob3.spec_quality.quality_score``. This shim re-exports the public
...
from bob3.spec_quality.score import (
...
class ScoreResult(_CompositeScoreResult):
    ...
    ``Function defined: bob3.spec_quality_score.ScoreResult``
    """
...
def filter_code_shaped_tokens(tokens: list[str]) -> list[str]:
    ...
    ``Function defined: bob3.spec_quality_score.filter_code_shaped_tokens``
...
def remediation_report(
    ...
    from bob3.spec_quality.quality_score import gate_for_ready
    report = _compute_score_internal(
        name=name,
        description=description,
        acceptance_criteria=acceptance_criteria,
...
def validate_score_thresholds(score: float) -> str:
    ...
    Function defined: bob3.spec_quality_score.validate_score_thresholds
