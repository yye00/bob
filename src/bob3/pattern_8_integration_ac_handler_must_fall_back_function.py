...
Root-cause (bob3 v.17 r1 ~09:43Z): feature 85790dc6 (orphan-subagent reaper)
NH'd with AC:
...
from bob3.enhanced_verification import _integration_wired  # noqa: F401 (integration: bob3.orchestrator.run_loop)
...
def pattern_8_integration_ac_handler_must_fall_back_function(
    ...
    Resolves an 'integration:' acceptance criterion by:
      1. Trying each dotted-path token (e.g. 'bob3.orchestrator.run_loop') via
