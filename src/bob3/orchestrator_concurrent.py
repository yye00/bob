"""Concurrent feature dispatch slot management for the Bob3 orchestrator.
...
- Re-exports :func:`dispatch_concurrent_features` from
  :mod:`bob3.orchestrator.run_loop` for backward-compatible access

The orchestrator tick loop can use these to maintain a pool of up to
``BOB3_MAX_CONCURRENT_FEATURES`` (default 3) concurrent feature workers,
...
def __getattr__(name: str) -> Any:
    """Lazy re-exports from bob3.orchestrator.run_loop to break circular import.

    orchestrator/__init__.py imports ConcurrentDispatchSlot from this module,
    and this module would normally import from bob3.orchestrator.run_loop —
    ...
    if name in ("dispatch_concurrent_features", "current_concurrency_slots", "_resolve_max_concurrent_features"):
        from bob3.orchestrator.run_loop import (  # noqa: F401
...
class ConcurrentDispatchSlot:
    ...
    a pool of these slots (up to ``BOB3_MAX_CONCURRENT_FEATURES``) and
