...
1. Write per-feature WORKER.md to .bob3/features/WORKER.md (slim context)
2. Write per-feature settings.json to .bob3/features/<id>/settings.json
3. Set ANTHROPIC_PROMPT_CACHING=1 in the worker environment
4. Pass --settings <path> to the claude -p invocation

See bob3.dispatch.spawn_worker for the canonical entry point.
