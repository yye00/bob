...
Problem (bob3 version 13 r10, 2026-05-29 00:37 UTC): a partial write left
...
Public API
----------
spec_findings_yaml_writer_must_use_atomic_tmp_rename_partial_write_corruption_kills_bob3_boot_scannererror_mapping_values_not_allowed(data, path, quarantine_if_corrupt=False)
    Main entry point. If *quarantine_if_corrupt* is True and *path* is
    an existing corrupt file, quarantine it and return
    ``{"quarantined": True}``. Otherwise, atomic-write *data* to *path*
    and return ``{"success": True, "path": str(path)}``.
"""
...
__all__ = [
    "spec_findings_yaml_writer_must_use_atomic_tmp_rename_partial_write_corruption_kills_bob3_boot_scannererror_mapping_values_not_allowed",
    "atomic_write_yaml",
...
def quarantine_corrupt_findings(path: Path | str) -> dict[str, Any]:
    ...
    On any corrupt file (regardless of content):
    - Renames it to ``<path>.corrupt.<unix_ts>``.
    - Logs a structured ``spec_findings_corrupt`` event at ERROR level.
    - Returns an empty dict so boot continues rather than crash-looping.
...
def load_spec_findings_safe(path: Path | str) -> dict[str, Any]:
    ...
    yaml.scanner.ScannerError), quarantines the corrupt file, logs the
    structured event, and returns an empty dict rather than propagating
    the exception.
    """
    p = Path(path)
    if not p.exists():
...
def spec_findings_yaml_writer_must_use_atomic_tmp_rename_partial_write_corruption_kills_bob3_boot_scannererror_mapping_values_not_allowed(
    ...
    if quarantine_if_corrupt and p.exists():
        try:
            ...
            if result is None or isinstance(result, dict):
                # File is valid — fall through to normal write if data provided
                pass
            else:
                quarantine_corrupt_findings(p)
