...
from spec_synthesis.uncertainty_loop import (
    ...
    trigger_clarification_questions as _trigger_clarification_questions,
)
...
def mark_ambiguous_slots(
    ...
    *,
    ...
    Groups stubs by slot name, computes a disagreement rate for each
    observable dimension (return type, raised exceptions, side effects),
    and returns slots whose rate exceeds ``threshold``.
...
def trigger_user_question(
    ...
    *,
    ...
    present, raises :class:`SpecNeedsHumanError` rather than confabulate.
    ...
    -------
    list[ClarificationAnswer]
        One answer per disagreement slot (interactive mode), or empty
