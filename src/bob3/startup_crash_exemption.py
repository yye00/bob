...
from bob3.run_loop import load_exemption_sidecar, increment_exemption_count  # type: ignore[import]

logger = logging.getLogger(__name__)

# Default sidecar directory (mirrors the orchestrator default).
_DEFAULT_SIDECAR_DIR: str = os.environ.get(
    "BOB3_STARTUP_EXEMPT_DIR",
    os.path.join(os.path.dirname(__file__), "..", "..", ".bob3_startup_exempt"),
)
...
def apply_startup_crash_exemption(
    ...
    *,
    ...
    sidecar_dir:
        ...
        ``None``, defaults to ``BOB3_STARTUP_EXEMPT_DIR`` env var or the
        built-in default next to the bob3 package root.
    ...
    if sidecar_dir is None:
        sidecar_dir = os.environ.get("BOB3_STARTUP_EXEMPT_DIR", _DEFAULT_SIDECAR_DIR)
