...
:mod:`bob3.orchestrator.prompt_source_reloader`, which maintains an
...
__all__ = ["reload_prompt_source_if_changed", "reload_prompt_sources", "reload_prompt_sources_if_changed"]
...
def reload_prompt_source_if_changed(module_name: str = "bob3.superpowers") -> bool:
    ...
    ``bob3.orchestrator.prompt_source_reloader._PROMPT_SOURCE_MODULES``)
    ...
    Args:
        module_name: Dotted Python module name to check and, if stale,
            reload.  Defaults to ``bob3.superpowers``, which is the
