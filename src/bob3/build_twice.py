...
Controlled by BOB3_BUILD_TWICE env var (default False).
Log nondeterminism events to .bob3/progress.jsonl and reviews/findings.yaml.
"""
...
def is_build_twice_enabled() -> bool:
    """Return True when BOB3_BUILD_TWICE env var is set to a truthy value."""
    val = os.environ.get("BOB3_BUILD_TWICE", "").strip().lower()
...
def _emit_progress_event(workspace: Path, feature_id: str, payload: dict) -> None:
    """Append a nondeterminism_detected event to .bob3/progress.jsonl."""
    progress_path = workspace / ".bob3" / "progress.jsonl"
    try:
