"""Top-level reaper module for bob3 — exponential backoff after reaper-reset (b75a279b).
...
The heavy logic lives in bob3.orchestrator.reap_backoff; this module exposes
...
from bob3.startup_crash_exempt import (  # noqa: F401 — startup-crash exemption integration
    is_transport_crash,
...
def handle_exponential_backoff(
    ...
    ``Function defined: bob3.reaper.handle_exponential_backoff``.
...
def reap_zombie_runs(project_id: str) -> list[str]:
    ...
    Delegates to bob3.orchestrator.zombie_run_reaper.scan_and_reap.
...
def update_reap_tracking(
    ...
    bob3.reaper.update_reap_tracking). It stamps last_reap_at and reap_count
...
def reap_stuck_executing(
    ...
    AC entry point: "Function defined: bob3.reaper.reap_stuck_executing"
...
# Alias satisfying AC: "Function defined: bob3.reaper.apply_exponential_backoff"
apply_exponential_backoff = handle_exponential_backoff

# Alias satisfying AC: "Function defined: bob3.reaper.reset_feature_status"
reset_feature_status = reset_stuck_feature

# Alias satisfying AC: "Function defined: bob3.reaper.close_zombie_sub_agent_runs"
...
def sweep_orphans_on_exit(project_id: str) -> list[str]:
    ...
    AC entry point: "Function defined: bob3.reaper.sweep_orphans_on_exit"
    ...
    bob3.final_reaper.sweep_orphans_on_exit which:
    ...
    """
    from bob3.final_reaper import sweep_orphans_on_exit as _impl  # noqa: PLC0415
