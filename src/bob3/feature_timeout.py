...
``BOB3_FEATURE_TIMEOUT_SECONDS`` environment variable; this module is the named,
importable home for that contract so callers (and tests) do not have to reach
into ``bob3.orchestrator.run_loop`` internals.
"""
...
from bob3.timeout import FeatureTimeoutError, FeatureTimeoutManager  # noqa: F401 — re-exported for AC
...
def resolve_feature_timeout_seconds() -> float:
    ...
    Reads ``BOB3_FEATURE_TIMEOUT_SECONDS`` from the environment; falls back to
...
async def enforce_feature_timeout(
    ...
    *,
    ...
    Delegates to :func:`bob3.timeout.enforce_wall_clock_timeout`.
