...
Background
----------
Bob3 gen-N installs its CLI as ``bobN`` (e.g. ``bob14``) via the editable
install entry_points. A naive ``pgrep bob3 run`` misses a running
...
orchestrator is alive before the lock file may be removed:
    ...
    1. No process matching ``bob[0-9]+ run`` (or ``bob3 run``) in argv
...
from bob3.orchestrator.probe_ancestry import collect_ancestor_pids, is_shell_wrapper

logger = logging.getLogger(__name__)

# Regex matches: 'bob3 run', 'bob14 run', '/path/to/bob14 run', etc.
...
def is_orchestrator_alive() -> bool:
    """Return True if any process matching 'bob[0-9]+ run' (or 'bob3 run') is alive.
    ...
    F-R7-580 (bob3 version 17 r1): substring-match on cmdline was too loose;
    the parent bash whose eval string contained "bob17 run" tripped the guard
    on every launch, blocking the bob3 version 17 boot indefinitely.
...
def safe_to_remove_lock(
    ...
    This is the three-signal gate used before removing .bob3.lock.
    """
    lock_path = pathlib.Path(lock_path)
...
def remove_stale_lock(
    ...
    AC: Function defined: bob3.orchestrator.remove_stale_lock
    """
    lock_path = pathlib.Path(lock_path)
