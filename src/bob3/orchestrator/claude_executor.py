...
# them via bob3.orchestrator.claude_executor.project_spawn_cost / should_spawn.
from bob3.orchestrator.cost_projection import (  # noqa: E402
    project_spawn_cost,
    should_spawn,
)

# Integration: crash-classifier imported so callers can access it via
# bob3.orchestrator.claude_executor.classify_exit (F-R6-300).
from bob3.orchestrator.crash_classifier import (  # noqa: E402
    classify_exit,
...
# This lets bob3 work against gateways (e.g. AMD APIM, Vertex deployments)
...
def _persist_failure_artifacts(
    *,
    ...
    """Write full sub-agent stderr (and any response text) to .bob3/agent_logs/.
    ...
    """
    try:
        log_dir = Path.cwd() / ".bob3" / "agent_logs"
...
class MessageStreamHandler:
    ...
    async def consume(
        ...
        *,
        ...
        message arrives within BOB3_SUBAGENT_IDLE_TIMEOUT_SECONDS, raise
        ...
        import os as _os
        try:
            _idle = float(_os.environ.get("BOB3_SUBAGENT_IDLE_TIMEOUT_SECONDS", "300"))
        ...
        # respawn (BOB3_SUBAGENT_IDLE_TIMEOUT_SECONDS is retained only as a hint
...
async def spawn_sub_agent(
    *,
    ...
    try:
        ...
        # sub-agent. Round 5 accumulated 59 orphan bob3.memory_mcp
        ...
        try:
            from bob3.mcp_lifecycle import unregister_mcp
