"""Disk-state reconciler for Bob3 (feature 2f69b554).
...
Run :func:`reconcile_from_disk` once at the start of every ``bob3 run``
...
Integration: called from ``bob3.orchestrator.run_loop._run_locked`` before
...
def reconcile_from_disk(project_id: str, workspace: pathlib.Path | None = None) -> int:
    ...
    # loop. Cap total time via BOB3_RECONCILE_BUDGET_SECONDS (default 120); on
    # exceed, stop and let the normal executor handle the rest.
    budget_raw = os.environ.get("BOB3_RECONCILE_BUDGET_SECONDS")
    try:
