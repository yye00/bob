...
rather than the UUID id field, which is minted fresh in every `bob3 init`.
"""
...
def compares_by_spec_slot(
    ...
    import os
    db_path = os.environ.get("BOB3_DATABASE_PATH")
...
def set_diff_uses_spec_slot() -> bool:
    ...
    get_completed_spec_slots() from bob3.migrations.add_spec_slot, which
...
def _assert_spec_slot_column(db_path: Union[str, Path]) -> None:
    ...
    if "spec_slot" not in cols:
        raise ValueError(
            ...
            "run bob3.migrations.add_spec_slot.upgrade() before using "
