"""Final reaper sweep on orchestrator exit — flips orphan 'executing' rows to 'failed'.

Implements the fix for the pattern where the orchestrator exits ALL_BLOCKED or
...
def _get_subagent_reaper():
    """Lazy import of subagent_reaper to break potential circular imports."""
    import importlib  # noqa: PLC0415
    return importlib.import_module("bob3.orchestrator.subagent_reaper")


def sweep_orphan_subagents():
    """Module-level proxy so patch('bob3.orchestrator.final_reaper.sweep_orphan_subagents') works in tests."""
    return _get_subagent_reaper().sweep_orphan_subagents()


def find_subagent_pid_for_feature(feature_id: str) -> list[int]:
    """Module-level proxy so patch('bob3.orchestrator.final_reaper.find_subagent_pid_for_feature') works in tests."""
...
def sweep_orphans_on_exit(project_id: str) -> list[str]:
    ...
    Invoked at ALL_BLOCKED/BUDGET_EXCEEDED termination to flip any 'executing' rows
    whose sub-agents have already exited to 'failed', preventing them from polluting
    inter-gen status reports indefinitely.
