...
* :func:`enforce_feature_timeout` — wraps a coroutine with a hard wall-clock
  ...
  expiry, and raises :class:`bob3.timeout.FeatureTimeoutError`.
...
--------------------
``BOB3_FEATURE_TIMEOUT_SECONDS`` (float, seconds) — configures the hard
...
_STARTUP_EXEMPT_DIR = ".bob3_startup_exempt"
...
async def enforce_feature_timeout(
    ...
    *,
    ...
    Delegates to :func:`bob3.timeout.enforce_wall_clock_timeout`, which wraps
    *coro* with ``asyncio.wait_for`` and raises :class:`FeatureTimeoutError`
    ...
    Args:
        ...
        timeout_seconds: Hard deadline in seconds.  When ``None``, reads
            ``BOB3_FEATURE_TIMEOUT_SECONDS`` from the environment (default
            1800 s).
...
def classify_timeout_attempt(
    ...
    *,
    ...
    The startup-exempt sentinel is stored in ``<workspace>/.bob3_startup_exempt/``
