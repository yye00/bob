...
``.bob3/progress.jsonl`` and (when available) the session log — to
...
# Event types written to ``.bob3/progress.jsonl`` that indicate the
...
def classify_sub_agent_exit(
    ...
    ----------
    progress_jsonl_path:
        Path to the sub-agent's ``.bob3/progress.jsonl``. May be
