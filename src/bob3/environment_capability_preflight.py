...
It re-exports the full API from ``bob3.preflight`` so that both
``bob3.environment_capability_preflight.*`` and the orchestrator integration
work via a single underlying implementation.
"""

from __future__ import annotations

from bob3.preflight import (  # noqa: F401
...
def probe_dependencies(ac_list):  # noqa: F811  # explicit re-export with same signature
    ...
    """
    from bob3.preflight import probe_dependencies as _probe_dependencies
...
def discover_workaround(probe_result):  # noqa: F811  # explicit re-export
    ...
    """
    from bob3.preflight import spawn_workaround_research
