...
spawn_next_generation.sh rsync-copies the parent bob3.db without re-running
``bob3 init``. This leaves ``projects.name`` pointing at the parent generation
...
Both delegate to ``bob3.run_loop.verify_project_metadata`` so the correction
...
def verify_project_metadata(
    ...
    Args:
        ...
        db_path: Path to the bob3.db database. Defaults to the
            ``BOB3_DATABASE_PATH`` env var or ``<workspace>/bob3.db``.
    ...
    """
    from bob3.run_loop import verify_project_metadata as _run_loop_verify, ProjectMetadataCheckResult  # noqa: F401
    ...
    if result.spec_path_was_stale:
        logger.warning(
            ...
            "re-run 'bob3 init --spec <correct-spec>' to fix."
        )
...
def reinit_after_spawn(
    ...
    Args:
        workspace: Workspace root directory.  Defaults to ``Path.cwd()``.
        db_path: Path to the bob3.db database.  See ``verify_project_metadata``.
