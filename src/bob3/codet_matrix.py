...
Both delegate to bob3.orchestrator.codet_triangulation for the heavy lifting.
This module exists as the stable, importable entry point required by the
integration AC (bob3.codet_matrix.*).

Integration hook: importing bob3.orchestrator triggers the codet_triangulation
module to be available via bob3.orchestrator.codet_triangulation.
"""
