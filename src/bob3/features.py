...
from bob75.bootstrap_override import should_bypass_readiness_gate
...
def add_spec_slot_column(
    ...
    Delegates to the add_spec_slot migration (idempotent).

    Parameters
    ----------
    db_path:
        Path to the SQLite database. Defaults to ``BOB3_DATABASE_PATH`` env var
        or ``./bob3.db``.
    spec_path:
        Optional path to a spec YAML file. When provided, rows with NULL
        spec_slot are backfilled by matching feature name to spec keys.
    """
...
def backfill_spec_slot(
    ...
    Runs the add_spec_slot migration (idempotent) then backfills NULL spec_slot
    rows by matching feature name against spec YAML keys.
