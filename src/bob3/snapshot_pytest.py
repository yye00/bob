"""bob3.snapshot_pytest — deterministic pytest snapshot enforcement.
...
enforce_maxfail_with_xdist(argv) -> list[str]
    Alias for enforce_maxfail_zero — enforces --maxfail=0 specifically
    at the xdist snapshot boundary to prevent non-deterministic early halt.
"""
...
__all__ = ["enforce_maxfail_zero", "enforce_maxfail_with_xdist", "MAXFAIL_ZERO"]
...
def enforce_maxfail_with_xdist(argv: list[str]) -> list[str]:
    """Enforce --maxfail=0 at the xdist snapshot boundary.

    pytest with xdist halts after ~20-25 failures non-deterministically,
    making before/after snapshots contain different subsets. This function
    strips any existing --maxfail flag and injects --maxfail=0 immediately
    after the first element (the pytest command), ensuring it appears before
    any xdist -n / --numprocesses flags.
