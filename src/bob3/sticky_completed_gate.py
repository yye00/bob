...
def is_sticky_completed(
    ...
    Args:
        ...
        db_path: Optional explicit path to the SQLite database. When None, the
            bob3 default db path is used (resolved from environment or cwd).
...
def _resolve_db_path() -> pathlib.Path:
    """Return the default bob3 DB path from environment or cwd convention."""
    import os

    env_path = os.environ.get("BOB3_DB_PATH")
    if env_path:
