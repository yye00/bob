"""bob3.linters.full_22_smell — Full 22-smell spec linter (F-R7-410 extension).

Provides the same public API as ``bob3.linter`` under the
``bob3.linters.full_22_smell`` namespace so that integration code can
import from either location.

Severities E/W/I; E-severity findings block ``bob3 plan --create``.
...
Public API::
    ...
    from bob3.linters.full_22_smell import detect_smells, SmellFinding, blocks_plan_create
