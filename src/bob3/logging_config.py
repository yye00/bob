"""Structured Python logging configuration for Bob3.
...
BOB3_LOG_FORMAT = "[%(asctime)s] [%(levelname)s] [%(component)s] %(message)s"


class Bob3Formatter(logging.Formatter):
    ...
    For example, "bob3.orchestrator.claude_executor" yields "claude_executor".
    """
...
def setup_logging(*, verbose: bool = False, level: int | None = None) -> None:
    """Configure the bob3 logger with structured output to stderr.
    ...
    if not any(isinstance(h, _StderrHandler) for h in logger.handlers):
        ...
        handler.setFormatter(Bob3Formatter())
