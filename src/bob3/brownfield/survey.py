...
class RepoMapperHandle:
    ...
    F-R7-611: This is the thin enforcement layer — bob3 delegates symbol-graph
...
def survey(
    ...
    This is the main entry point for brownfield survey in bob3.
    RepoMapper does the heavy lifting; bob3 is just the thin MCP client.
    """
    if db_path is None:
...
def _walk_sources(workspace: Path) -> list[Path]:
    """Walk workspace, respecting .gitignore simple patterns and .bob3ignore."""
    ignore_dirs: set[str] = {'.git', '__pycache__', '.venv', 'node_modules', '.bob3'}
    bob3ignore = workspace / '.bob3ignore'
    extra_ignore: set[str] = set()
    if bob3ignore.exists():
        for line in bob3ignore.read_text().splitlines():
            line = line.strip()
...
def build_survey(
    ...
    Args:
        ...
        db_path:   Path for survey.db.  Defaults to workspace/.bob3/survey.db.

    Returns:
        List of dicts with keys {name, path, lineno, docstring, kind}.
    """
    if db_path is None:
        bob3_dir = workspace / '.bob3'
        bob3_dir.mkdir(exist_ok=True)
        db_path = bob3_dir / 'survey.db'
...
def run_repomapper_mcp(
    ...
    Delegates to launch_repomapper_mcp — bob3 is a thin MCP client, not
...
def index_repository(
    ...
    This is the canonical BF-1 entry point for `bob3 init --brownfield`.
...
def walk_repo(
    ...
    "Function defined: bob3.brownfield.survey.walk_repo".
...
def parse_and_extract_symbols(
    ...
    *,
    ...
    "Function defined: bob3.brownfield.survey.parse_and_extract_symbols".
...
def incremental_update(
    ...
    "Function defined: bob3.brownfield.survey.incremental_update".
