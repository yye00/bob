...
def write_worker_config(
    ...
    *,
    ...
    1. ``.bob3/features/<id>/settings.json`` — permission allowlist for the worker.
    2. ``.bob3/features/WORKER.md`` — slim per-feature context (feature title,
    ...
    Args:
        ...
        workspace:   Project workspace path (used for WORKER.md). If None,
                     defaults to the parent of bob3_dir.
    ...
    if workspace is None:
        workspace = bob3_dir.parent

    settings_path = write_feature_settings(
        feature, bob3_dir=bob3_dir, extra_allow=extra_allow
    )
