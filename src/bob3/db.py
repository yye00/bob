"""Database connection and initialization for Bob3.
...
from bob3.models import BugLedger, CalibrationAlert, CalibrationData, ConfidenceHistory, EvidenceArtifact, ExecutionLog, Feature, FeatureDependency, FeatureReviewIssue, FlakyTestRun, ForgettingEvent, Project, ReadinessHistory, RegressionEvent, ResourceCheckpoint, RollbackEvent, ResearchResult, ReviewHistory, ScopeChange, SubAgentRun, Task
...
def _row_to_feature(row: tuple) -> Feature:
    ...
    for bool_field in ("exceeds_size_limits", "tdd_mode", "sub_agent_mode"):
    ...
    with connect() as conn:
        conn.execute(
            """INSERT INTO features
               (id, project_id, parent_feature_id, decomposition_depth,
                name, description, acceptance_criteria, status,
                priority, tdd_mode, sub_agent_mode, risk_category, created_at, updated_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (fid, project_id, parent_feature_id, decomposition_depth,
             name, description, acceptance_criteria, status,
             priority, tdd_mode, sub_agent_mode, risk_category, now.isoformat(), now.isoformat()),
        )
...
def assess_feature_confidence(feature_id: str) -> dict[str, float]:
    ...
    # Overall readiness: minimum of all dimensions
...
def detect_regression(
    *,
    ...
    test_to_feature_map: dict[str, str] | None = None,
    ...
    - Creates a regression_events record
    - Updates the affected feature's status to 'regression'
    ...
    Args:
        ...
        test_to_feature_map: Optional dict mapping test name -> feature_id.
            Used to determine which feature is affected. If not provided,
            the first affected feature found in the project is used.

    Returns:
        RegressionEvent if regressions were detected, None otherwise.
    """
    ...
    # Determine the affected feature
    affected_feature_id = None
    if test_to_feature_map:
        # Use the mapping to find which feature owns the failing tests
        feature_ids = {
            test_to_feature_map[t]
            for t in newly_failing
            if t in test_to_feature_map
        }
        if feature_ids:
            affected_feature_id = next(iter(feature_ids))

    # Fallback: try to find the affected feature from project's completed features
    if affected_feature_id is None:
        features = list_features(project_id=project_id, status="completed")
        if features:
            # Use the first completed feature that isn't the causing feature
            for f in features:
                if f.id != causing_feature_id:
                    affected_feature_id = f.id
                    break

    # If we still don't have an affected feature, use causing as self-regression
    if affected_feature_id is None:
        affected_feature_id = causing_feature_id
    ...
    event = create_regression_event(
        ...
        affected_feature_id=affected_feature_id,
        causing_feature_id=causing_feature_id,
        affected_tests=json.dumps(sorted(newly_failing)),
    )

    # Update the affected feature's status to 'regression'
    if affected_feature_id != causing_feature_id:
        update_feature(affected_feature_id, status="regression")
...
def diff_features(
    ...
    """
    old_by_name = {f["name"]: f for f in old_features}
    new_by_name = {f["name"]: f for f in new_features}
    ...
    if has_changes:
        ...
        for feat in changes["added"]:
            ...
            create_feature(
                ...
                priority=feat.get("priority", 100),
        ...
        # Log the change
        details = json.dumps({
            "added": [f["name"] for f in changes["added"]],
            "modified": [f["name"] for f in changes["modified"]],
            "removed": [f["name"] for f in changes["removed"]],
        })
        log_event(
            project_id=project_id,
