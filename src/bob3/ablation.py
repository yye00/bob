"""BOB3_ABLATION_MODE environment variable and CLI flag support.
...
The active mode is read from the BOB3_ABLATION_MODE environment variable
...
def _promotion_value() -> int | None:
    ...
    from bob3.learnings import get_promote_on_n  # local import avoids circular
...
def get_ablation_mode() -> AblationMode:
    """Return the active AblationMode from the BOB3_ABLATION_MODE env var.
    ...
    """
    raw = os.environ.get("BOB3_ABLATION_MODE")
    if raw is None:
    ...
    if mode is None:
        ...
        raise ValueError(
            f"BOB3_ABLATION_MODE={raw!r} is not a valid ablation mode. "
