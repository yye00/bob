...
2. ``BOB3_FEATURE_TIMEOUT_SECONDS`` environment variable.
...
def resolve_execution_timeout_seconds() -> float:
    ...
    try:
        ...
        logger.warning(
            "Invalid BOB3_FEATURE_TIMEOUT_SECONDS=%r; using default %.0fs",
            raw,
    ...
    if value <= 0:
        logger.warning(
            "Non-positive BOB3_FEATURE_TIMEOUT_SECONDS=%r; using default %.0fs",
            raw,
            _DEFAULT_TIMEOUT_SECONDS,
        )
        return _DEFAULT_TIMEOUT_SECONDS
    ...
    *,
    ...
    Args:
        ...
        timeout_seconds: Explicit override.  When ``None`` (default), reads
            ``BOB3_FEATURE_TIMEOUT_SECONDS`` from the environment.  Must be a
