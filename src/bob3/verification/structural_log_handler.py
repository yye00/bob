"""Structural log-line AC handler for bob3 verification.

Provides ``match_log_line_ac`` — a thin public alias over
``bob3.enhanced_verification.handle_structural_log_line`` — so that ACs of the
form::

    Function defined: bob3.verification.structural_log_handler.match_log_line_ac

are satisfiable from the ``bob3.verification`` namespace without duplicating
logic.
...
def match_log_line_ac(
    *,
    ...
    Delegates to :func:`bob3.enhanced_verification.handle_structural_log_line`
    ...
    Args:
        criterion_body: AC text *after* stripping the ``structural:`` prefix,
            e.g. ``"src/bob3/run_loop.py emits a 'Run finished:
