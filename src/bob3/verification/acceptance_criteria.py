"""bob3.verification.acceptance_criteria — AC checking entry point.

Exposes acceptance-criteria validation functions from the canonical
``bob3.enhanced_verification`` and ``bob3.verification.verifier`` implementations
under the ``bob3.verification.acceptance_criteria`` namespace.

This module satisfies the integration AC

    integration: bob3.verification.acceptance_criteria

by providing a single importable surface for the Pattern 8 integration
AC resolver (``bob3.verification.integration_ac_resolver``) and the
...
resolve_integration_criterion(criterion, workspace) -> tuple[bool, str]
    Delegates to ``bob3.verification.integration_ac_resolver.resolve_integration_ac``
...
from bob3.verification.integration_ac_resolver import (  # noqa: F401
...
def check_criterion(
    ...
    Delegates to ``bob3.enhanced_verification._check_criterion`` for all
...
def resolve_integration_criterion(
    ...
    ``bob3.verification.integration_ac_resolver.resolve_integration_ac``.
    """
