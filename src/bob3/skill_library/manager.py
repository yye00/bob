"""Module-level API for the Voyager-style persistent skill library (F-R7-477).

Provides two module-level functions required by ACs:
- search_skill_library: search by semantic similarity on env preflight.
- write_skill_to_library: persist a newly discovered workaround shim.

Both delegate to bob3.skill_library.registry for storage operations.
...
def write_skill_to_library(
    ...
    """Write a newly discovered workaround shim into the persistent skill library.
    ...
    Raises:
        ValueError: If capability_description or shim_module_src is empty or
            whitespace-only.
