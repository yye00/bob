...
Fix applied across THREE sources (all must be correct simultaneously):
  ...
  2. ``cli/__init__.py`` ``bob3 init`` command — sets max_cost_usd from BOB3_MAX_COST_USD
     on the INSERT, not relying on the schema default.
  3. ``db.create_project`` — env-aware default when max_cost_usd is None.
  4. ``models.Project.max_cost_usd`` — Field default_factory reads BOB3_MAX_COST_USD.
...
Rules
-----
- Empty or malformed ``BOB3_MAX_COST_USD`` → unlimited (1_000_000.0), NEVER 0.
- Per-attempt cap (``BOB3_PER_ATTEMPT_COST_CAP``) and per-feature telemetry
  ceiling (``BOB3_PER_FEATURE_COST_CEILING``) remain in force.
"""
...
def resolve_max_cost_usd() -> float:
    ...
    Reads ``BOB3_MAX_COST_USD`` from the environment.  An absent, empty, or
...
def per_project_cost_cap_must_env_overridable_default() -> dict[str, object]:
    ...
    Returns a status dict suitable for inclusion in healthcheck output::

        {
            ...
            "source": "default",   # or "BOB3_MAX_COST_USD"
        }
    ...
    """
    from bob3.models import Project  # local import to avoid circular deps

    env_val = os.environ.get("BOB3_MAX_COST_USD", "")
    ...
    source = "BOB3_MAX_COST_USD" if env_val else "default"
