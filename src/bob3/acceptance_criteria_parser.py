"""Acceptance criteria parser — sixth EARS-style behavior AC grammar.

Adds the ``behavior: <subject> <verb> <object> when <condition>`` grammar
to bob3's acceptance criteria layer. Strings are parsed at load time into a
structured :class:`BehaviorCriterion` named-tuple so the evaluator can
verify each discrete field rather than relying on freeform prose.

Public API
----------
BehaviorCriterion
    Named-tuple with fields ``subject``, ``verb``, ``object_``, ``condition``.
parse_behavior_criterion(ac) -> BehaviorCriterion | None
    Parse a ``behavior:`` AC string. Returns ``None`` for non-behavior ACs,
    raises ``ValueError`` for malformed behavior ACs (prefix present but
    ``when`` clause missing).
"""
...
def parse_behavior_criterion(ac: str) -> BehaviorCriterion | None:
    ...
    Examples::
        ...
        >>> parse_behavior_criterion("behavior: parser returns BehaviorCriterion when AC matches grammar")
        BehaviorCriterion(subject='parser', verb='returns', object_='BehaviorCriterion',
