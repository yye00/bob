"""AC artifact-existence verifier for the bob3 package.
...
check_baseline_collection(workspace, *, test_dir, timeout) -> CollectionResult
    ...
    Re-exported from bob3.stable_baseline_gate for integration use.
...
from bob3.stable_baseline_gate import (  # noqa: F401 — stable baseline gate integration
...
def check_artifact_exists(ac_text: str, workspace: Union[str, Path]) -> bool:
    ...
    Args:
        ac_text: A single acceptance criteria string, e.g.
            ``"File exists: src/bob3/foo.py"`` or
            ``"Function defined: bob3.foo.my_func"``.
