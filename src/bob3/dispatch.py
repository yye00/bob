...
try:
    from bob3.extended_thinking import classifier as _extended_thinking_classifier
...
def should_refuse_redispatch(
    ...
    Delegates to bob3.reaper.should_refuse_redispatch to apply exponential
    backoff and needs_human escalation logic.  Integrated here so the
    dispatch layer (bob3.dispatch) satisfies the integration AC.
    ...
    """
    from bob3.reaper import should_refuse_redispatch as _refuse  # noqa: PLC0415
...
def compute_backoff_deadline(
    ...
    from bob3.orchestrator.reap_backoff import compute_backoff_seconds  # noqa: PLC0415
...
def should_refuse_redispatch_after_reap(
    ...
    This is the canonical AC-required entry point on bob3.dispatch for the
    ...
    Delegates to bob3.reaper.should_refuse_redispatch for the core logic.
...
def check_reap_backoff(
    ...
    This is the AC-required entry point on bob3.dispatch for the exponential
...
# Re-export should_refuse_recent_reap from bob3.dispatch_backoff so that the
# integration AC "integration: bob3.dispatch" is satisfied by a real callable
# on this module.

def _import_dispatch_backoff_integration() -> None:
    """Register dispatch_backoff functions on this module namespace."""
    import bob3.dispatch_backoff as _db  # noqa: PLC0415
...
def write_feature_settings(
    ...
    *,
    ...
    """Write per-feature settings.json under *.bob3/features/<id>/settings.json*.
    ...
    settings_dir = bob3_dir / "features" / feature_id
...
def spawn_worker_with_cache(
    ...
    *,
    ...
    (C) Per-worker settings — writes .bob3/features/<id>/settings.json and
    ...
    worker_md_path = bob3_dir / "features" / "WORKER.md"
    ...
    if _EXTENDED_THINKING_AVAILABLE:
        ...
        worker_env["BOB3_EXTENDED_THINKING"] = "1" if et_enabled else "0"
...
def spawn_worker(
    ...
    *,
    ...
    AC-required entry point on bob3.dispatch. Delegates to spawn_worker_with_cache,
...
def get_repo_tree(workspace: str | Path, *, max_lines: int = _REPO_TREE_MAX_LINES) -> str:
    ...
    bob3.dispatch.get_repo_tree to satisfy the function-defined AC for
...
def generate_worker_settings(
    ...
    *,
    ...
    Returns:
        Path to the written settings.json file.
    """
    return write_feature_settings(feature, bob3_dir=bob3_dir, extra_allow=extra_allow)
