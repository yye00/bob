...
def verify_and_reinit_after_spawn(
    ...
    Delegates to ``bob3.run_loop.verify_project_metadata``, which:
    ...
    workspace:
        ...
        ``ValueError``.
    db_path:
        Path to the bob3.db database.  Defaults to ``BOB3_DATABASE_PATH``
        env var or ``<workspace>/bob3.db``.
    ...
    """
    from bob3.run_loop import verify_project_metadata, ProjectMetadataCheckResult  # noqa: F401
