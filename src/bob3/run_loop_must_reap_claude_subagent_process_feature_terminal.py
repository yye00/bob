...
Closes the orphan subagent resource leak observed in bob3 version 14 round 11:
...
completion handler so that other modules can import from the top-level bob3
...
def reap_subagent_for_feature(feature_id: str) -> list[int]:
    """Re-export of bob3.orchestrator.subagent_reaper.reap_subagent_for_feature.

    Lazy import avoids circular dependency (bob3.orchestrator.__init__ →
    run_loop → this module → bob3.orchestrator.subagent_reaper).
    """
...
def _sweep_orphans() -> list[tuple[str, int]]:
    """Lazy wrapper around bob3.orchestrator.subagent_reaper.sweep_orphan_subagents."""
...
def run_loop_must_reap_claude_subagent_process_feature_terminal(
    ...
    to bob3.orchestrator.subagent_reaper.reap_subagent_for_feature which:
