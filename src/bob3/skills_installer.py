...
def get_progress_path() -> Path:
    ...
    Intentionally mirrors ``bob3.progress_events.get_progress_path`` so
    callers can patch ``bob3.skills_installer.get_progress_path`` in tests
    without pulling in the full progress_events module.
    """
    from bob3.progress_events import get_progress_path as _get
...
def log_skill_activation(
    ...
    """Emit a structured skill_activation_logged event to .bob3/progress.jsonl.
...
# in ``bob3.orchestrator.claude_executor.spawn_evaluator_agent`` opts in
...
def install_skills_to_workspace(
    ...
    *,
    ...
    receive them. Any pre-existing bob3-managed install of an excluded
    ...
    for skill_src in sorted(source_dir.iterdir()):
        ...
        # default (implementation-agent) install. If a previous bob3
        # version installed them here, remove the bob3-managed entry so
        # the implementation workspace is brought back into compliance.
        # User-owned entries with the same name are left alone — we only
        # touch what we own (see ``_is_bob3_managed``).
        if skill_src.name in EVALUATOR_ONLY_SKILLS and not include_evaluator_skills:
            ...
            elif dest.is_dir() and _is_bob3_managed(dest, source_dir):
                _remove_dest(dest)
                logger.info(
