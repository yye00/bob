"""Seventh AC grammar: property-based acceptance criteria for bob3.
...
Delegates to :mod:`bob3.spec_quality.example_grammar` for the actual parsing
and emitting logic. This module is the canonical ``bob3``-namespaced entry
...
def parse_key_example_ac(ac: dict | str | None) -> "KeyExample | None":
    ...
    Delegates to :func:`~bob3.spec_quality.example_grammar.parse_key_example`.
    This is the canonical ``bob3``-namespaced entry point for parsing
...
def generate_hypothesis_test(prop: "PropertyAC", *, seed: int = 0) -> str:
    ...
    requirement: ``bob3.property_based_ac.generate_hypothesis_test``.

    Args:
        prop: A parsed :class:`~bob3.spec_quality.example_grammar.PropertyAC`.
...
def generate_parametrized_pytest(
    ...
    *,
    ...
    requirement: ``bob3.property_based_ac.generate_parametrized_pytest``.

    Args:
        examples:  List of :class:`~bob3.spec_quality.example_grammar.KeyExample` objects.
