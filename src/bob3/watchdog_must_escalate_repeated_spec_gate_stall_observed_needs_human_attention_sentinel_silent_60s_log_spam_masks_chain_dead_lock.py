...
~3h of wall time was lost silently (bob3 version 13 r10, 2026-05-28).

Fix: after N consecutive stall observations (default 5, overridable via
BOB3_STALL_ESCALATION_COUNT), escalate to a distinct needs_human_attention
sentinel:
...
def _read_escalation_threshold() -> int:
    """Return BOB3_STALL_ESCALATION_COUNT as int, or _DEFAULT_ESCALATION_COUNT."""
    raw = os.environ.get("BOB3_STALL_ESCALATION_COUNT")
    if not raw:
