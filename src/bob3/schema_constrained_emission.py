"""Schema-constrained spec emission — bob3 integration facade.

Exposes ``emit_with_schema_constraint``, ``emit_with_schema``,
``validate_and_reject_invalid``, and ``validate_spec_against_schema`` as the
canonical bob3 entry-points for schema-constrained spec emission.  All
...
Public API::
    ...
    from bob3.schema_constrained_emission import (
...
Integration: bob3.orchestrator imports this module so that spec validation
...
# __file__ = src/bob3/schema_constrained_emission.py → 3 parents up = workspace root
...
def emit_with_schema_constraint(
    ...
    *,
    ...
    Canonical entry point for schema-constrained spec emission in the bob3
