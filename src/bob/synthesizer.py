"""bob.synthesizer — guaranteed boundary + error-path AC coverage and object-format LLM parsing.
...
def parse_criteria_response(response_text: str) -> list[str] | None:
    ...
    for x in parsed:
        if isinstance(x, str):
            ...
            # Extract criterion text from recognized keys; never str(dict).
...
def inject_boundary_error_criteria(
    criteria: list[str],
    title: str = "",
) -> list[str]:
    ...
    if not isinstance(criteria, list):
        raise TypeError(
            f"inject_boundary_error_criteria: criteria must be a list, got {type(criteria).__name__!r}"
        )
    for item in criteria:
        if not isinstance(item, str):
            raise ValueError(
                f"inject_boundary_error_criteria: all criteria must be strings, "
                f"got {type(item).__name__!r}: {item!r}"
            )

    # Build a probe string from all ACs, but for structural ACs (pytest:, File exists:, etc.)
    # only check the description portion after " — " to avoid false matches on slugs.
    # Pure prose ACs (no structural prefix) are included in full.
