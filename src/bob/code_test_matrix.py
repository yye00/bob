...
``bob.code_test_matrix.*``.  Heavy lifting is in
``bob.orchestrator.codet_triangulation``; this module re-exports everything and
adds the ``spawn_kxk_matrix`` convenience function required by the AC suite.

Public API::

    from bob.code_test_matrix import (
        spawn_kxk_matrix,          # AC: Function defined: bob.code_test_matrix.spawn_kxk_matrix
        mutual_agreement_score,    # AC: Function defined: bob.code_test_matrix.mutual_agreement_score
    )
...
def spawn_kxk_matrix(
    ...
    sycophancy that the recursive bob framework has documented.
