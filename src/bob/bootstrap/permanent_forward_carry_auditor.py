...
class PermanentForwardCarryMissing(BootstrapAuditError):
    ...
    the ``Function defined: bob.permanent_forward_carry_auditor.PermanentForwardCarryMissing``
...
def required_feature_ids() -> frozenset[str]:
    ...
    BOB_PERMANENT_CARRY_IDS as a comma-separated list.

    Returns:
        Frozen set of required feature ID strings.
    """
    base = set(_CANONICAL_REQUIRED_IDS)
    extra_env = os.environ.get("BOB_PERMANENT_CARRY_IDS", "").strip()
    if extra_env:
