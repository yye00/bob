"""Public subagent reaper for bob — feature terminal-state process cleanup.

Re-exports the canonical implementation from bob.orchestrator.subagent_reaper
so that code can import from the top-level ``bob`` namespace without depending
...
def sweep_orphan_subagents() -> list[tuple[str, int]]:
    ...
    Delegates to ``bob.orchestrator.subagent_reaper.sweep_orphan_subagents``
...
def reap_on_terminal_transition(feature_id: str) -> list[int]:
    ...
    AC-required entry point (Function defined: bob.subagent_reaper.reap_on_terminal_transition).
...
def reap_stale_orphans() -> list[tuple[str, int]]:
    ...
    AC-required entry point (Function defined: bob.subagent_reaper.reap_stale_orphans).
    Reaps subagents whose tagged feature is in a terminal state for >5 minutes.
