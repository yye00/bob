...
- Emits a heartbeat event to .bob/progress.jsonl every ``heartbeat_interval_s`` seconds.
...
The default timeout is read from BOB_CRITERION_EXEC_TIMEOUT (same env var used by
...
from bob.convergence_checker import check_convergence, compare_by_name  # noqa: F401
...
def _read_timeout_from_env() -> int:
    """Return BOB_CRITERION_EXEC_TIMEOUT as int, or _DEFAULT_TIMEOUT_S."""
...
class SpawnWatchdog:
    ...
    timeout_s:
        ...
        ``BOB_CRITERION_EXEC_TIMEOUT`` env var, defaulting to 3600s.
    feature_id:
        Feature identifier written into every progress event.
    progress_path:
        Path to the JSONL progress file. Defaults to ``.bob/progress.jsonl``.
