"""Root-cause analysis helpers exposed at the bob.rca top level.
...
def auto_reset_on_code_defect(
    ...
    Classification logic (see ``bob.rca_classifier``):
...
def classify_failure_cause(failed_acs: list[str]) -> Classification:
    ...
    This name satisfies the F-R7-479 AC "Function defined: bob.rca.classify_failure_cause"
...
def classify_failure_as_code_emission_defect(failed_acs: list[str]) -> bool:
    ...
    Satisfies the F-R7-479 AC "Function defined: bob.rca.classify_failure_as_code_emission_defect".
...
def should_grant_fresh_attempt_on_code_defect(
    ...
    Satisfies the F-R7-479 AC "Function defined: bob.rca.should_grant_fresh_attempt_on_code_defect".
