...
Public API::
    ...
    from bob.self_discover_selector import select_spec_sections
...
that are relevant to the feature::
    ...
    from bob.spec_critic import SpecCritic
    from bob.self_discover_selector import select_spec_sections
...
__all__ = ["select_spec_sections", "run_focused_extractor", "run_self_discover_meta_agent"]
...
def select_spec_sections(
    ...
    dict[str, str]
        ...
        every section in :func:`bob.spec_quality.section_selector.module_set`.
...
def run_self_discover_meta_agent(
    feature_id: str,
    name: str,
    description: str,
    acceptance_criteria: list[str],
    ...
    Performs the full Self-Discover meta-agent pass: selects which spec
    sections matter for this feature, then returns an extraction result that
    downstream critics and verifiers can use to skip irrelevant sections.

    This is the primary entry point for the Self-Discover meta-agent pattern
    (Agent 4 Section 7, ICML 2024) as integrated by bob's orchestrator.
