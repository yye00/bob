...
The implementation delegates to :func:`bob.run_loop.classify_evaluator_mcp_transient`,
...
def classify_evaluator_result(
    *,
    ...
    This is the public API entry point defined in ``bob.retry_classifier``
    (AC: "Function defined: bob.retry_classifier.classify_evaluator_result").
    It delegates all logic to :func:`bob.run_loop.classify_evaluator_mcp_transient`.
...
def classify_evaluator_failure(
    *,
    ...
    AC alias: "Function defined: bob.retry_classifier.classify_evaluator_failure".
...
def is_mcp_transient(stderr: str | None) -> bool:
    """Return True if stderr contains any MCP-transient token.

    AC: "Function defined: bob.retry_classifier.is_mcp_transient".
    Convenience predicate for callers that only need a boolean check
    without full classification context (no verdict/confidence/is_error
    checks applied — raw token scan only).
    """
    ...
    return result["classification"] == "mcp_transient"
...
def classify_evaluator_mcp_transient(
    *,
    ...
    AC: "Function defined: bob.retry_classifier.classify_evaluator_mcp_transient".
    Delegates to :func:`bob.run_loop.classify_evaluator_mcp_transient`.

    This function is the canonical entry point for F-R7-597 MCP-transient
    classification. It examines evaluator sub-agent exit conditions and
    determines whether a crash was caused by infrastructure noise (cert/conn
    failures) rather than real feature-correctness failures.
    """
    from bob.run_loop import classify_evaluator_mcp_transient as _classify
    return _classify(
        verdict=verdict,
        confidence=confidence,
        is_error=is_error,
...
def is_mcp_transient_failure(stderr: str | None) -> bool:
    """Return True if stderr contains any MCP-transient token.

    AC: "Function defined: bob.retry_classifier.is_mcp_transient_failure".
    Alias for :func:`is_mcp_transient` with a name that matches the F-R7-597
    spec language ("classify as retryable, not feature-failure").
    """
    return is_mcp_transient(stderr)
