"""Startup validation for bob project metadata after spawn_next_generation.sh rsync.

spawn_next_generation.sh rsync-copies the parent directory without re-running
``bob init``, so the child's bob.db can retain stale ``projects.name`` (still
...
::
    ...
    from bob.startup_validation import validate_startup_metadata
...
def validate_startup_metadata(
    ...
    """Validate and correct stale project metadata at bob startup.
