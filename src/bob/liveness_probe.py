"""bob.liveness_probe — Public API for orchestrator liveness checks (feature 28e1082f).

Thin wrappers over bob.orchestrator.liveness_probe with AC-mandated names:
  - check_orchestrator_alive  (AC: Function defined: bob.liveness_probe.check_orchestrator_alive)
  - is_lock_holder_alive      (AC: Function defined: bob.liveness_probe.is_lock_holder_alive)
...
def check_orchestrator_running() -> bool:
    ...
    (e.g. bob14, bob59) in addition to the legacy ``bob run`` form.

    Excludes the current process, its ancestors, and shell wrappers.

    Signal #1 of the three-signal liveness gate.

    AC: Function defined: bob.liveness_probe.check_orchestrator_running
    """
...
def check_orchestrator_alive() -> bool:
    ...
    addition to the legacy ``bob run`` form.
    ...
    AC: Function defined: bob.liveness_probe.check_orchestrator_alive
    """
    return is_orchestrator_alive()
...
def is_lock_holder_alive(lock_path: str | pathlib.Path) -> bool:
    ...
    AC: Function defined: bob.liveness_probe.is_lock_holder_alive
    """
...
def validate_lock_file_holder(lock_path: str | pathlib.Path) -> bool:
    ...
    AC: Function defined: bob.liveness_probe.validate_lock_file_holder
    """
    return lock_holder_pid_alive(lock_path)
...
def safe_remove_lock_file(
    ...
    Delegates to bob.orchestrator.liveness_probe.remove_stale_lock.

    AC: Function defined: bob.liveness_probe.safe_remove_lock_file
    """
    from bob.orchestrator.liveness_probe import remove_stale_lock
