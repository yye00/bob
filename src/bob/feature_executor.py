...
configured via ``BOB_FEATURE_TIMEOUT_SECONDS`` (default 1800 s / 30 min).
...
Integration: imported by bob.orchestrator via
:func:`bob.timeout.enforce_wall_clock_timeout` (which is the shared
    ...
    *,
    ...
    Args:
        ...
        timeout_seconds: Override the wall-clock timeout.  When ``None``
            (default), reads ``BOB_FEATURE_TIMEOUT_SECONDS`` from the
...
# Public alias matching the AC requirement for bob.feature_executor.execute_feature_with_timeout
execute_feature_with_timeout = enforce_timeout
