"""Cross-project registry transfer for Bob.
...
When BOB_REGISTRY_TRANSFER_PATH is set, load_transfer_registry_if_configured()
automatically loads the exported registry into a new project at startup.

When BOB_FROZEN_REGISTRY=1 (or --frozen-registry flag), freeze_registry() pins
...
def freeze_registry(*, warn: bool = True) -> None:
    ...
    if warn:
        warnings.warn(
            "BOB FROZEN-REGISTRY MODE ACTIVE: registry writes are disabled for this run. "
...
def _check_not_frozen(operation: str) -> None:
    ...
    if _FROZEN:
        raise RuntimeError(
            ...
            "Unset BOB_FROZEN_REGISTRY or remove --frozen-registry to allow writes."
        )
...
def load_transfer_registry_if_configured(
    ...
    *,
    ...
    """Load registry from BOB_REGISTRY_TRANSFER_PATH if set.
    ...
    transfer_path = os.environ.get("BOB_REGISTRY_TRANSFER_PATH")
    if not transfer_path:
        return None

    in_path = Path(transfer_path)
    if not in_path.is_file():
        logger.warning("BOB_REGISTRY_TRANSFER_PATH=%s is not a file; skipping", transfer_path)
        return None
...
def activate_frozen_registry_if_configured() -> bool:
    """Activate frozen-registry mode from env var BOB_FROZEN_REGISTRY if set.
    ...
    """
    value = os.environ.get("BOB_FROZEN_REGISTRY", "").strip()
