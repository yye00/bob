...
Two-layer architecture:
  - F-R7-478 (first line): spawn-layer regex guard ``bob.orchestrator.spawn_retry``
...
def diagnose_infra_only(
    ...
    Named entry-point required by AC: ``bob.rca_infra_recovery.diagnose_infra_only``.
...
def reset_feature_on_infra_verdict(
    ...
    Named entry-point required by AC: ``bob.rca_infra_recovery.reset_feature_on_infra_verdict``.
...
def append_signature_to_spawn_retry(
    ...
    Named entry-point required by AC: ``bob.rca_infra_recovery.append_signature_to_spawn_retry``.
...
def analyze_infra_causation(
    ...
    Named entry-point required by F-R7-479 AC: ``bob.rca_infra_recovery.analyze_infra_causation``.
...
def reset_feature_from_infra_error(
    ...
    ``bob.rca_infra_recovery.reset_feature_from_infra_error``.
...
def inspect_failure_history(
    feature_id: str,
    workspace: "str | os.PathLike[str] | None" = None,
) -> "Verdict":
    """Inspect the failure history of a feature and classify its failure mode.

    Named entry-point required by F-R7-479 AC:
    ``bob.rca_infra_recovery.inspect_failure_history``.

    Second-line defense: called BEFORE the orchestrator transitions a feature
    to ``needs_human``. Delegates to ``classify_attempts`` from the orchestrator
    layer to apply multi-signal heuristics and determine whether ALL failures
    were infrastructure-caused.
...
def is_infra_only(
    ...
    Named entry-point required by F-R7-479 AC:
    ``bob.rca_infra_recovery.is_infra_only``.

    Convenience boolean predicate wrapping ``inspect_failure_history``. Use
    when only a True/False decision is needed (e.g. "should we auto-reset?").
    ...
    return classify_attempts(feature_id, workspace=workspace) == "infra_only"
...
def reset_feature_on_infra_only(
    ...
    Named entry-point required by AC: ``bob.rca_infra_recovery.reset_feature_on_infra_only``.
