...
outer ``bob run --max-cost`` flag caps TOTAL run cost but not per-feature
attempts.  This module closes that gap.

The cap defaults to $10 USD and is controlled by the
``BOB_PER_ATTEMPT_COST_CAP`` environment variable (clamped to [0.5, 100]).

Integration
-----------
``bob.orchestrator.run_loop`` calls :func:`enforce_per_attempt_cap` in the
...
``bob.orchestrator.per_attempt_cost_cap``.
"""
...
def get_cost_cap_limit() -> float | None:
    ...
    bob.cost_cap.get_cost_cap_limit``; it delegates to
    :func:`~bob.orchestrator.per_attempt_cost_cap.get_per_attempt_cap`.
...
def get_cost_cap_threshold() -> float | None:
    ...
    This is the AC-required canonical name ``bob.cost_cap.get_cost_cap_threshold``.
    It delegates to
    :func:`~bob.orchestrator.per_attempt_cost_cap.get_per_attempt_cap`.
...
def enforce_per_attempt_cap(
    *,
    ...
    1. Check whether ``reported_cost`` exceeds the configured cap (via
       :func:`~bob.orchestrator.per_attempt_cost_cap.should_terminate_subagent`).
...
def enforce_per_attempt_cost_cap(
    *,
    ...
    the name ``bob.cost_cap.enforce_per_attempt_cost_cap``; this wrapper
...
def send_sigterm_on_cost_exceeded(
    *,
    ...
    (``BOB_PER_ATTEMPT_COST_CAP``, default 10.0 USD) and, if so, delegates
    to :func:`~bob.orchestrator.per_attempt_cost_cap.terminate_subagent_on_cost_cap`
