...
(B) Slim worker context — generates per-feature WORKER.md at dispatch time.
    bob's CLAUDE.md (loop-operator memory ~70 bullets) is irrelevant to a
...
(C) Per-worker settings — writes .bob/features/<id>/settings.json at dispatch
...
Public API
----------
claude_code_worker_leverage_enable_prompt_cache_slim_per(feature, workspace, bob_dir, ...)
    Main entry point: applies all three fixes and returns metadata dict.

apply_worker_leverage(feature, workspace, bob_dir, ...)
...
def apply_worker_leverage(
    ...
    *,
    ...
    (B) Writes per-feature WORKER.md to .bob/features/WORKER.md.
    (C) Writes per-feature settings.json to .bob/features/<id>/settings.json.
    ...
    worker_md_dir = bob_dir / "features"
...
def claude_code_worker_leverage_enable_prompt_cache_slim_per(
    ...
    *,
    ...
    This is the canonical function satisfying the feature AC:
      "Function defined: bob.claude_code_worker_leverage_enable_prompt_cache_slim_per
    ...
    Args:
        ...
        bob_dir:    Path to the .bob directory where feature dirs are written.
