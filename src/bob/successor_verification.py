...
When a feature's diff modifies ``src/bob/enhanced_verification.py`` or any module
...
VERIFIER_EXTENSION_MODULES
    ...
    re-exported from ``bob.spec_quality.spec_extractor``.
"""
...
def set_pending_successor_verify(
    ...
    Entry point exported as ``bob.orchestrator.set_pending_successor_verify``.
    Delegates to ``bob.pending_successor_verify.set_pending_successor_verify``
