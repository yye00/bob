...
`bob.dispatch.build_worker_md` at dispatch time. That file is placed at
`.bob/features/WORKER.md` before the worker subprocess is launched.
...
(C) **Per-worker settings** — `.bob/features/<id>/settings.json` is written
