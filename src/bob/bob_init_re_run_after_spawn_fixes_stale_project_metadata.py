...
``bob.orchestrator.project_metadata_check`` and wraps them in the same
...
def bob_init_re_run_after_spawn_fixes_stale_project_metadata(
    ...
    try:
        ...
        logger.warning(
            "bob_init_re_run check: stale pytest tmpdir in spec_path — "
    ...
    if name_was_stale:
        logger.info(
            "bob_init_re_run: corrected stale project name → %r "
