...
trigger_decomposition(feature, *, previous_readiness_score=None, db_update=None) -> Feature | None
    Check if stuck and, if so, mark pending_decomposition and return the updated
    feature. Returns None when the feature is not stuck (no action taken).

should_trigger_decomposition(feature, *, previous_readiness_score=None) -> bool
    Alias for check_stuck_readiness — for callers that prefer this name.
...
Integration: bob.orchestrator calls ``check_stuck_readiness`` before re-queuing a
...
def trigger_decomposition(
    feature: "Feature",
    *,
    previous_readiness_score: float | None = None,
    db_update: Any | None = None,
) -> "Feature | None":
    """Check if stuck and, if so, mark the feature pending_decomposition.

    Returns the updated Feature (with status ``pending_decomposition``) when the
    trigger fires, or None when the feature is not stuck and no action is taken.

    When ``db_update`` is provided it is forwarded to ``mark_pending_decomposition``
    and called as ``db_update(feature.id, status="pending_decomposition")``.

    Raises ValueError if feature.refinement_attempts is negative (data corruption).
    """
    if not check_stuck_readiness(feature, previous_readiness_score=previous_readiness_score):
        return None
