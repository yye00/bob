"""Install bob skills into sub-agent workspaces.

Bob ships a set of skills at ``src/bob/skills/``. When a sub-agent is
spawned with ``cwd=<workspace>``, Claude Code discovers skills at
``<workspace>/.claude/skills/``. This module links bob's bundled skills
into that directory so sub-agents can find them.

Called from ``bob init`` (one-time per workspace) and defensively from
the sub-agent spawn path (so upgrades of bob pick up new skills even on
existing workspaces).
"""
...
def get_progress_path() -> Path:
    ...
    Intentionally mirrors ``bob.progress_events.get_progress_path`` so
    callers can patch ``bob.skills_installer.get_progress_path`` in tests
    without pulling in the full progress_events module.
    """
    from bob.progress_events import get_progress_path as _get
...
def log_skill_activation(
    ...
    """Emit a structured skill_activation_logged event to .bob/progress.jsonl.

    Args:
        spawn_id: Identifier for the sub-agent spawn.
        feature_id: Feature being implemented in this spawn.
        skills_activated: List of skill IDs that were installed/selected.
        skills_considered: Full list of skill IDs that were evaluated.
        selection_reason: How skills were chosen ("heuristic" or "registry-driven").
    """
...
# Skills reserved for the independent evaluator sub-agent (Round 0 Task 1,
# Gap #1). These must NOT land in an implementation agent's workspace,
# because installing the adversarial-review checklist into the same
# context window that produced the code is the homework-grading failure
# mode the evaluator agent was created to fix. The evaluator spawn path
# in ``bob.orchestrator.claude_executor.spawn_evaluator_agent`` opts in
# explicitly via ``include_evaluator_skills=True``.
EVALUATOR_ONLY_SKILLS: frozenset[str] = frozenset({"adversarial-self-review"})


def get_bundled_skills_dir() -> Path:
    """Return the filesystem path to bob's bundled skills directory."""
    # importlib.resources.files works both for editable installs and wheels.
    skills_root = resources.files("bob") / "skills"
    return Path(str(skills_root))


def list_bundled_skills() -> list[str]:
    """Return the names of all bob-bundled skills."""
...
def _is_bob_managed(dest: Path, bundled_skills_dir: Path) -> bool:
    """Return True if ``dest`` is a bob-managed install (symlink or copy).

    A path counts as bob-managed if:
    - It is a symlink that resolves INTO the current bob bundled
      skills directory (i.e. a link we installed). User-created
      symlinks pointing at their own external skill dir are NOT
      considered ours.
    - It is a real directory containing the ``.bob-installed`` marker
...
def _symlink_resolves_into(dest: Path, expected_dir: Path) -> bool:
    ...
    given bob bundled skills dir so an old workspace pointing at a
    deleted bob install is detected as stale.
    """
...
def _install_one(skill_src: Path, dest: Path) -> bool:
    ...
    try:
        ...
        # Drop a marker so we know this is a bob-managed copy and may
        # safely refresh it on subsequent runs.
        try:
            (dest / ".bob-installed").write_text("bob\n")
        except OSError:
            # Non-fatal: marker is a nice-to-have. If we can't write it
            # the directory is still usable; we just won't auto-refresh.
            logger.debug("Could not write .bob-installed marker in %s", dest)
...
def install_skills_to_workspace(
    ...
    *,
    ...
    include_evaluator_skills: bool = False,
) -> list[str]:
    """Install all bob skills into ``<workspace>/.claude/skills/``.

    Each skill is symlinked if possible (to stay in sync with bob
    ...
    - Subsequent calls: refreshes any stale entries (broken symlinks or
      symlinks pointing outside the current bob bundled skills dir),
      and leaves working entries alone.
    - If ``dest`` is a real directory with no ``.bob-installed`` marker
      it's treated as user-owned and never modified.

    A symlink that already resolves into the current bob bundled skills
    ...
    Round 0 Task 1 (Gap #1): skills listed in :data:`EVALUATOR_ONLY_SKILLS`
    (currently ``{"adversarial-self-review"}``) are excluded by default
    so they do NOT land in the implementation agent's workspace — that
    was the "agent grading its own homework" failure mode. The
    evaluator spawn path passes ``include_evaluator_skills=True`` to
    receive them. Any pre-existing bob-managed install of an excluded
    skill is removed during the same pass so a workspace that was set
    up before this change is brought back into compliance.
    """
    ...
    for skill_src in sorted(source_dir.iterdir()):
        ...
        # Round 0 Task 1: evaluator-only skills are excluded from the
        # default (implementation-agent) install. If a previous bob
        # version installed them here, remove the bob-managed entry so
        # the implementation workspace is brought back into compliance.
        # User-owned entries with the same name are left alone — we only
        # touch what we own (see ``_is_bob_managed``).
        if skill_src.name in EVALUATOR_ONLY_SKILLS and not include_evaluator_skills:
            if dest.is_symlink() and _symlink_resolves_into(dest, source_dir):
                _remove_dest(dest)
                logger.info(
                    "Removed evaluator-only skill %s from implementation "
                    "workspace %s (Round 0 Task 1, Gap #1)",
                    skill_src.name,
                    dest,
                )
            elif dest.is_dir() and _is_bob_managed(dest, source_dir):
                _remove_dest(dest)
                logger.info(
                    "Removed evaluator-only skill copy %s from implementation "
                    "workspace %s (Round 0 Task 1, Gap #1)",
                    skill_src.name,
                    dest,
                )
        ...
        if dest.is_symlink():
            ...
            if dest.is_dir() and _is_bob_managed(dest, source_dir):
                # A previous copy-install we own — refresh it.
                if not _remove_dest(dest):
                    logger.warning(
                        "Could not refresh bob copy at %s; skipping %s",
                        dest,
                        skill_src.name,
                    )
                    continue
                ...
                logger.debug(
                    "Skipping %s; existing entry is user-owned (not bob-managed)",
                    dest,
                )
                continue
...
def verify_skills_integrity(
    workspace: str | Path, *, include_evaluator_skills: bool = False
) -> list[str]:
    ...
    ``.claude/skills/`` — including replacing one of bob's symlinks with a
    ...
    1. The destination at ``<workspace>/.claude/skills/<name>`` exists and
       is a symlink.
    2. The symlink resolves into the *current* bob bundled skills
    ...
    for skill_src in sorted(source_dir.iterdir()):
        ...
        name = skill_src.name
        # Round 0 Task 1: don't audit evaluator-only skills in
        # implementation workspaces — they are intentionally absent.
        if name in EVALUATOR_ONLY_SKILLS and not include_evaluator_skills:
            continue
        ...
        if not dest.exists() and not dest.is_symlink():
            ...
            logger.warning(
                "SECURITY: bob skill %r missing from %s; reinstalling",
                name,
                target_dir,
            )
        ...
        if not dest.is_symlink():
            # A real directory (or file) is sitting where bob's symlink
            # should be. This is the canonical poisoning shape: a
            # sub-agent wrote SKILL.md inside a directory it created at
            # the bob skill name. Refuse to trust it and force-replace.
            kind = "directory" if dest.is_dir() else "file"
            logger.warning(
                "SECURITY: bob skill %r at %s is a %s, not a symlink — "
        ...
        # It's a symlink — but does it resolve into THIS bob install?
        if not _symlink_resolves_into(dest, source_dir):
            try:
                ...
                actual = "<unresolvable>"
            logger.warning(
                "SECURITY: bob skill %r at %s resolves to %s, which is "
                "outside the current bob bundled skills dir %s — "
    ...
    if tampered:
        ...
        install_skills_to_workspace(
            workspace_path, include_evaluator_skills=include_evaluator_skills
        )
        logger.warning(
            "SECURITY: force-replaced %d bob skill(s) after integrity "
            "check: %s",
            len(tampered),
            tampered,
        )
...
def clean_workspace_skills(workspace: str | Path) -> list[str]:
    """Remove only bob-installed skills from a workspace.

    Walks ``<workspace>/.claude/skills/`` and removes only entries that
    are bob-managed, as determined by :func:`_is_bob_managed`:

    - symlinks that resolve into the current bob bundled skills
      directory (i.e. links we created), and
    - real directories that contain the ``.bob-installed`` marker
      file (copy-fallback installs we own).

    User-created entries — including symlinks pointing at the user's
    own skills outside the bob bundled tree, and plain directories
    ...
    for entry in sorted(target_dir.iterdir()):
        if _is_bob_managed(entry, source_dir):
            if _remove_dest(entry):
                removed.append(entry.name)
        # else: user-owned, skip
    logger.info("Removed %d bob skills from %s", len(removed), target_dir)
    return removed
