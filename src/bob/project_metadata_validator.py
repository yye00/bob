"""Project metadata validator for bob spawn lifecycle.

After spawn_next_generation.sh rsync-copies the parent directory, the child
bob.db still contains the parent's project name. This module provides:
...
def verify_project_name_matches_workspace(
    ...
    db_path:
        ...
        environment variable, or ``<workspace>/bob.db`` when the env var is
        unset.
            ...
            for project_id, current_name, spec_path in rows:
                ...
                if spec_path and _PYTEST_TMPDIR_MARKER in spec_path:
                    logger.warning(
                        ...
                        "project %r — re-run 'bob init --spec <correct-spec>' to fix. "
                        "spec_path=%r",
                        current_name,
                        spec_path,
                    )
