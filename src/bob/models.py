"""Pydantic data models for all Bob database entities.
...
class Feature(BaseModel):
    ...
    # (BOB_MODEL_ESCALATION_LADDER). 0 = first/least-capable model. Bumped when
    # the feature exhausts attempts on its current model. See bob.model_escalation.
    model_tier: int = 0
...
class ForgettingEvent(BaseModel):
    """Audit log for bob memory forgetting actions.

    Note: table/column names retain the legacy 'titans' prefix from the
    pre-bob-memory schema; only docs/labels have been updated.
    """
...
class SecurityFinding(BaseModel):
    ...
    See ``bob.security_checks.run_security_checks`` for the runner and
...
class EvaluatorVerdict(BaseModel):
    ...
    verdict. See ``bob.orchestrator.claude_executor.spawn_evaluator_agent``
    for the runner and
