...
def trace_ac(
    ...
    *,
    ...
    db_path:
        Optional path to the SQLite database. Falls back to the ``BOB_DATABASE_PATH``
        environment variable and then to ``bob.db``.
    ...
    if db_path is None:
        db_path = os.environ.get("BOB_DATABASE_PATH", "bob.db")
