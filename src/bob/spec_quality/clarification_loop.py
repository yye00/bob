...
def code_consistency_check(
    ...
    *,
    ...
    ci_mode:
        When True, CI mode is forced on. When None, the ``BOB_CI_MODE``
    ...
    """
    if ci_mode is None:
        _ci_raw = os.environ.get("BOB_CI_MODE", "").strip().lower()
...
def run_clarification_loop(
    ...
    *,
    ...
    ci_mode:
        Force CI mode on/off; None reads BOB_CI_MODE env var.
    audit_log_path:
...
def exit_spec_needs_human_in_ci(
    ...
    *,
    ...
    ci_mode:
        Force CI mode; None reads BOB_CI_MODE env var.
