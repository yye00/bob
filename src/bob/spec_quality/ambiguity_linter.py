...
Run at bob plan time. Fails the plan if any feature has an ambiguous AC
...
def has_concrete_identifier(ac: str) -> bool:
    ...
    if _matches_structured_form(stripped):
        return True
    # Check for path-like patterns (e.g. src/bob/foo.py, tests/test_bar.py)
    if re.search(r'\b\w+/\w+[\w./]*\.py\b', stripped):
        return True
    # Check for dotted module paths (e.g. bob.module.function)
