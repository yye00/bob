...
Public API::
    ...
    from bob.spec_quality.spec_critic import critique_feature, persist_findings
...
def critique_feature(
    ...
    if workspace is None:
        # Walk up from src/bob/spec_quality/ → bob12/
...
def _load_spec_findings(path: Path) -> dict[str, Any]:
    ...
    from bob.spec_findings_writer import load_spec_findings_safe
...
def _save_spec_findings(findings_by_hash: dict[str, Any], path: Path) -> None:
    from bob.spec_findings_writer import write_atomic as atomic_write_yaml
...
def persist_findings(
    ...
    *,
    ...
    try:
        from bob.spec_quality.spec_findings_registry import record as _sfr_record
...
def run_auto_repair(
    ...
    Delegates to :func:`bob.spec_quality.ac_auto_repair.repair_feature_acs`.
    Provides the integration point between spec_critic and ac_auto_repair.
    """
    from bob.spec_quality.ac_auto_repair import repair_feature_acs  # noqa: PLC0415
