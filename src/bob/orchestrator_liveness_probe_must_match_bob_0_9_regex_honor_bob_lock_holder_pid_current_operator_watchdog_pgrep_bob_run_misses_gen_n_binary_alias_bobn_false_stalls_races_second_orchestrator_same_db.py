"""Feature 53afc69e: orchestrator-liveness probe MUST match bob[0-9]+ regex AND honor .bob.lock holder PID.

Operational defect: pgrep bob run missed gen-N binary aliases (e.g. bob14),
...
the .bob.lock file may be safely removed:

  Signal 1 — pgrep-style: no process matching ``bob[0-9]+ run`` in argv
  Signal 2 — kill-0:      .bob.lock holder PID is no longer alive
...
from bob.orchestrator.liveness_probe.
"""
...
__all__ = [
    "orchestrator_liveness_probe_must_match_bob_0_9_regex_honor_bob_lock_holder_pid_current_operator_watchdog_pgrep_bob_run_misses_gen_n_binary_alias_bobn_false_stalls_races_second_orchestrator_same_db",
]


def orchestrator_liveness_probe_must_match_bob_0_9_regex_honor_bob_lock_holder_pid_current_operator_watchdog_pgrep_bob_run_misses_gen_n_binary_alias_bobn_false_stalls_races_second_orchestrator_same_db(
    ...
    Implements the three-signal liveness gate contract:
      ...
      1. Regex pgrep: scans /proc for processes matching ``bob[0-9]+ run``
         — covers gen-N aliases (bob14, bob59, …) that ``pgrep bob run`` misses.
      2. Lock PID: probes the holder PID from .bob.lock with kill(pid, 0).
