"""bob.provenance — Sub-translation provenance public API.
...
def add_ac_provenance(
    ...
    *,
    ...
    Given an AC (as a string or dict with an ``"ac"`` key) and the original
    human-intent text, locates the contiguous character span(s) of *intent*
    that produced the AC and returns a provenance record dict.

    Parameters
    ----------
    ac:
        The acceptance-criterion text (str), or a dict with at least an
        ``"ac"`` key whose value is the AC text.  Extra dict keys are
        preserved in the returned dict.
    intent:
        The original human-intent text from which the AC was derived.
    strict:
        When True (default), raises ``ValueError`` if no provenance span
        is found.  When False, returns an empty spans/provenance list.
    ...
    dict with keys:
        ...
        ``spans``      — list of ``{"start": int, "end": int}`` dicts
        ``provenance`` — list of source substrings for each span
        (plus any extra keys from an input dict AC)

    Raises
    ------
    TypeError
        When *ac* is neither a str nor a dict.
    ValueError
        When *intent* is not a str, or when *strict* is True and no span
        could be found.
    """
    from bob_legacy.provenance import add_provenance_field as _add_provenance_field

    return _add_provenance_field(ac, intent, strict=strict)
...
def trace_ac_to_source(
    ...
    *,
    ...
    Delegates to ``bob.spec_quality.provenance.trace_ac`` and returns a
...
def build_ac_provenance_map(
    acs: list[str],
    intent: str,
    *,
    strict: bool = False,
) -> dict[str, list[dict[str, int]]]:
    """Build a mapping from each AC text to its provenance spans.

    For each AC in *acs*, locates the contiguous character span(s) of *intent*
    that produced it and returns a dict keyed by AC text.
    ...
    Returns
    -------
    dict mapping each AC string to a list of ``{"start": int, "end": int}`` dicts.
    ...
    TypeError
        When *acs* contains a non-string element.
    """
    records = extract_provenance_spans(acs, intent, strict=strict)
    return {rec["ac"]: rec["spans"] for rec in records}
