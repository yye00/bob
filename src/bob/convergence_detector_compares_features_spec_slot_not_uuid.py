...
feature sets by features.id (UUID), which is minted fresh on every ``bob init``,
...
def convergence_detector_compares_features_spec_slot_not_uuid(
    ...
    """Compare two bob databases by completed spec_slot sets, not by UUID.

    Feature IDs (UUID) are minted fresh on every ``bob init``, making UUID-based
