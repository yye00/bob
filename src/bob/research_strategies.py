...
Public API::
    ...
    from bob.research_strategies import (
        ...
        retry_generation_on_validation_failure,
    )
"""
...
def emit_canonical_acs(topic: Any) -> list[str]:
    ...
    module_path = f"bob.{slug}" if not slug.startswith("bob.") else slug
    ...
    acs: list[str] = [
        ...
        f"File exists: src/bob/research_strategies.py",
        f"pytest: {test_path}",
        f"integration: bob.synthesis",
...
def validate_against_spec_quality(acceptance_criteria: Any) -> dict[str, Any]:
    """Validate a list of ACs against the canonical spec_quality gate rules.

    Alias for :func:`validate_against_spec_quality_gate` using the canonical
    function name required by the AC:
    ``Function defined: bob.research_strategies.validate_against_spec_quality``.
...
def generate_canonical_acs(topic: Any) -> list[str]:
    ...
    bob.research_strategies.generate_canonical_acs``.
...
def generate_research_strategy_acs(
    ...
    ``Function defined: bob.research_strategies.generate_research_strategy_acs``.
...
def validate_acs_canonical_form(acceptance_criteria: Any) -> dict[str, Any]:
    """Validate a list of AC strings against the canonical spec_quality gate form rules.

    Each AC is individually checked for canonical prefix compliance.  Returns a
    result dict with ``passed`` (bool), ``non_canonical`` (list[str]), and
    ``per_ac_results`` (list[dict]) with per-criterion detail.
    ...
    Returns:
        ...
        - ``passed`` (bool): True when all ACs are canonical.
        - ``non_canonical`` (list[str]): ACs that failed validation.
        - ``per_ac_results`` (list[dict]): Per-AC validation results.
    ...
    per_ac_results = [validate_ac_canonical_form(ac) for ac in acceptance_criteria]
    non_canonical = [r["non_canonical"][0] for r in per_ac_results if not r["passed"]]
    return {
        "passed": len(non_canonical) == 0 and len(acceptance_criteria) > 0,
        "non_canonical": non_canonical,
...
def validate_ac_against_canonical_forms(ac: Any) -> dict[str, Any]:
    """Validate a single AC string against the canonical structured forms required by the spec_quality gate.

    This is the canonical entry-point referenced by the AC:
    ``Function defined: bob.research_strategies.validate_ac_against_canonical_forms``.

    Checks whether *ac* matches one of the canonical structured prefixes
    (``File exists:``, ``Function defined:``, ``Class defined:``, ``pytest:``,
    ``integration:``, ``behavior: ... when ...``, ``python:``, ``Field exists:``).
    ...
    is_ok = _is_canonical(ac)
    reason = "AC matches a canonical structured form" if is_ok else (
        "AC does not match any canonical structured form "
        "(expected one of: 'File exists:', 'Function defined:', 'Class defined:', "
        "'pytest:', 'integration:', 'behavior: ... when ...', 'python:', 'Field exists:')"
    )
    return {
...
def emit_canonical_ac(topic: Any, ac_type: str = "function") -> str:
    """Emit a single canonical-form AC for the given feature *topic*.

    This is the canonical entry-point referenced by the AC:
    ``Function defined: bob.research_strategies.emit_canonical_ac``.

    Emits a single AC string that matches a canonical structured prefix
    required by the spec_quality gate.

    Args:
        topic: Feature name or description string. Must be a non-empty string.
        ac_type: Type of canonical AC to emit. One of ``"function"``,
                 ``"file"``, ``"pytest"``, ``"integration"``, ``"error"``.
                 Defaults to ``"function"``.

    Returns:
        A single canonical-form AC string.

    Raises:
        TypeError: When *topic* is not a string.
        ValueError: When *topic* is empty, whitespace-only, or *ac_type* is unknown.
    """
    if not isinstance(topic, str):
        raise TypeError(
            f"topic must be a non-empty string, got {type(topic).__name__!r}"
    ...
    _ac_map: dict[str, str] = {
        "function": f"Function defined: {module_path}.main",
        "file": f"File exists: src/bob/{test_slug}.py",
        "pytest": f"pytest: tests/test_{test_slug}.py",
        "integration": f"integration: {module_path}",
        "error": f"behavior: {slug} raises ValueError when input is invalid",
    }

    if ac_type not in _ac_map:
        raise ValueError(
            f"ac_type must be one of {sorted(_ac_map)!r}, got {ac_type!r}"
        )

    return _ac_map[ac_type]


def validate_ac_structure(ac: Any) -> dict[str, Any]:
    """Validate a single AC string against the canonical structure required by the spec_quality gate.

    This is the canonical entry-point referenced by the feature AC:
    ``Function defined: bob.research_strategies.validate_ac_structure``.

    Checks whether *ac* matches one of the canonical structured prefixes and
    returns a structured result dict describing the outcome.
    ...
    Returns:
        Dict with keys:
        - ``passed`` (bool): True when the AC matches a canonical structure.
        - ``reason`` (str): Human-readable explanation of the result.
        - ``non_canonical`` (list[str]): The AC if it failed, else empty list.
        - ``has_error_path`` (bool): True when the AC references an error/failure path.
    ...
    is_ok = _is_canonical(ac)
    has_error = any(kw in ac.lower() for kw in _ERROR_KEYWORDS)
    reason = "AC matches canonical structured form" if is_ok else (
        "AC does not match any canonical structured form "
        "(expected one of: 'File exists:', 'Function defined:', 'Class defined:', "
        "'pytest:', 'integration:', 'behavior: ... when ...', 'python:', 'Field exists:')"
    )
    return {
        ...
        "reason": reason,
        "non_canonical": [] if is_ok else [ac],
        "has_error_path": has_error,
    }


def emit_structured_acs(topic: Any) -> list[str]:
    """Emit a structured canonical-form AC list for the given feature *topic*.

    Alias for :func:`emit_canonical_acs` using the name required by the AC
    ``Function defined: bob.research_strategies.emit_structured_acs``.

    Every emitted AC matches one of the canonical structured prefixes required
    by the spec_quality gate.  At least one AC references an error/failure path
    to satisfy the gate's negative-AC requirement.
...
def retry_generation_on_validation_failure(
    feature_topic: Any,
    max_retries: int = 3,
) -> dict[str, Any]:
    """Generate canonical ACs with automatic retry on spec_quality gate failure.

    Calls :func:`emit_structured_acs` to produce ACs and validates them with
    :func:`validate_acs_canonical_form`.  On validation failure, retries up to
    *max_retries* times.  Persistent failure marks the synthesis attempt as
    ``synthesis_blocked_invalid_acs`` and returns without writing a feature row.

    This satisfies the AC: ``Function defined:
    bob.research_strategies.retry_generation_on_validation_failure``.
    ...
    for attempt in range(1, max_retries + 1):
        acs = emit_structured_acs(stripped)
        result = validate_acs_canonical_form(acs)
