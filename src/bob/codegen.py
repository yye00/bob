"""bob.codegen — code generation utilities for acceptance criteria.
...
from bob.behavior_contract import apply_design_by_contract
from bob.design_by_contract import (
...
def emit_hypothesis_test(prop: PropertyAC, *, seed: int = 0) -> str:
    ...
    Wraps :func:`bob.spec_quality.example_grammar.emit_hypothesis_test`
    and is the canonical integration point in ``bob.codegen``.

    The generated test uses ``@settings(deriving=["database"])`` and
    ``@seed(seed)`` so the test run is reproducible with ``seed=0`` by default.

    Args:
        prop: A :class:`~bob.spec_quality.example_grammar.PropertyAC` dataclass
...
def emit_key_example_test(
    ...
    *,
    ...
    Wraps :func:`bob.spec_quality.example_grammar.emit_parametrize_test`
    and is the canonical integration point in ``bob.codegen``.
...
def emit_behavior_contract_decorators(ac: dict) -> str:
    ...
    Convenience wrapper around :func:`bob.behavior_contract.apply_design_by_contract`
