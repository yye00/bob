...
def discover_workarounds(probe_results: List[Dict[str, Any]]) -> List[Optional[Dict[str, Any]]]:
    ...
    Calls ``discover_workaround`` on each probe result and returns a parallel
    list. Present deps yield None; missing deps yield a workaround dict.
    ...
    return [_discover_workaround(pr) for pr in probe_results]
