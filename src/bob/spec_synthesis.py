"""bob.spec_synthesis — Structured-uncertainty clarification loop with AskUserQuestion.
...
#: Re-exported from bob.self_discover_meta_agent for spec_synthesis consumers.
...
def deterministic_fallback(
    ...
    **kwargs: Any,
    ...
    Delegates to :func:`bob.spec_synthesizer.deterministic_fallback` which
...
def emit_plan_ready_event(
    ...
    from bob.orchestrator.plan_gate import emit_plan_ready_event as _emit
...
def ask_for_clarification(
    ...
    *,
    ...
    ci_mode:
        Force CI mode on/off. When ``None``, reads ``BOB_CI_MODE``.
    audit_log_path:
