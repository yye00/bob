"""bob.linters.smell_detector — canonical integration point for the
...
- :func:`check_severity_blocks` — return True when any E-severity finding
  is present (blocking ``bob plan --create``)

Severities: E (Error), W (Warning), I (Informational).
E-severity smells block ``bob plan --create`` via :func:`check_severity_blocks`.
...
def detect_smells(
    ...
    integrating with ``bob.orchestrator`` via ``check_severity_blocks``.
...
def check_severity_blocks(findings: list[SmellFinding]) -> bool:
    ...
    E-severity findings block ``bob plan --create``. Use this function as
