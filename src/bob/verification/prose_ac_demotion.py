...
Prose-connector tokens are split into two distinct partitions:
  ...
  1. Descriptive-prose connectors (bob.verification.structural_prefix_match
  ...
  2. Policy-verb connectors (bob.verification.policy_verb_registry
...
The integration-AC handler (bob.verification.integration_ac_resolver)
consumes BOTH partitions when deciding whether a body is prose-policy.  This
prevents the failure mode observed in bob version 16 round 13 where feature
...
logger = logging.getLogger("bob.verification.prose_demotion")
...
def log_prose_ac_demoted(criterion: str, feature_id: str | None = None) -> None:
    ...
    'bob.verification.prose_demotion' so callers can route or suppress it
