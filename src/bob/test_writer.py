...
Public API::
    ...
    from bob.test_writer import emit_failing_test, triple_filter_one

    emitted = emit_failing_test(
        feature_id="abc123",
        ac_index=0,
        ac_text="File exists: src/bob/mymod.py",
    )
...
class TestWriter:
    """Test-writer sub-agent entry point — emits failing tests before the implementer fires.

    Sits between the spec-critic (F-R7-450) and the implementer sub-agent.
    Calls ``generate_failing_tests`` to produce one failing pytest file per AC
    and applies the TestGen-LLM triple filter (compile / fails-on-stub /
    coverage heuristic).
    """
