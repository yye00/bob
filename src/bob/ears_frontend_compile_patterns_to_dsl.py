"""EARS frontend — compile all five EARS patterns into bob's acceptance criterion DSL.
...
Each recognised EARS pattern is compiled into one or more bob DSL criterion
...
def compile_ears_to_dsl(pattern: EARSPattern) -> list[str]:
    """Compile one :class:`EARSPattern` into a list of bob DSL criterion strings.

    Each criterion string is in one of the canonical bob formats:
...
def ears_text_to_dsl_criteria(text: str) -> list[str]:
    ...
    and compiles matched patterns into bob DSL criterion strings.  The result
