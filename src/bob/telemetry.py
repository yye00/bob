...
from bob.version_probe import get_model_id, get_sdk_version
from bob.registry_transfer import is_registry_frozen

_DEFAULT_RUN_JSONL_PATH = Path(".bob") / "run.jsonl"
...
def emit_telemetry_line(run_id: str, **fields: Any) -> None:
    """Append one fixed-schema JSON record to <workspace>/.bob/run.jsonl.
    ...
    BOB_ABLATION_MODE unless explicitly overridden in ``fields``.
    """
