"""bob.explain_gate_block — operator-visibility subcommand for spec_quality_gate.
...
:func:`score_feature` lives in ``bob.enhanced_verification``. This module
re-exports those functions under the ``bob.explain_gate_block`` namespace so
that ACs of the form ``Function defined: bob.explain_gate_block.<symbol>``
...
CLI integration::
    ...
    bob explain-gate-block --json <feature_id_prefix>
...
from bob.enhanced_verification import explain_gate_block, score_feature  # noqa: F401
...
def compute_cheapest_fixes(
    ...
    ``Function defined: bob.explain_gate_block.compute_cheapest_fixes`` resolves.
...
def score_and_breakdown(
    ...
    ``Function defined: bob.explain_gate_block.score_and_breakdown`` resolves.
...
def explain_feature_block(
    ...
    Primary entry point for the ``bob explain-gate-block`` CLI subcommand.
    Equivalent to :func:`explain_gate_block`; exposed under this name so that
    the AC ``Function defined: bob.explain_gate_block.explain_feature_block``
    resolves correctly.
...
def format_breakdown(result: dict[str, Any]) -> str:
    """Format an :func:`explain_gate_block` result dict as a human-readable string.

    Produces the multi-line breakdown output shown by ``bob explain-gate-block``,
    including score vs threshold, per-component sub-scores, and remediation hints.

    Parameters
    ----------
    result:
        Dict returned by :func:`explain_gate_block` with keys ``feature_id``,
        ``feature_name``, ``score``, ``threshold``, ``components``, and
        ``remediation_hints``.
