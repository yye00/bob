"""bob.synthesizer.peas_extractor — PEAS pipeline adapter.

AC: File exists: src/bob/synthesizer/peas_extractor.py
AC: Function defined: bob.synthesizer.peas_extractor.parse_peas_markdown
AC: Function defined: bob.synthesizer.peas_extractor.extract_to_features_yaml

Exposes the PEAS pipeline (parse → stub → synthesize → score-gate) as functions
in the bob.synthesizer namespace.  Implementation delegates to
``bob.extract_from_peas`` which owns the canonical logic.
"""
...
from bob.extract_from_peas import (
