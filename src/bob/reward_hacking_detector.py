"""Reward-hacking detector for Bob (LLM judge).
...
Verdict is recorded in bob.db (reward_hacking_verdicts table).
"""
...
def _get_db_path() -> pathlib.Path:
    """Return the path to bob.db, searching upward from this module."""
    here = pathlib.Path(__file__).resolve()
    for parent in (here.parent, *here.parents):
        candidate = parent / "bob.db"
