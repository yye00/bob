---
name: using-bob-memory
description: Use when starting any feature implementation, when encountering a bug, or when a past decision is relevant. Bob ships with semantic memory (mem0 + FastEmbed + Qdrant) exposed as MCP tools. Searching lessons before implementing prevents repeating past mistakes; recording lessons after solving bugs helps future agents.
---

# Using bob memory

Bob gives every sub-agent access to a persistent, semantic memory store backed by mem0 + FastEmbed + Qdrant. The memory is shared across all agents and all features, so what you learn here helps the next one.

## Tools available to you

All exposed via MCP under the `bob-memory` server:
...
If you don't specify a pool, bob auto-classifies by keyword. Auto-classification is coarse — prefer to specify.
...
This feeds into the memory's usefulness score and lets bob demote bad memories over time.
