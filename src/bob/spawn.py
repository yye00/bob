"""bob.spawn — spawn lifecycle helpers for project metadata verification.

After spawn_next_generation.sh rsync-copies the parent directory, the child
bob.db retains stale project name and spec_path values.  This module exposes
``verify_project_metadata`` as the canonical entry point for detecting and
correcting that state.  The same function is also exposed via ``bob.run_loop``
...
from bob.ac_form_validator import validate_acceptance_criteria  # noqa: F401
from bob.orchestrator.spawn_retry import (  # noqa: F401
    classify_exit,
    spawn_with_retry,
)
...
def re_init_after_spawn(
    ...
    ``bob init``, leaving ``projects.name`` set to the parent generation name
    ...
    Args:
        ...
        db_path: Path to the bob.db database. Defaults to the
            ``BOB_DATABASE_PATH`` environment variable or
            ``<workspace>/bob.db``.
...
#: Canonical alias: "Function defined: bob.spawn.reinit_after_spawn"
