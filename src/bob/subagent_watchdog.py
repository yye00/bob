...
Both entry points delegate to bob.orchestrator.feature_watchdog, which
...
from bob.feature_watchdog import cancel_subagent_by_pid
...
def cancel_subagent_process(pid: int, feature_id: str = "") -> None:
    """Cancel a hung subagent process by OS PID (AC: bob.subagent_watchdog.cancel_subagent_process).
...
def cancel_subagent_on_timeout(pid: int, feature_id: str = "") -> None:
    ...
    derived from ``BOB_FEATURE_TIMEOUT_SECONDS`` expires.
