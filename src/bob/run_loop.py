...
def _lazy_import_charge_feature_from_test():
    ...
    from bob.verification.blame_feature_charger import charge_feature_from_test  # noqa: F401
...
def check_worktree_artifacts(
    ...
    """Count implementation artifacts in worktree paths matching .worktrees/hotfix-*/src/bob/.
    ...
    Scans ``<bob_root>/.worktrees/hotfix-*/src/bob/`` for Python files.
    Returns 0 and never raises on missing or unreadable paths.

    Parameters
    ----------
    bob_root:
        Root of the bob repository tree.  Defaults to ``Path.cwd()`` if None.
    ...
    try:
        for candidate in worktrees_dir.iterdir():
            ...
            if not candidate.name.startswith("hotfix-"):
                continue
            bob_src = candidate / "src" / "bob"
            if not bob_src.exists():
                continue
            try:
                for entry in bob_src.rglob("*.py"):
                    if entry.is_file():
                        count += 1
            except OSError as exc:
                logger.debug(
                    "check_worktree_artifacts: could not scan %s: %s", bob_src, exc
                )
    except OSError as exc:
        logger.debug(
...
# F-R7-613 aliases: ACs require these names importable from bob.run_loop.
classify_startup_crash_exempt = classify_subagent_startup_crash
classify_startup_crash = classify_subagent_startup_crash
# be23bbf0: AC requires bob.run_loop.classify_and_exempt_startup_crash
...
# AC alias: bob.run_loop.is_transport_transient_signature
...
def is_subagent_startup_crash(exit_signature: str | None) -> bool:
    ...
    ``Function defined: bob.run_loop.is_subagent_startup_crash``.
...
def verify_project_metadata(
    ...
    spawn_next_generation.sh copies the parent DB via rsync. If bob init is not
    ...
    db_path:
        Path to the bob.db database.  Defaults to BOB_DATABASE_PATH env var
        or ``<workspace>/bob.db``.
    ...
    """
    import importlib
    _pmc = importlib.import_module("bob.orchestrator.project_metadata_check")
...
#: Canonical alias required by AC: "Function defined: bob.run_loop.verify_project_metadata_consistency"
verify_project_metadata_consistency = verify_project_metadata

# Integration alias: bob.init_re_run.verify_project_metadata delegates here.
...
def sigterm_subagent_on_terminal_transition(feature_id: str) -> list[int]:
    ...
    """
    from bob.orchestrator.subagent_reaper import reap_subagent_for_feature
...
def detect_pending_successor_verify(
    ...
    Delegates to ``bob.pending_successor_detector.detect_pending_successor_verify``.
...
def handle_pending_successor_verify(
    ...
    Delegates to bob.status_handlers.handle_pending_successor_verify.
    """
...
def should_defer_to_successor_verifier(
    ...
    Delegates to bob.status_handler.should_defer_to_successor_verifier.
...
def should_defer_to_successor_gen(
    ...
    AC: ``Function defined: bob.run_loop.should_defer_to_successor_gen``.
...
# Alias satisfying AC: "Function defined: bob.run_loop.should_defer_to_successor_verify"
...
# AC alias: "Function defined: bob.run_loop.reap_orphan_subagents_backstop"
reap_orphan_subagents_backstop = backstop_reap_orphan_subagents

# AC alias: "Function defined: bob.run_loop.reap_orphan_subagents_sweeper"
...
def backstop_reaper(stale_minutes: int | float | None = None) -> list[tuple[str, int]]:
    ...
    Named entry point satisfying AC 'Function defined: bob.run_loop.backstop_reaper'.
...
def backstop_orphan_reaper(
    ...
    Named entry point satisfying AC 'Function defined: bob.run_loop.backstop_orphan_reaper'.
...
def backstop_reaper_for_orphan_subagents(
    ...
    Named entry point satisfying AC 'Function defined: bob.run_loop.backstop_reaper_for_orphan_subagents'.
    Catches handler-bypass paths (e.g. SIGKILL'd orchestrator restart mid-completion)
    where the completion handler never fired. Reaps any claude subagent whose tagged
    feature_id has been in a terminal state for longer than the dwell threshold (default
    5 minutes). Emits audit sentinel subagent_reaped_on_terminal=<feature_id> for each
...
def sigterm_subagent_on_terminal_state(feature_id: str) -> list[int]:
    ...
    Named entry point satisfying AC 'Function defined: bob.run_loop.sigterm_subagent_on_terminal_state'.
...
def sigkill_orphan_subagents_sweeper(
    ...
    Named entry point satisfying AC 'Function defined: bob.run_loop.sigkill_orphan_subagents_sweeper'.
    Catches handler-bypass paths (e.g. SIGKILL'd orchestrator restart mid-completion)
    where the completion handler never fired.  Reaps any claude subagent whose tagged
...
def _final_exit_sweep(project_id: str) -> None:
    ...
    """
    from bob.orchestrator.run_loop import _final_exit_sweep as _orch_final_exit_sweep
...
def _final_exit_sweep_with_reconciler(project_id: str) -> None:
    ...
    "Function defined: bob.run_loop._final_exit_sweep_with_reconciler".
...
def final_exit_sweep_with_disk_reconciliation(project_id: str) -> None:
    ...
    "Function defined: bob.run_loop.final_exit_sweep_with_disk_reconciliation".
...
def _run_locked(project_id: str) -> None:
    ...
    Delegates to bob.final_reaper.sweep_orphans_on_exit for the full sweep logic.

    This function satisfies the AC 'Function defined: bob.run_loop._run_locked'
...
def disk_reconciler_promote_check(
    ...
    Named entry point satisfying AC 'Function defined: bob.run_loop.disk_reconciler_promote_check'.
...
def reconciler_before_sweep_guard(
    project_id: str,
    feature_id: str,
    feature_name: str,
    acceptance_criteria_json: str,
) -> bool:
    """Reconciler-before-sweep guard: check disk ACs before flipping orphan-executing to failed.

    Entry point for F-R7-598. For each orphan-executing feature (no live subagent PID),
    invokes disk_reconciler's promote-if-ACs-satisfied check before the final sweep
    flips the feature to 'failed'.

    If all ACs are satisfied on disk (e.g. from prior generation inheritance), the
    feature is promoted to 'completed' and True is returned — the caller skips the
    flip-to-failed path. If any AC fails or an error occurs, returns False and the
    caller falls through to the existing flip-to-failed behavior.
    ...
    if not project_id:
        raise ValueError(
            f"reconciler_before_sweep_guard: project_id must be a non-empty string, "
            f"got {project_id!r}"
        )
    if not feature_id:
        raise ValueError(
            f"reconciler_before_sweep_guard: feature_id must be a non-empty string, "
            f"got {feature_id!r}"
        )
    from bob.orchestrator.run_loop import _check_executing_feature_acs
    return _check_executing_feature_acs(
        project_id=project_id,
        feature_id=feature_id,
        feature_name=feature_name,
...
def create_subagent_watchdog(
    ...
    """
    from bob.feature_watchdog import create_subagent_watchdog as _create_watchdog
...
async def dispatch_subagent(
    ...
    *,
    ...
    derived from ``BOB_FEATURE_TIMEOUT_SECONDS`` and forcibly cancels the
    ...
    from bob.feature_watchdog import create_subagent_watchdog as _arm_watchdog
...
def rca_auto_reset_on_verification_failure(
    ...
    """
    from bob.rca_auto_reset import auto_reset_on_code_defect
...
def classify_failure_cause(failed_acs: list[str]) -> str:
    ...
    Wraps ``bob.rca.classify_verification_failure_cause`` so callers inside the
    orchestration loop can import from ``bob.run_loop`` without pulling in the
    ...
    """
    from bob.rca import classify_verification_failure_cause
...
def apply_rca_auto_reset(
    ...
    This is the canonical ``bob.run_loop.apply_rca_auto_reset`` entry point
    that the orchestrator calls.  It delegates to
    ``bob.rca.auto_reset_on_code_defect``.
...
def should_emit_function_ac(ac_text: str) -> bool:
    """Return True when an AC requires emitted code to satisfy (F-R7-479 decision gate).

    Behavior, integration, and pytest ACs all require the subagent to emit
    correct code — a different subagent attempt may produce the right emission,
    so failures on these ACs are classified as ``code_emission_defect`` and
    eligible for a fresh attempt budget via F-R7-479.
    ...
    -------
    True
        The AC is a behavior, integration, or pytest AC (emitted-code AC).
    False
        The AC is a function-definition, spec_ambiguity, or other non-emitted-code AC.
    """
    if not isinstance(ac_text, str):
        raise TypeError(f"ac_text must be a str, got {type(ac_text).__name__!r}")
    stripped = ac_text.strip().lower()
    return (
        stripped.startswith("behavior:")
        or stripped.startswith("integration:")
        or stripped.startswith("pytest:")
    )


def handle_rca_reset_on_code_defect(
    feature_id: str,
    db_update_fn,
    failed_acs: list[str],
    refinement_attempts: int,
    ...
    """Reset feature to 'ready' when a verification-gate failure is a code-emission defect.

    Canonical F-R7-479 entry point for the orchestration loop.  Grants a fresh
    attempt when ``classify_failure_cause(failed_acs) == "code_emission_defect"``
    and ``refinement_attempts < 5``.  Returns True when the feature has been
    reset (caller must skip NH-demotion); False when NH-demotion should proceed.
    ...
    True
        Feature reset to ``ready``; caller must not NH-demote.
    False
        Failure is terminal or cap reached; caller continues with normal recovery.
    """
    from bob.rca_auto_reset import auto_reset_on_code_defect
    return auto_reset_on_code_defect(
        feature_id=feature_id,
...
def classify_mcp_transient_error(
    *,
    ...
    """Alias for classify_mcp_transient — satisfies AC 'Function defined: bob.run_loop.classify_mcp_transient_error'."""
    return classify_mcp_transient(stderr=stderr, retry_count=retry_count, feature_id=feature_id)
...
def classify_mcp_transient_before_hook(
    *,
    ...
    """Alias for classify_mcp_transient — satisfies AC 'Function defined: bob.run_loop.classify_mcp_transient_before_hook'.
...
def disk_reconciler_promotion_on_verify_fail(
    ...
    Canonical entry point required by AC "Function defined: bob.run_loop.disk_reconciler_promotion_on_verify_fail".
...
def seed_readiness_at_iteration_start(project_id: str) -> int:
    ...
    """
    import sqlite3
    from bob.db import get_connection, get_database_path, assess_feature_confidence, update_feature
...
def seed_readiness_zero_features(project_id: str) -> int:
    """Public entry point for AC 'Function defined: bob.run_loop.seed_readiness_zero_features'.
...
def seed_readiness_for_ready_features(project_id: str) -> int:
    """Public entry point for AC 'Function defined: bob.run_loop.seed_readiness_for_ready_features'.
...
def seed_readiness_on_ready_features(project_id: str) -> int:
    ...
    'Function defined: bob.run_loop.seed_readiness_on_ready_features'.
...
def apply_stuck_readiness_decomposition(
    ...
    *,
    ...
    """
    from bob.readiness.stuck_decomposition import (
...
def classify_evaluator_transient(
    *,
    ...
    Satisfies AC 'Function defined: bob.run_loop.classify_evaluator_transient'.
...
def classify_mcp_transient_failure(
    *,
    ...
    Satisfies AC 'Function defined: bob.run_loop.classify_mcp_transient_failure'.
    Same signature, same return value, same semantics.
    """
    return classify_evaluator_mcp_transient(
        verdict=verdict,
...
def load_exemption_sidecar(
    ...
    The sidecar directory defaults to ``<cwd>/.bob_startup_exempt/`` or the path
    specified by the ``BOB_STARTUP_EXEMPT_DIR`` environment variable.
    ...
    sidecar_dir:
        ...
        in the ``BOB_STARTUP_EXEMPT_DIR`` environment variable, or
        ``<cwd>/.bob_startup_exempt/`` if the env var is not set.
    ...
    if sidecar_dir is None:
        env_dir = os.environ.get("BOB_STARTUP_EXEMPT_DIR")
        if env_dir:
            resolved_dir = Path(env_dir)
        else:
            resolved_dir = Path.cwd() / ".bob_startup_exempt"
    else:
...
def increment_exemption_count(
    ...
    The sidecar directory defaults to ``<cwd>/.bob_startup_exempt/`` or the
    path specified by the ``BOB_STARTUP_EXEMPT_DIR`` environment variable.
    ...
    sidecar_dir:
        ...
        ``BOB_STARTUP_EXEMPT_DIR`` environment variable, or
        ``<cwd>/.bob_startup_exempt/`` if the env var is not set.
...
def reset_feature_ready_exempt(
    ...
    *,
    ...
    sidecar_dir:
        Directory for per-feature sidecar files.  Defaults to the path in the
        ``BOB_STARTUP_EXEMPT_DIR`` environment variable, or
        ``<cwd>/.bob_startup_exempt/`` if not set.
...
def readiness_score_derived(
    *,
    ...
    Public entry point satisfying AC
    'Function defined: bob.run_loop.readiness_score_derived'.
...
def seed_readiness_at_loop_start(project_id: str) -> int:
    ...
    Public entry point satisfying AC
    'Function defined: bob.run_loop.seed_readiness_at_loop_start'.
...
def readiness_seed_sweep(project_id: str) -> int:
    ...
    Public entry point satisfying AC
    'Function defined: bob.run_loop.readiness_seed_sweep'.
...
# Alias required by AC "Function defined: bob.run_loop.claim_first_feature_concurrent".
...
# Alias required by AC "Function defined: bob.run_loop.claim_batch_before_loop".
# The canonical implementation is claim_first_feature_before_batch_building; this alias
# satisfies the AC name without duplicating logic.
claim_batch_before_loop = claim_first_feature_before_batch_building

# Alias required by AC "Function defined: bob.run_loop.claim_and_dispatch_batch".
# The canonical implementation is claim_first_feature_before_batch_building; this alias
# satisfies the AC name without duplicating logic.
claim_and_dispatch_batch = claim_first_feature_before_batch_building
...
# F-R7-613 AC alias: "Function defined: bob.run_loop.check_startup_crash_exemption"
check_startup_crash_exemption = check_subagent_startup_crash_exemption

# F-2cbd8e87 AC alias: "Function defined: bob.run_loop.check_subagent_startup_crash_exempt"
check_subagent_startup_crash_exempt = check_subagent_startup_crash_exemption

# aac22c5e AC: "Function defined: bob.run_loop.check_subagent_startup_crash"
check_subagent_startup_crash = check_subagent_startup_crash_exemption

# F-R7-613 AC alias: "Function defined: bob.run_loop.load_exempt_count_sidecar"
...
def get_exempt_count(
    ...
    ``Function defined: bob.run_loop.get_exempt_count``.
    ...
    sidecar_dir:
        ...
        uses the ``BOB_STARTUP_EXEMPT_DIR`` env var or
        ``<cwd>/.bob_startup_exempt/``.
...
def verify_artifact_existence_pre_pytest(
    ...
    """
    from bob.ac_verifier import verify_artifact_existence
...
# Canonical short alias required by AC: "Function defined: bob.run_loop.claim_first_feature"
def claim_first_feature(
    *,
    ...
    bob.run_loop.claim_first_feature AC while preserving the longer descriptive name.
...
def build_concurrent_batch(
    *,
    ...
    Alias for claim_first_feature_before_batch_building — satisfies the
    bob.run_loop.build_concurrent_batch AC.
...
# Alias required by AC "Function defined: bob.run_loop.claim_batch_head".
# The canonical implementation is claim_first_feature_before_batch_building; this alias
# satisfies the AC name without duplicating logic.
claim_batch_head = claim_first_feature_before_batch_building

# Alias required by AC "Function defined: bob.run_loop.claim_batch_executing".
# Satisfies the a94e8021-d4c2-44ed-abfe-1b17cf2e3a5d feature AC without duplicating logic.
claim_batch_executing = claim_first_feature_before_batch_building
...
def claim_batch_feature(feature, *, update_feature) -> None:
    ...
    bob.run_loop.claim_batch_feature AC.
...
def check_reap_backoff_eligibility(feature, now=None) -> bool:
    ...
    """
    from bob.reaper import should_refuse_redispatch  # noqa: PLC0415
...
def handle_zero_cost_with_work_events(
    ...
    """
    from bob.cost_enforcement import validate_reported_cost  # noqa: PLC0415
...
def log_cost_telemetry_lost(
    ...
    Delegates to bob.cost_enforcement.log_cost_telemetry_lost. Exposed here so
    ...
    """
    from bob.cost_enforcement import log_cost_telemetry_lost as _log  # noqa: PLC0415
...
async def enforce_feature_timeout(
    ...
    *,
    ...
    from bob.run_loop so that ACs that reference this module can import it
    directly.  Delegates to :func:`bob.timeout.enforce_wall_clock_timeout`.
    ...
    Raises:
        ...
        bob.timeout.FeatureTimeoutError: When *coro* exceeds the wall-clock timeout.
    """
    from bob.timeout import enforce_wall_clock_timeout  # noqa: PLC0415
...
def load_spec_findings_at_boot(path: "Path | str | None" = None) -> dict:
    ...
    Delegates to bob.spec_findings.load_with_corruption_recovery so that any
    ScannerError (e.g. from a prior partial write) quarantines the corrupt file
    and returns {} rather than crash-looping the chain.

    Integration AC: bob.run_loop
    """
    from pathlib import Path as _Path

    from bob.spec_findings import load_with_corruption_recovery
...
def save_spec_findings_atomic(data: dict, path: "Path | str | None" = None) -> None:
    ...
    bob.spec_findings.write_atomic to prevent partial-write corruption.

    Integration AC: bob.run_loop
    """
    from pathlib import Path as _Path

    from bob.spec_findings import write_atomic
...
def is_structural_prefix_match(criterion: str) -> bool:
    ...
    Delegates to bob.demoter.is_structural_prefix_match — the canonical
    ...
    Integration AC: bob.run_loop
    """
    from bob.demoter import is_structural_prefix_match as _impl
...
def get_prose_connectors() -> "frozenset[str]":
    ...
    Delegates to bob.demoter.get_prose_connectors — the single source of
    ...
    Integration AC: bob.run_loop
    """
    from bob.demoter import get_prose_connectors as _impl
    return _impl()
...
def handle_shell_script_integration(
    ...
    """
    from bob.ac_handler import demote_shell_script_integration_ac
...
__all__ = [
    ...
    "handle_rca_reset_on_code_defect",
