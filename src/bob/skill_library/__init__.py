...
from bob.skill_library.embeddings import default_backend, load_from_env
from bob.skill_library.shim_manager import register_skill
...
def apply_shim(
    ...
    Thin public alias for bob.skill_library.registry.apply_skill that
    satisfies the AC "Function defined: bob.skill_library.apply_shim".
...
def write_shim(
    ...
    """Persist a new workaround shim into the persistent skill library.

    AC-required function: bob.skill_library.write_shim.
    Alias for write_discovery — persists a newly discovered workaround so
    future preflight calls can find and apply it via search_by_similarity.
...
def persist_workaround(
    capability_description: str,
    shim_module_src: str,
    workspace: Optional[pathlib.Path] = None,
    skill_id: Optional[str] = None,
) -> str:
    """AC-required alias for write_discovery (F-R7-473).

    Persist a newly discovered workaround shim into the persistent skill
    library so future preflight calls can find and apply it via
    search_library instead of re-spawning research agents.
...
def write_discovery(
    ...
    "Function defined: bob.skill_library.write_discovery".
...
def write_to_library(
    ...
    """AC-required function: bob.skill_library.write_to_library.

    Persist a newly discovered workaround shim into the persistent skill library.
    Called after a successful research-path discovery so future preflight calls
    hit the library instead of re-spawning research agents.
...
def search_skill_library(
    ...
    AC-required function: bob.skill_library.search_skill_library.
...
def persist_skill_to_library(
    ...
    AC-required function: bob.skill_library.persist_skill_to_library.
...
__all__ = [
    # AC-required names (this feature — 2f8dbfe9)
    "search_skill_library",
    "persist_skill_to_library",
    # AC-required names (feature — bb74fbcc)
