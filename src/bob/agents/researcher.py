...
Protocol (BF-2):
  ...
  2. Researcher prompt is built from ``bob.agents.roles.build_researcher_prompt``
     (no ticket text included).
  3. Research output is written to .bob/features/<id>/research_notes.md.
...
def run_researcher(
    *,
    ...
    required by the feature AC: "Function defined: bob.agents.researcher.run_researcher".
...
def dispatch_researcher(
    *,
    ...
    "Function defined: bob.agents.researcher.dispatch_researcher".
