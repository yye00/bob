"""bob.db_path_resolver — canonical DB path resolution.
...
priority chain:
  ...
  3. ``<cwd>/bob.db`` (lowest priority fallback)
...
def get_absolute_db_path(
    ...
    2. ``BOB_DATABASE_PATH`` environment variable
    3. ``cwd/bob.db`` (lowest priority, cwd-relative fallback)
    ...
    if db_path is not None:
        ...
        env_val = os.environ.get("BOB_DATABASE_PATH")
        if env_val:
            resolved = pathlib.Path(env_val).resolve()
        else:
            resolved = (pathlib.Path.cwd() / "bob.db").resolve()
