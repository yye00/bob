...
spec-quality gate. The implementation is :func:`bob.spec_synthesizer.score_gate_loop`;
...
:mod:`bob.gate_resynth` so the orchestrator has a single, stable import point.
"""
...
async def re_synthesize_acceptance_criteria(
    *,
    ...
    Delegates to :func:`bob.spec_synthesizer.score_gate_loop`, defaulting the
...
def resynthesize_gate_blocked_features(
    ...
    via the module-level set in :mod:`bob.gate_resynth` — prevents the livelock
    where gate-blocked features loop the blocked→test-writer→CodeT cycle forever.

    Delegates to :func:`bob.gate_resynth.resynthesize_gate_blocked_feature`.
    ...
    """
    from bob.gate_resynth import resynthesize_gate_blocked_feature as _resynth  # noqa: PLC0415
    return _resynth(
...
# Alias required by AC: "Function defined: bob.score_gate_loop.resynthesizegate_blocked_features"
...
def robust_import_scorer():
    ...
    :func:`bob.spec_synthesizer.resilient_import_scorer`.
...
def is_already_resynthesized(feature_id: str) -> bool:
    ...
    Delegates to :func:`bob.gate_resynth.is_synthesis_attempted`.
    ...
    """
    from bob.gate_resynth import is_synthesis_attempted as _is_attempted  # noqa: PLC0415
