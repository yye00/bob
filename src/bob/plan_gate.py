"""Public plan_gate façade for bob.
...
Delegates to bob.orchestrator.plan_gate for all implementation details.
"""
...
from bob.linter_22smell import (  # noqa: F401 — integration: bob.plan_gate
...
def trigger_spec_critic_on_edit(
    feature_id: str,
    name: str,
    ...
    """Re-trigger the spec critic when plan.yaml is edited and drift is detected.

    Alias for retrigger_critic_on_edit — satisfies the AC-required name
    bob.plan_gate.trigger_spec_critic_on_edit.

    When plan.yaml is edited, this function detects drift via
    compute_plan_vs_spec_drift and incrementally re-triggers the F-R7-450
    spec critic per F-R7-451 provenance rules.

    Returns
    -------
    list
        Defect list from the spec critic when drift is detected;
        empty list when no drift is found.
    ...
    return retrigger_critic_on_edit(
...
def lint_acceptance_criteria(
    ...
    *,
    ...
    :class:`ValueError` — blocking ``bob plan --create`` per spec.
