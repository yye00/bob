...
The ladder is read from ``BOB_MODEL_ESCALATION_LADDER`` (comma-separated, ordered
...
_ENV_VAR = "BOB_MODEL_ESCALATION_LADDER"
...
def parse_ladder(raw: str | None = None) -> list[str]:
    ...
    ``raw`` defaults to ``os.environ[BOB_MODEL_ESCALATION_LADDER]`` (or the
