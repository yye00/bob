"""Triton kernel synthesizer — public facade for bob.
...
Uses a direct file-path import for ``bob.implementers.triton_kernel`` to
avoid the circular import chain:
  bob.triton_kernel_synthesizer
    → bob.implementers.__init__
      → bob.orchestrator.plan_gate
        → bob.orchestrator.__init__
          → bob.triton_kernel_synthesizer  (partially initialised)
"""
...
# Direct file-path load to bypass bob.implementers.__init__, which would
