...
in :mod:`bob.orchestrator.cost_telemetry_guard`.  The orchestrator's
...
def validate_reported_cost(
    ...
    per_feature_ceiling:
        ...
        telemetry is lost (env ``BOB_PER_FEATURE_COST_CEILING``).
    feature_id:
...
def should_treat_cost_as_unknown(
    ...
    Wraps :func:`bob.orchestrator.cost_telemetry_guard.is_cost_telemetry_lost`
...
def validate_cost_and_work_events(
    ...
    ``bob.cost_enforcement.validate_cost_and_work_events``.
...
def should_apply_max_cost_ceiling(
    ...
    Named to match the AC predicate ``bob.cost_enforcement.should_apply_max_cost_ceiling``.
    Wraps :func:`bob.orchestrator.cost_telemetry_guard.is_cost_telemetry_lost`.
...
def enforce_zero_cost_protection(
    ...
    This is the canonical AC function (``bob.cost_enforcement.enforce_zero_cost_protection``).
