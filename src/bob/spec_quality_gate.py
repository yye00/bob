...
Integration with bob.synthesizer::

    from bob.spec_quality_gate import bypass_quality_threshold
    import bob.synthesizer  # synthesizer uses bob.spec_quality_gate internally
"""
...
def bypass_quality_threshold(feature: "Feature") -> bool:
    ...
    Additional patterns can be injected via ``BOB_ALLOWLIST_PATTERNS``
...
def should_bypass_quality_threshold(feature: "Feature") -> bool:
    ...
    Integration with bob.feature_reset: when a feature reset occurs (e.g. RCA
    ...
    ``bob.feature_reset`` can import and call this function directly.
...
# Alias: check_quality_gate_exemption is the function required by AC "Function defined: bob.spec_quality_gate.check_quality_gate_exemption".
...
def load_allowlist() -> list[str]:
    ...
    This is the canonical AC-required entry-point for feature 3f732534.
    Delegates to :func:`load_allowlist_patterns` from bob.spec_quality_allowlist.

    Returns
    -------
    list[str]
        Deduplicated list of pattern strings to match against feature spec_slot or name.
        Reads from ``BOB_ALLOWLIST_PATTERNS`` env var if set; falls back to defaults
...
def get_allowlist_config() -> dict[str, object]:
    ...
    dict[str, object]
        ...
        - ``"source"``: str — ``"env"`` when overridden via BOB_ALLOWLIST_PATTERNS,
          otherwise ``"defaults"``
    ...
    source = "env" if _os.environ.get("BOB_ALLOWLIST_PATTERNS", "").strip() else "defaults"
    return {
        "patterns": patterns,
        "source": source,
