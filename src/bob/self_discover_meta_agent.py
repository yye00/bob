...
Integration
-----------
Both functions delegate to :mod:`bob.spec_quality.section_selector` for
section classification and :mod:`bob.spec_quality.spec_extractor` for
extraction-time section pruning. They are intended to be called by
:mod:`bob.spec_synthesizer` before the extraction pass so that the critic
...
def select_spec_sections(
    ...
    Wraps :func:`bob.spec_quality.section_selector.select_sections` to
...
def extract_with_focused_sections(
    ...
    AC naming requirement ``bob.self_discover_meta_agent.extract_with_focused_sections``.
