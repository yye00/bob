"""Permanent-forward-carry auditor — public module for bob.permanent_forward_carry_auditor.

Re-exports the canonical implementation from bob.bootstrap.permanent_forward_carry_auditor
so that `from bob.permanent_forward_carry_auditor import audit_merged_spec` works as
specified by the acceptance criterion.

The actual implementation lives in bob.bootstrap.permanent_forward_carry_auditor.
Both import paths resolve to the same objects.

Also exports match_by_canonical_id and resolve_shortname_to_canonical from
bob.auditor.carry_forward_matcher to fix the silent-drop defect (F-R7-554):
...
from bob.auditor.carry_forward_matcher import (  # noqa: F401
...
def match_feature_by_canonical_id(
    ...
    bob.auditor.carry_forward_matcher. Uses regex matching to detect the
