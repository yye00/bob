"""Bob in-process memory client.

Provides an async-compatible API over bob.memory.BobMemory. This client
calls BobMemory directly — no sub-agent round-trip — because bob itself
runs in the same process as the memory backend. Sub-agents spawned by bob
still use the MCP server (bob.memory_mcp) to access the same underlying
store.
...
class BobMemoryClient:
    """High-level async client for bob in-process memory.

    Wraps BobMemory with the async API shape expected by bob's
    ...
    async def create_lesson_from_bug(
        ...
        *,
        ...
        Args:
            ...
            db_module: Optional module override (defaults to bob.db).
                Useful for testing.
        """
        if db_module is None:
            from bob import db as db_module  # type: ignore
        ...
        if result.success:
            ...
            if memory_id and hasattr(db_module, "update_bug"):
                try:
                    ...
                    # pre-bob-memory schema. It now holds bob memory IDs but
