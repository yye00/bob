"""Spec versioning and diff tracking for bob.
...
- SpecChangedAbort — exception raised when BOB_ABORT_ON_SPEC_CHANGE=true
"""
...
class SpecChangedAbort(Exception):
    """Raised when the spec changes mid-run and BOB_ABORT_ON_SPEC_CHANGE is true."""


def _abort_on_spec_change() -> bool:
    """Return True if BOB_ABORT_ON_SPEC_CHANGE is enabled (default: True)."""
    val = os.environ.get("BOB_ABORT_ON_SPEC_CHANGE", "true").strip().lower()
...
def check_spec_change_abort(*, old_version: str, new_version: str) -> None:
    ...
    if _abort_on_spec_change():
        raise SpecChangedAbort(
            ...
            "Set BOB_ABORT_ON_SPEC_CHANGE=false to continue anyway."
        )
