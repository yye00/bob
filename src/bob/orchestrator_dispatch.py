...
dispatching up to ``BOB_MAX_CONCURRENT_FEATURES`` (default 3) ready features
...
def resolve_max_concurrent_features() -> int:
    ...
    Reads ``BOB_MAX_CONCURRENT_FEATURES``; falls back to 3 on parse errors
...
def claim_ready_features(
    ...
    *,
    ...
    Returns:
        List of :class:`~bob.models.Feature` objects ready to dispatch.
...
async def dispatch_concurrent_features(
    ...
    *,
    ...
    if not claimed:
        return []
    # Lazy import to avoid circular dependency: bob.orchestrator.__init__
    # imports from this module; importing concurrent_executor at module level
    # would re-enter bob.orchestrator before it is fully initialised.
    from bob.orchestrator.concurrent_executor import run_concurrent  # noqa: PLC0415
