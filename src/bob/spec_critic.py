"""Adversarial spec-critic sub-agent — top-level bob module.

Thin wrapper around :mod:`bob.spec_quality.spec_critic` that exposes a
``critique_spec`` entry point matching the bob.spec_critic AC contract.

Public API::

    from bob.spec_critic import critique_spec
...
def critique_spec(
    ...
    *,
    ...
    constitution_path:
        ...
        then to ``src/bob/spec_quality/spec_constitution.md``.
    findings_path:
