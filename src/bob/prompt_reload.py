...
Thin public API over ``bob.orchestrator.prompt_source_reloader``.
...
def reload_if_modified(module_name: str = "bob.superpowers") -> bool:
    """Reload *module_name* if its on-disk source has changed since last call.

    Wraps :func:`bob.orchestrator.prompt_source_reloader.reload_if_stale`.
...
def reload_if_changed(module_name: str = "bob.superpowers") -> bool:
    """Reload *module_name* if its on-disk source has changed since last call.
