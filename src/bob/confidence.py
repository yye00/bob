"""Live confidence assessment for bob features.
...
from bob.db import assess_feature_confidence  # re-export canonical implementation


def decay_confidence_after_failure(feature_id: str) -> object:
    """Decay confidence components after a failed feature attempt.

    Public entry point satisfying AC 'Function defined: bob.confidence.decay_confidence_after_failure'.
    Delegates to the orchestrator's decay_confidence_components — decays only
    conf_impl_correctness, conf_spec_understanding, and conf_test_adequacy;
    readiness_score is NOT written (it is derived on read).
    ...
    return decay_confidence_components(feature_id)


__all__ = ["assess_feature_confidence", "decay_confidence_after_failure"]
