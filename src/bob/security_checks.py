"""Security verification check #9 for Bob.
    ...
    if src.is_dir():
        for child in src.iterdir():
            if child.is_dir() and (child / "__init__.py").exists():
                ...
                # This handles src/bob/spec_quality/ (no bob/__init__.py) correctly.
                for grand in child.iterdir():
                    if grand.is_dir() and (grand / "__init__.py").exists():
                        pkgs.add(grand.name)
                    elif grand.is_file() and grand.suffix == ".py" and grand.stem != "__init__":
    ...
    if tools_dir.is_dir():
        ...
        for child in tools_dir.iterdir():
            if child.is_file() and child.suffix == ".py" and child.stem != "__init__":
                ...
                pkgs.add(child.name)
    # The gen's OWN lineage names (bob, bob71, bob72, ...) are first-party: the
...
def _pypi_package_exists(name: str, *, timeout: int) -> bool | None:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "bob-security-checks/1.0"})
    try:
    ...
    try:
        ...
        with default_settings():
            if diff:
                ...
                # real risk. BOB_SECRETS_RECENT_SECONDS (default 3600) and a hard
                # file cap keep it bounded; .venv/tests/etc. are excluded.
                import os as _os3
                try:
                    _recent = float(_os3.environ.get("BOB_SECRETS_RECENT_SECONDS", "3600"))
                except (TypeError, ValueError):
                    _recent = 3600.0
                _now = time.time()
                _skip_parts = {".venv", "venv", "__pycache__", ".git", "build", "dist",
                               "tests", ".bob", "workspace", "node_modules"}
                _scanned = 0
                _CAP = 400
    ...
    *,
    ...
    Args:
        workspace: directory containing the implementation under
            evaluation. May or may not be the same as the bob repo
