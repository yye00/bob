...
Before bob dispatches a fresh implementer for a "new" feature, this module
...
def write_resurrection_report(
    ...
    The report is written to:
        <bob_root>/features/<feature_id>/resurrection_report.md
    ...
    Args:
        ...
        bob_root: Root of the .bob directory (default: '.bob').

    Returns:
        Absolute path to the written report file.
    """
    report_dir = Path(bob_root) / "features" / feature_id
    ...
    lines += [
        "---",
        "*Generated by bob.brownfield.resurrection (BF-5)*",
    ]
...
def filter_signals_by_feature_flags(
    ...
    to bob. Gates B and C behind deep_resurrection_scan, which defaults OFF.
...
# AC alias: singular form required by BF-5 045b527f ACs.
detect_exported_stub = detect_export_without_impl
...
# AC alias: BF-5 7ea1e9ae requires bob.brownfield.resurrection.detect_stub_exports.
detect_stub_exports = detect_export_without_impl

# AC alias: BF-5 acceptance criteria require bob.brownfield.resurrection.detect_signals.
detect_signals = detect_resurrection_signals

# AC alias: BF-5 0a43b348 requires bob.brownfield.resurrection.check_resurrection_signals.
...
def get_graveyard_signal(
    ...
    Mode. Only the GitHub-graveyard signal is unique to bob.
...
# AC alias: singular form required by F-R7-611 AC (bob.brownfield.resurrection.filter_signals_by_feature_flag)
