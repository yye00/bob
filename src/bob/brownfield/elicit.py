...
#       Claude Code cannot prompt the user; bob must BRANCH-INTO-CANDIDATES.
...
# Only the headless path contains bob-specific logic.
#
# BF-3 adds:
...
from bob.brownfield.localizer import localize as _localize_intent
...
def _emit_ask_user_question(request: ElicitationRequest) -> dict[str, Any]:
    ...
    The AskUserQuestion tool handles the Pydantic intent schema; bob
...
def _branch_into_candidates(request: ElicitationRequest) -> list[dict[str, Any]]:
    ...
    When running under `claude -p` (no human), bob cannot prompt the user.
...
def elicit(
    ...
    The headless path is bob-specific (Claude Code cannot prompt the user there).
    """
...
# AC alias: AC requires bob.brownfield.elicit.clarification_gate
...
def branch_into_candidates(request: ElicitationRequest) -> list[dict]:
    ...
    When running under `claude -p` (no human), bob branches into multiple
...
# AC alias: F-R7-611 requires bob.brownfield.elicit.branch_headless_candidates
...
def elicit_with_localization(
    ...
    *,
    ...
    BF-4 integration: calls bob.brownfield.localizer.localize to produce
...
def route_by_mode(
    ...
    AC-required entry point for bob.brownfield.elicit.route_by_mode.

    Dispatches based on feature.mode:
      - 'interactive': emit AskUserQuestion via host SDK (thin redirect only).
      - 'headless':    BRANCH-INTO-CANDIDATES (F-R7-605 path; unique to bob).
...
# AC alias: BF-3 AC requires bob.brownfield.elicit.extract_brownfield_intent
...
clarification_policy = apply_clarification_gate

# AC alias: BF-3 AC requires bob.brownfield.elicit.gate_clarification
...
def branch_or_ask(
    ...
    AC-required entry point for bob.brownfield.elicit.branch_or_ask (F-R7-611).
