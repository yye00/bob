...
Brownfield EDIT workflow:
  1. emit_diff_plan  — write .bob/features/<id>/diff_plan.yaml before any edits
  2. apply_diff_plan — backup originals → apply hunks → return modified paths
  3. rollback_changes — restore from .bob/features/<id>/orig/<path> backups
...
def _feature_dir(feature_id: str, workspace: Path) -> Path:
    return workspace / _BOB_DIR / _FEATURES_DIR / feature_id
...
def apply_diff_plan(
    ...
    *,
    ...
    For each touched file:
      1. Backup the original to .bob/features/<id>/orig/<path>.
...
def rollback_changes(
    ...
    *,
    ...
    """Restore original file contents from .bob/features/<id>/orig/ backups.
    ...
    *,
    ...
    This is the primary entry point for implementer subagents.  It:
      ...
      2. Emits the reviewable diff_plan.yaml artifact under
         .bob/features/<feature_id>/diff_plan.yaml.
...
# ---------------------------------------------------------------------------
# rollback_edits — AC-required alias for rollback_changes
# ---------------------------------------------------------------------------
...
def rollback_edits(
    ...
    *,
    ...
    """Restore original file contents from backups (AC alias for rollback_changes).
