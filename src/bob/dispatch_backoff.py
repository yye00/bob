...
should_refuse_recent_reap(feature, now)
    ...
    escalated. Delegates to bob.reaper for the heavy logic.
...
def check_backoff_eligibility(
    feature: "Feature",
    now: datetime | None = None,
) -> bool:
    """Return True if the feature is ELIGIBLE for dispatch (not in backoff).

    This is the inverse of should_refuse_recent_reap: returns True when dispatch
    may proceed, False when it should be refused (feature is within the backoff
    window or has been escalated to needs_human).
    ...
    Returns:
        True if dispatch may proceed; False if it should be refused.

    Raises:
        ValueError: If feature is None or lacks an 'id' attribute.
    """
    return not should_refuse_recent_reap(feature, now=now)
...
def check_reap_backoff(
    ...
    This is the AC-required entry point on bob.dispatch_backoff satisfying
    the acceptance criterion "Function defined: bob.dispatch_backoff.check_reap_backoff".
    ...
    Raises:
        ValueError: If feature is None or lacks an 'id' attribute.
    """
    return should_refuse_recent_reap(feature, now=now)
...
def record_reap_event(
    ...
    """Record a reap event by stamping last_reap_at and reap_count on the feature row.

    Called by the stuck_executing_reaper after resetting a feature to 'ready'.
    The stamped fields are the persistent memory the dispatch loop uses to
    enforce exponential backoff via check_reap_backoff / should_refuse_recent_reap.

    This is the AC-required entry point satisfying
    "Function defined: bob.dispatch_backoff.record_reap_event".
    ...
    Raises:
        ValueError: If feature_id is empty or reap_count is not a non-negative int.
    """
    stamp_reap_metadata(feature_id, reap_count=reap_count, now=now)


__all__ = [
    "check_backoff_eligibility",
    "check_reap_backoff",
    "record_reap_event",
