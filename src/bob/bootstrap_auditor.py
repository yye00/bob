...
This module is a thin coordination layer that delegates to:
    bob.bootstrap.permanent_forward_carry_auditor

The re-export at the public ``bob.bootstrap_auditor`` path satisfies the
acceptance criterion:
    File exists: src/bob/bootstrap_auditor.py
    Function defined: bob.bootstrap_auditor.audit_permanent_forward_carry
    Function defined: bob.bootstrap_auditor.PermanentForwardCarryMissing
"""
...
def audit_permanent_forward_carry(spec: dict[str, Any]) -> None:
    ...
    Single-call entrypoint for the bootstrap auditor at the ``bob.bootstrap_auditor``
