"""Carry-forward auditor — regex-based canonical ID matching for bob.
...
the public API so imports from bob.carry_forward_auditor work as specified
...
from bob.auditor.carry_forward_matcher import resolve_feature_reference  # noqa: F401
...
def audit_carry_forward_by_canonical_id(
    spec: dict[str, Any],
    *,
    required: frozenset[str] | None = None,
) -> frozenset[str]:
    """Audit a spec for carry-forward features by canonical F-R7-NNN ID regex.

    Matches features by scanning id, title, and description fields for
    F-R7-NNN tokens using a regex — not exact string comparison. This prevents
    false-positive missing reports and false-negative silent drops that occur
    when sidecars are renamed or features are referenced by shortname only.
...
__all__ = [
    "BootstrapAuditError",
    "audit_carry_forward_by_canonical_id",
