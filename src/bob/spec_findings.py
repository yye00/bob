"""bob.spec_findings — atomic write and quarantine for spec_findings.yaml.

Feature b37d0f34-5d02-4fdc-be9b-b205e2839fcb

Problem (bob version 13 r10, 2026-05-29 00:37 UTC): a partial write left a
...
def write_atomic(data: dict[str, Any], path: Path | str) -> None:
    """AC-facing alias for :func:`atomic_write` (AC: bob.spec_findings.write_atomic)."""
    atomic_write(data, path)


def load_with_corruption_recovery(path: Path | str) -> dict[str, Any]:
    """AC-facing alias for :func:`load_safe` (AC: bob.spec_findings.load_with_corruption_recovery)."""
