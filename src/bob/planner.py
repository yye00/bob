"""bob.planner — planning-time AC validation integration.

Wires :func:`bob.validators.validate_acceptance_criteria` and the full
22-smell linter (:mod:`bob.linter.smella_22`) into the feature-creation
flow so malformed or smelly acceptance criteria are rejected before
features are persisted to the database.

Usage (called internally by ``bob plan --create``)::

    from bob.planner import validate_feature_acs
...
from bob.validators import MalformedACError, validate_acceptance_criteria
from bob.spec_quality.composite_score import compute_composite_score  # noqa: F401 — integration
from bob.linter.smella_22 import detect_all_smells, filter_by_severity, blocks_plan_create  # noqa: F401 — integration
from bob.orchestrator.plan_gate import emit_plan_ready_event as _emit_plan_ready_event  # noqa: F401 — re-export
...
def emit_plan_ready_event(
    ...
    Delegates to :func:`bob.orchestrator.plan_gate.emit_plan_ready_event`.
    Exposed here so callers can import from bob.planner directly.
...
def validate_feature_acs(feature: dict) -> list[str]:
    ...
    if blocking_details:
        raise ValueError(
            "E-severity smell(s) block bob plan --create:\n"
