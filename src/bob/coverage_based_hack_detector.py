"""Coverage-based hack detector for Bob (feature 8c495c04).

Rejects implementations whose passing tests cover less than a configurable
threshold (BOB_MIN_COVERAGE, default 70%) of source lines. Uses coverage.py
...
def _resolve_threshold(threshold: float | None) -> float:
    """Return the effective threshold, reading BOB_MIN_COVERAGE from env if needed."""
...
def measure_coverage(
    *,
    ...
    The threshold defaults to the value of the ``BOB_MIN_COVERAGE`` environment
