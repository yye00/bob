...
Usage:
    from bob.memory import BobMemory
...
DEFAULT_USER_ID = "bob_default"


def _bob_data_dir() -> Path:
    """Return the persistent data directory for bob memory."""
    override = os.environ.get("BOB_MEMORY_DIR")
    if override:
        path = Path(override)
    else:
...
def _default_config() -> dict[str, Any]:
    """Build the default mem0 configuration for bob.
    ...
    """
    data_dir = _bob_data_dir()
    qdrant_path = str(data_dir / "qdrant")

    embedder_model = os.environ.get("BOB_EMBEDDER_MODEL", DEFAULT_EMBEDDER_MODEL)
    embedding_dims = int(os.environ.get("BOB_EMBEDDING_DIMS", str(DEFAULT_EMBEDDING_DIMS)))
...
class BobMemory:
    ...
    Use this from bob itself. Sub-agents should call the memory MCP
