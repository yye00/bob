...
Architecture:
  - Delegates to :func:`bob.spec_synthesizer.score_gate_loop` for the
    retry/scoring logic and :func:`bob.spec_synthesizer.deterministic_fallback`
    for exhaustion fallback.
  - The score gate loop calls the supplied ``synthesize_fn`` (default:
    :func:`bob.spec_synthesizer.synthesize_for_feature`) up to ``max_retries``
...
async def spec_synthesizer_score_gate_loop_re_synthesize_tbd_acs_until(
    *,
    ...
    Wraps :func:`~bob.spec_synthesizer.score_gate_loop` to provide a
    ...
    project_id:
        Bob project ID forwarded to the LLM sub-agent spawn call.
    synthesize_fn:
        Async callable with signature ``(*, project_id, title, description,
        project_context, workspace, retry_feedback) -> list[str] | None``.
        Defaults to :func:`~bob.spec_synthesizer.synthesize_for_feature`.
    threshold:
        Composite score threshold [0, 1] that the ACs must reach.
        Defaults to ``BOB_SPEC_QUALITY_THRESHOLD`` env var (0.85 if unset).
    max_retries:
        Maximum number of synthesis attempts before falling back.
    use_fallback:
        When True (default), use :func:`~bob.spec_synthesizer.deterministic_fallback`
