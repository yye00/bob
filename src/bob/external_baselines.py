...
emit the same F-105 telemetry schema that internal bob runs produce.

This lets the sweep orchestrator compare external tool performance against
bob's internal ablation variants without any post-processing.
"""
...
def run_claude_code_baseline(
    ...
    *,
    ...
    Claude Code's own token/cost accounting is independent of bob's internal
    sub-agent budget tracking. The result is recorded with variant="claude-code"
    in run.jsonl so the sweep orchestrator can distinguish it from bob runs.
