...
Context
-------
bob v.15 r12 observed feature 9b2e1060 crash with work_events=176217,
...
--------------------------------------
:mod:`bob.orchestrator.cost_telemetry_guard` already contains
...
__all__ = [
    ...
    "is_telemetry_loss_likely",
    "enforce_budget_on_zero_cost_with_work_events",
    "enforce_cost_with_work_events",
...
def enforce_budget_on_zero_cost(
    ...
    """AC-required entry point: enforce budget even when reported cost is zero.

    Delegates to :func:`enforce_budget_on_zero_cost_with_work_events`.
    ...
    """
    # Forward to the canonical implementation defined later in this module.
    # The function is defined after EnforceBudgetWithWorkEventsResult, so
    # this forward call works at runtime (not at import time).
...
def is_telemetry_loss_likely(
    reported_cost: float | None,
    work_events: int,
) -> bool:
    """Return True when stream-json telemetry loss is likely for this run.

    Telemetry loss is detected when reported_cost is zero (or None/negative)
    AND work_events exceeds the configured threshold (default: 100, env
    ``BOB_COST_TELEMETRY_LOST_WORK_THRESHOLD``).
    ...
    -------
    bool
        True  — cost is zero AND work_events > threshold: telemetry likely lost.
        False — cost > 0 (telemetry arrived), OR work_events == 0 (genuine
                spawn crash with no work, free-retry path).
    """
    return _is_cost_telemetry_lost(
        reported_cost=reported_cost,
        work_events=work_events,
    )
...
def enforce_budget_on_zero_cost_with_work_events(
    ...
    per_feature_ceiling:
        ...
        when telemetry is lost (env BOB_PER_FEATURE_COST_CEILING).
    feature_id:
        Feature UUID for structured logging.
    exit_code:
        Sub-agent exit code (None if unknown).
