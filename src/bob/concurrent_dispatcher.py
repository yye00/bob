"""Concurrent feature dispatcher for the Bob orchestrator.
...
``bob.orchestrator.run_loop`` and ``bob.orchestrator.concurrent_executor``
...
async def dispatch_concurrent_features(
    ...
    *,
    ...
    This wraps :func:`bob.orchestrator.run_loop.dispatch_concurrent_features`
    to expose the function at the ``bob.concurrent_dispatcher`` namespace as
...
def fill_empty_slots(
    ...
    *,
    ...
    This wraps :func:`bob.orchestrator.run_loop.dispatch_up_to_concurrency`
    to expose slot-filling at the ``bob.concurrent_dispatcher`` namespace as
    required by the feature ACs.
    ...
    Returns:
        List of :class:`~bob.models.Feature` objects that were claimed and
...
async def gather_completed_features(
    ...
    This wraps :func:`bob.orchestrator.run_loop.gather_completed_dispatches`
    to expose the function at the ``bob.concurrent_dispatcher`` namespace as
    required by the feature ACs.
...
# AC alias: "Function defined: bob.concurrent_dispatcher.gather_completed_tasks"
