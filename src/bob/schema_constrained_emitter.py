...
the bob pipeline.  Uses the pinned ``schemas/spec.v1.json`` schema to
...
import bob.critic  # noqa: F401 — integration hook: validated specs feed into bob.critic
...
def emit_with_schema_constraint(
    ...
    *,
    ...
    requirement ``bob.schema_constrained_emitter.emit_with_schema_constraint``.
...
def emit_constrained_spec(
    ...
    *,
    ...
    Validated specs feed directly into :mod:`bob.spec_critic` to gate
...
def validate_against_spec(
    ...
    *,
    ...
    requirement ``bob.schema_constrained_emitter.validate_against_spec``.
