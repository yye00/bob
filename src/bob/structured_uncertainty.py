"""bob.structured_uncertainty — Structured-uncertainty clarification loop.
...
def clarify_ambiguous_slots(
    ...
    *,
    ...
    if uncertain_slots:
        _ci = ci_mode
        if _ci is None:
            env_val = os.environ.get("BOB_CI_MODE", "").lower()
...
def trigger_clarification_loop(
    ...
    *,
    ...
    max_per_round:
        Maximum questions per interactive round (1-5, default 5).
    ci_mode:
        Force CI mode on/off. When None, reads ``BOB_CI_MODE`` env var.
    audit_log_path:
...
def generate_stub_implementations(
    ...
    *,
    ...
    ``Function defined: bob.structured_uncertainty.generate_stub_implementations``
