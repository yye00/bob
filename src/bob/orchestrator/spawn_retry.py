...
4. Logs each retry attempt to ``.bob/agent_logs/<job>.retry.jsonl``.
...
def _cleanup_partial_state(feature_id: str, workspace: str | os.PathLike[str] | None) -> None:
    ...
    # Orphan lock file under .bob/locks/<feature_id>
    lock_path = ws / ".bob" / "locks" / feature_id
    if lock_path.exists():
        try:
            lock_path.unlink()
...
async def spawn_with_retry(
    ...
    *,
    ...
    if log_dir is not None:
        ...
        _log_dir = Path(workspace) / ".bob" / "agent_logs"
    else:
        _log_dir = Path(".bob") / "agent_logs"
