"""Project metadata consistency checks for bob spawn/init lifecycle.

After spawn_next_generation.sh rsync-copies the parent directory, the
child bob.db still contains the parent's project name and spec_path.
The functions here detect and fix those stale values, and are called
both by the spawn script (via ``bob init``) and by run_loop at startup.
"""
...
def run_bob_init_post_rsync(
    ...
    *,
    ...
    """Execute ``bob init`` with the correct --name and --spec after rsync.
    ...
    child bob.db row reflects bob<NEXT> metadata.
    ...
    logger.info("Running post-rsync bob init: %s", " ".join(cmd))
...
def verify_project_name_matches_dir(
    *,
    ...
    Args:
        db_path: Path to bob.db; defaults to BOB_DATABASE_PATH or cwd/bob.db.
...
def reject_pytest_tmpdir_in_spec_path(
    *,
    ...
    ``bob init`` with a tempdir spec, the projects row ends up with
    ...
    if stale:
        ...
        raise StaleSpecPathError(
            f"spec_path contains 'pytest-of-' tmpdir leak — re-run bob init with "
...
def _resolve_db_path(db_path: Optional[pathlib.Path]) -> pathlib.Path:
    """Resolve a database path, falling back to env var then cwd/bob.db."""
