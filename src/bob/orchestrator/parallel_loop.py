...
that uses asyncio.gather with a semaphore capped at BOB_PARALLELISM
...
Performance motivation:
  ...
  parallel: BOB_PARALLELISM=4  → ~40 min on the same hardware
"""
...
from bob import db
from bob.orchestrator.run_loop import OrchestrationLoop, cascade_update_dependents
...
def _resolve_parallelism() -> int:
    ...
    Reads BOB_PARALLELISM from the environment; falls back to 4 for
    missing, non-integer, zero, or negative values.
    """
    raw = os.environ.get("BOB_PARALLELISM")
    if raw is not None:
        try:
            value = int(raw)
            if value > 0:
...
async def run_parallel_loop(
    ...
    **kwargs: Any,
    ...
    a semaphore capped at BOB_PARALLELISM, calls cascade_update_dependents
