...
def sweep_orphan_subagents():
    """Module-level proxy so patch('bob.orchestrator.final_reaper.sweep_orphan_subagents') works in tests."""
    return _get_subagent_reaper().sweep_orphan_subagents()


def find_subagent_pid_for_feature(feature_id: str) -> list[int]:
    """Module-level proxy so patch('bob.orchestrator.final_reaper.find_subagent_pid_for_feature') works in tests."""
