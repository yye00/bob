...
def _read_threshold() -> int:
    """Read BOB_COST_TELEMETRY_LOST_WORK_THRESHOLD, clamped to [1, 10000]."""
    raw = os.environ.get("BOB_COST_TELEMETRY_LOST_WORK_THRESHOLD", "")
    if raw:
        try:
            value = int(raw)
...
def enforce_budget_on_zero_cost(
    ...
    work_events:
        Count of substantive progress events in progress.jsonl.
    per_feature_ceiling:
        The per-feature max-cost ceiling (e.g. from BOB_PER_FEATURE_COST_CEILING).
    feature_id:
...
def enforce_budget_with_telemetry_loss(
    ...
    Satisfies AC: Function defined: bob.orchestrator.enforce_budget_with_telemetry_loss
