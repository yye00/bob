...
Integration with the Anthropic tool-use ``input_schema`` constrained-decoding
path is provided by :mod:`bob3.spec_quality.schema_constrained_emit`.
"""
...
def emit_with_schema(
    ...
    *,
    ...
    and then retrying on parse failure, callers must emit conforming JSON
    on the first attempt (via Anthropic tool-use ``input_schema`` or Outlines
    logit masking).  Any spec that does not conform is REJECTED immediately.
