...
def research_workaround(probe_result: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """Spawn a research sub-agent to surface a concrete workaround for a missing dep.

    When a dependency is not found, this function simulates spawning a research
    sub-agent that queries for installation or emulation strategies. The workaround
    is marked ``low_risk=True`` when it involves only Python package installation
    (pip install) — these can be auto-applied. CLI workarounds are marked
    ...
    Returns:
        A dict with keys ``dep_name``, ``description``, ``low_risk``, ``commands``;
        or None if the dep is already present.
    ...
    # Known workarounds database (simulates what the research sub-agent would surface)
...
def preflight(
    ...
    1. enumerate_dependencies -> list of deps
    2. probe_dependency for each dep
    3. research_workaround for each missing dep
    4. Auto-apply low-risk workarounds; raise MissingDependencyError for high-risk

    Args:
        ac_list: List of acceptance criteria strings. May be empty.
        workspace: Optional project root path (unused currently, reserved for future persistence).
    ...
    Returns:
        ...
        - halted: always False (raises MissingDependencyError instead of halting silently)
