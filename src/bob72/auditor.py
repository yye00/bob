...
Integration with bob.evaluator is provided via evaluate_canonical_carry so
that the evaluator can verify carry-forward compliance using the same regex
logic rather than exact string comparison.

Implementation delegates to bob.auditor.carry_forward_matcher which is
self-contained (no bob.__init__ import chain) to avoid circular imports.
"""
...
def required_feature_ids() -> frozenset[str]:
    ...
    Reads the base set plus any extra IDs from the BOB_PERMANENT_CARRY_IDS
    ...
    """
    extra_raw = os.environ.get("BOB_PERMANENT_CARRY_IDS", "")
    extras: set[str] = set()
