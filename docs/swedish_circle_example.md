# Swedish Circle — a bob case study

This document walks through the swedish-circle example end-to-end:
the geotechnical method itself, how the spec was written, what bob
did with it, what got escalated to a human, what bob features were
...
acceptance criteria, and bob only marks a feature `completed` when
the criteria pass.

---

## How this was built with bob
...
The build went through several phases, each ended by `bob run`
...
3. **Resume after R10-020 fix** — the test-run timeout fix shipped
   in bob itself; resume completed F011 and F012.
4. **Recovery via `bob verify-feature`** — F010, F014, F016 were
   marked `needs_human` due to bugs in bob's verifier (see below);
...
|---|---|---|
| F010 (Abramson V&V) | `code_changes_made` failed on a re-run because mtimes were stale (R10-021) | `bob verify-feature` after `touch` |
| F014 (Slope canvas) | Same: mtime-stale re-run (R10-021) | `bob verify-feature` |
| F016 (Material dialog) | Same: prior attempt had completed work; re-run didn't modify files (R10-021) | `bob verify-feature` |
...
## bob features exercised

The build pulled on most of bob's machinery. Each item below was
...
After each sub-agent run, bob ran a checklist:
...
OOM under the 9 GB swap pressure during long concurrent builds). bob
...
When a sub-agent crashed mid-run, bob still ran the post-execution
...
low-confidence on first encounter (`spec=0.50, impl=0.30`). bob
...
### `bob verify-feature` CLI for human recovery
...
The registry isn't just a bug list; it's how bob learns across
specs which classes of mistakes its verifier and orchestrator make
under real load.

### Session resumability

The build spanned five `bob run` invocations over several hours.
...
Filed as `R10-*` in the registry. Roughly half were real bob bugs
...
| R10-020 | high | `BOB_TEST_RUN_TIMEOUT` 300 s default rejects valid V&V suites |
...
R10-013, -014, -015, -016, -018, -020 were fixed in bob itself
...
After bob finished, the swedish-circle workspace was a working
...
four small improvements were made by hand on top of what bob shipped.
...
misread as "bob produced exactly this."

### 1. FoS contour overlay rewrite

bob's sub-agent shipped F012 with a working but visually rough
...
heatmap and the original blocky red-and-white grid bob produced.
...
This is the kind of failure pattern bob's verifier *should* catch
...
Honest summary: bob produced a 19-feature, 105-test, 2-method,
