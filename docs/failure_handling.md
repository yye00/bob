# When implementations fail — bob's recovery pipeline
...
expensive in different ways. Bob sits between them: it classifies
...
> [`src/bob/orchestrator/run_loop.py`](../src/bob/orchestrator/run_loop.py)
> unless noted.
...
gates exist precisely because earlier versions of bob failed in
those specific ways.
...
[`run_loop.py:245`](../src/bob/orchestrator/run_loop.py):
...
After the spawn check, bob runs the verification checklist
...
The checklist (in [`superpowers.py`](../src/bob/superpowers.py)):
...
When verification fails, bob increments
`feature.refinement_attempts` (against the cap
`max_refinement_attempts`, default 5) and then **decays the
confidence scores** ([`run_loop.py:1339`](../src/bob/orchestrator/run_loop.py)):
...
The decay rate is `BOB_CONFIDENCE_DECAY_PER_FAILURE` (default
...
`refinement_attempts >= 2` post-increment), bob spawns an **RCA
sub-agent** ([`run_loop.py:2171`](../src/bob/orchestrator/run_loop.py)).
...
1. **Feature flag** — `BOB_RCA_ENABLED` (default 1; tests set it
...
`BOB_RCA_TIMEOUT_SECONDS` (default 600 s = 10 minutes). RCA is
...
([`claude_executor.py:1162`](../src/bob/orchestrator/claude_executor.py))
...
([`claude_executor.py:1198`](../src/bob/orchestrator/claude_executor.py))
...
([`run_loop.py:2941`](../src/bob/orchestrator/run_loop.py)):
...
gate ([`run_loop.py:1383`](../src/bob/orchestrator/run_loop.py))
...
([`run_loop.py:2446`](../src/bob/orchestrator/run_loop.py)):
...
call site ([`run_loop.py:1078`](../src/bob/orchestrator/run_loop.py)).
...
([`run_loop.py:1383`](../src/bob/orchestrator/run_loop.py)):

1. **Explicit marker** — feature description contains
   `research_required=True`.
2. **Failure threshold** — `count_feature_failures(feature) >=
   BOB_FAILURE_THRESHOLD_FOR_RESEARCH` (default 2). If a feature
...
This is the terminal state for the automated loop: bob emits
warnings, persists the failure evidence, and continues to the next
feature.

A human can recover via:

- **`bob verify-feature <id>`** — re-runs the verification
...
- **`bob run`** again — `needs_human` features are *not*
...
|---|---|---|
| `BOB_RCA_ENABLED` | `1` | Master switch for RCA spawn |
| `BOB_RCA_TIMEOUT_SECONDS` | `600` | Wall-clock cap on a single RCA |
| `BOB_FAILURE_THRESHOLD_FOR_RESEARCH` | `2` | Failures before research fires (Trigger 2) |
| `BOB_CONFIDENCE_DECAY_PER_FAILURE` | `0.15` | Per-failure decay step |
| `BOB_TEST_RUN_TIMEOUT` | (scaled) | Per-test pytest cap; otherwise scales with project test count |
| `BOB_FEATURE_TIMEOUT_SECONDS` | `3600` | Hard wall on a single feature run |
...
When you're tailing `bob run`, the structured log lines that
...
`evidence_artifacts` and `sub_agent_runs` tables; `bob status`
and `bob show-reviews` surface the most-relevant slices.
