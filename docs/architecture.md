# Architecture — what bob actually is

This is the high-level tour. It explains what each major bob
...
You write a YAML spec describing the project you want built. Bob
...
- An **MCP integration layer** that selectively enables Perplexity
  (research), Bob Memory (always), and Puppeteer (per-feature) for
  sub-agents.
...
├── mcp_lifecycle.py          # starts/stops the bob-memory MCP server
...
Source: [`src/bob/orchestrator/run_loop.py`](../src/bob/orchestrator/run_loop.py).

The single class `OrchestrationLoop` is the heart of bob. It owns
...
Source: [`src/bob/orchestrator/claude_executor.py`](../src/bob/orchestrator/claude_executor.py).

Bob spawns four kinds of sub-agent. Each gets a different system
prompt, MCP set, and tool allowlist.

| Sub-agent | Spawned by | MCP set | Purpose |
|---|---|---|---|
| **Implementation** | `spawn_implementation_agent` | bob-memory + puppeteer (opt) + perplexity (when key set) | Write the feature. Driven by orientation prompt + acceptance criteria + skills. |
...
Source: [`src/bob/skills/`](../src/bob/skills/) and
[`src/bob/skills_installer.py`](../src/bob/skills_installer.py).

Skills are markdown files Claude Code auto-discovers in a project
workspace. Bob ships nine of them, each a `SKILL.md` with YAML
...
|---|---|
| `using-bob-memory` | At session start; teaches the memory_search / memory_add tool surface |
...
Skills are **bob-managed**: the installer audits symlinks before
...
a path outside the current bob install. Users who want to add
...
adversarial review — without bob having to inject those rules
...
Source: [`src/bob/superpowers.py`](../src/bob/superpowers.py)
and [`src/bob/enhanced_verification.py`](../src/bob/enhanced_verification.py).
...
The AST stub detector ([`ast_checks.py`](../src/bob/ast_checks.py))
...
([`enhanced_verification.py`](../src/bob/enhanced_verification.py))
recognizes:
...
timeout. The default per-criterion timeout is `BOB_CRITERION_EXEC_TIMEOUT`
(default 600 s).
...
Source: [`src/bob/memory.py`](../src/bob/memory.py),
[`src/bob/memory_mcp.py`](../src/bob/memory_mcp.py),
[`src/bob/mcp_lifecycle.py`](../src/bob/mcp_lifecycle.py).

Bob's memory is **in-process and local**: mem0ai for the higher-level
...
across-pool. Sub-agents are taught (via the `using-bob-memory`
skill) to:
...
the lifecycle log lines `bob-memory MCP server started/stopped`
in `bob run` output are this.

---

## MCP integration

Source: [`src/bob/orchestrator/mcp_config.py`](../src/bob/orchestrator/mcp_config.py).

Bob supports three MCP plugins:

| Plugin | Where it runs | Sub-agents that see it |
|---|---|---|
| `bob-memory` | Local Python subprocess | All implementation sub-agents (always) |
...
- `bob.reviews.Registry` — Python API: load, search, add, mark_fixed
- `bob show-reviews` — CLI for human / sub-agent search
...
Source: [`src/bob/db.py`](../src/bob/db.py) (`create_or_update_calibration`,
`list_calibration_alerts`), CLI: `bob show-calibration`.

Every feature attempt is recorded with `(task_class, confidence_bucket,
predicted_outcome, actual_outcome)`. Over time, bob can tell you:
...
This is how bob learns whether its own confidence estimates are
trustworthy.
...
[`run_loop.py`](../src/bob/orchestrator/run_loop.py),
`db.detect_regression`, `db.rollback_feature_cascade`,
[`git_ops.revert_feature`](../src/bob/git_ops.py).
...
3. If a regression is detected, the operator can run
   `bob show-regressions` to see exactly which tests broke and
   in which feature.
...
Sources: signal handler ([`signal_handler.py`](../src/bob/signal_handler.py)),
...
On the next `bob run`, `_resume_interrupted_work` runs first.
...
bob thinks the work needs attention or just got cut short.
...
billing visible to the SDK). Bob still needs to enforce a budget
in those cases, so:
...
- The proxy gets logged once per feature so operators don't get
  surprised by a "$0 cost" in `bob status`.
...
The spec at `examples/00_bootstrap_spec.yaml` describes bob
itself — the orchestration loop, the verification checklist, the
memory layer, the registry. Running `bob run` against
`bootstrap_spec.yaml` will rebuild bob from scratch (in a
separate workspace).
...
because bob fails to build itself. New features that change the
...
5. `src/bob/orchestrator/run_loop.py` — read top-to-bottom; the
   sections are clearly delimited and many comments reference
   specific R10-* findings.
6. `src/bob/superpowers.py` — the verification checklist
   implementation.
7. `src/bob/skills/*/SKILL.md` — what every sub-agent reads on
   spawn.
