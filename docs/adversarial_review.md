# Adversarial review — bob3's red-team loop
...
Bob3 has a **second** defense — adversarial review — that runs
...
Source: [`src/bob3/skills/adversarial-self-review/SKILL.md`](../src/bob3/skills/adversarial-self-review/SKILL.md).
...
6. **Verifier-gaming** — could a hostile implementation pass the
   bob3 verifier without solving the problem? `touch` files to
...
[`src/bob3/reviews.py`](../src/bob3/reviews.py).
...
Programmatic API in `bob3.reviews`:
...
reg = add_finding(
    ...
    files=["src/bob3/orchestrator/run_loop.py"],
    severity="high",
...
Source: [`src/bob3/reviews.py`](../src/bob3/reviews.py) — the
...
The CLI's `bob3 show-reviews --tag <tag>` surfaces the full
...
Source:
[`src/bob3/skills/checking-review-registry/SKILL.md`](../src/bob3/skills/checking-review-registry/SKILL.md).
...
```python
from bob3.reviews import load_registry
...
bob3 show-reviews --tag signal-safety --status open
```
...
the moment they'd be useful. Bob3's skills system makes querying
the default behavior.

---

## 5. The CLI surface — `bob3 show-reviews`

Source: [`src/bob3/cli.py:1723`](../src/bob3/cli.py).

```bash
bob3 show-reviews                    # full registry
bob3 show-reviews --summary          # status + severity counts
bob3 show-reviews --status open      # only open findings
bob3 show-reviews --severity critical
bob3 show-reviews --tag non-atomic-counter
bob3 show-reviews --file-glob 'orchestrator/*.py'
bob3 show-reviews --query "cascade"
```
...
## 7. How adversarial review interacts with the rest of bob3
...
- **Memory** (`bob3.memory`): findings filed during a run can
...
- **Run `bob3 show-reviews --summary` periodically.** A growing
