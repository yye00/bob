...
class TestInferTaskClass:
    ...
    def test_file_manipulation_keyword(self):
        from bob3.calibration import infer_task_class
    ...
    def test_valid_task_classes_returned(self):
        from bob3.calibration import TASK_CLASSES, infer_task_class
...
class TestComputeEceByBucket:
    ...
    def test_returns_dict(self):
        from bob3.calibration import compute_ece_by_bucket
...
class TestCalibrationReportCLI:
    """Tests for the 'bob3 calibration-report' CLI command."""
    ...
    def test_command_shows_ece_data_when_available(self, tmp_path, monkeypatch):
        ...
        from bob3.cli import main
        from bob3.db import create_or_update_calibration, create_project, init_database

        monkeypatch.setenv("BOB3_DATABASE_PATH", str(tmp_path / "test.db"))
        init_database()

        project = create_project(name="Test Project", workspace_path=str(tmp_path))
        monkeypatch.setenv("BOB3_PROJECT_ID", project.id)
