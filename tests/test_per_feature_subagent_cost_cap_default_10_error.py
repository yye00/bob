...
Covers:
- Non-numeric BOB_PER_ATTEMPT_COST_CAP env var → get_per_attempt_cap uses default (no ValueError)
...
from bob.cost_cap import enforce_per_attempt_cost_cap
...
class TestGetPerAttemptCapInvalidEnvSafe:
    ...
    def test_non_numeric_env_var_falls_back_to_default(self, monkeypatch):
        """BOB_PER_ATTEMPT_COST_CAP=notanumber → falls back to default 10.0."""
        monkeypatch.setenv("BOB_PER_ATTEMPT_COST_CAP", "notanumber")
        cap = get_per_attempt_cap()
        assert cap == 10.0

    def test_whitespace_only_env_var_returns_default(self, monkeypatch):
        """BOB_PER_ATTEMPT_COST_CAP='  ' (whitespace) → falls back to default 10.0."""
        monkeypatch.setenv("BOB_PER_ATTEMPT_COST_CAP", "   ")
        cap = get_per_attempt_cap()
        assert cap == 10.0

    def test_alpha_env_var_does_not_raise(self, monkeypatch):
        """BOB_PER_ATTEMPT_COST_CAP='abc' must not raise, returns default."""
    ...
    def test_mixed_env_var_does_not_raise(self, monkeypatch):
        """BOB_PER_ATTEMPT_COST_CAP='10abc' must not raise, returns default."""
        monkeypatch.setenv("BOB_PER_ATTEMPT_COST_CAP", "10abc")
        cap = get_per_attempt_cap()
        assert isinstance(cap, float)
        assert cap == 10.0
