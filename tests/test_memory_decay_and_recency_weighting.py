"""Tests for src/bob/memory_decay_and_recency_weighting.py."""
import os
...
from bob.memory_decay_and_recency_weighting import (
    MemoryDecayConfig,
...
class TestGetDecayConfig:
    def test_defaults_to_30_days(self):
        with patch.dict(os.environ, {}, clear=False):
            env = {k: v for k, v in os.environ.items() if k != "BOB_LESSON_HALF_LIFE_DAYS"}
            with patch.dict(os.environ, env, clear=True):
                config = get_decay_config()
        assert config.half_life_days == 30.0

    def test_reads_env_var(self):
        with patch.dict(os.environ, {"BOB_LESSON_HALF_LIFE_DAYS": "60"}):
            config = get_decay_config()
        assert config.half_life_days == 60.0

    def test_fractional_half_life(self):
        with patch.dict(os.environ, {"BOB_LESSON_HALF_LIFE_DAYS": "7.5"}):
            config = get_decay_config()
        assert config.half_life_days == 7.5

    def test_raises_for_non_numeric(self):
        with patch.dict(os.environ, {"BOB_LESSON_HALF_LIFE_DAYS": "not-a-number"}):
            with pytest.raises(ValueError, match="not a valid number"):
                get_decay_config()

    def test_raises_for_zero(self):
        with patch.dict(os.environ, {"BOB_LESSON_HALF_LIFE_DAYS": "0"}):
            with pytest.raises(ValueError, match="must be positive"):
                get_decay_config()

    def test_raises_for_negative(self):
        with patch.dict(os.environ, {"BOB_LESSON_HALF_LIFE_DAYS": "-10"}):
            with pytest.raises(ValueError, match="must be positive"):
                get_decay_config()

    def test_context_pool_has_shorter_half_life(self):
        with patch.dict(os.environ, {"BOB_LESSON_HALF_LIFE_DAYS": "30"}):
            config = get_decay_config()
