...
Tests that Bob handles SIGINT/SIGTERM gracefully by:
...
7. Logging: 'Interrupted. Run bob run to resume.'
...
# ============================================================
# Step 7: Log: 'Interrupted. Run bob run to resume.'
# ============================================================
...
class TestInterruptionLogMessage:
    ...
    async def test_log_message_on_shutdown(self, tmp_db, project, ready_features):
        """'Interrupted. Run bob run to resume.' is logged on shutdown."""
        with patch("bob.db.get_database_path", return_value=tmp_db):
            loop = OrchestrationLoop(project_id=project.id)
            ...
            with patch(
                ...
                with patch("bob.orchestrator.run_loop.stop_mcp_server"):
                    with patch("bob.orchestrator.run_loop.logger") as mock_logger:
                        await loop.run()
...
# ============================================================
# Step 8: Test: Start bob, send SIGINT, verify checkpoint created
# ============================================================
...
class TestSignalHandlerIsAsyncSignalSafe:
    ...
    def test_handler_does_not_call_db_create_checkpoint(self):
        ...
        from bob.signal_handler import GracefulShutdownHandler
        ...
        with patch(
            "bob.db.create_checkpoint"
        ) as create_cp, patch(
            "bob.db.update_feature"
        ) as upd, patch(
            "bob.mcp_lifecycle.stop_mcp_server"
        ) as stop_mcp:
