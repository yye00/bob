...
And that bob3.models.resolve_max_cost_usd is importable and correct.
"""
...
def _clean_env(monkeypatch):
    """Ensure BOB3_MAX_COST_USD is unset before each test."""
    monkeypatch.delenv("BOB3_MAX_COST_USD", raising=False)


# ---------------------------------------------------------------------------
# AC: Function defined — bob3.models.resolve_max_cost_usd
# ---------------------------------------------------------------------------


def test_resolve_max_cost_usd_importable():
    """bob3.models.resolve_max_cost_usd is importable and callable."""
    from bob3.models import resolve_max_cost_usd
...
def test_resolve_max_cost_usd_honors_env(monkeypatch):
    """BOB3_MAX_COST_USD=750 → returns 750.0."""
    monkeypatch.setenv("BOB3_MAX_COST_USD", "750")
    from bob3.models import resolve_max_cost_usd
...
def test_resolve_max_cost_usd_empty_env_returns_unlimited(monkeypatch):
    """Empty BOB3_MAX_COST_USD → unlimited, never 0."""
...
def test_resolve_max_cost_usd_malformed_env_returns_unlimited(monkeypatch):
    """Malformed BOB3_MAX_COST_USD → unlimited, not 0, does not raise."""
...
def test_resolve_max_cost_usd_nan_returns_unlimited(monkeypatch):
    """NaN BOB3_MAX_COST_USD → unlimited."""
...
def test_resolve_max_cost_usd_inf_returns_unlimited(monkeypatch):
    """Inf BOB3_MAX_COST_USD → unlimited."""
    monkeypatch.setenv("BOB3_MAX_COST_USD", "inf")
    from bob3.models import resolve_max_cost_usd
...
def test_resolve_max_cost_usd_zero_is_honored(monkeypatch):
    """BOB3_MAX_COST_USD=0 → 0.0 (explicit user budget; empty/malformed must not produce this)."""
...
def test_resolve_max_cost_usd_negative_clamped_to_zero(monkeypatch):
    """Negative BOB3_MAX_COST_USD is clamped to 0.0, not raises."""
    monkeypatch.setenv("BOB3_MAX_COST_USD", "-50")
    from bob3.models import resolve_max_cost_usd
...
def test_project_default_max_cost_usd_reads_env(monkeypatch):
    """Project() picks up BOB3_MAX_COST_USD via default_factory."""
    monkeypatch.setenv("BOB3_MAX_COST_USD", "300")
    import bob3.models as _m
    importlib.reload(_m)
    try:
...
def test_schema_sql_default_is_unlimited():
    ...
    schema_candidates = [
        ...
        pathlib.Path("src/bob3/schema.sql"),
        pathlib.Path("src/bob3/migrations/schema.sql"),
    ]
...
def test_db_create_project_honors_env(monkeypatch):
    """create_project reads BOB3_MAX_COST_USD when max_cost_usd not explicitly passed."""
    monkeypatch.setenv("BOB3_MAX_COST_USD", "1234.56")
    from bob3.db import create_project, init_database
...
def test_orchestrator_imports_resolve_max_cost_usd():
    """bob3.orchestrator module is importable and run_loop reads project.max_cost_usd."""
