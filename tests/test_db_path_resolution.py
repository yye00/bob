...
- export_database_path sets BOB_DATABASE_PATH to an absolute path
- Sub-agents that inherit BOB_DATABASE_PATH use the same DB as the project
...
def _make_db(path: pathlib.Path) -> None:
    """Create a minimal bob schema in a fresh SQLite file at path."""
...
class TestCreateAgentRunDbPath:
    ...
    def test_writes_to_explicit_db_path(self, tmp_path):
        ...
        from bob.db import create_agent_run, init_database
    ...
    def test_explicit_db_path_not_cwd_default(self, tmp_path, monkeypatch):
        """When db_path is given, cwd/bob.db is NOT touched even if it exists."""
        from bob.db import create_agent_run, init_database

        # Create a stale 0-byte file that would shadow the real DB
        stale_db = tmp_path / "bob.db"
        ...
        assert stale_size == 0, "Stale cwd/bob.db must not be written to"
...
class TestExportDatabasePath:
    """export_database_path exports an absolute path into BOB_DATABASE_PATH."""

    def test_returns_absolute_path(self, tmp_path, monkeypatch):
        """Always returns an absolute pathlib.Path."""
        from bob.orchestrator import export_database_path
    ...
    def test_sets_env_var(self, tmp_path, monkeypatch):
        """Sets BOB_DATABASE_PATH to the resolved absolute path string."""
        from bob.orchestrator import export_database_path
        ...
        assert os.environ.get("BOB_DATABASE_PATH") == str(result)

    def test_honors_existing_env_when_no_arg(self, tmp_path, monkeypatch):
        """When no db_path supplied, resolves from BOB_DATABASE_PATH env."""
        from bob.orchestrator import export_database_path
        ...
        assert os.environ["BOB_DATABASE_PATH"] == str(result)

    def test_explicit_arg_overrides_env(self, tmp_path, monkeypatch):
        """Explicit db_path argument overrides BOB_DATABASE_PATH env."""
        from bob.orchestrator import export_database_path
        ...
        assert os.environ["BOB_DATABASE_PATH"] == str(explicit_db.resolve())
    ...
    def test_subagent_inherits_exported_path(self, tmp_path, monkeypatch):
        ...
        from bob.orchestrator import export_database_path
        from bob.db import get_database_path
