"""Tests for bob.superpowers.reload_prompt_sources (feature 27f56284).
...
- is callable and importable from bob.superpowers
"""
...
def test_reload_prompt_sources_importable():
    """reload_prompt_sources must be importable from bob.superpowers."""
    from bob.superpowers import reload_prompt_sources  # noqa: F401


def test_reload_prompt_sources_is_callable():
    """reload_prompt_sources must be a callable."""
    from bob.superpowers import reload_prompt_sources
...
def test_reload_prompt_sources_returns_reloaded_modules():
    ...
    reloaded = ["bob.superpowers", "bob.models"]
...
def test_reload_prompt_sources_covers_superpowers_module():
    """maybe_reload_all should be watching bob.superpowers."""
...
def test_reload_prompt_sources_covers_models_module():
    """maybe_reload_all should be watching bob.models."""
    from bob.orchestrator.prompt_source_reloader import _PROMPT_SOURCE_MODULES
...
def test_reload_if_stale_detects_mtime_change(tmp_path):
    ...
    try:
        ...
        result_first = prompt_source_reloader.reload_if_stale("bob.superpowers")
        ...
        with patch.object(
            ...
            with patch("importlib.reload") as mock_reload_fn:
                result_changed = prompt_source_reloader.reload_if_stale("bob.superpowers")
