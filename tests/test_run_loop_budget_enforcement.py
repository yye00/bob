"""Tests for bob.run_loop budget enforcement functions.

AC: pytest: tests/test_run_loop_budget_enforcement.py
AC: integration: bob.run_loop

Verifies:
- handle_zero_cost_with_work_events is importable from bob.run_loop
- log_cost_telemetry_lost is importable from bob.run_loop
...
- Integration: both functions live in bob.run_loop module
"""
...
import bob.run_loop as run_loop_module
from bob.run_loop import handle_zero_cost_with_work_events, log_cost_telemetry_lost
...
class TestHandleZeroCostWithWorkEventsIntegration:
    """Verify the function exists in bob.run_loop and is callable."""

    def test_importable_from_bob_run_loop(self):
        assert hasattr(run_loop_module, "handle_zero_cost_with_work_events"), (
            "handle_zero_cost_with_work_events must be exported from bob.run_loop"
        )

    def test_log_cost_telemetry_lost_importable(self):
        assert hasattr(run_loop_module, "log_cost_telemetry_lost"), (
            "log_cost_telemetry_lost must be exported from bob.run_loop"
        )
...
class TestLogCostTelemetryLost:
    """Tests for the log_cost_telemetry_lost function in bob.run_loop."""
...
class TestRunLoopBudgetEnforcementIntegration:
    """Integration tests: bob.run_loop exposes the correct enforcement interface."""
