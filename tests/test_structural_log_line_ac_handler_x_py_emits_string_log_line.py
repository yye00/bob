"""Tests for bob.structural_log_line_ac_handler_x_py_emits_string_log_line (feature f78ce68d).
...
from bob.structural_log_line_ac_handler_x_py_emits_string_log_line import (
    structural_log_line_ac_handler_x_py_emits_string_log_line,
)


def _write_src(tmp_path: pathlib.Path, content: str, rel: str = "src/bob/run_loop.py") -> pathlib.Path:
    target = tmp_path / rel
...
def test_non_emits_criterion_returns_none(tmp_path):
    ...
    workspace = _write_src(tmp_path, src)
    result = structural_log_line_ac_handler_x_py_emits_string_log_line(
        criterion_body="src/bob/run_loop.py defines function some_function",
        workspace=workspace,
    )
    assert result is None
...
def test_file_does_not_exist_returns_none(tmp_path):
    ...
    result = structural_log_line_ac_handler_x_py_emits_string_log_line(
        criterion_body="src/bob/nonexistent_module.py emits a 'Some log line' log line",
        workspace=tmp_path,
    )
    assert result is None
...
def test_with_quote_style_variants(tmp_path):
    ...
    workspace = _write_src(tmp_path, src)
    result = structural_log_line_ac_handler_x_py_emits_string_log_line(
        criterion_body='src/bob/run_loop.py emits a "Queue drained" log line',
        workspace=workspace,
    )
    assert result is True
...
def test_different_file_path(tmp_path):
    ...
    workspace = _write_src(tmp_path, src, rel="src/bob/orchestrator/run_loop.py")
    result = structural_log_line_ac_handler_x_py_emits_string_log_line(
        criterion_body="src/bob/orchestrator/run_loop.py emits a 'Heartbeat: pid=%d' log line",
        workspace=workspace,
    )
    assert result is True
