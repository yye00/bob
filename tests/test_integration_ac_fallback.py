"""Tests for bob.integration_ac_fallback.resolve_integration_target.
...
from bob.integration_ac_fallback import resolve_integration_target
...
def test_dotted_module_path_resolves(tmp_path: pathlib.Path) -> None:
    ...
    _write_src(tmp_path, "orchestrator/run_loop.py", """
        def main():
            pass
    """)
    importer = tmp_path / "src" / "bob" / "main.py"
    importer.write_text("from bob.orchestrator.run_loop import main\n")
...
def test_dotted_module_not_imported_returns_false(tmp_path: pathlib.Path) -> None:
    ...
    criterion = "integration: bob.some_module"
...
def test_file_exists_criterion_returns_false(tmp_path: pathlib.Path) -> None:
    ...
    criterion = "File exists: src/bob/foo.py"
    assert resolve_integration_target(criterion, tmp_path) is False
...
def test_bad_dotted_path_falls_through_to_function_existence(tmp_path: pathlib.Path) -> None:
    ...
    _write_src(tmp_path, "watchdog.py", """
        def periodic_sweep():
            pass
    """)
    criterion = "integration: bob.nonexistent periodic_sweep runs every 60s"
    assert resolve_integration_target(criterion, tmp_path) is True
...
def test_module_importable() -> None:
    """Module bob.integration_ac_fallback is importable."""
    import importlib
    mod = importlib.import_module("bob.integration_ac_fallback")
...
def test_in_all_exports() -> None:
    ...
    from bob import integration_ac_fallback
