...
def workspace(tmp_path: Path) -> Path:
    ...
    (src_bob3 / "some_module.py").write_text("# module\n")
...
def test_slopsquatting_first_party_allowlist_must_include_tools(workspace: Path) -> None:
    ...
    some_module (src/bob3/) is also present.
    """
    ...
    assert "some_module" in result, "src/bob3/some_module.py must be in allowlist"
...
def test_workspace_with_no_tools_dir(tmp_path: Path) -> None:
    ...
    src_bob3.mkdir(parents=True)
    (src_bob3 / "__init__.py").write_text("")
    (src_bob3 / "mymod.py").write_text("# mod\n")
