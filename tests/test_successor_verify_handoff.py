"""Tests for bob3.status_handler — successor-gen verification handoff (b89c45f9).

Acceptance criteria:
- File exists: src/bob3/status_handler.py
- Function defined: bob3.status_handler.should_defer_to_successor_verifier
...
# ---------------------------------------------------------------------------
# AC 1: File exists — src/bob3/status_handler.py
# ---------------------------------------------------------------------------


class TestFileExists:
    def test_module_file_exists(self):
        import bob3.status_handler

        module_file = Path(bob3.status_handler.__file__)
        assert module_file.exists()
        assert module_file.name == "status_handler.py"

    def test_module_importable(self):
        import bob3.status_handler  # noqa: F401
    ...
    def test_all_contains_expected_names(self):
        from bob3.status_handler import __all__
...
# ---------------------------------------------------------------------------
# AC 2: Function defined — bob3.status_handler.should_defer_to_successor_verifier
# ---------------------------------------------------------------------------


class TestFunctionDefined:
    def test_function_is_callable(self):
        from bob3.status_handler import should_defer_to_successor_verifier
        assert callable(should_defer_to_successor_verifier)
    ...
    def test_function_returns_bool(self, tmp_path):
        from bob3.status_handler import should_defer_to_successor_verifier
        with patch("bob3.status_handler.set_pending_successor_verify", return_value=False):
...
class TestShouldDeferGuards:
    def test_returns_false_when_structural_ac_not_passed(self, tmp_path):
        ...
        with patch("bob3.status_handler.set_pending_successor_verify") as mock_set:
...
class TestShouldDeferHappyPath:
    ...
    def test_delegates_to_set_pending_successor_verify(self, tmp_path):
        ...
        with patch("bob3.status_handler.set_pending_successor_verify", return_value=True) as mock_set:
            result = should_defer_to_successor_verifier(
    ...
    def test_logs_info_when_deferring(self, tmp_path, caplog):
        ...
        with patch("bob3.pending_successor_verify.db"):
            with caplog.at_level(logging.INFO, logger="bob3.status_handler"):
                should_defer_to_successor_verifier(
...
class TestLoggingBehaviour:
    def test_logs_debug_when_no_structural_ac(self, tmp_path, caplog):
        from bob3.status_handler import should_defer_to_successor_verifier
        with caplog.at_level(logging.DEBUG, logger="bob3.status_handler"):
...
class TestRunLoopIntegration:
    ...
    def test_status_handler_importable_from_bob3(self):
        import bob3.status_handler as sh
    ...
    def test_should_defer_consistent_with_set_pending(self, tmp_path):
        ...
        with patch("bob3.pending_successor_verify.db") as mock_db1:
            r1 = should_defer_to_successor_verifier("feat-c1", tmp_path, structural_ac_passed=True)

        with patch("bob3.pending_successor_verify.db") as mock_db2:
