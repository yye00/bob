"""Tests for bob3.orchestrator.reload_prompt_source_if_changed (feature e1bb8261).

Verifies that hot-reload of prompt-source modules works correctly when accessed
through the bob3.orchestrator namespace, so code-fixes to superpowers.py land
...
# ---------------------------------------------------------------------------
# Importability and callable contract via bob3.orchestrator
# ---------------------------------------------------------------------------


def test_reload_prompt_source_if_changed_importable_from_orchestrator():
    """reload_prompt_source_if_changed must be importable from bob3.orchestrator."""
    from bob3.orchestrator import reload_prompt_source_if_changed  # noqa: F401


def test_reload_prompt_source_if_changed_is_callable_via_orchestrator():
    """reload_prompt_source_if_changed imported from bob3.orchestrator must be callable."""
    from bob3.orchestrator import reload_prompt_source_if_changed
...
# ---------------------------------------------------------------------------
# Default module is bob3.superpowers
# ---------------------------------------------------------------------------


def test_default_module_is_superpowers():
    """Default module_name must be bob3.superpowers."""
    from bob3.orchestrator import reload_prompt_source_if_changed
    import bob3.orchestrator.prompt_source_reloader as _reloader
    ...
    assert called_with == ["bob3.superpowers"]
...
def test_explicit_module_name_forwarded():
    ...
    with patch.object(_reloader, "reload_if_stale", return_value=True) as mock_fn:
        result = reload_prompt_source_if_changed("bob3.models")
...
def test_returns_false_for_missing_module():
    ...
    with patch.object(_reloader, "reload_if_stale", return_value=False):
        result = reload_prompt_source_if_changed("bob3.nonexistent_module_xyz")
...
def test_detects_mtime_change():
    ...
    try:
        ...
        first_result = reload_prompt_source_if_changed("bob3.superpowers")
        ...
        with patch.object(
            ...
            with patch("importlib.reload"):
                second_result = reload_prompt_source_if_changed("bob3.superpowers")
...
def test_orchestrator_exposes_maybe_reload_all():
    """maybe_reload_all must also be importable from bob3.orchestrator."""
    from bob3.orchestrator import maybe_reload_all  # noqa: F401

    assert callable(maybe_reload_all)


def test_prompt_source_modules_includes_superpowers():
    """The watched module list must include bob3.superpowers."""
...
def test_prompt_source_modules_includes_models():
    """The watched module list must include bob3.models."""
    from bob3.orchestrator.prompt_source_reloader import _PROMPT_SOURCE_MODULES
...
def test_idempotent_consecutive_calls():
    ...
    try:
        ...
        # Establish baseline
        reload_prompt_source_if_changed("bob3.superpowers")
        mtime = prompt_source_reloader._MTIME_CACHE.get("bob3.superpowers", 0.0)

        with patch.object(prompt_source_reloader, "get_prompt_mtime", return_value=mtime):
            r1 = reload_prompt_source_if_changed("bob3.superpowers")
            r2 = reload_prompt_source_if_changed("bob3.superpowers")
