"""Tests for bob.spec_to_test_plan_generator.
...
class TestParseAcceptanceCriteria:
    def test_parse_json_list_string(self):
        ac = '["File exists: src/bob/foo.py", "pytest: tests/test_foo.py"]'
        result = parse_acceptance_criteria(ac)
        assert len(result) == 2
        assert "File exists: src/bob/foo.py" in result
        assert "pytest: tests/test_foo.py" in result

    def test_parse_plain_list(self):
        ac = ["File exists: src/bob/bar.py", "Function defined: bob.bar.baz"]
        result = parse_acceptance_criteria(ac)
        assert len(result) == 2
        assert result[0] == "File exists: src/bob/bar.py"

    def test_parse_single_string(self):
        ac = "File exists: src/bob/foo.py"
        result = parse_acceptance_criteria(ac)
        assert len(result) == 1
        assert result[0] == "File exists: src/bob/foo.py"
...
class TestTestScenarioModel:
    def test_basic_construction(self):
        scenario = TestScenario(
            ...
            expected_outputs=["File src/bob/foo.py present"],
            edge_cases=[],
            covers_criteria=["File exists: src/bob/foo.py"],
        )
...
class TestGenerateTestPlan:
    def _make_feature(self, **kwargs) -> dict[str, Any]:
        defaults: dict[str, Any] = {
            ...
            "description": "Add a helper function foo() to src/bob/util.py that returns True.",
            "acceptance_criteria": '["File exists: src/bob/util.py", "pytest: tests/test_util.py"]',
        }
    ...
    def test_plan_has_parsed_acceptance_criteria(self):
        ...
        assert "File exists: src/bob/util.py" in plan.acceptance_criteria
    ...
    def test_edge_case_plain_list_criteria(self):
        feature = self._make_feature(
            acceptance_criteria=["File exists: src/bob/foo.py", "pytest: tests/test_foo.py"]
        )
        plan = generate_test_plan(feature)
        assert len(plan.acceptance_criteria) == 2
    ...
    def test_file_exists_criterion_generates_scenario(self):
        feature = self._make_feature(
            acceptance_criteria='["File exists: src/bob/mymodule.py"]'
        )
        plan = generate_test_plan(feature)
    ...
    def test_function_defined_criterion_generates_scenario(self):
        feature = self._make_feature(
            acceptance_criteria='["Function defined: bob.mymodule.my_func"]'
        )
        plan = generate_test_plan(feature)
        covered = set()
        for scenario in plan.scenarios:
            covered.update(scenario.covers_criteria)
        assert "Function defined: bob.mymodule.my_func" in covered

    def test_multiple_criteria_generate_multiple_scenarios(self):
        feature = self._make_feature(
            acceptance_criteria='["File exists: src/bob/a.py", "pytest: tests/test_a.py", "Function defined: bob.a.foo"]'
        )
        plan = generate_test_plan(feature)
...
class TestFormatTestPlanMarkdown:
    def _make_plan(self) -> TestPlan:
        scenario = TestScenario(
            ...
            inputs=["Feature with File exists AC"],
            expected_outputs=["src/bob/foo.py exists on disk"],
            edge_cases=["File already exists (idempotent)", "Parent dir missing"],
            covers_criteria=["File exists: src/bob/foo.py"],
        )
        return TestPlan(
            feature_name="Add foo module",
            feature_description="Creates src/bob/foo.py with helper functions",
            acceptance_criteria=["File exists: src/bob/foo.py", "pytest: tests/test_foo.py"],
            scenarios=[scenario],
        )
    ...
    def test_contains_feature_description(self):
        plan = self._make_plan()
        md = format_test_plan_markdown(plan)
        assert "Creates src/bob/foo.py" in md

    def test_contains_acceptance_criteria(self):
        plan = self._make_plan()
        md = format_test_plan_markdown(plan)
        assert "File exists: src/bob/foo.py" in md
    ...
    def test_contains_expected_outputs(self):
        plan = self._make_plan()
        md = format_test_plan_markdown(plan)
        assert "src/bob/foo.py exists on disk" in md
