"""Tests for prompt-cache worker leverage in bob.dispatch.
...
class TestSpawnWorkerCacheIntegration:
    ...
    def test_spawn_worker_passes_cache_env_var(self, tmp_path):
        ...
        with mock.patch("subprocess.run", side_effect=fake_run):
            spawn_worker(feature, "do something", tmp_path, bob_dir=bob_dir)
    ...
    def test_spawn_worker_none_feature_raises_value_error(self, tmp_path):
        ...
        with pytest.raises(ValueError):
            spawn_worker(None, "prompt", tmp_path, bob_dir=bob_dir)
