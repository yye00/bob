...
from bob3.spec_quality_score import (
    ...
    extract_concrete_py_paths as bob3_extract_concrete_py_paths,
    is_code_shaped_token as bob3_is_code_shaped_token,
)
...
class TestIsCodeShapedToken:
    ...
    def test_bob3_alias_matches_tools_version(self):
        for tok in ["compute_quality", "defined", "ACs", "CompositeScore", ""]:
            assert bob3_is_code_shaped_token(tok) == is_code_shaped_token(tok)
...
class TestExtractConcretePyPaths:
    ...
    def test_finds_path_with_slash(self):
        desc = "implement src/bob3/brownfield/survey.py for the survey"
    ...
    def test_bob3_alias_matches_tools_version(self):
        desc = "implement src/bob3/brownfield/survey.py"
        assert bob3_extract_concrete_py_paths(desc) == extract_concrete_py_paths(desc)

    def test_finds_multiple_paths(self):
        desc = "modify tools/spec_quality_score.py and src/bob3/synthesizer.py"
        paths = extract_concrete_py_paths(desc)
        assert "tools/spec_quality_score.py" in paths
        assert "src/bob3/synthesizer.py" in paths
...
class TestEmitFileExistsAcs:
    ...
    def test_does_not_duplicate_already_covered_path(self):
        ...
        result = emit_file_exists_acs(criteria, desc)
        count = sum(1 for ac in result if "src/bob3/brownfield/survey.py" in ac)
        assert count == 1
    ...
    def test_adds_multiple_uncovered_paths(self):
        ...
        assert "src/bob3/synthesizer.py" in added_paths
...
class TestContractCompletenessTokenFiltering:
    ...
    def test_code_shaped_token_covered_by_ac_gives_full_score(self):
        ...
        desc = "The function extract_py_paths extracts .py paths from descriptions."
        acs = [
            "Function defined: bob3.spec_quality_score.extract_py_paths",
            "pytest: tests/test_foo.py",
