...
Several modules under ``bob`` keep module-level singletons. Without the
...
def test_a_set_bob_database_path_env_directly():
    """Set ``BOB_DATABASE_PATH`` via ``os.environ`` (NOT via
    ...
    os.environ["BOB_DATABASE_PATH"] = "/tmp/leaked-from-prior-test.db"
    assert os.environ.get("BOB_DATABASE_PATH") == "/tmp/leaked-from-prior-test.db"


def test_b_bob_database_path_env_was_restored():
    """The autouse fixture should restore the ``BOB_DATABASE_PATH``
