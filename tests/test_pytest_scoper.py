"""Tests for bob.pytest_scoper (F-R6-301).
...
def _mk_fake_repo(tmp_path: Path) -> Path:
    """Create an empty src/bob + tests skeleton under ``tmp_path``."""
    (tmp_path / "src" / "bob").mkdir(parents=True)
    (tmp_path / "tests").mkdir()
    (tmp_path / "src" / "bob" / "__init__.py").write_text("")
    return tmp_path


def _src(repo: Path, name: str, body: str) -> None:
    (repo / "src" / "bob" / name).write_text(body)
...
def test_direct_importer_selects_only_relevant_tests(tmp_path: Path) -> None:
    ...
    _src(repo, "user_a.py", "from bob.leaf import VALUE\n")
    _src(repo, "user_b.py", "import bob.leaf\n")
    _src(repo, "unrelated.py", "X = 2\n")

    _test(repo, "test_leaf.py", "from bob.leaf import VALUE\n\ndef test_x():\n    assert VALUE == 1\n")
    _test(repo, "test_user_a.py", "from bob.user_a import VALUE\n\ndef test_y():\n    pass\n")
    _test(repo, "test_user_b.py", "from bob.user_b import bob as _\n\ndef test_b():\n    pass\n")
    _test(repo, "test_unrelated.py", "from bob.unrelated import X\n\ndef test_z():\n    pass\n")

    result = scope_tests_for_diff(["src/bob/leaf.py"], repo)
...
def test_transitive_closure_a_imports_b_imports_c(tmp_path: Path) -> None:
    ...
    _src(repo, "b_mod.py", "from bob.c_mod import C\nB = C + 1\n")
    _src(repo, "a_mod.py", "from bob.b_mod import B\nA = B + 1\n")
    _src(repo, "spectator.py", "S = 9\n")  # uninvolved

    _test(repo, "test_a_mod.py", "from bob.a_mod import A\n\ndef test():\n    pass\n")
    _test(repo, "test_b_mod.py", "from bob.b_mod import B\n\ndef test():\n    pass\n")
    _test(repo, "test_c_mod.py", "from bob.c_mod import C\n\ndef test():\n    pass\n")
    _test(repo, "test_spectator.py", "from bob.spectator import S\n\ndef test():\n    pass\n")

    result = scope_tests_for_diff(["src/bob/c_mod.py"], repo)
...
def test_max_depth_limits_expansion(tmp_path: Path) -> None:
    ...
    result = scope_tests_for_diff(["src/bob/c_mod.py"], repo, max_depth=1)
...
def test_few_tests_returns_none(tmp_path: Path) -> None:
    ...
    _test(repo, "test_lonely.py", "from bob.lonely import X\ndef test(): pass\n")
    # No other tests reference lonely / no other importer chain → only 1 test.

    result = scope_tests_for_diff(["src/bob/lonely.py"], repo)
    assert result is None
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "src/bob/evaluator.py",
        "src/bob/orchestrator/run_loop.py",
        "src/bob/orchestrator/claude_executor.py",
    ],
)
def test_critical_path_returns_none(tmp_path: Path, critical_path: str) -> None:
    ...
    _src(repo, "x.py", "X = 1\n")
    for i in range(5):
        _test(repo, f"test_x_{i}.py", "from bob.x import X\ndef test(): pass\n")

    # A diff that includes both a critical-path file AND benign files still
    # bails to None.
    result = scope_tests_for_diff([critical_path, "src/bob/x.py"], repo)
    assert result is None
...
def test_missing_file_does_not_crash(tmp_path: Path) -> None:
    ...
    _test(repo, "test_alive.py", "from bob.alive import A\ndef test(): pass\n")
    _test(repo, "test_alive_two.py", "from bob.alive import A\ndef test(): pass\n")
    _test(repo, "test_alive_three.py", "from bob.alive import A\ndef test(): pass\n")

    # ``ghost.py`` was deleted; we still pass it as a changed file.
    result = scope_tests_for_diff(
        ["src/bob/ghost.py", "src/bob/alive.py"], repo
    )
...
def test_missing_repo_root_returns_none(tmp_path: Path) -> None:
    """If src/bob or tests/ are missing, scoping is impossible."""
    # tmp_path is empty — no src/, no tests/.
    result = scope_tests_for_diff(["src/bob/foo.py"], tmp_path)
    assert result is None
...
def test_non_python_changed_files_are_ignored(tmp_path: Path) -> None:
    ...
    repo = _mk_fake_repo(tmp_path)
    _src(repo, "x.py", "X = 1\n")
    _test(repo, "test_x.py", "from bob.x import X\ndef test(): pass\n")
...
def test_changed_test_file_is_included(tmp_path: Path) -> None:
    ...
    result = scope_tests_for_diff(
        ["src/bob/leaf.py", "tests/test_my_new.py"], repo
    )
    assert result is not None
    names = {Path(p).name for p in result}
...
def test_smoke_against_real_bob8(tmp_path: Path) -> None:
    ...
    """
    # Walk up to find the worktree root (the dir containing src/bob).
    here = Path(__file__).resolve()
    repo = here.parent.parent  # tests/ -> repo root
    src_root = repo / "src" / "bob"
    if not src_root.is_dir():
        pytest.skip("Not running inside a bob checkout")
    ...
    # (over-broad pattern).
    target = "src/bob/container_runner.py"
...
def test_smoke_critical_path_real_bob8_returns_none() -> None:
    ...
    if not src_root.is_dir():
        pytest.skip("Not running inside a bob checkout")
    ev = repo / "src" / "bob" / "enhanced_verification.py"
    if not ev.is_file():
        pytest.skip("enhanced_verification.py not present")
    assert scope_tests_for_diff(["src/bob/enhanced_verification.py"], repo) is None
...
def test_enhanced_verification_bridge_helper_returns_scope() -> None:
    ...
    from bob.enhanced_verification import pytest_scope_for_feature_diff

    here = Path(__file__).resolve()
    repo = here.parent.parent
    if not (repo / "src" / "bob").is_dir():
        pytest.skip("Not running inside a bob checkout")

    # Critical-path → None (bail to full suite).
    assert pytest_scope_for_feature_diff(
        ["src/bob/enhanced_verification.py"], repo
    ) is None

    # Leaf module → real scope.
    leaf = "src/bob/container_runner.py"
...
def test_capture_pytest_snapshot_accepts_changed_files_kw(tmp_path: Path) -> None:
    ...
    assert capture_pytest_snapshot(None, changed_files=["src/bob/foo.py"]) is None
    assert capture_pytest_snapshot(
        "", changed_files=["src/bob/enhanced_verification.py"]
    ) is None
    ...
    assert capture_pytest_snapshot(
        str(fake_ws), changed_files=["src/bob/foo.py"]
    ) is None
    ...
    empty_ws.mkdir()
    assert capture_pytest_snapshot(
        str(empty_ws), changed_files=["src/bob/foo.py"]
    ) is None
...
def test_capture_pytest_snapshot_scopes_real_workspace(tmp_path: Path) -> None:
    ...
    (ws / "src" / "bob").mkdir(parents=True)
    (ws / "tests").mkdir()
    (ws / "src" / "bob" / "__init__.py").write_text("")
    (ws / "src" / "bob" / "leaf.py").write_text("VALUE = 1\n")
    (ws / "src" / "bob" / "user_a.py").write_text(
        "from bob.leaf import VALUE\n"
    )
    (ws / "src" / "bob" / "user_b.py").write_text(
        "import bob.leaf\n"
    )
    ...
    snap = capture_pytest_snapshot(
        str(ws), changed_files=["src/bob/leaf.py"]
    )
