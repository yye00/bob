...
def _make_feature(feature_id: str, ac_json: str | None = None) -> MagicMock:
    ...
    f.id = feature_id
    f.name = "test feature"
    f.acceptance_criteria = ac_json or '["File exists: src/bob3/run_loop.py"]'
...
# ---------------------------------------------------------------------------
# Tests for disk_reconciler_promotion_check in bob3.run_loop (public facade)
# ---------------------------------------------------------------------------
...
class TestDiskReconcilerPromotionCheckPublicFacade:
    ...
    def test_function_defined_in_run_loop_module(self):
        """disk_reconciler_promotion_check must be importable from bob3.run_loop."""
        from bob3.run_loop import disk_reconciler_promotion_check
        assert callable(disk_reconciler_promotion_check)

    def test_function_in_dunder_all(self):
        """disk_reconciler_promotion_check must appear in bob3.run_loop.__all__."""
    ...
    def test_delegates_to_check_executing_feature_acs(self):
        ...
        with patch(
            ...
            result = disk_reconciler_promotion_check(
                ...
                acceptance_criteria_json='["File exists: src/bob3/run_loop.py"]',
            )
...
class TestHandleExecutionResultDiskPromotion:
    ...
    def test_disk_promote_fires_before_needs_human_when_tests_pass_only_failing(self):
        ...
        feature = _make_feature(
            feature_id, '["File exists: src/bob3/run_loop.py"]'
        )
        ...
        with patch("bob3.orchestrator.run_loop.db") as mock_db, \
             ...
             patch("bob3.orchestrator.run_loop._rca_auto_reset_if_infra",
                   return_value=False):
...
class TestVerifyFailDiskPromotedEvent:
    ...
    def test_verify_fail_disk_promoted_string_in_orchestrator_run_loop(self):
        ...
        src = pathlib.Path("src/bob3/orchestrator/run_loop.py")
        if not src.exists():
            pytest.skip("orchestrator run_loop not found")
        content = src.read_text()
        assert "VERIFY_FAIL_DISK_PROMOTED" in content, (
            "src/bob3/orchestrator/run_loop.py must contain 'VERIFY_FAIL_DISK_PROMOTED'"
        )
    ...
    def test_f_r7_612_marker_in_orchestrator_run_loop(self):
        ...
        assert "F-R7-612" in content, (
            "src/bob3/orchestrator/run_loop.py must contain 'F-R7-612'"
        )
...
class TestDiskPromotionGuardCondition:
    ...
    @pytest.mark.parametrize("ac_json,expect_attempted", [
        ...
        ('["File exists: src/bob3/run_loop.py"]', True),
        ('["Function defined: bob3.run_loop.disk_reconciler_promotion_check"]', True),
        # Integration AC only (counts as structural check) → guard passes
        ('["integration: bob3.run_loop"]', True),
...
# ---------------------------------------------------------------------------
# Integration: bob3.run_loop module structure
# ---------------------------------------------------------------------------

class TestRunLoopModuleIntegration:
    """Verify the public bob3.run_loop module has the required function."""
    ...
    def test_module_runs_without_error(self):
        """bob3.run_loop module imports cleanly."""
        import bob3.run_loop  # noqa: F401
