"""Tests for bob.spec_synthesizer.emit_file_exists_acs and
...
class TestEmitFileExistsAcs:
    ...
    def test_emits_ac_for_concrete_py_path(self):
        """When description names src/bob/brownfield/survey.py, emit File exists AC."""
        description = "Implement the survey in src/bob/brownfield/survey.py."
    ...
    def test_does_not_double_add_already_covered_path(self):
        ...
        existing = ["File exists: src/bob/brownfield/survey.py"]
    ...
    def test_multiple_paths_all_emitted(self):
        ...
        description = (
            "Implement src/bob/brownfield/survey.py "
...
class TestFilterApiSurfaces:
    ...
    def test_keeps_dotted_tokens(self):
        result = self.fn(["tools.spec_quality_score", "bob.spec_synthesizer"])
...
def test_parse_object_format_criteria():
    ...
    response = '```json\n[{"id":1,"criterion":"File exists: src/bob/foo.py","description":"The module"}]\n```'
    result = parse_criteria_response(response)
    assert result is not None, "Should parse object-format LLM response"
    assert len(result) == 1
    assert result[0] == "File exists: src/bob/foo.py", f"Expected criterion text, got: {result[0]}"
    ...
    response3 = '```json\n["Function defined: bob.foo.bar",{"criterion":"integration: bob.orchestrator"}]\n```'
    result3 = parse_criteria_response(response3)
    assert result3 is not None
    assert len(result3) == 2
    assert "Function defined: bob.foo.bar" in result3
    assert "integration: bob.orchestrator" in result3
...
def test_composite_score_nonzero_with_injected_acs():
    ...
    structural_only = [
        "File exists: src/bob/synthesizer.py",
        "Function defined: bob.synthesizer.parse_criteria_response",
        "Function defined: bob.synthesizer.inject_boundary_and_error_acs",
        "integration: bob.orchestrator",
    ]
