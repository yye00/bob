"""Tests for bob.orchestrator.final_reaper.sweep_orphans_on_exit.

AC: pytest: tests/test_orchestrator_final_reaper.py
AC: File exists: src/bob/orchestrator/final_reaper.py
AC: Function defined: bob.orchestrator.final_reaper.sweep_orphans_on_exit
...
class TestOrchestratorFinalReaperModule(unittest.TestCase):
    ...
    def test_module_importable(self):
        """bob.orchestrator.final_reaper must be importable."""
        import bob.orchestrator.final_reaper  # noqa: F401

    def test_sweep_orphans_on_exit_exists(self):
        """sweep_orphans_on_exit must be a callable in bob.orchestrator.final_reaper."""
        from bob.orchestrator.final_reaper import sweep_orphans_on_exit
    ...
    def test_function_returns_list(self):
        ...
        with patch("bob.orchestrator.final_reaper.sweep_orphan_subagents") as mock_sweep, \
             patch("bob.orchestrator.final_reaper.db") as mock_db, \
             patch("bob.orchestrator.final_reaper.find_subagent_pid_for_feature"):
...
class TestOrchestratorFinalReaperBehavior(unittest.TestCase):
    ...
    def test_orphan_feature_flipped_to_failed(self):
        ...
        with patch("bob.orchestrator.final_reaper.sweep_orphan_subagents") as mock_sweep, \
             patch("bob.orchestrator.final_reaper.db") as mock_db, \
             patch("bob.orchestrator.final_reaper.find_subagent_pid_for_feature") as mock_pids:
            mock_sweep.return_value = []
    ...
    def test_sweep_orphan_subagents_called_first(self):
        ...
        with patch("bob.orchestrator.final_reaper.sweep_orphan_subagents", side_effect=mock_sweep), \
             patch("bob.orchestrator.final_reaper.db") as mock_db:
