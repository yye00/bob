"""Tests for bob.stuck_executing_reaper — top-level module.
...
class TestModuleImports:
    def test_module_importable(self):
        import bob.stuck_executing_reaper  # noqa: F401

    def test_detect_and_reset_stuck_features_callable(self):
        from bob.stuck_executing_reaper import detect_and_reset_stuck_features
...
class TestDetectAndResetStuckFeatures:
    ...
    def test_logs_reap_event(self, caplog):
        ...
        with patch("bob.orchestrator.stuck_executing_reaper.db") as mock_db:
            mock_db.list_features.return_value = [feature]
            with caplog.at_level(logging.INFO, logger="bob.orchestrator.stuck_executing_reaper"):
                detect_and_reset_stuck_features("proj-1")
        assert any(
...
# ---------------------------------------------------------------------------
# Orchestrator integration: bob.orchestrator exports sweep via stuck_executing_reaper
# ---------------------------------------------------------------------------

class TestOrchestratorIntegration:
    def test_orchestrator_stuck_executing_reaper_importable(self):
        from bob.orchestrator.stuck_executing_reaper import sweep_stuck_executing
        assert callable(sweep_stuck_executing)
    ...
    def test_top_level_delegates_to_orchestrator_sweep(self):
        ...
        from bob.stuck_executing_reaper import detect_and_reset_stuck_features
        with patch(
            "bob.orchestrator.stuck_executing_reaper.sweep_stuck_executing"
        ) as mock_sweep:
    ...
    def test_orchestrator_run_loop_imports_sweep_stuck_executing(self):
        ...
        run_loop_path = Path(__file__).parent.parent / "src" / "bob" / "orchestrator" / "run_loop.py"
