...
Root cause addressed: bob v.14 round 11 — feature fbd68fee verification
...
# ---------------------------------------------------------------------------
# Integration: verify all symbols importable from bob.verifier
# ---------------------------------------------------------------------------


class TestOrchestratorIntegrationAC:
    """AC: integration: bob.orchestrator — verifier scoping wired into the stack."""

    def test_scope_pytest_to_feature_in_bob_verifier(self):
        from bob.verifier import scope_pytest_to_feature as fn
        assert callable(fn)

    def test_collect_feature_test_paths_in_bob_verifier(self):
        from bob.verifier import collect_feature_test_paths as fn
        assert callable(fn)

    def test_build_scoped_pytest_argv_in_bob_verifier(self):
        from bob.verifier import build_scoped_pytest_argv as fn
        assert callable(fn)

    def test_assert_no_sibling_collection_in_bob_verifier(self):
    ...
    def test_orchestrator_run_loop_importable(self):
        ...
        from bob.orchestrator import run_loop  # noqa: F401
        assert True
