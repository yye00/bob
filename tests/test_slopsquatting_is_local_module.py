"""Tests for is_local_module function in bob.security.slopsquatting_scan."""
from __future__ import annotations

import pytest
from pathlib import Path
from bob.security.slopsquatting_scan import is_local_module


@pytest.fixture
def fixture_tree(tmp_path: Path) -> Path:
    """Create a fixture src/bob tree with example_local.py and example_pkg/__init__.py."""
    src_bob = tmp_path / "src" / "bob"
    src_bob.mkdir(parents=True)
    # A single .py file module
    (src_bob / "example_local.py").write_text("# local module\n")
    # A package
    pkg_dir = src_bob / "example_pkg"
    pkg_dir.mkdir()
...
def test_is_local_module_returns_true_for_py_file(fixture_tree: Path) -> None:
    """is_local_module returns True for a .py file at src/bob/example_local.py."""
    py_file = fixture_tree / "src" / "bob" / "example_local.py"
...
def test_is_local_module_returns_true_for_package_init(fixture_tree: Path) -> None:
    ...
    init_file = fixture_tree / "src" / "bob" / "example_pkg" / "__init__.py"
...
def test_is_local_module_returns_false_for_nonexistent_file(tmp_path: Path) -> None:
    ...
    nonexistent = tmp_path / "src" / "bob" / "does_not_exist.py"
...
def test_is_local_module_returns_false_for_non_python_file(tmp_path: Path) -> None:
    ...
    src_bob = tmp_path / "src" / "bob"
    src_bob.mkdir(parents=True)
    txt_file = src_bob / "some_file.txt"
