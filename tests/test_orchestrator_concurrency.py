...
- Is importable from bob3.orchestrator
...
- Respects BOB3_MAX_CONCURRENT_FEATURES env override
...
# ---------------------------------------------------------------------------
# Integration AC: dispatch_concurrent_features importable from bob3.orchestrator
# ---------------------------------------------------------------------------

def test_dispatch_concurrent_features_importable_from_bob3_orchestrator():
    """dispatch_concurrent_features must be importable from bob3.orchestrator."""
    from bob3.orchestrator import dispatch_concurrent_features as dcf
    assert callable(dcf)
...
# ---------------------------------------------------------------------------
# BOB3_MAX_CONCURRENT_FEATURES env variable
# ---------------------------------------------------------------------------
...
def test_resolve_max_concurrent_features_env_override(monkeypatch):
    """BOB3_MAX_CONCURRENT_FEATURES env var overrides the default."""
...
def test_resolve_max_concurrent_features_clamps_non_positive(monkeypatch):
    """Non-positive BOB3_MAX_CONCURRENT_FEATURES is clamped to 1."""
...
async def test_dispatch_concurrent_features_runs_workers_concurrently():
    ...
    with mock.patch("bob3.orchestrator.run_loop.db") as mock_db, \
         mock.patch.dict("os.environ", {"BOB3_MAX_CONCURRENT_FEATURES": "3"}):
        mock_db.update_feature = mock.MagicMock()
        results = await dispatch_concurrent_features(
            loop,
            worker=timed_worker,
