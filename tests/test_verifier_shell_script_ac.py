"""Tests for bob.verifier.demote_shell_script_integration_ac (Pattern 9, F-R7-594).

Verifies that check_shell_script_integration / demote_shell_script_integration_ac
in bob.verifier correctly handles integration ACs referencing shell scripts.
"""
...
from bob.verifier import demote_shell_script_integration_ac
...
class TestDemoteShellScriptIntegrationAc:
    ...
    def test_integration_with_python_module_returns_none(self, tmp_path: pathlib.Path) -> None:
        result = demote_shell_script_integration_ac("integration: bob.verifier", tmp_path)
        assert result is None
    ...
    def test_function_is_importable_from_verifier_module(self) -> None:
        """Confirms the alias is exported from bob.verifier."""
