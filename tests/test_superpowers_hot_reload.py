"""Tests for bob3.superpowers.reload_prompt_source_if_changed (feature cedc291e).

Verifies that hot-reload of prompt-source modules works correctly:
- reload_prompt_source_if_changed is importable from bob3.superpowers
...
# ---------------------------------------------------------------------------
# AC: Function defined: bob3.superpowers.reload_prompt_source_if_changed
# ---------------------------------------------------------------------------
...
def test_reload_prompt_source_if_changed_default_arg():
    """reload_prompt_source_if_changed defaults to 'bob3.superpowers' module."""
    from bob3.superpowers import reload_prompt_source_if_changed
...
# ---------------------------------------------------------------------------
# AC: integration: bob3.orchestrator (run_loop integrates the reloader)
# ---------------------------------------------------------------------------
...
def test_reload_if_stale_first_call_records_baseline():
    ...
    try:
        ...
        assert result is False
        assert "bob3.superpowers" in prompt_source_reloader._MTIME_CACHE
    finally:
        prompt_source_reloader._MTIME_CACHE.clear()
...
def test_prompt_source_modules_includes_superpowers():
    """_PROMPT_SOURCE_MODULES must include bob3.superpowers."""
...
def test_prompt_source_modules_includes_models():
    """_PROMPT_SOURCE_MODULES must include bob3.models."""
    from bob3.orchestrator.prompt_source_reloader import _PROMPT_SOURCE_MODULES
