...
def test_integration_with_bob_spec_quality():
    """Integration: bob.spec_quality — delegates to the existing quality scoring."""
    ...
    bob_report = compute_score(name=name, description=None, acceptance_criteria=acs)
    assert abs(spec_quality_score - bob_report.score) < 1e-6, (
        f"calculate_spec_quality_score ({spec_quality_score}) should match "
        f"compute_score ({bob_report.score})"
    )
...
class TestExtractPyPaths:
    ...
    def test_extracts_path_with_directory(self):
        desc = "The implementation lives in src/bob/brownfield/survey.py."
        paths = self.fn(desc)
        assert "src/bob/brownfield/survey.py" in paths

    def test_extracts_multiple_paths(self):
        desc = "See src/bob/foo.py and tools/spec_quality_score.py for details."
        paths = self.fn(desc)
        assert "src/bob/foo.py" in paths
        assert "tools/spec_quality_score.py" in paths

    def test_deduplicates_paths(self):
        desc = "Use src/bob/foo.py and also src/bob/foo.py again."
        paths = self.fn(desc)
    ...
    def test_returns_list_type(self):
        result = self.fn("some text src/bob/x.py here")
...
class TestExtractConcretePyPaths:
    ...
    def test_returns_same_result_as_extract_py_paths(self):
        ...
        from tools.spec_quality_score import extract_py_paths
        desc = "Use src/bob/foo.py and tools/spec_quality_score.py."
...
def test_synthesizer_emits_file_exists_for_py_paths():
    ...
    assert "src/bob/brownfield/survey.py" in py_paths, (
        f"extract_py_paths must detect 'src/bob/brownfield/survey.py'; got {py_paths}"
    )
...
def _all_metrics(value: float) -> dict:
    from bob.spec_quality.composite_score import SUB_METRIC_WEIGHTS
...
def test_boundary_conditions_65_80_green():
    ...
    """
    from bob.spec_quality.composite_score import compute_composite_score
    from bob.spec_quality_score import validate_score_thresholds
