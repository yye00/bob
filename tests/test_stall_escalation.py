"""Tests for bob.stall_escalation.

AC: File exists: src/bob/stall_escalation.py
    Function defined: bob.stall_escalation.escalate_stall_observation
    Function defined: bob.stall_escalation.write_stall_attention_marker
    integration: bob.weekend_watchdog
"""
...
from bob.stall_escalation import escalate_stall_observation, write_stall_attention_marker
...
class TestModuleInterface:
    ...
    def test_module_has_all_exports(self):
        import bob.stall_escalation as mod
...
class TestEscalateStallObservation:
    ...
    def test_escalation_logs_warn(self, tmp_path, caplog):
        marker = tmp_path / "STALL_ATTENTION.txt"
        with caplog.at_level(logging.WARNING, logger="bob.stall_escalation"):
            escalate_stall_observation(observation_count=5, marker_path=marker)
...
class TestWeekendWatchdogIntegration:
    """Verify bob.weekend_watchdog re-exports the stall escalation functions."""

    def test_weekend_watchdog_exports_escalate_stall_observation(self):
        from bob.weekend_watchdog import escalate_stall_observation as fn
        assert callable(fn)

    def test_weekend_watchdog_exports_write_stall_attention_marker(self):
        from bob.weekend_watchdog import write_stall_attention_marker as fn
        assert callable(fn)
