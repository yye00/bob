...
from bob.spec_quality.ac_auto_repair import apply_repairs
...
class TestApplyRepairs:
    ...
    def test_error_severity_applied_when_equivalent(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("bob.spec_quality.ac_auto_repair.verify_semantic_equivalence", return_value=equiv_result),
        ):
    ...
    def test_no_rewrite_skips_finding(self, tmp_path: Path) -> None:
        ...
        with (
            patch("bob.spec_quality.ac_auto_repair.suggest_rewrite", return_value=None),
        ):
            repairs = apply_repairs(
                findings=[finding],
                feature_id="feat-001",
    ...
    def test_multiple_findings_mixed_outcomes(self, tmp_path: Path) -> None:
        ...
        with (
            patch("bob.spec_quality.ac_auto_repair.suggest_rewrite", side_effect=mock_suggest),
            patch("bob.spec_quality.ac_auto_repair.verify_semantic_equivalence", return_value=equiv_result),
        ):
            repairs = apply_repairs(
