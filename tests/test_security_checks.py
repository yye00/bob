"""Tests for bob.security_checks (Round 0 Task 2 — Gap #2).
...
from bob.security_checks import (
...
def test_pip_audit_parses_vulnerabilities(workspace: Path):
    ...
    with patch("bob.security_checks.subprocess.run") as m_run:
...
def test_pip_audit_timeout_returns_tool_failure(workspace: Path):
    with patch("bob.security_checks.subprocess.run", side_effect=subprocess.TimeoutExpired("pip-audit", 60)):
        findings, err = _run_pip_audit(workspace, timeout=60)
    assert findings == []
...
def test_slopsquatting_flags_404_packages(workspace: Path):
    ...
    with patch("bob.security_checks._pypi_package_exists", side_effect=fake_exists):
...
def test_slopsquatting_clean_returns_no_findings(workspace: Path):
    with patch("bob.security_checks._pypi_package_exists", return_value=True):
        findings, err = _run_slopsquatting(workspace, diff=SLOP_DIFF, timeout=60)
    assert err is None
    assert findings == []
...
def test_slopsquatting_all_network_failures_records_tool_failure(workspace: Path):
    with patch("bob.security_checks._pypi_package_exists", return_value=None):
        findings, err = _run_slopsquatting(workspace, diff=SLOP_DIFF, timeout=60)
    assert findings == []
...
def test_pypi_package_exists_handles_404():
    ...
    with patch("bob.security_checks.urllib.request.urlopen", side_effect=fake_err):
...
def test_pypi_package_exists_handles_200():
    ...
    with patch("bob.security_checks.urllib.request.urlopen", return_value=fake_resp):
...
def test_run_security_checks_clean_returns_no_hard_fail(workspace: Path):
    with (
        patch("bob.security_checks._run_pip_audit", return_value=([], None)),
        patch("bob.security_checks._run_detect_secrets", return_value=([], None)),
        patch("bob.security_checks._run_bandit", return_value=([], None)),
        patch("bob.security_checks._run_slopsquatting", return_value=([], None)),
    ):
...
def test_run_security_checks_hard_fail_on_secret(workspace: Path):
    ...
    with (
        patch("bob.security_checks._run_pip_audit", return_value=([], None)),
        patch("bob.security_checks._run_detect_secrets", return_value=([secret_finding], None)),
        patch("bob.security_checks._run_bandit", return_value=([], None)),
        patch("bob.security_checks._run_slopsquatting", return_value=([], None)),
    ):
...
def test_run_security_checks_tool_failure_recorded(workspace: Path):
    with (
        patch("bob.security_checks._run_pip_audit", return_value=([], "pip-audit timed out")),
        patch("bob.security_checks._run_detect_secrets", return_value=([], None)),
        patch("bob.security_checks._run_bandit", return_value=([], None)),
        patch("bob.security_checks._run_slopsquatting", return_value=([], None)),
    ):
...
def test_run_security_checks_one_subcheck_crash_does_not_block_others(workspace: Path):
    ...
    with (
        patch("bob.security_checks._run_pip_audit", side_effect=RuntimeError("kaboom")),
        patch("bob.security_checks._run_detect_secrets", return_value=([], None)),
        patch("bob.security_checks._run_bandit", return_value=([], None)),
        patch("bob.security_checks._run_slopsquatting", return_value=([], None)),
    ):
...
def test_run_security_checks_invokes_all_four_sub_checks(workspace: Path):
    with (
        patch("bob.security_checks._run_pip_audit", return_value=([], None)) as m1,
        patch("bob.security_checks._run_detect_secrets", return_value=([], None)) as m2,
        patch("bob.security_checks._run_bandit", return_value=([], None)) as m3,
        patch("bob.security_checks._run_slopsquatting", return_value=([], None)) as m4,
    ):
...
def test_check_9_invoked_from_run_verification_checklist(tmp_path: Path):
    ...
    with patch("bob.superpowers.run_security_checks", return_value=fake_clean_result) as m:
...
def test_check_9_hard_fail_blocks_overall_passed(tmp_path: Path):
    ...
    with patch("bob.superpowers.run_security_checks", return_value=fake_bad):
        result = superpowers.run_verification_checklist(
            workspace=str(tmp_path),
            diff="+SECRET=...\n",
            enable_security_check=True,
...
def test_check_9_can_be_disabled(tmp_path: Path):
    ...
    with patch("bob.superpowers.run_security_checks") as m:
        result = superpowers.run_verification_checklist(
            workspace=str(tmp_path),
...
def test_read_first_party_packages_includes_tools_and_root_modules(tmp_path: Path) -> None:
    """_read_first_party_packages includes tools/ and project-root .py modules.

    Regression test for recurring NH: slopsquatting hard-fails on
    ``import spec_quality_score`` when _read_first_party_packages misses
    tools/spec_quality_score.py. This is the canonical test name required
    by F-R7-481 acceptance criteria.
    """
    from bob.security_checks import _read_first_party_packages
    ...
    (tmp_path / "root_mod.py").write_text("# root-level module\n")
    ...
    assert "foo" in result, "foo.py in tools/ must be first-party"
    assert "root_mod" in result, "root-level .py files must be first-party"
    assert "__init__" not in result


def test_slopsquatting_allowlist_includes_tools_and_project_root(tmp_path: Path) -> None:
    """Slopsquatting first-party allowlist must include tools/ and project-root .py modules.

    Canonical regression test for feature 10784e1e: _read_first_party_packages must
    include tools/*.py and workspace-root .py files so that local scripts like
    spec_quality_score.py are never probed against PyPI.
    """
    from bob.security_checks import _read_first_party_packages
