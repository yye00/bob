...
class TestWarningStubHeuristics:
    ...
    def test_warning_does_not_break_passed_flag(self):
        ...
        """
        sources = {
            "src/bob/module.py": textwrap.dedent("""\
                def returns_none():
                    return None
            """),
        }
...
class TestDetectMockUsage:
    ...
    def test_mock_import_in_src_flagged(self):
        ...
        source = textwrap.dedent("""\
            ...
            def something():
                return 42
        """)
        findings = detect_mock_usage(source, "src/bob/mymodule.py")
        assert len(findings) >= 1
    ...
    def test_magicmock_usage_in_src_flagged(self):
        ...
        source = textwrap.dedent("""\
            ...
            mock_obj = MagicMock()
        """)
        findings = detect_mock_usage(source, "src/bob/bad.py")
        assert len(findings) >= 1
    ...
    def test_no_mock_usage_clean(self):
        ...
        """)
        findings = detect_mock_usage(source, "src/bob/clean.py")
        assert len(findings) == 0
...
class TestVerifyNoStubsOrMocks:
    ...
    def test_mixed_findings(self):
        ...
        sources = {
            "src/bob/a.py": textwrap.dedent("""\
                def stub():
                    pass
            """),
            "src/bob/b.py": textwrap.dedent("""\
