...
class TestE2EStep4ResumeFromFeature3:
    ...
    async def test_resume_continues_from_feature_3(self, tmp_db, project_with_5_features):
        ...
        with patch("bob.db.get_database_path", return_value=tmp_db):
            ...
            # Start a new orchestration loop (simulates bob run after interrupt)
            loop = OrchestrationLoop(
                project_id=project.id,
                workspace="/tmp/resume-e2e",
            )
...
class TestE2EStep9FullWorkflow:
    ...
    async def test_full_interrupt_resume_spec_change_workflow(self, tmp_db, tmp_path):
        ...
        with patch("bob.db.get_database_path", return_value=tmp_db):
            ...
            # Set feature 5 and 6 to ready (simulating bob plan/readiness assessment)
    ...
    async def test_three_interrupt_resume_cycles(self, tmp_db, project_with_5_features):
        ...
        with patch("bob.db.get_database_path", return_value=tmp_db):
            ...
            with patch(
                ...
                "bob.orchestrator.run_loop.git_commit_feature",
                return_value="c1",
            ), patch("bob.orchestrator.run_loop.stop_mcp_server"), patch(
                "bob.orchestrator.run_loop.run_verification_checklist",
                side_effect=_passing_verification,
            ):
                t1 = await loop1.run()
