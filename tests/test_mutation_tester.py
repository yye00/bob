"""Tests for bob3.mutation_tester — the canonical mutation gate entry point.

AC: pytest: tests/test_mutation_tester.py

Covers the public API of bob3.mutation_tester:
...
- verifier integration (module is importable from bob3.verifier scope)
"""
...
import bob3.mutation_tester as mutation_tester_module
from bob3.mutation_tester import (
...
def test_module_exists():
    """src/bob3/mutation_tester.py is importable."""
    assert mutation_tester_module is not None


def test_run_mutation_tests_is_callable():
    """bob3.mutation_tester.run_mutation_tests is a callable."""
...
def test_gate_passes_when_score_meets_threshold(tmp_path):
    ...
    with patch(
        "bob3.mutation_testing.mutmut_verifier.run_mutation_test",
...
def test_gate_fails_when_score_below_threshold(tmp_path):
    ...
    ), patch("bob3.mutation_testing.mutmut_verifier.persist_surviving_mutants"):
        result = run_mutation_tests(
            feature_id="feat-fail",
            src_files=["src/bob3/foo.py"],
            test_dir=str(tmp_path),
...
# ---------------------------------------------------------------------------
# Verifier integration — importable from bob3.verifier context
# ---------------------------------------------------------------------------


def test_verifier_imports_run_mutation_tests():
    """bob3.verifier exposes run_mutation_tests via its mutation_testing import."""
    import bob3.verifier  # noqa: F401 — integration smoke test
    # verifier.py imports run_mutation_tests from bob3.mutation_testing
    from bob3.mutation_testing import run_mutation_tests as verifier_rmt
    assert callable(verifier_rmt)


def test_mutation_tester_is_consistent_with_mutation_testing():
    """bob3.mutation_tester.run_mutation_tests and bob3.mutation_testing.run_mutation_tests are the same callable."""
    from bob3.mutation_testing import run_mutation_tests as mt_rmt
    from bob3.mutation_tester import run_mutation_tests as mtr_rmt
