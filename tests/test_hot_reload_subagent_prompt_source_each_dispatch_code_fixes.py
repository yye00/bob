"""Tests for bob.hot_reload_subagent_prompt_source_each_dispatch_code_fixes.
...
def test_hot_reload_subagent_prompt_source_each_dispatch_code_fixes():
    ...
    from bob.hot_reload_subagent_prompt_source_each_dispatch_code_fixes import (
...
def test_module_importable():
    ...
    import bob.hot_reload_subagent_prompt_source_each_dispatch_code_fixes  # noqa: F401
...
def test_returns_reloaded_module_names_when_stale():
    ...
    with patch.object(
        _reloader, "maybe_reload_all", return_value=["bob.superpowers"]
    ):
        result = hot_reload_subagent_prompt_source_each_dispatch_code_fixes()
...
def test_detects_mtime_change_and_reloads():
    ...
    try:
        ...
        assert "bob.superpowers" in result
    finally:
        prompt_source_reloader._MTIME_CACHE.clear()
        prompt_source_reloader._MTIME_CACHE.update(original_cache)
