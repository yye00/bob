"""Word-boundary detection tests for hippy.spec_synthesis._ensure_boundary_and_error_coverage.
...
matches "fail", "length-capped" matches "limit"), so the AC was skipped while
the scorer still saw 0 coverage → composite 0.0. These tests pin the fix.
"""
...
from hippy.spec_synthesis import _ensure_boundary_and_error_coverage


def _joined(criteria):
    return "\n".join(criteria).lower()


class TestSlugFalsePositivesAreNotDetected:
    ...
    def test_failing_slug_still_injects_error_ac(self):
        ...
        out = _ensure_boundary_and_error_coverage(criteria, title="Failing tests reporter")
        # An error-path AC must be injected because no PROSE AC has an error token.
        assert any("_error.py" in c for c in out)
        assert any("_boundary.py" in c for c in out)

    def test_length_capped_slug_still_injects_boundary_ac(self):
        # "length-capped" contains "limit"? no — but "capped"/"cap" near "limit"
        # semantics. Use the documented regressing case: a slug carrying "limit".
        criteria = [
            "File exists: src/bob/length_capped_slug.py",
            "Function defined: bob.length_capped_slug.derive",
            "pytest: tests/test_length_capped_slug.py",
        ]
        out = _ensure_boundary_and_error_coverage(criteria, title="Length-capped module slug")
        assert any("_boundary.py" in c for c in out)
        assert any("_error.py" in c for c in out)


class TestProseCoverageSuppressesInjection:
    """When a PROSE AC carries a real coverage token, do not double-inject."""
    ...
    def test_prose_behavior_acs_suppress_both_injections(self):
        # The mandated detector counts prose/behaviour ACs only; a behaviour AC
        # carrying both tokens suppresses all injection.
        criteria = [
            "File exists: src/bob/x.py",
            "behavior: on empty input returns a result; on invalid input raises",
        ]
        out = _ensure_boundary_and_error_coverage(criteria, title="x")
        assert out == criteria
...
class TestInjectionShape:
    ...
    def test_injected_acs_are_pytest_form(self):
        ...
        injected = [c for c in out if c not in ("File exists: src/bob/x.py",)]
