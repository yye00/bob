...
class TestSetModel:
    ...
    def test_set_model_sonnet(self):
        ...
        assert "sonnet" in opts.model.lower()
        assert opts.model.lower().startswith("claude-")
    ...
    def test_set_model_haiku(self):
        ...
        assert opts.model is not None
        assert "haiku" in opts.model.lower()
        assert opts.model.lower().startswith("claude-")
    ...
    def test_model_aliases_map_to_valid_ids(self):
        from bob.orchestrator.claude_executor import MODEL_ALIASES, VALID_MODEL_IDS
...
class TestSetCwd:
    ...
    def test_cwd_string_path(self):
        ...
        opts = build_sub_agent_options(cwd="/home/captain/clawd/work/bob")
        assert opts.cwd == "/home/captain/clawd/work/bob"

    def test_cwd_pathlib_path(self):
        from bob.orchestrator.claude_executor import build_sub_agent_options

        p = pathlib.Path("/home/captain/clawd/work/bob")
...
class TestSpawnAgentWithOptions:
    ...
    def test_executor_passes_options_to_query(self):
        ...
        assert "haiku" in passed_opts.model.lower()
    ...
    def test_executor_per_call_options_override_default(self):
        ...
        assert "opus" in passed_opts.model.lower()
    ...
    def test_spawn_sub_agent_passes_options(self, monkeypatch):
        ...
        With ``PERPLEXITY_API_KEY`` unset bob does not need to merge in
        any MCP servers, so the original options object should be passed
        through untouched. (When the key is set, bob may build a new
        ...
        async def _run():
            with (
                patch("bob.orchestrator.claude_executor.query", mock_query),
                patch("bob.db.create_agent_run", return_value=mock_agent_run),
                patch("bob.db.update_agent_run", return_value=mock_agent_run),
            ):
    ...
    def test_rca_agent_uses_configured_options(self):
        ...
        from bob.orchestrator.claude_executor import spawn_rca_agent
        from claude_code_sdk import AssistantMessage, ResultMessage, TextBlock
