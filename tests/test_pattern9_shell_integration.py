"""Tests for src/bob3/verifier/pattern9_shell_integration.py (F-R7-594).
...
from bob3.verifier.pattern9_shell_integration import (
...
def test_is_shell_script_integration_false_for_python_module_body() -> None:
    assert is_shell_script_integration("integration: bob3.orchestrator") is False
...
def test_demote_none_for_python_module_body(tmp_path: pathlib.Path) -> None:
    result = demote_to_pass_with_warning("integration: bob3.verifier", tmp_path)
    assert result is None
...
def test_demote_warns_with_f_r7_594_tag(tmp_path: pathlib.Path, caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.WARNING, logger="bob3.verifier.pattern9_shell_integration"):
