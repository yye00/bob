...
from bob3.auto_repair_smelly_acs_semantic_equivalence_verification import (
...
class TestAutoRepairSmelly:
    ...
    def test_error_ac_repaired_when_equivalent(self, tmp_path: Path) -> None:
        ...
        with (
            patch("bob3.spec_quality.ac_auto_repair._call_llm_judge") as mock_judge,
        ):
    ...
    def test_apply_repairs_warn_severity_not_applied(self, tmp_path: Path) -> None:
        ...
        with (
            patch("bob3.spec_quality.ac_auto_repair.suggest_rewrite", return_value="rewrite"),
            patch("bob3.spec_quality.ac_auto_repair.verify_semantic_equivalence", return_value=(True, "Equiv.")),
        ):
            repairs = apply_repairs(
                findings=[finding],
                feature_id="feat-001",
    ...
    def test_apply_repairs_non_equivalent_rejected(self, tmp_path: Path) -> None:
        ...
        with (
            patch("bob3.spec_quality.ac_auto_repair.suggest_rewrite", return_value="totally different"),
            patch("bob3.spec_quality.ac_auto_repair.verify_semantic_equivalence", return_value=(False, "Not equiv.")),
        ):
            repairs = apply_repairs(
                findings=[finding],
                feature_id="feat-001",
    ...
    def test_verify_semantic_equivalence_llm_error(self) -> None:
        with patch("bob3.spec_quality.ac_auto_repair._call_llm_judge", side_effect=Exception("down")):
    ...
    def test_suggest_rewrite_error_returns_string(self) -> None:
        finding = _make_error_finding()
        rewrite = "The system shall process requests."
        with patch("bob3.spec_quality.ac_auto_repair._call_llm_judge", return_value=_llm_rewrite(rewrite)):
            result = suggest_rewrite(finding)
    ...
    def test_reject_unequivalent_rewrite_raises_on_non_equiv(self) -> None:
        with (
            patch("bob3.spec_quality.ac_auto_repair._call_llm_judge", return_value=_llm_equiv_false()),
    ...
    def test_handle_missing_judge_raises_on_llm_error(self) -> None:
        with (
            patch("bob3.spec_quality.ac_auto_repair._call_llm_judge", side_effect=Exception("unreachable")),
