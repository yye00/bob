"""Tests for bob3.enhanced_verification.explain_gate_block and the CLI command.

Acceptance criteria:
  - CLI command: bob3 explain-gate-block
  - Function defined: bob3.enhanced_verification.explain_gate_block
...
class TestExplainGateBlockCLI:
    def _make_temp_db(self, tmp_path, acs, feature_name="Test Feature", description="A test"):
        """Create a temporary bob3.db with one feature for testing."""
        import os
        import sys
        ...
        schema_path = pathlib.Path(__file__).parent.parent / "src" / "bob3" / "schema.sql"
    ...
    def test_cli_text_output_contains_score(self, runner, tmp_path):
        ...
        result = runner.invoke(main, ["explain-gate-block", fid[:8]], env={
            "BOB3_DATABASE_PATH": str(db_path),
        })
