"""Tests for bob3.run_loop: classify_subagent_startup_crash and compute_persisted_artifact_count.
...
class TestFinalExitSweepPromotesOnDiskEvidence:
    ...
    def test_final_exit_sweep_promotes_on_disk_evidence(self) -> None:
        ...
        with (
            patch("bob3.orchestrator.run_loop.db") as mock_db,
            patch(
                "bob3.orchestrator.run_loop.find_subagent_pid_for_feature",
                return_value=[],
            ),
            patch(
                "bob3.orchestrator.run_loop._check_executing_feature_acs",
                return_value=True,
            ) as mock_check,
        ):
...
def test_disk_reconciler_promotion_on_verify_fail() -> None:
    ...
    acs = ["File exists: src/bob3/run_loop.py", "Function defined: bob3.run_loop.test_fn"]
...
def test_disk_reconciler_promotion_guards() -> None:
    ...
    acs_with_structural = json.dumps(["File exists: src/bob3/run_loop.py"])
...
def test_final_exit_sweep_summary_events(caplog: pytest.LogCaptureFixture) -> None:
    ...
    promoted_feature = SimpleNamespace(
        ...
        acceptance_criteria=json.dumps(["File exists: src/bob3/verification/ac_artifact_check.py"]),
    )
    ...
    with (
        ...
        caplog.at_level(logging.INFO, logger="bob3.orchestrator.run_loop"),
    ):
...
def test_projects_metadata_verification(tmp_path):
    ...
    Simulates a post-rsync child workspace whose bob3.db still has the parent
...
def test_disk_reconciler_promotes_verification_fail_on_disk() -> None:
    ...
    ac_json = json.dumps([
        ...
        "Function defined: bob3.run_loop.disk_reconciler_verify_fail_promotion",
    ])
...
def test_disk_reconciler_promotes_on_verify_fail_with_structural_and_behavior() -> None:
    ...
    from bob3.run_loop import disk_reconciler_promotion_on_verify_fail

    acs = [
        "File exists: src/bob3/run_loop.py",
        "Function defined: bob3.run_loop.disk_reconciler_promotion_on_verify_fail",
    ]
    ac_json = json.dumps(acs)
...
def test_verify_fail_disk_promotion_emits_event() -> None:
    ...
    with patch(
        ...
        with patch("bob3.run_loop.logger") as mock_logger:
...
def test_pre_hook_transient_summary_telemetry() -> None:
    ...
    """
    from bob3.run_loop import drain_mcp_transient_summary
    import logging
...
def test_mcp_transient_classified_before_git_hook_rejection() -> None:
    ...
    """
    from bob3.run_loop import classify_mcp_transient_pre_hook
...
def test_rca_auto_reset_on_code_emission_defect():
    ...
    from bob3.run_loop import apply_rca_auto_reset
...
def test_rca_auto_reset_budget_grant_verification_gate_failure():
    ...
    result = apply_rca_auto_reset(
        ...
        db_update_fn=fake_db_update,
        failed_acs=["integration: bob3.run_loop"],
...
def test_classify_failure_cause_pytest_ac():
    ...
    from bob3.run_loop import classify_failure_cause
...
def test_classify_failure_cause_integration_ac():
    ...
    result = classify_failure_cause(["integration: bob3.run_loop"])
    assert result == "code_emission_defect"
...
def test_pending_successor_verify_on_verifier_extension_ac_failure(tmp_path):
    ...
    with patch(
        "bob3.pending_successor_verify.is_verifier_extension_feature",
        return_value=True,
    ), patch("bob3.pending_successor_verify.db") as mock_db:
...
def test_concurrent_batch_claims_first_feature_before_building() -> None:
    ...
    from bob3.run_loop import build_concurrent_batch
...
# ---------------------------------------------------------------------------
# de62dd33: bob3 init re-run after spawn fixes stale project metadata
# ---------------------------------------------------------------------------
...
def test_orphan_reaper_on_orchestrator_exit_all_blocked() -> None:
    ...
    Verifies that bob3.run_loop._run_locked calls sweep_orphans_on_exit so that
    orphan executing rows are cleaned up when the orchestrator exits due to
    ALL_BLOCKED termination.
    """
    from unittest.mock import patch
    from bob3.run_loop import _run_locked

    project_id = "proj-all-blocked-0000-0000-000000000001"

    # _run_locked does `from bob3.final_reaper import sweep_orphans_on_exit` locally,
    # so we patch the function in final_reaper's namespace.
    with patch("bob3.final_reaper.sweep_orphans_on_exit") as mock_sweep:
...
def test_orphan_reaper_on_orchestrator_exit_budget_exceeded() -> None:
    ...
    Verifies that bob3.run_loop._run_locked calls sweep_orphans_on_exit for
...
def test_shell_script_integration_pass_with_warning(tmp_path, caplog) -> None:
    ...
    to an existing, executable .sh file, bob3.run_loop.handle_shell_script_integration
    returns (True, "") and emits a WARNING log line tagged 'F-R7-594'.
    """
    import logging
    import stat
    from bob3.run_loop import handle_shell_script_integration
    ...
    with caplog.at_level(logging.WARNING, logger="bob3.verifier.shell_script_ac"):
...
def test_verify_fail_disk_promoted_on_structural_and_behavior_pass() -> None:
    ...
    from bob3.run_loop import disk_reconciler_verify_fail_gate

    structural_acs = json.dumps([
        "File exists: src/bob3/run_loop.py",
        "Function defined: bob3.run_loop.disk_reconciler_verify_fail_gate",
    ])
...
def test_metadata_consistency_check(tmp_path: "Path") -> None:
    ...
    Verifies that:
    1. verify_project_metadata_consistency is importable from bob3.run_loop.
    ...
    from bob3.run_loop import verify_project_metadata_consistency, ProjectMetadataCheckResult
...
def test_concurrent_batch_claims_first_feature_before_loop():
    ...
    """
    from bob3.run_loop import claim_first_feature_for_batch
...
def test_batch_reaches_max_concurrent_size():
    ...
    """
    from bob3.run_loop import claim_batch_head
...
def test_batch_size_equals_min_claimable_and_max_concurrent():
    ...
    """
    from bob3.run_loop import claim_first_feature
...
def test_final_exit_sweep_emits_summary_events(caplog: pytest.LogCaptureFixture) -> None:
    ...
    failed_feature = SimpleNamespace(
        id=failed_id,
        name="genuinely incomplete",
        acceptance_criteria=json.dumps(["File exists: src/bob3/does_not_exist.py"]),
    )
...
def test_final_exit_sweep_flips_failed_when_reconciler_fails() -> None:
    ...
    feature = SimpleNamespace(
        ...
        acceptance_criteria=json.dumps(["File exists: src/bob3/does_not_exist_anywhere.py"]),
    )
...
def test_subagent_startup_crash_exempt_transport_errors(tmp_path: Path) -> None:
    ...
    """
    from bob3.run_loop import check_subagent_startup_crash
...
def test_disk_reconciler_promotion_verification_fail_with_passed_gates() -> None:
    ...
    from bob3.run_loop import disk_reconciler_promote_verification_fail
...
def test_mcp_transient_classification_before_git_hook_rejection() -> None:
    ...
    (bob3.run_loop) fires BEFORE the git-hook-rejection demotion path and correctly
    ...
    """
    from bob3.run_loop import classify_mcp_transient_pre_hook, drain_mcp_transient_summary
...
def test_final_exit_sweep_disk_promotes_orphans():
    ...
    from bob3.run_loop import _final_exit_sweep
    ...
    with (
        patch("bob3.orchestrator.run_loop.db") as mock_db,
        patch("bob3.orchestrator.run_loop.find_subagent_pid_for_feature", return_value=[]),
        patch(
            "bob3.orchestrator.run_loop._check_executing_feature_acs",
            return_value=True,
        ),
...
def test_final_exit_sweep_flipped_failed() -> None:
    ...
    feature = SimpleNamespace(
        ...
        name="genuinely incomplete feature",
        acceptance_criteria=json.dumps(
            ["File exists: src/bob3/nonexistent_artifact_12345.py"]
        ),
    )
