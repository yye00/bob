"""Test: arm_feature_watchdog reads BOB_FEATURE_TIMEOUT_SECONDS from env.

Verifies that _resolve_timeout_seconds (called by arm_feature_watchdog when
timeout_seconds is None) honours the BOB_FEATURE_TIMEOUT_SECONDS env var and
...
def test_env_not_set_returns_default(monkeypatch):
    """When BOB_FEATURE_TIMEOUT_SECONDS is not set, default (3600) is returned."""
...
def test_env_set_to_valid_value(monkeypatch):
    """When BOB_FEATURE_TIMEOUT_SECONDS=120, 120.0 is returned."""
    monkeypatch.setenv("BOB_FEATURE_TIMEOUT_SECONDS", "120")
    result = _resolve_timeout_seconds()
    assert result == 120.0
...
def test_env_set_to_float(monkeypatch):
    ...
    monkeypatch.setenv("BOB_FEATURE_TIMEOUT_SECONDS", "45.5")
    result = _resolve_timeout_seconds()
    assert result == 45.5


def test_env_invalid_string_returns_default(monkeypatch):
    """Non-numeric BOB_FEATURE_TIMEOUT_SECONDS falls back to the default."""
    monkeypatch.setenv("BOB_FEATURE_TIMEOUT_SECONDS", "not_a_number")
    result = _resolve_timeout_seconds()
    assert result == float(_DEFAULT_FEATURE_TIMEOUT_SECONDS)


def test_env_zero_returns_default(monkeypatch):
    """Zero BOB_FEATURE_TIMEOUT_SECONDS falls back to the default."""
...
def test_env_negative_returns_default(monkeypatch):
    """Negative BOB_FEATURE_TIMEOUT_SECONDS falls back to the default."""
...
async def test_arm_watchdog_uses_env_timeout(monkeypatch):
    """arm_feature_watchdog with timeout_seconds=None reads BOB_FEATURE_TIMEOUT_SECONDS."""
    monkeypatch.setenv("BOB_FEATURE_TIMEOUT_SECONDS", "300")
    ...
    with patch(
        "bob.orchestrator.feature_watchdog.cancel_subagent_at_deadline",
...
async def test_arm_watchdog_explicit_timeout_overrides_env(monkeypatch):
    ...
    monkeypatch.setenv("BOB_FEATURE_TIMEOUT_SECONDS", "9999")
