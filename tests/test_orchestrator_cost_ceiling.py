...
def test_apply_pessimistic_cost_importable_from_run_loop():
    """apply_pessimistic_cost must be importable from bob3.orchestrator.run_loop."""
...
def test_env_override_ceiling():
    """BOB3_PER_FEATURE_COST_CEILING env var overrides the default ceiling."""
    os.environ["BOB3_PER_FEATURE_COST_CEILING"] = "5.0"
    try:
        ceiling = compute_per_feature_ceiling()
        assert ceiling == pytest.approx(5.0)
    finally:
        del os.environ["BOB3_PER_FEATURE_COST_CEILING"]


def test_invalid_env_ceiling_falls_back_to_default():
    """Invalid BOB3_PER_FEATURE_COST_CEILING falls back to default (no raise)."""
    os.environ["BOB3_PER_FEATURE_COST_CEILING"] = "not-a-number"
    try:
        ceiling = compute_per_feature_ceiling()
...
def test_negative_env_ceiling_falls_back_to_default():
    """Negative BOB3_PER_FEATURE_COST_CEILING is rejected and falls back to default."""
    os.environ["BOB3_PER_FEATURE_COST_CEILING"] = "-5.0"
    try:
        ceiling = compute_per_feature_ceiling()
        assert ceiling > 0.0
...
def test_zero_env_ceiling_falls_back_to_default():
    """Zero BOB3_PER_FEATURE_COST_CEILING is rejected and falls back to default."""
    os.environ["BOB3_PER_FEATURE_COST_CEILING"] = "0.0"
    try:
        ceiling = compute_per_feature_ceiling()
        assert ceiling > 0.0
    finally:
...
def test_apply_pessimistic_cost_with_env_ceiling():
    ...
    os.environ["BOB3_PER_FEATURE_COST_CEILING"] = "7.5"
    try:
        ceiling = compute_per_feature_ceiling()
        charged = apply_pessimistic_cost(
            reported_cost=0.0,
