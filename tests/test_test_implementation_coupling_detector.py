...
Verifies that ``bob.test_implementation_coupling_detector`` exports the full
...
from bob.test_implementation_coupling_detector import (
    CouplingFinding,
    CouplingResult,
...
class TestDetectInternalImports:
    ...
    def test_private_module_import_flagged(self, tmp_path):
        _write(tmp_path, "tests/test_priv.py", """\
            from bob._private import helper
            def test_it(): pass
        """)
        findings = detect_internal_imports(workspace=tmp_path)
        assert len(findings) >= 1
