...
class TestGetDatabasePath:
    """Step 2: get_database_path() returns Path.cwd()/bob.db or BOB_DATABASE_PATH."""

    def test_default_path_is_cwd_bob_db(self, monkeypatch, tmp_path):
    ...
    def test_default_uses_cwd_not_install_dir(self, monkeypatch, tmp_path):
        ...
        result = get_database_path()
        install_default = get_package_dir().parent.parent / "bob.db"
    ...
    def test_env_var_overrides_default(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("BOB_DATABASE_PATH", custom_path)
        from bob.db import get_database_path
    ...
    def test_env_var_overrides_cwd(self, monkeypatch, tmp_path):
        """BOB_DATABASE_PATH wins even when cwd is something else."""
        cwd_dir = tmp_path / "elsewhere"
        cwd_dir.mkdir()
        custom_path = tmp_path / "custom.db"
        monkeypatch.chdir(cwd_dir)
        monkeypatch.setenv("BOB_DATABASE_PATH", str(custom_path))

        from bob.db import get_database_path

        result = get_database_path()
        assert result == custom_path
        assert result != cwd_dir / "bob.db"
...
class TestGetConnection:
    ...
    def test_returns_sqlite3_connection(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("BOB_DATABASE_PATH", db_path)
    ...
    def test_connection_has_busy_timeout(self, tmp_path, monkeypatch):
        ...
        ``bob run`` invocations and overlapping ``bob status`` queries
...
class TestConnectionContextManager:
    ...
    def test_context_manager_commits_on_success(self, tmp_path, monkeypatch):
        db_path = str(tmp_path / "test.db")
        monkeypatch.setenv("BOB_DATABASE_PATH", db_path)
        from bob.db import init_database, connect
...
class TestAllTablesAndViews:
    ...
    def setup_db(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("BOB_DATABASE_PATH", self.db_path)
        from bob.db import init_database
