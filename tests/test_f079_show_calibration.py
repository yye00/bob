...
class TestShowCalibrationCommandRegistered:
    ...
    def test_show_calibration_uses_rich_table(self):
        ...
        from bob.cli import show_calibration_cmd
...
class TestShowCalibrationQueryView:
    ...
    def test_calls_query_calibration_drift_summary(self):
        ...
        runner = CliRunner()
        with patch("bob.cli.query_calibration_drift_summary", return_value=[]) as mock_query, \
             patch("bob.cli._get_current_project_id", return_value="proj-1"):
    ...
    def test_no_project_message(self):
        ...
        runner = CliRunner()
        with patch("bob.cli._get_current_project_id", return_value=None):
            result = runner.invoke(main, ["show-calibration"])
        assert result.exit_code == 0
...
class TestShowCalibrationDisplay:
    ...
    def test_displays_task_class(self):
        ...
        runner = CliRunner()
        with patch("bob.cli.query_calibration_drift_summary", return_value=entries), \
             patch("bob.cli._get_current_project_id", return_value="proj-1"):
            result = runner.invoke(main, ["show-calibration"])
        assert result.exit_code == 0
    ...
    def test_no_data_message(self):
        ...
        runner = CliRunner()
        with patch("bob.cli.query_calibration_drift_summary", return_value=[]), \
             patch("bob.cli._get_current_project_id", return_value="proj-1"):
            result = runner.invoke(main, ["show-calibration"])
        assert result.exit_code == 0
        output_lower = result.output.lower()
...
class TestShowCalibrationWithDriftData:
    ...
    def test_show_calibration_with_real_db(self, db_with_calibration):
        ...
        runner = CliRunner()
        with patch("bob.db.get_database_path", return_value=db_path), \
             patch("bob.cli._get_current_project_id", return_value=project_id), \
             patch("bob.cli.query_calibration_drift_summary", wraps=query_calibration_drift_summary) as mock_query:
            result = runner.invoke(main, ["show-calibration"])
    ...
    def test_overconfident_bucket_shown_in_drift_data(self, db_with_calibration):
        ...
        with patch("bob.db.get_database_path", return_value=db_path), \
             patch("bob.cli._get_current_project_id", return_value=project_id), \
             patch("bob.cli.query_calibration_drift_summary", wraps=query_calibration_drift_summary):
            result = runner.invoke(main, ["show-calibration"])
