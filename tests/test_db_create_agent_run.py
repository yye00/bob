"""Tests for bob.db.create_agent_run — explicit db_path threading.

Verifies that create_agent_run correctly writes to the project's database
when db_path is supplied explicitly, and that the BOB_DATABASE_PATH env
var is honoured when it is set.

Root-cause context: without an explicit db_path, create_agent_run resolved
the database from cwd or BOB_DATABASE_PATH, which could differ from the
...
def test_create_agent_run_env_var_db_path_honoured(project_with_db, monkeypatch):
    """When BOB_DATABASE_PATH is set to the project DB, create_agent_run succeeds."""
    project, db_path = project_with_db
...
def test_create_agent_run_importable_from_bob_db():
    """bob.db.create_agent_run is importable via the canonical entry point."""
    from bob.db import create_agent_run as fn
    assert callable(fn)


def test_create_agent_run_importable_from_orchestrator():
    """bob.orchestrator re-exports create_agent_run (integration AC)."""
    from bob.orchestrator import create_agent_run as fn
    assert callable(fn)
