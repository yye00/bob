"""Tests for bob.research_notes.generate_research_notes (BF-2)."""
...
class TestGenerateResearchNotesBasic:
    ...
    def test_returns_dict(self):
        from bob.research_notes import generate_research_notes
...
class TestGenerateResearchNotesWithPathGlob:
    ...
    def test_prompt_includes_path_glob(self):
        from bob.research_notes import generate_research_notes
        result = generate_research_notes(path_glob="src/bob/**")
        assert "src/bob/**" in result["researcher_prompt"]

    def test_prompt_includes_symbols(self):
        from bob.research_notes import generate_research_notes
        result = generate_research_notes(
            path_glob="src/bob/**", symbol_shortlist=["Coordinator", "dispatch_slot"]
        )
...
class TestGenerateResearchNotesFileWrite:
    ...
    def test_writes_file_when_content_and_feature_id(self, tmp_path):
        ...
        notes_file = tmp_path / ".bob" / "features" / "feat-write" / "research_notes.md"
    ...
    def test_file_contains_frontmatter(self, tmp_path):
        ...
        notes_file = tmp_path / ".bob" / "features" / "feat-front" / "research_notes.md"
        text = notes_file.read_text()
        assert "survey_sha: sha999" in text
        assert "path_glob: src/bob/**" in text
