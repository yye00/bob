"""Tests for bob.synth_retry — retry primitives for transient upstream API failures.
...
from bob.synth_retry import retry_with_backoff, synthesize_with_retry
...
class TestRetryWithBackoff:
    ...
    async def test_retry_logs_warning_on_empty(self, caplog):
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock):
            with caplog.at_level(logging.WARNING, logger="bob.synth_retry"):
    ...
    async def test_env_var_controls_max_attempts(self):
        """BOB_SYNTH_MAX_ATTEMPTS env var controls max attempts when max_attempts is None."""
        call_count = 0
    ...
    async def test_invalid_env_var_falls_back_to_default(self):
        """Invalid BOB_SYNTH_MAX_ATTEMPTS value does not crash; falls back to default."""
        async def fn(**kwargs):
            return "success"
...
class TestSynthesizeWithRetry:
    ...
    async def test_success_on_first_attempt(self):
        ...
        with patch("bob.synth_retry.synthesize_with_retry.__wrapped__" if hasattr(synthesize_with_retry, "__wrapped__") else "bob.spec_synthesizer.synthesize_for_feature", new=mock_synthesize):
            pass

        # Use the actual function with a mock via the import path
        with patch("bob.synth_retry._synthesize_import", create=True):
            pass

        # Patch at the import level inside the function
        with patch("bob.spec_synthesizer.synthesize_for_feature", new=mock_synthesize):
...
class TestModuleImports:
    ...
    def test_module_imports_successfully(self):
        """bob.synth_retry imports without error."""
        import bob.synth_retry as m
