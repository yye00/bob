"""Tests for bob.extract_peas — verifies the public PEAS API module.

These tests confirm that:
- ``bob.extract_peas`` is importable.
...
import bob.extract_peas as _mod
from bob.extract_peas import parse_peas_markdown, synthesize_features
...
class TestSynthesizeFeatures:
    ...
    def test_non_tbd_criteria_left_unchanged(self):
        original_ac = ["File exists: src/bob/existing.py", "pytest: tests/test_existing.py"]
        stubs = [
            {
                "key": "F-R7-302",
                "title": "Existing",
...
class TestRoundTrip:
    def test_parse_then_synthesize_produces_valid_yaml(self):
        ...
        from bob.extract_peas import emit_stub_features
