...
Verifies handle_shell_script_ac from bob.patterns.pattern_9:
...
from bob.patterns.pattern_9 import handle_shell_script_ac
...
def test_returns_none_for_non_shell_integration_body(tmp_workspace: pathlib.Path) -> None:
    assert handle_shell_script_ac("integration: bob.verifier", tmp_workspace) is None
...
def test_warning_emitted_on_pass(tmp_workspace: pathlib.Path, caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.WARNING, logger="bob.patterns.pattern_9"):
...
def test_returns_none_for_python_module_path(tmp_workspace: pathlib.Path) -> None:
    assert handle_shell_script_ac("integration: bob.patterns.pattern_9", tmp_workspace) is None
