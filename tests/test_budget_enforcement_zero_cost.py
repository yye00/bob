...
- function is importable from bob3.orchestrator directly
"""
...
class TestEnforceBudgetWithCostTelemetryFallback:
    ...
    def test_env_threshold_override(self, monkeypatch):
        """BOB3_COST_TELEMETRY_LOST_WORK_THRESHOLD env var is respected."""
...
class TestBob3OrchestratorIntegration:
    """Verify enforce_budget_with_cost_telemetry_fallback is exported from bob3.orchestrator."""

    def test_function_importable_from_orchestrator_package(self):
        """enforce_budget_with_cost_telemetry_fallback accessible via bob3.orchestrator."""
        fn = getattr(bob3.orchestrator, "enforce_budget_with_cost_telemetry_fallback", None)
        assert fn is not None, (
            "enforce_budget_with_cost_telemetry_fallback not found in bob3.orchestrator"
        )
        assert callable(fn)

    def test_orchestrator_import_returns_same_function(self):
        """Function imported from bob3.orchestrator is identical to direct import."""
        from bob3.orchestrator import enforce_budget_with_cost_telemetry_fallback as via_pkg
        assert via_pkg is enforce_budget_with_cost_telemetry_fallback

    def test_orchestrator_function_works_end_to_end(self):
        """Function callable via bob3.orchestrator namespace produces correct result."""
        from bob3.orchestrator import enforce_budget_with_cost_telemetry_fallback as fn
        result = fn(
            reported_cost=0.0,
            work_events=176217,
