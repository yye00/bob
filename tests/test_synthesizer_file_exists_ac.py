...
(4) integration: bob3.synthesizer — the synthesizer module is importable and
...
class TestBob3SynthesizerIntegration:
    """AC: integration: bob3.synthesizer"""

    def test_synthesizer_importable(self):
        mod = importlib.import_module("bob3.synthesizer")
...
class TestEmitFileExistsAcs:
    ...
    def test_emits_file_exists_for_described_path(self):
        """Description naming src/bob3/brownfield/survey.py → File-exists AC emitted."""
        ...
        assert any("File exists: src/bob3/brownfield/survey.py" in ac for ac in result), (
            f"Expected 'File exists: src/bob3/brownfield/survey.py' in ACs; got {result}"
        )
    ...
    def test_does_not_duplicate_existing_file_exists_ac(self):
        ...
        acs_in = ["File exists: src/bob3/brownfield/survey.py"]
        result = self.fn(acs_in, description)
        matching = [ac for ac in result if "src/bob3/brownfield/survey.py" in ac]
    ...
    def test_emits_multiple_file_exists_for_multiple_paths(self):
        ...
        description = (
            "Implement src/bob3/brownfield/survey.py and tools/spec_quality_score.py."
        )
        acs_in: list[str] = []
        result = self.fn(acs_in, description)
    ...
    def test_original_acs_preserved(self):
        ...
        description = "Implement src/bob3/bar.py."
        acs_in = ["pytest: tests/test_bar.py", "Function defined: bob3.bar.run"]
        result = self.fn(acs_in, description)
        assert "pytest: tests/test_bar.py" in result
        assert "Function defined: bob3.bar.run" in result
    ...
    def test_empty_acs_and_description_with_path(self):
        ...
        description = "Implement src/bob3/alpha.py."
        result = self.fn([], description)
        assert any("src/bob3/alpha.py" in ac for ac in result)
