"""Tests for bob.periodic_resume_scanner.scan_and_promote_interrupted.
...
def test_module_importable():
    """bob.periodic_resume_scanner must be importable."""
    import bob.periodic_resume_scanner  # noqa: F401


def test_scan_and_promote_interrupted_is_callable():
    """scan_and_promote_interrupted must be a callable."""
    from bob.periodic_resume_scanner import scan_and_promote_interrupted
...
def test_no_interrupted_features_returns_empty_list():
    ...
    from bob.periodic_resume_scanner import scan_and_promote_interrupted
    with patch("bob.periodic_resume_scanner.db") as mock_db:
...
def test_orchestrator_exposes_resume_interrupted_work():
    """bob.orchestrator must expose resume_interrupted_work."""
    from bob.orchestrator import resume_interrupted_work
    assert callable(resume_interrupted_work)


def test_orchestrator_periodic_resume_scan_exposes_scan_and_promote():
    """bob.orchestrator.periodic_resume_scan must expose scan_and_promote_interrupted alias."""
    from bob.orchestrator import periodic_resume_scan
