...
class TestFinalReaperSweepIntegration(unittest.TestCase):
    ...
    def test_sweep_orphan_subagents_is_defined_in_reaper(self):
        """sweep_orphan_subagents must be defined in bob.orchestrator.subagent_reaper."""
    ...
    def test_sweep_orphan_subagents_importable_from_orchestrator(self):
        """sweep_orphan_subagents must be importable from bob.orchestrator package."""
        from bob.orchestrator import sweep_orphan_subagents
        assert callable(sweep_orphan_subagents)

    def test_final_exit_sweep_is_defined_in_run_loop(self):
        """_final_exit_sweep must be defined and callable in bob.orchestrator.run_loop."""
    ...
    def test_final_exit_sweep_called_on_all_blocked_path(self):
        ...
        from bob.orchestrator.run_loop import _final_exit_sweep, LoopTermination
    ...
    def test_sweep_orphan_subagents_invoked_before_row_updates(self):
        ...
        with patch("bob.orchestrator.run_loop.db") as mock_db, \
             patch("bob.orchestrator.run_loop.find_subagent_pid_for_feature") as mock_pids, \
             patch("bob.orchestrator.run_loop._check_executing_feature_acs") as mock_disk, \
             patch("bob.orchestrator.run_loop._sweep_orphan_subagents", side_effect=record_sweep):
