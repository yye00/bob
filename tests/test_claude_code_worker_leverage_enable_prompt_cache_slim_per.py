"""Tests for bob.claude_code_worker_leverage_enable_prompt_cache_slim_per.
...
ACs tested:
  ...
  - File exists: src/bob/claude_code_worker_leverage_enable_prompt_cache_slim_per.py
...
from bob.claude_code_worker_leverage_enable_prompt_cache_slim_per import (
...
class TestMainFunction:
    ...
    def test_settings_file_created(self, tmp_path):
        ...
        settings_path = tmp_path / ".bob" / "features" / "feat-main-04" / "settings.json"
...
class TestApplyWorkerLeverage:
    ...
    def test_extra_allow_merged_into_settings(self, tmp_path):
        ...
        settings_path = tmp_path / ".bob" / "features" / "feat-awl-04" / "settings.json"
...
class TestEmitPermPromptEvent:
    ...
    def test_logs_warning(self, caplog):
        import logging
        with caplog.at_level(logging.WARNING, logger="bob.claude_code_worker_leverage_enable_prompt_cache_slim_per"):
...
class TestRunLoopIntegration:
    ...
    def test_apply_worker_leverage_importable(self):
        from bob.claude_code_worker_leverage_enable_prompt_cache_slim_per import apply_worker_leverage
        assert callable(apply_worker_leverage)

    def test_build_worker_context_importable(self):
        from bob.claude_code_worker_leverage_enable_prompt_cache_slim_per import build_worker_context
        assert callable(build_worker_context)

    def test_emit_perm_prompt_event_importable(self):
        from bob.claude_code_worker_leverage_enable_prompt_cache_slim_per import emit_perm_prompt_event
