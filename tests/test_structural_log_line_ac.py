...
def _make_workspace(tmp_path: pathlib.Path, src_content: str, rel_path: str = "src/bob3/run_loop.py") -> pathlib.Path:
...
class TestStructuralLogLineAC:
    ...
    def test_pass_when_literal_present(self, tmp_path):
        ...
        result = _check(
            "structural: src/bob3/run_loop.py emits a 'Run finished: termination=%s' log line",
            workspace,
        )
