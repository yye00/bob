"""Tests for bob.ac_provenance — AC-to-source-intent provenance tracing.
...
- Integration with bob.spec_synthesizer (import-level wiring)
"""
...
from bob.ac_provenance import compute_coverage, trace_ac_to_spans
from bob.spec_quality.provenance import Span
...
def tmp_db(tmp_path):
    ...
    db_path = str(tmp_path / "test_bob.db")
...
def test_cli_spec_trace_prints_ac_and_spans(tmp_db, capsys):
    ...
    from bob.cli.spec_trace import spec_trace
...
# ---------------------------------------------------------------------------
# Integration: bob.spec_synthesizer imports ac_provenance
# ---------------------------------------------------------------------------


def test_ac_provenance_importable_from_bob():
    """bob.ac_provenance is importable and exposes the required public API."""
    import bob.ac_provenance as m
...
def test_spec_synthesizer_module_imports_successfully():
    """bob.spec_synthesizer can be imported without errors (integration wiring)."""
    import bob.spec_synthesizer  # noqa: F401 — import-level integration check


def test_spec_synthesizer_has_sanitize_spec_file():
    """bob.spec_synthesizer.sanitize_spec_file exists (a core synthesis entry point)."""
