"""Tests for bob3.property_based_ac_parser — seventh AC grammar parser.
...
- Integration with bob3.acceptance_criteria
"""
...
from bob3.property_based_ac_parser import (
    KeyExample,
    PropertyAC,
...
class TestParsePropertyCriterionHappyPath:
    ...
    def test_file_exists_ac_returns_none(self):
        result = parse_property_criterion("File exists: src/bob3/foo.py")
        assert result is None

    def test_function_defined_ac_returns_none(self):
        result = parse_property_criterion("Function defined: bob3.foo.bar")
        assert result is None
...
class TestAcceptanceCriteriaIntegration:
    ...
    def test_parse_key_example_variant_returns_correct_type(self):
        from bob3.property_based_ac_parser import parse_key_example_variant, KeyExample
        result = parse_key_example_variant({"given": "input", "then": "output"})
        assert isinstance(result, KeyExample)
