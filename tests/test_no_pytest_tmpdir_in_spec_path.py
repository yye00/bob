...
The bug: rsync copies the parent generation's bob3.db. If the parent ran
tests and a test had called ``bob3 init`` with a tempdir spec, the projects
...
3. A live DB (bob3.db in this workspace) does not have a tmpdir spec_path.
"""
...
def _make_db(tmp_path: Path) -> Path:
    """Create a minimal bob3.db in tmp_path with one project row."""
...
def test_init_upsert_overwrites_tmpdir_spec_path(tmp_path):
    ...
    from bob3.cli import main as bob3_main
    from click.testing import CliRunner

    runner = CliRunner()
    with patch.dict(os.environ, {"BOB3_DATABASE_PATH": str(db_path)}):
        # Patch MCP so init doesn't try to start the real MCP server
        with patch("bob3.cli.start_mcp_server"), \
             patch("bob3.cli.stop_mcp_server"), \
             patch("bob3.cli._check_runtime_dependencies"):
...
def test_run_loop_startup_check_fixes_stale_name(tmp_path, monkeypatch):
    ...
    from bob3.orchestrator.run_loop import _verify_project_name_matches_workspace
...
def test_run_loop_startup_check_noop_when_name_matches(tmp_path, monkeypatch):
    ...
    db_path = bob3_db.init_database(db_path=tmp_path / "bob3.db") or tmp_path / "bob3.db"
    db_path = tmp_path / "bob3.db"
    bob3_db.init_database(db_path=db_path)
...
def test_live_db_spec_path_not_in_tmpdir():
    """The bob3.db in this workspace must not have a /tmp spec_path."""
    workspace = Path(__file__).parents[1]
    db_path = workspace / "bob3.db"

    if not db_path.exists():
        pytest.skip("bob3.db not found in workspace; skipping live DB check")
    ...
    assert not tmpdir_rows, (
        f"Live bob3.db has project row(s) with tmpdir spec_path: {tmpdir_rows}"
    )
