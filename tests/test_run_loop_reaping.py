"""Tests for bob.run_loop subagent reaping on feature terminal-state transition.

Acceptance criteria (feature 4de21232-7ef4-46d8-98b2-1163050ec482):
  - Function defined: bob.run_loop.handle_terminal_transition
  - Function defined: bob.run_loop.reap_subagent
  - Function defined: bob.run_loop.sweep_orphan_subagents
  - pytest: tests/test_run_loop_reaping.py
  - integration: bob.run_loop

Verifies that all three functions are importable from bob.run_loop, that
...
class TestHandleTerminalTransitionImport(unittest.TestCase):
    """handle_terminal_transition is importable from bob.run_loop."""

    def test_function_importable_from_bob_run_loop(self):
        import bob.run_loop as rl
        self.assertTrue(callable(rl.handle_terminal_transition))

    def test_direct_import(self):
        from bob.run_loop import handle_terminal_transition
...
class TestReapSubagentImport(unittest.TestCase):
    """reap_subagent is importable from bob.run_loop."""
    ...
    def test_direct_import(self):
        from bob.run_loop import reap_subagent
...
class TestSweepOrphanSubagentsImport(unittest.TestCase):
    """sweep_orphan_subagents is importable from bob.run_loop."""
...
class TestIntegrationBobRunLoop(unittest.TestCase):
    """Integration tests ensuring bob.run_loop publishes the full API surface."""
    ...
    def test_module_importable_without_side_effects(self):
        """Importing bob.run_loop must not trigger DB connections or process scans."""
        import importlib
        # Re-importing the module must not raise
        import bob.run_loop
        importlib.reload(bob.run_loop)
