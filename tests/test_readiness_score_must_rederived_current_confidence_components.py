...
Acceptance criteria:
- File exists: src/bob3/readiness_score_must_rederived_current_confidence_components.py
- Function defined: bob3.readiness_score_must_rederived_current_confidence_components
...
import bob3.readiness_score_must_rederived_current_confidence_components as mod
from bob3.readiness_score_must_rederived_current_confidence_components import (
    readiness_score_must_rederived_current_confidence_components,
)
...
def test_readiness_score_must_rederived_current_confidence_components():
    ...
    src = Path(__file__).parents[1] / "src" / "bob3" / \
...
class TestDelegation:
    """The facade delegates to bob3.readiness.derive_readiness_score."""

    def test_delegates_to_derive_readiness_score(self):
        with patch(
            "bob3.readiness_score_must_rederived_current_confidence_components.derive_readiness_score"
        ) as mock_derive:
