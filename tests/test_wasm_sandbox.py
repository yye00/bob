...
from bob.container_runner import (
    ContainerResult,
    WasmSandboxConfig,
    run_wasm_sandbox,
)
...
class TestWatToWasmCompilation:
    ...
    def test_docker_command_includes_wat2wasm(self, tmp_path):
        ...
        with patch("bob.container_runner._docker_available", return_value=True):
            with patch("bob.container_runner.subprocess.run", return_value=mock_result) as mock_run:
...
class TestWasmOutputCapture:
    ...
    def test_returns_wasmtime_stdout(self, tmp_path):
        ...
        with patch("bob.container_runner._docker_available", return_value=True):
            with patch("bob.container_runner.subprocess.run", side_effect=[mock_compile, mock_run_wasm]):
                result = run_wasm_sandbox(wat_source="(module)", workspace=tmp_path)
    ...
    def test_compilation_failure_propagated(self, tmp_path):
        ...
        with patch("bob.container_runner._docker_available", return_value=True):
            with patch("bob.container_runner.subprocess.run", return_value=mock_compile_fail):
