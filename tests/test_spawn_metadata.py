"""Tests for feature 1809afa5: bob init re-run after spawn fixes stale metadata.

Verifies:
  - bob.spawn.verify_project_metadata exists and is callable
...
# ---------------------------------------------------------------------------
# AC: Function defined: bob.spawn.verify_project_metadata
# ---------------------------------------------------------------------------

def test_verify_project_metadata_importable_from_spawn():
    """bob.spawn.verify_project_metadata must be importable."""
    import bob.spawn as spawn_mod
    assert hasattr(spawn_mod, "verify_project_metadata"), (
        "bob.spawn does not export verify_project_metadata"
    )
    assert callable(spawn_mod.verify_project_metadata)


def test_projectmetadatacheckresult_importable_from_spawn():
    """bob.spawn.ProjectMetadataCheckResult must be importable."""
    import bob.spawn as spawn_mod
    assert hasattr(spawn_mod, "ProjectMetadataCheckResult"), (
        "bob.spawn does not export ProjectMetadataCheckResult"
    )


def test_verify_project_metadata_importable_from_run_loop():
    """bob.run_loop.verify_project_metadata must be importable."""
    import bob.run_loop as rl
    assert hasattr(rl, "verify_project_metadata"), (
        "bob.run_loop does not export verify_project_metadata"
    )
...
def test_result_is_named_tuple():
    ...
    from bob.spawn import verify_project_metadata, ProjectMetadataCheckResult
    import inspect
...
def test_logs_correction_on_stale_name(tmp_path, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="bob"):
...
# ---------------------------------------------------------------------------
# AC: integration: bob.run_loop — _run_locked calls verify_project_metadata
# ---------------------------------------------------------------------------
...
def test_run_locked_calls_verify_project_metadata():
    ...
    import ast
    import inspect
    from bob.orchestrator import run_loop as orl
...
def test_run_locked_integration_imports_from_bob_run_loop():
    """The _run_locked integration must import from bob.run_loop (not orchestrator)."""
    import inspect
    from bob.orchestrator import run_loop as orl

    source = inspect.getsource(orl.OrchestrationLoop._run_locked)
    assert "from bob.run_loop import verify_project_metadata" in source, (
        "_run_locked does not import verify_project_metadata from bob.run_loop"
    )
...
def test_spawn_verify_is_same_fn_as_run_loop_verify():
    """bob.spawn.verify_project_metadata must be the same callable as bob.run_loop's."""
    from bob import spawn as spawn_mod
    from bob import run_loop as rl_mod

    assert spawn_mod.verify_project_metadata is rl_mod.verify_project_metadata, (
        "bob.spawn.verify_project_metadata is not the same object as "
        "bob.run_loop.verify_project_metadata — spawn.py must re-export from run_loop"
    )
