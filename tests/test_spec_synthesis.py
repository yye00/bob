...
class TestExitSpecNeedsHuman:
    ...
    def test_reads_bob_ci_mode_env_var(self, monkeypatch):
        slots = self._make_slots()
        monkeypatch.setenv("BOB_CI_MODE", "1")
        with pytest.raises(SpecNeedsHumanError):
            exit_spec_needs_human(slots)

    def test_bob_ci_mode_false_string_does_not_raise(self, monkeypatch):
        slots = self._make_slots()
        monkeypatch.setenv("BOB_CI_MODE", "false")
