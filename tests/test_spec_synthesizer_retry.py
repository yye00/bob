"""Tests for bob.spec_synthesizer.synthesize_with_retry.
...
Verifies:
- synthesize_with_retry is importable from bob.spec_synthesizer
...
- BOB_SYNTH_MAX_ATTEMPTS env var is honored
...
from bob.spec_synthesizer import synthesize_with_retry
from bob.synthesizer import score_gate_loop, ScoreGateReport


class TestSynthesizeWithRetryImport:
    """AC: Function defined: bob.spec_synthesizer.synthesize_with_retry"""

    def test_synthesize_with_retry_is_callable(self):
        """synthesize_with_retry must be a callable defined in bob.spec_synthesizer."""
        assert callable(synthesize_with_retry)

    def test_synthesize_with_retry_importable_from_synthesizer(self):
        """synthesize_with_retry must be re-exported from bob.synthesizer."""
        from bob.synthesizer import synthesize_with_retry as swr
        assert callable(swr)
...
class TestSynthesizeWithRetryOnTransientEmpty:
    ...
    async def test_retries_on_exception_then_succeeds(self):
        ...
        with patch("bob.spec_synthesizer._llm_spawn_synthesizer", side_effect=exception_then_success):
            with patch("asyncio.sleep", new_callable=AsyncMock):
                result = await synthesize_with_retry(
                    project_id="proj-1",
    ...
    async def test_exhausts_all_attempts_before_returning_none(self):
        """When all attempts return empty, must exhaust BOB_SYNTH_MAX_ATTEMPTS before returning None."""
        call_count = 0
...
class TestEnvTuning:
    """BOB_SYNTH_MAX_ATTEMPTS env var must control retry count."""

    @pytest.mark.asyncio
    async def test_max_attempts_env_var_honored(self):
        """BOB_SYNTH_MAX_ATTEMPTS=5 must result in exactly 5 attempts."""
        call_count = 0
    ...
    async def test_invalid_max_attempts_falls_back_to_default(self):
        """Invalid BOB_SYNTH_MAX_ATTEMPTS (non-integer) must fall back gracefully."""
        ...
        with patch("bob.spec_synthesizer._llm_spawn_synthesizer", side_effect=immediate_success):
            with patch.dict(os.environ, {"BOB_SYNTH_MAX_ATTEMPTS": "INVALID"}):
                with patch("asyncio.sleep", new_callable=AsyncMock):
                    result = await synthesize_with_retry(
                        project_id="proj-1",
