"""Tests for bob.weekend_watchdog.check_convergence.

Verifies that check_convergence compares feature sets by spec_slot
(stable YAML key) rather than by UUID (minted fresh on every bob init).
"""
...
def test_check_convergence_is_importable():
    """check_convergence must be importable from bob.weekend_watchdog."""
    from bob.weekend_watchdog import check_convergence  # noqa: F401
...
def test_check_convergence_converged_by_spec_slot(tmp_path):
    ...
    from bob.weekend_watchdog import check_convergence
...
def test_check_convergence_by_spec_slot(tmp_path):
    ...
    UUID churn across bob init calls must not prevent convergence detection.
    """
    from bob.weekend_watchdog import check_convergence
