"""Tests for src/bob3/learnings.py - Per-skill learning ledger."""
import os
import re
import tempfile
...
from bob3.learnings import append_learning, read_learnings
...
class TestAppendLearning:
    ...
    def test_creates_learnings_file(self, tmp_path):
        ...
        with patch("bob3.learnings.SKILLS_DIR", skills_dir):
            append_learning(
                skill="test-skill",
                lesson="Something important was learned",
                evidence={"file": "src/bob3/foo.py", "line": 42},
    ...
    def test_entry_contains_evidence_pointer(self, tmp_path):
        ...
        with patch("bob3.learnings.SKILLS_DIR", skills_dir):
            append_learning(
                ...
                lesson="Lesson",
                evidence={"file": "src/bob3/db.py", "line": 99},
                source_feature_id=None,
            )
        content = (skills_dir / "evidence-skill" / "LEARNINGS.md").read_text()
        assert "src/bob3/db.py" in content
...
class TestReadLearnings:
    ...
    def test_each_entry_has_required_fields(self, tmp_path):
        ...
        with patch("bob3.learnings.SKILLS_DIR", skills_dir):
            append_learning(
                ...
                evidence={"file": "src/bob3/foo.py", "line": 10},
    ...
    def test_evidence_preserved(self, tmp_path):
        skills_dir = tmp_path / "skills"
        evidence = {"file": "src/bob3/db.py", "line": 55}
        with patch("bob3.learnings.SKILLS_DIR", skills_dir):
