"""Tests for handle_structural_log_line in bob.enhanced_verification.
...
from bob.enhanced_verification import (
    ...
    check_integration_wired_with_fallback,
)
from bob.verification.prose_ac_demotion import is_executable_or_structural_criterion
...
class TestNonMatchingCriteria:
    ...
    def test_defines_function_returns_none(self, tmp_path):
        result = _call("src/bob/foo.py defines function bar", tmp_path)
    ...
    def test_no_quoted_string_returns_none(self, tmp_path):
        result = _call("src/bob/foo.py emits a log line", tmp_path)
        assert result is None
...
class TestExactMatch:
    ...
    def test_exact_string_double_quotes_in_source(self, tmp_path):
        ...
        result = _call(
            'src/bob/run_loop.py emits a "Queue drained: all features blocked" log line',
            workspace,
        )
        assert result is True
    ...
    def test_exact_string_single_quotes_in_criterion(self, tmp_path):
        ...
        result = _call(
            "src/bob/orchestrator/run_loop.py emits a 'heartbeat tick' log line",
            workspace,
        )
        assert result is True
    ...
    def test_exact_match_ignores_surrounding_code(self, tmp_path):
        ...
        result = _call(
            "src/bob/foo.py emits a 'dispatch started' log line",
            workspace,
        )
        assert result is True
...
class TestAdjacentLiteralConcat:
    ...
    def test_split_across_newline(self, tmp_path):
        ...
        workspace = _write_py(tmp_path, "src/bob/run_loop.py", src)
        result = _call(
            "src/bob/run_loop.py emits a 'Run finished: termination=%s features_completed=%d features_failed=%d total=%d' log line",
            workspace,
        )
        assert result is True
    ...
    def test_split_across_newline_partial_prefix(self, tmp_path):
        ...
        workspace = _write_py(tmp_path, "src/bob/run_loop.py", src)
        result = _call(
            "src/bob/run_loop.py emits a 'Run finished: termination=%s features_completed=%d' log line",
            workspace,
        )
        assert result is True
    ...
    def test_split_with_indentation(self, tmp_path):
        ...
        workspace = _write_py(tmp_path, "src/bob/run_loop.py", src)
        result = _call(
            "src/bob/run_loop.py emits a 'Queue drained: all_blocked termination=%s' log line",
            workspace,
        )
        assert result is True
    ...
    def test_three_part_concat(self, tmp_path):
        ...
        workspace = _write_py(tmp_path, "src/bob/foo.py", src)
        result = _call(
            "src/bob/foo.py emits a 'Part one part two part three' log line",
            workspace,
        )
        assert result is True
...
class TestTokenOrderFallback:
    ...
    def test_token_order_fallback_emits_warning(self, tmp_path, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="bob.enhanced_verification"):
            result = _call(
                "src/bob/foo.py emits a 'feature=%s blocked' log line",
                workspace,
            )
        assert result is True
        assert any("token-order fallback" in r.message for r in caplog.records)
...
class TestStringNotFound:
    ...
    def test_partial_token_match_not_sufficient(self, tmp_path):
        ...
        result = _call(
            "src/bob/foo.py emits a 'Run finished termination=%s' log line",
            workspace,
        )
...
class TestIntegrationWithCheckCriterion:
    ...
    def test_structural_criterion_passes_exact(self, tmp_path):
        ...
        result = _check_criterion(
            criterion="structural: src/bob/run_loop.py emits a 'Run finished: termination=%s' log line",
            workspace=workspace,
    ...
    def test_structural_criterion_passes_adjacent_concat(self, tmp_path):
        ...
        workspace = _write_py(tmp_path, "src/bob/run_loop.py", src)
        result = _check_criterion(
            criterion="structural: src/bob/run_loop.py emits a 'Run finished: termination=%s features_completed=%d features_failed=%d' log line",
            workspace=workspace,
            is_python_project=True,
            is_cmake_project=False,
            is_opm_project=False,
...
class TestEdgeCases:
    ...
    def test_criterion_body_with_no_log_line_suffix(self, tmp_path):
        ...
        result = _call(
            "src/bob/foo.py emits a 'hello world'",
            workspace,
        )
        assert result is True
    ...
    def test_emits_without_article_a(self, tmp_path):
        ...
        result = _call(
            "src/bob/foo.py emits 'startup complete' log line",
            workspace,
        )
        assert result is True
    ...
    def test_nested_directory_path(self, tmp_path):
        ...
        result = _call(
            "src/bob/orchestrator/run_loop.py emits a 'tick: features=%d' log line",
            workspace,
        )
        assert result is True
    ...
    def test_case_insensitive_emits_keyword(self, tmp_path):
        ...
        result = _call(
            "src/bob/foo.py EMITS a 'done' log line",
            workspace,
        )
        assert result is True
...
class TestExtractQuotedLiterals:
    ...
    def setup_method(self):
        from bob.enhanced_verification import extract_quoted_literals
...
class TestVerifySubstringPresence:
    ...
    def setup_method(self):
        from bob.enhanced_verification import verify_substring_presence
    ...
    def test_integration_with_check_criterion_behavior_ac(self, tmp_path):
        ...
        from bob.enhanced_verification import _check_criterion
        workspace = self._write_src(tmp_path, "bob/cli/__init__.py", '"Queue drained"\n')
        result = _check_criterion(
    ...
    def test_integration_check_criterion_fails_when_mention_absent(self, tmp_path):
        from bob.enhanced_verification import _check_criterion
        workspace = self._write_src(tmp_path, "bob/cli/__init__.py", '"Something else"\n')
        result = _check_criterion(
...
class TestVerifyBehaviorAcWithSubstringGrep:
    ...
    def setup_method(self):
        from bob.enhanced_verification import verify_behavior_ac_with_substring_grep
    ...
    def test_integration_verifier_import(self):
        """verify_behavior_ac_with_substring_grep is importable from bob.verifier."""
        from bob.verifier import verify_behavior_ac_with_substring_grep  # noqa: F401
...
class TestVerifyQuotedSubstringAc:
    ...
    def test_integration_verifier_import(self):
        """verify_quoted_substring_ac is importable from bob.verifier."""
        from bob.verifier import verify_quoted_substring_ac  # noqa: F401
...
def test_class_defined_ac_verification(tmp_path):
    ...
    """
    from bob.enhanced_verification import verify_class_defined, _check_criterion

    # Workspace with a plain class
    src_dir = tmp_path / "src" / "bob" / "verification"
    ...
    result_missing = verify_class_defined(
        "Class defined: bob.verification.mutation_gate.NoSuchClass",
        tmp_path,
    )
    ...
    result_via_check_missing = _check_criterion(
        criterion="Class defined: bob.verification.mutation_gate.NoSuchClass",
        workspace=tmp_path,
        is_python_project=True,
        is_cmake_project=False,
        is_opm_project=False,
...
def test_class_defined_ac(tmp_path):
    """check_class_defined is importable from bob.enhanced_verification and works.
    ...
    """
    from bob.enhanced_verification import check_class_defined
...
class TestDemoteCrossFeatureCriterion:
    ...
    def test_returns_none_for_non_feature_token(self):
        result = demote_cross_feature_criterion("File exists: src/bob/foo.py")
        assert result is None
...
class TestStructuralLogLineHandlerAlias:
    ...
    def test_alias_accepts_same_kwargs(self, tmp_path):
        ...
        py_path = tmp_path / "src" / "bob" / "foo.py"
        py_path.parent.mkdir(parents=True, exist_ok=True)
        py_path.write_text('logger.info("hello world")\n')
        result = structural_log_line_handler(
            criterion_body="src/bob/foo.py emits a 'hello world' log line",
            workspace=tmp_path,
        )
        assert result is True
    ...
    def test_alias_returns_none_for_non_matching(self, tmp_path):
        result = structural_log_line_handler(
            criterion_body="src/bob/foo.py defines function do_something",
            workspace=tmp_path,
        )
        assert result is None

    def test_alias_tolerates_adjacent_literal_concat(self, tmp_path):
        py_path = tmp_path / "src" / "bob" / "run_loop.py"
        py_path.parent.mkdir(parents=True, exist_ok=True)
        py_path.write_text(
            'logger.info(\n'
            '    "Run finished: termination=%s features_completed=%d "\n'
        ...
        result = structural_log_line_handler(
            criterion_body="src/bob/run_loop.py emits a 'Run finished: termination=%s features_completed=%d features_failed=%d total=%d' log line",
            workspace=tmp_path,
        )
        assert result is True
...
class TestPattern8IntegrationWired:
    ...
    def test_dotted_module_wired_passes(self, tmp_path):
        ...
        _write_src(tmp_path, "src/bob/enhanced_verification.py", "def foo(): pass\n")
        importer = tmp_path / "src" / "bob" / "orchestrator.py"
        importer.parent.mkdir(parents=True, exist_ok=True)
        importer.write_text("from bob.enhanced_verification import foo\n")
    ...
    def test_no_matching_identifier_returns_false(self, tmp_path):
        ...
        _write_src(tmp_path, "src/bob/unrelated.py", "def unrelated_thing(): pass\n")
...
def test_pattern_8_fallback_to_function_existence(tmp_path):
    ...
    src_file = tmp_path / "src" / "bob" / "reaper.py"
...
def test_pattern_8_fallback_returns_false_when_no_match(tmp_path):
    ...
    (tmp_path / "src" / "bob").mkdir(parents=True, exist_ok=True)
    (tmp_path / "src" / "bob" / "unrelated.py").write_text("def unrelated(): pass\n")
...
class TestFallbackToFunctionExistence:
    ...
    def test_function_absent_returns_false(self, tmp_path):
        ...
        _write_src(tmp_path, "src/bob/foo.py", "def something_else(): pass\n")
    ...
    def test_no_snake_case_in_criterion_returns_false(self, tmp_path):
        ...
        _write_src(tmp_path, "src/bob/foo.py", "def bar(): pass\n")
...
def test_prose_ac_demoted_to_warning(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("BOB_STRICT_VERIFICATION", "0")
    prose_criterion = (
...
class TestDemoteCrossFeaturePolicyAC:
    ...
    def test_no_cross_feature_ref_returns_none(self):
        ...
        result = demote_cross_feature_policy_ac(
            "integration: bob.ac_handler must be importable"
        )
        assert result is None
    ...
    def test_plain_function_criterion_returns_none(self):
        ...
        result = demote_cross_feature_policy_ac(
            "function defined: bob.enhanced_verification.demote_cross_feature_policy_ac"
        )
        assert result is None
...
def test_structural_ac_fuzzy_lookup_fallback(tmp_path: pathlib.Path) -> None:
    ...
    """
    from bob.enhanced_verification import structural_ac_handler_with_fallback
    ...
    criterion = "structural: src/bob/X.py defines function target_fn"
...
# --- bob v.72 force-drain: feature 0b0e084a + 4aafbb8f ---
...
def test_pattern8_prose_integration_fallback(tmp_path):
    ...
    import pathlib
    from bob.enhanced_verification import pattern_8_integration_wired
    # Build a tiny workspace with a first-party module.
    (tmp_path / "src" / "bob").mkdir(parents=True)
    (tmp_path / "src" / "bob" / "spec_linter.py").write_text("def lint():\n    return True\n")
...
def test_bespoke_ac_demote_on_failure(tmp_path):
    ...
    import logging
    import pathlib
    from bob.enhanced_verification import bespoke_ac_handler

    mod = tmp_path / "src" / "bob" / "mymod.py"
...
def test_cross_feature_reference_demotion():
    ...
    result_none = demote_cross_feature_reference_ac(
        "function defined: bob.enhanced_verification.some_function"
    )
...
def test_class_defined_criterion(tmp_path):
    ...
    result_plain = criterion_checker(
        "Class defined: bob.verification.plain_class.PlainResult",
        tmp_path,
    )
...
def test_pattern8_integration_ac_fallback(tmp_path: pathlib.Path) -> None:
    ...
    """
    from bob.enhanced_verification import fallback_function_existence_check
...
def test_bespoke_ac_handler_demotes_on_failure(tmp_path):
    ...
    """
    import logging
    from bob.enhanced_verification import bespoke_handler_with_fallback

    mod = tmp_path / "src" / "bob" / "target_module.py"
    mod.parent.mkdir(parents=True, exist_ok=True)
...
def test_structural_ac_fuzzy_fallback_exact_match_found(tmp_path: pathlib.Path) -> None:
    ...
    """
    from bob.enhanced_verification import structural_ac_fuzzy_fallback
...
def test_handle_bespoke_ac_with_demotion_probe_passes(tmp_path: pathlib.Path) -> None:
    ...
    from bob.enhanced_verification import handle_bespoke_ac_with_demotion
...
def test_bespoke_handler_demotes_on_failure(tmp_path: pathlib.Path) -> None:
    ...
    """
    from bob.enhanced_verification import demote_on_bespoke_failure
...
def test_bespoke_handler_logs_f_r7_584_warning(
    ...
    """
    import logging
    from bob.enhanced_verification import should_demote_bespoke_on_failure
...
def test_pattern8_fallback_to_function_existence(
    ...
    non_integration = "File exists: src/bob/orphan_reaper.py"
    ...
    dotted_criterion = "integration: bob.enhanced_verification"
...
def test_pattern_8_function_fallback(tmp_path: pathlib.Path) -> None:
    ...
    """
    from bob.enhanced_verification import integration_wired_with_function_fallback
...
def test_prose_integration_ac_bare_name(tmp_path: pathlib.Path) -> None:
    ...
    as a module path.
    """
    from bob.enhanced_verification import _integration_wired_with_fallback
...
def test_structural_log_line_handler(tmp_path: pathlib.Path) -> None:
    ...
    py_file = tmp_path / "src" / "bob" / "run_loop.py"
    ...
    result = handle_structural_log_line(
        criterion_body="src/bob/run_loop.py emits a 'Run finished: termination=%s' log line",
        workspace=tmp_path,
    )
    assert result is True, (
    ...
    result_miss = handle_structural_log_line(
        criterion_body="src/bob/run_loop.py emits a 'No such string here XYZ' log line",
        workspace=tmp_path,
    )
    ...
    result_no_match = handle_structural_log_line(
        criterion_body="src/bob/run_loop.py defines function run_loop",
        workspace=tmp_path,
    )
    ...
    with pytest.raises(ValueError, match="workspace"):
        handle_structural_log_line(
            criterion_body="src/bob/run_loop.py emits a 'x' log line",
...
def test_adjacent_string_literal_concat(tmp_path: pathlib.Path) -> None:
    ...
    py_file = tmp_path / "src" / "bob" / "orchestrator" / "run_loop.py"
    py_file.parent.mkdir(parents=True, exist_ok=True)
...
def test_structural_ac_fuzzy_fallback_function_found(tmp_path: pathlib.Path) -> None:
    ...
    """
    from bob.enhanced_verification import handle_structural_ac_with_fuzzy_fallback
    ...
    criterion = "structural: src/bob/X.py defines function fuzzy_target_func"
...
def test_structural_ac_fuzzy_fallback_function_not_found(tmp_path: pathlib.Path) -> None:
    ...
    criterion = "structural: src/bob/X.py defines function completely_absent_func"
    passed, reason = handle_structural_ac_with_fuzzy_fallback(
        criterion=criterion,
        workspace=tmp_path,
        findings_path=findings_path,
...
def test_cross_feature_policy_ac_demotion_no_token_returns_none():
    ...
    result = handle_cross_feature_policy_ac("integration: bob.enhanced_verification importable")
    assert result is None
...
def test_deterministic_fallback_includes_boundary_ac():
    ...
    """
    from bob.enhanced_verification import ensure_boundary_and_error_coverage
    import re

    structural_only = [
        "File exists: src/bob/my_module.py",
        "pytest: tests/test_my_module.py",
        "Function defined: bob.my_module.do_thing",
    ]
...
def test_behavior_ac_demotion_fallback(tmp_path: pathlib.Path) -> None:
    ...
    hard-failing.
    """
    from bob.enhanced_verification import handle_behavior_ac_fallback
...
def test_class_defined_handler(tmp_path: pathlib.Path) -> None:
    ...
    src = tmp_path / "src" / "bob" / "verification"
    src.mkdir(parents=True)
    ...
    result_present = criterion_checker(
        "Class defined: bob.verification.mutation_gate.MutationReport", tmp_path
    )
    assert result_present is True, (
    ...
    result_absent = criterion_checker(
        "Class defined: bob.verification.mutation_gate.NonExistentClass", tmp_path
    )
    assert result_absent is False, (
...
def test_cross_feature_policy_demotion():
    ...
    result_no_token = demote_cross_feature_policy_ac("function defined: bob.some_module.fn")
...
def test_bespoke_handler_demote_on_failure(tmp_path: pathlib.Path) -> None:
    ...
    module_file = tmp_path / "src" / "bob" / "behavior_ac_parser.py"
    ...
    absent_module = tmp_path / "src" / "bob" / "nonexistent_module.py"
...
def test_bespoke_probe_soft_fail(tmp_path: pathlib.Path) -> None:
    ...
    from bob.enhanced_verification import handle_bespoke_probe_with_demotion
...
def test_cross_feature_ac_demotion():
    ...
    plain = "function defined: bob.enhanced_verification.verify_ac"
...
def test_structural_ac_fuzzy_function_lookup_fallback(tmp_path: pathlib.Path) -> None:
    ...
    - AC says "module src/bob/X.py defines function function_in_z"
    - Implementation landed function_in_z in src/bob/Z.py
...
def test_pattern8_integration_fallback_to_function_existence(
    ...
    When the first token after 'integration:' is a bare snake_case function name
    (not a dotted module path like bob.foo), _integration_wired returns False
...
def test_bespoke_handler_demotes_to_warning_when_module_exists(
    ...
    """
    from bob.enhanced_verification import demote_on_failure, demote_bespoke_on_module_exists

    # --- Setup: create a fake module file ---
    mod_path = tmp_path / "src" / "bob" / "behavior_ac_parser.py"
    ...
    absent_path = tmp_path / "src" / "bob" / "nonexistent_module.py"
...
# ---------------------------------------------------------------------------
# Structural-AC fuzzy function-lookup fallback (73b9b311)
# Required ACs: test_structural_ac_fuzzy_fallback_warns_on_module_mismatch
#               test_structural_ac_fuzzy_fallback_hard_fails_when_symbol_not_found
# ---------------------------------------------------------------------------

def test_structural_ac_fuzzy_fallback_warns_on_module_mismatch(tmp_path: pathlib.Path) -> None:
    """fallback_structural_ac_lookup returns True AND emits a WARNING when symbol is in the
    wrong module (fuzzy hit: found in workspace but not in expected module).

    AC: pytest: tests/test_enhanced_verification.py::test_structural_ac_fuzzy_fallback_warns_on_module_mismatch
    """
    from bob.enhanced_verification import fallback_structural_ac_lookup

    # Set up workspace: X.py does NOT define the target; Z.py does.
    src_dir = tmp_path / "src" / "bob"
    src_dir.mkdir(parents=True)
    (src_dir / "__init__.py").write_text("")
    (src_dir / "X.py").write_text("# placeholder — no target function here\n")
    (src_dir / "Z.py").write_text("def misplaced_fn(arg):\n    return arg\n")
    ...
    result = fallback_structural_ac_lookup(
        workspace=tmp_path,
        symbol_name="misplaced_fn",
        expected_module_path="src/bob/X.py",
        is_class=False,
    ...
    assert result is True, (
        "fallback_structural_ac_lookup must return True (PASS) when symbol is found "
        "in the workspace but NOT in the expected module (fuzzy hit)"
    )

    content = findings_path.read_text(encoding="utf-8")
    assert "misplaced_fn" in content, (
        "WARNING finding must mention the symbol name"
    )
    assert "warning" in content.lower(), (
        "WARNING finding must have severity=warning"
    )


def test_structural_ac_fuzzy_fallback_hard_fails_when_symbol_not_found(tmp_path: pathlib.Path) -> None:
    """fallback_structural_ac_lookup returns False when symbol is absent from entire workspace.

    AC: pytest: tests/test_enhanced_verification.py::test_structural_ac_fuzzy_fallback_hard_fails_when_symbol_not_found
    """
    from bob.enhanced_verification import fallback_structural_ac_lookup

    # Workspace has Python files but NONE define the target symbol.
    src_dir = tmp_path / "src" / "bob"
    src_dir.mkdir(parents=True)
    (src_dir / "__init__.py").write_text("")
    (src_dir / "X.py").write_text("def some_other_fn():\n    pass\n")
    (src_dir / "Z.py").write_text("def yet_another_fn():\n    pass\n")
    ...
    result = fallback_structural_ac_lookup(
        workspace=tmp_path,
        symbol_name="completely_absent_fn",
        expected_module_path="src/bob/X.py",
        is_class=False,
        findings_path=findings_path,
    )
    ...
    assert result is False, (
        "fallback_structural_ac_lookup must return False (hard-fail) when the symbol "
        "is not found anywhere in the workspace"
    )

    # No warning finding should be emitted when nothing is found.
    content = findings_path.read_text(encoding="utf-8")
    assert "completely_absent_fn" not in content, (
        "No WARNING finding should be emitted when symbol is absent from entire workspace"
    )


# ---------------------------------------------------------------------------
# test_pattern_8_integration_function_fallback
# AC: pytest: tests/test_enhanced_verification.py::test_pattern_8_integration_function_fallback
# ---------------------------------------------------------------------------

def test_pattern_8_integration_function_fallback(tmp_path: pathlib.Path) -> None:
    """check_integration_wired_with_fallback resolves prose-policy integration ACs.

    When the first token after 'integration:' is a bare snake_case function name
    (not a dotted module path), _integration_wired returns False because no module
    file by that name exists. check_integration_wired_with_fallback must fall back
    to function-existence scanning in the workspace src tree and return True when
    the named function is defined there.
    """
    # Write a Python file containing the bare function referenced in the AC.
    src_dir = tmp_path / "src" / "mypackage"
    src_dir.mkdir(parents=True)
    (src_dir / "reaper.py").write_text(
        "def sweep_orphan_subagents(db):\n    pass\n",
        encoding="utf-8",
    )

    # Prose-policy AC: first token is a bare function name, not a dotted path.
    criterion = (
        "integration: sweep_orphan_subagents runs at the same cadence as the "
        "existing stuck_executing reaper (watchdog tick); both reapers are "
        "idempotent and safe to run concurrently"
    ...
    result = check_integration_wired_with_fallback(criterion, workspace=tmp_path)
    assert result is True, (
        "check_integration_wired_with_fallback must return True when the bare "
        "function name in the prose-integration AC exists in the workspace src tree"
    )


def test_pattern_8_integration_function_fallback_absent(tmp_path: pathlib.Path) -> None:
    """check_integration_wired_with_fallback returns False when function is absent."""
    # Empty workspace — no Python files define the referenced function.
    criterion = (
        "integration: nonexistent_function_xyz does something important"
    )
    result = check_integration_wired_with_fallback(criterion, workspace=tmp_path)
    assert result is False, (
        "check_integration_wired_with_fallback must return False when no function "
        "matching the prose AC identifier exists in the workspace"
    )


def test_pattern_8_integration_function_fallback_non_integration(tmp_path: pathlib.Path) -> None:
    """check_integration_wired_with_fallback returns False for non-integration ACs."""
    criterion = "File exists: src/bob/enhanced_verification.py"
    result = check_integration_wired_with_fallback(criterion, workspace=tmp_path)
    assert result is False, (
        "check_integration_wired_with_fallback must return False for non-integration ACs"
    )


def test_pattern_8_integration_function_fallback_invalid_input() -> None:
    """check_integration_wired_with_fallback raises ValueError on non-string input."""
    with pytest.raises(ValueError):
        check_integration_wired_with_fallback(None)  # type: ignore[arg-type]
