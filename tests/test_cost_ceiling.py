"""Tests for bob.orchestrator.cost_ceiling.compute_per_feature_ceiling.
...
from bob.orchestrator.cost_ceiling import compute_per_feature_ceiling
...
class TestComputePerFeatureCeiling:
    ...
    def test_env_zero_falls_back_to_default(self, monkeypatch):
        monkeypatch.setenv("BOB_PER_FEATURE_COST_CEILING", "0")
        assert compute_per_feature_ceiling() == pytest.approx(20.0)
    ...
    def test_env_empty_string_falls_back_to_default(self, monkeypatch):
        monkeypatch.setenv("BOB_PER_FEATURE_COST_CEILING", "")
        assert compute_per_feature_ceiling() == pytest.approx(20.0)
