"""Tests for per-worker settings redeclaration in bob.dispatch.
...
class TestWriteFeatureSettingsRedeclaration:
    def test_settings_file_created_under_bob_features(self, tmp_path):
    ...
    def test_extra_allow_patterns_merged_with_defaults(self, tmp_path):
        ...
        path = write_feature_settings(
            feature, bob_dir=tmp_path, extra_allow=["Read(tests/*)", "Bash(pytest*)"]
        )
        data = json.loads(path.read_text())
        allow = data["permissions"]["allow"]
...
class TestSpawnWorkerWithCacheSettingsPath:
    def test_settings_flag_passed_to_subprocess(self, tmp_path):
        ...
        with mock.patch("bob.dispatch.subprocess.run", side_effect=fake_run):
            spawn_worker_with_cache(
                feature, "run", tmp_path, bob_dir=tmp_path / ".bob"
            )
    ...
    def test_independent_settings_per_worker_invocation(self, tmp_path):
        ...
        with mock.patch("bob.dispatch.subprocess.run", side_effect=fake_run):
            spawn_worker_with_cache(feat_a, "run", tmp_path, bob_dir=tmp_path / ".bob")
            spawn_worker_with_cache(feat_b, "run", tmp_path, bob_dir=tmp_path / ".bob")
