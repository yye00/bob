"""Tests for bob3.research_strategies canonical AC generator and spec_quality gate validator."""
...
class TestValidateAgainstSpecQualityGate:
    ...
    def test_integration_prefix_is_canonical(self):
        result = validate_against_spec_quality_gate(["integration: bob3.orchestrator"])
        assert result["passed"] is True
    ...
    def test_class_defined_prefix_is_canonical(self):
        result = validate_against_spec_quality_gate(["Class defined: bob3.foo.MyClass"])
        assert result["passed"] is True


class TestValidateAcAgainstSpecQualityGate:
    def test_canonical_ac_passes(self):
        result = validate_ac_against_spec_quality_gate("Function defined: bob3.foo.bar")
        assert result["passed"] is True
        assert result["non_canonical"] == []
...
def test_emit_invalid_acs_triggers_retry():
    ...
    _canonical_acs = [
        "Function defined: bob3.my_feature.fn",
    ...
    with patch("bob3.research_strategies.emit_canonical_acs", side_effect=_fake_emit):
...
def test_synthesis_blocked_on_persistent_failure():
    ...
    with patch(
        "bob3.research_strategies.emit_canonical_acs",
...
def test_negative_error_path_ac():
    ...
    from bob3.research_strategies import emit_canonical_acs, _ERROR_KEYWORDS
