...
from bob3.orchestrator.prompt_source_reloader import reload_if_stale, _MTIME_CACHE
...
class TestPromptReloaderPicksUpMtimeChange:
    ...
    def test_reload_triggered_on_mtime_change(self, tmp_path):
        ...
        mod = types.ModuleType("bob3.superpowers")
        ...
        with patch.dict(sys.modules, {"bob3.superpowers": mod}):
            with patch.object(importlib, "reload", side_effect=fake_reload):
                ...
                result2 = reload_if_stale("bob3.superpowers")
                assert result2 is True
                assert "bob3.superpowers" in reload_calls
    ...
    def test_cache_updated_after_reload(self, tmp_path):
        ...
        with patch.dict(sys.modules, {"bob3.superpowers": mod}):
            with patch.object(importlib, "reload", return_value=mod):
                reload_if_stale("bob3.superpowers")
                old_mtime = _MTIME_CACHE.get("bob3.superpowers")
                ...
                reload_if_stale("bob3.superpowers")
                new_mtime = _MTIME_CACHE.get("bob3.superpowers")
    ...
    def test_reload_count_matches_change_count(self, tmp_path):
        ...
        with patch.dict(sys.modules, {"bob3.superpowers": mod}):
            with patch.object(importlib, "reload", side_effect=counting_reload):
                reload_if_stale("bob3.superpowers")  # seeds cache
