...
def test_is_orchestrator_alive_matches_bob3_run():
    """is_orchestrator_alive returns True for legacy 'bob3 run' process."""
    with patch(
        "bob3.orchestrator.liveness_probe._iter_candidate_pids",
        return_value=_fake_process_list(["bob3 run --all"]),
    ):
        assert is_orchestrator_alive() is True
...
def test_is_orchestrator_alive_does_not_match_bob_without_run():
    ...
    with patch(
        "bob3.orchestrator.liveness_probe._iter_candidate_pids",
        return_value=_fake_process_list(["bob14 status", "bob3 plan --create"]),
    ):
        assert is_orchestrator_alive() is False
...
def test_is_orchestrator_alive_matches_bob3_run_variant():
    """'bob3 run' (without gen suffix) also triggers is_orchestrator_alive=True."""
    with patch(
        "bob3.orchestrator.liveness_probe._iter_candidate_pids",
        return_value=_fake_process_list(["/usr/local/bin/bob3 run --all --fresh"]),
    ):
        assert is_orchestrator_alive() is True
