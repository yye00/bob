...
Root cause (bob3 v.17 r1): run_loop.py used min(self.max_cost, self._project_max_cost_usd)
...
Fix: clamp to BOB3_PER_FEATURE_COST_CEILING (default $20), NOT project max_cost_usd.
...
from bob3.db import create_project, get_project, init_database
...
def db_path(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("BOB3_COST_PROJECTION_GATE", "0")
    init_database()
    return p
...
class TestNoBudgetExceededWithinCap:
    ...
    def test_per_feature_ceiling_env_var_respected(self, monkeypatch, db_path):
        """BOB3_PER_FEATURE_COST_CEILING env var controls the pessimistic ceiling.

        AC: the ceiling is configurable and defaults to $20, NOT project max_cost_usd.
        """
        from bob3.orchestrator.cost_telemetry_guard import apply_pessimistic_cost
    ...
    def test_per_feature_ceiling_env_var_default_is_twenty(self, monkeypatch):
        """Default BOB3_PER_FEATURE_COST_CEILING is $20 (not max_cost_usd)."""
        import os
...
class TestStructuralAccumulatorPath:
    ...
    def test_per_feature_ceiling_not_max_cost_usd(self, monkeypatch):
        """Verify run_loop.py uses BOB3_PER_FEATURE_COST_CEILING, not _project_max_cost_usd.
        ...
        ceiling_default = float(os.environ.get("BOB3_PER_FEATURE_COST_CEILING", "20.0"))
