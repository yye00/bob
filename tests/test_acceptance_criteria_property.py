"""Tests for bob.acceptance_criteria.parse_property_ac.

Verifies the seventh AC grammar: ``property: <name> for <generator> assert <predicate>``
is correctly parsed and exported from bob.acceptance_criteria.
"""
...
from bob.acceptance_criteria import parse_property_ac
...
class TestParsePropertyACNonProperty:
    ...
    def test_returns_none_for_function_defined_ac(self):
        result = parse_property_ac("Function defined: bob.foo.bar")
        assert result is None
...
class TestParsePropertyACImport:
    def test_function_importable_from_acceptance_criteria(self):
        from bob.acceptance_criteria import parse_property_ac as fn
        assert callable(fn)
