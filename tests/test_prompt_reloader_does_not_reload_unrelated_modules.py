...
class TestPromptReloaderDoesNotReloadUnrelatedModules:
    ...
    def test_watchlist_contains_superpowers_and_models(self):
        """_PROMPT_SOURCE_MODULES includes both bob.superpowers and bob.models."""
    ...
    def test_maybe_reload_all_does_not_touch_unrelated_module(self, tmp_path):
        ...
        unrelated_mod = types.ModuleType("bob._unrelated_test_module_xyz")
        ...
        with patch.dict(sys.modules, {"bob._unrelated_test_module_xyz": unrelated_mod}):
            with patch.object(importlib, "reload", side_effect=tracking_reload):
                # Seed the mtime for the unrelated module manually
                _MTIME_CACHE["bob._unrelated_test_module_xyz"] = 1.0
        ...
        assert "bob._unrelated_test_module_xyz" not in reloaded
        assert "bob._unrelated_test_module_xyz" not in reload_calls
    ...
    def test_reload_if_stale_is_independent_per_module(self, tmp_path):
        ...
        mod_a = types.ModuleType("bob._test_mod_a")
        mod_a.__file__ = str(src_a)
        mod_b = types.ModuleType("bob._test_mod_b")
        mod_b.__file__ = str(src_b)

        with patch.dict(sys.modules, {
            "bob._test_mod_a": mod_a,
            "bob._test_mod_b": mod_b,
        }):
            with patch.object(importlib, "reload", return_value=mod_a):
                reload_if_stale("bob._test_mod_a")
                # mod_b's cache should not be set by the above call
                assert "bob._test_mod_b" not in _MTIME_CACHE
