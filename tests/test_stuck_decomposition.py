"""Tests for bob.readiness.stuck_decomposition."""
...
class TestShouldMarkForDecomposition:
    ...
    def test_no_previous_score_treated_as_no_improvement(self):
        f = make_feature(refinement_attempts=2, readiness_score=0.65)
        assert should_mark_for_decomposition(f, previous_readiness_score=None) is True

    def test_default_previous_score_is_none(self):
        f = make_feature(refinement_attempts=2, readiness_score=0.65)
        assert should_mark_for_decomposition(f) is True
    ...
    def test_three_attempts_returns_true(self):
        ...
        assert should_mark_for_decomposition(f, previous_readiness_score=0.50) is True

    def test_negative_attempts_raises_value_error(self):
        f = make_feature(refinement_attempts=-1)
        with pytest.raises(ValueError, match="negative"):
            should_mark_for_decomposition(f)
    ...
    def test_readiness_just_below_threshold_triggers(self):
        ...
        assert should_mark_for_decomposition(f, previous_readiness_score=0.799) is True
...
class TestMarkPendingDecomposition:
    ...
    def test_returns_updated_feature_not_same_object(self):
        f = make_feature(refinement_attempts=2, readiness_score=0.65)
        result = mark_pending_decomposition(f)
