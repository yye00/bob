"""Tests for find_local_modules function in bob3.security.slopsquatting_scan."""
...
def fixture_tree(tmp_path: Path) -> Path:
    """Create a fixture src/bob3 tree with various modules."""
    src_bob3 = tmp_path / "src" / "bob3"
    src_bob3.mkdir(parents=True)
    # Single-file modules
    (src_bob3 / "module_a.py").write_text("# module a\n")
    (src_bob3 / "module_b.py").write_text("# module b\n")
    # Package
    pkg = src_bob3 / "my_package"
    pkg.mkdir()
    (pkg / "__init__.py").write_text("# package\n")
    # __init__.py at the bob3 level itself (should NOT be included as a separate name)
    (src_bob3 / "__init__.py").write_text("")
    return tmp_path
...
def test_find_local_modules_only_top_level_names(fixture_tree: Path) -> None:
    """find_local_modules only returns top-level module names from src/bob3/."""
    modules = find_local_modules(fixture_tree)
...
def test_find_local_modules_with_additional_py_file(tmp_path: Path) -> None:
    """find_local_modules finds each additional .py module in src/bob3/."""
    src_bob3 = tmp_path / "src" / "bob3"
    src_bob3.mkdir(parents=True)
    (src_bob3 / "spec_quality_score.py").write_text("# spec quality\n")
