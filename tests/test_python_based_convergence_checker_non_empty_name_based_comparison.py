"""Tests for python_based_convergence_checker_non_empty_name_based_comparison.
...
from bob3.python_based_convergence_checker_non_empty_name_based_comparison import (
    python_based_convergence_checker_non_empty_name_based_comparison,
)
...
def _make_db(path: Path, rows: list[tuple[str, str]], status: str = "completed") -> None:
    ...
    conn.execute(
        "CREATE TABLE features (id TEXT, name TEXT, status TEXT)"
    )
...
def test_python_based_convergence_checker_non_empty_name_based_comparison():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        converged, diff = python_based_convergence_checker_non_empty_name_based_comparison(
            d0, d1, d2
        )
...
def test_returns_diff_set_on_partial_mismatch():
    """Diff set must contain names present in some but not all generations."""
    with tempfile.TemporaryDirectory() as td:
        d0 = Path(td) / "g0.db"
        d1 = Path(td) / "g1.db"
        d2 = Path(td) / "g2.db"

        _make_db(d0, [("u1", "feat-a"), ("u2", "feat-b")])
        _make_db(d1, [("u3", "feat-a"), ("u4", "feat-b")])
        _make_db(d2, [("u5", "feat-a")])  # feat-b missing
