"""Tests for regression_attribution_requires_test_ownership_map_no_scapegoats.
...
from bob3.regression_attribution_requires_test_ownership_map_no_scapegoats import (
    UNATTRIBUTED_KEY,
...
# ---------------------------------------------------------------------------
# Top-level canonical AC test
# ---------------------------------------------------------------------------

def test_regression_attribution_requires_test_ownership_map_no_scapegoats():
    ...
    result = regression_attribution_requires_test_ownership_map_no_scapegoats(
    ...
    # feat-beta owns no failing test → NOT in result at all
...
class TestBuildTestOwnershipMap:
    ...
    def test_case_insensitive_pytest_prefix(self):
        features = [{"id": "feat-ci", "acceptance_criteria": json.dumps(["Pytest: tests/test_ci.py::test_x"])}]
        result = build_test_ownership_map(features)
        assert "tests/test_ci.py::test_x" in result
...
class TestRegressionAttributionNoScapegoats:
    ...
    def test_mix_of_owned_and_unowned_tests(self):
        ownership = {"tests/test_known.py::test_k": "feat-known"}
        result = regression_attribution_requires_test_ownership_map_no_scapegoats(
            newly_failing_tests=["tests/test_known.py::test_k", "tests/test_ghost.py::test_g"],
            test_ownership_map=ownership,
        )
        assert "feat-known" in result
        assert result["feat-known"]["demote"] is True
        assert UNATTRIBUTED_KEY in result
        assert result[UNATTRIBUTED_KEY]["demote"] is False
