"""Tests for bob.cost_cap.enforce_per_attempt_cap (e93f7fc4).
...
class TestEnforcePerAttemptCapNoAction:
    ...
    def test_cost_at_cap_returns_false(self, monkeypatch):
        ...
        with patch("bob.cost_cap.terminate_subagent_on_cost_cap") as mock_terminate:
...
class TestEnforcePerAttemptCapCustomCap:
    """Custom BOB_PER_ATTEMPT_COST_CAP env override is respected."""
...
class TestEnforcePerAttemptCapIntegration:
    ...
    def test_sigterm_sent_to_pid_on_cap_exceeded(self, monkeypatch):
        ...
        with (
            patch(
                "bob.orchestrator.per_attempt_cost_cap._send_signal",
                side_effect=fake_send_signal,
            ),
            patch(
                "bob.orchestrator.per_attempt_cost_cap._wait_for_exit",
                return_value=True,
            ),
            patch("bob.orchestrator.per_attempt_cost_cap.db") as mock_db,
        ):
    ...
    def test_refinement_attempt_charged_via_db(self, monkeypatch):
        ...
        with (
            patch("bob.orchestrator.per_attempt_cost_cap._send_signal"),
            patch(
                "bob.orchestrator.per_attempt_cost_cap._wait_for_exit",
                return_value=True,
            ),
