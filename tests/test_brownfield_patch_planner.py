"""BF-7 — tests for bob3.brownfield.patch_planner.
...
# ---------------------------------------------------------------------------
# plan_diff (required by AC: Function defined: bob3.brownfield.patch_planner.plan_diff)
# ---------------------------------------------------------------------------
...
class TestEmitDiffPlan:
    def test_creates_diff_plan_yaml_in_bob3_dir(self, tmp_path):
        touches = [
            {
                "path": "src/x.py",
...
class TestApplyDiffPlan:
    ...
    def test_backup_created_before_modification(self, tmp_path):
        ...
        apply_diff_plan(plan_path, workspace=tmp_path)
        backup = tmp_path / ".bob3" / "features" / "feat-ap-003" / "orig" / "src" / "target.py"
...
class TestOrchestratorIntegration:
    def test_plan_diff_importable_from_orchestrator(self):
        ...
        assert hasattr(orch, "plan_diff") or hasattr(orch, "emit_diff_plan") or hasattr(orch, "apply_diff_plan"), (
            "bob3.orchestrator must expose at least one patch_planner symbol"
        )

    def test_patch_planner_module_has_required_symbols(self):
        from bob3.brownfield import patch_planner
