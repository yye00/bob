"""Tests for sub_translation_provenance_every_ac_traces_source_intent_span."""

import pytest

from bob3.sub_translation_provenance_every_ac_traces_source_intent_span import (
    sub_translation_provenance_every_ac_traces_source_intent_span,
)
...
def test_sub_translation_provenance_every_ac_traces_source_intent_span():
    """Each AC in the result carries a non-empty provenance span list."""
    result = sub_translation_provenance_every_ac_traces_source_intent_span(
...
def test_raises_on_insufficient_coverage():
    ...
    The intent has 4 sentences; one AC matches only one sentence (25% coverage).
    """
    # INTENT has 4 sentences; a single AC maps to one sentence (~25% coverage)
