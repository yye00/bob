"""Tests for bob3.orchestrator_dispatch — feature f9de5db3.

Verifies that dispatch_concurrent_features:
- Is importable from bob3.orchestrator_dispatch
- Is also integrated into bob3.orchestrator
- Returns empty list when no ready features exist or cap is saturated
- Correctly resolves BOB3_MAX_CONCURRENT_FEATURES env var (default 3)
...
class TestModuleImportability:
    ...
    def test_all_exports_listed_in_dunder_all(self):
        import bob3.orchestrator_dispatch as mod
...
class TestResolveMaxConcurrentFeatures:
    ...
    def test_invalid_env_falls_back_to_default(self, monkeypatch):
        monkeypatch.setenv("BOB3_MAX_CONCURRENT_FEATURES", "not-a-number")
        from bob3.orchestrator_dispatch import resolve_max_concurrent_features
        assert resolve_max_concurrent_features() == 3
...
class TestClaimReadyFeatures:
    def test_returns_empty_when_no_ready(self):
        ...
        with mock.patch("bob3.db.update_feature") as mock_update:
    ...
    def test_claims_up_to_cap(self):
        ...
        with mock.patch("bob3.db.update_feature"):
...
class TestDispatchConcurrentFeatures:
    ...
    async def test_returns_empty_when_no_ready(self):
        ...
        with mock.patch("bob3.orchestrator_dispatch.db"):
    ...
    async def test_env_override_limits_concurrency(self, monkeypatch):
        ...
        # With BOB3_MAX_CONCURRENT_FEATURES=2, at most 2 workers run simultaneously
    ...
    async def test_concurrent_tasks_run_in_parallel(self, monkeypatch):
        ...
        from bob3.orchestrator_dispatch import dispatch_concurrent_features
        monkeypatch.setenv("BOB3_MAX_CONCURRENT_FEATURES", "3")
...
# ---------------------------------------------------------------------------
# AC 4: Integration — dispatch_concurrent_features accessible from bob3.orchestrator
# ---------------------------------------------------------------------------

class TestOrchestratorIntegration:
    def test_dispatch_concurrent_features_in_bob3_orchestrator(self):
    ...
    def test_dispatch_concurrent_features_v2_is_callable(self):
        from bob3.orchestrator import dispatch_concurrent_features_v2
    ...
    def test_orchestrator_dispatch_module_importable_standalone(self):
        import bob3.orchestrator_dispatch
        assert bob3.orchestrator_dispatch is not None

    def test_both_dispatch_functions_have_same_signature_shape(self):
        from bob3.orchestrator_dispatch import dispatch_concurrent_features as new_fn
        from bob3.orchestrator.run_loop import dispatch_concurrent_features as old_fn
