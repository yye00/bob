...
BOB3_SPEC_QUALITY_THRESHOLD env var.

Acceptance criteria verified:
  - File exists: src/bob3/spec_quality/quality_score.py
  - Function defined: bob3.spec_quality.quality_score._resolve_threshold
  - Function defined: bob3.spec_quality.quality_score.score_threshold
...
import bob3.spec_quality.quality_score as qs
import bob3.spec_quality.threshold_resolver as tr


# ---------------------------------------------------------------------------
# AC: File exists: src/bob3/spec_quality/quality_score.py
# ---------------------------------------------------------------------------

class TestFileExists:
    def test_quality_score_module_importable(self):
        """quality_score.py must exist and be importable."""
        module = importlib.import_module("bob3.spec_quality.quality_score")
        assert module is not None

    def test_quality_score_file_on_disk(self):
        from pathlib import Path
        p = Path(__file__).parent.parent / "src" / "bob3" / "spec_quality" / "quality_score.py"
        assert p.exists(), f"Expected file at {p}"


# ---------------------------------------------------------------------------
# AC: Function defined: bob3.spec_quality.quality_score._resolve_threshold
# ---------------------------------------------------------------------------
...
class TestResolveThresholdDefined:
    ...
    def test_reads_env_var_on_every_call(self, monkeypatch):
        ...
        monkeypatch.setenv("BOB3_SPEC_QUALITY_THRESHOLD", "0.40")
    ...
    def test_clamps_above_1_0(self, monkeypatch):
        ...
        monkeypatch.setattr(tr, "_frozen_initialized", False)
        monkeypatch.setattr(tr, "_frozen_value", None)
        monkeypatch.setenv("BOB3_SPEC_QUALITY_THRESHOLD", "2.5")
    ...
    def test_unparseable_falls_back_to_0_85(self, monkeypatch):
        ...
        monkeypatch.setattr(tr, "_frozen_initialized", False)
        monkeypatch.setattr(tr, "_frozen_value", None)
        monkeypatch.setenv("BOB3_SPEC_QUALITY_THRESHOLD", "banana")
        assert qs._resolve_threshold() == pytest.approx(0.85)

    def test_frozen_escape_hatch_pins_value(self, monkeypatch):
        """BOB3_SPEC_QUALITY_THRESHOLD_FROZEN pins threshold for deterministic tests."""
        monkeypatch.setattr(tr, "_frozen_initialized", False)
        monkeypatch.setattr(tr, "_frozen_value", None)
        monkeypatch.setenv("BOB3_SPEC_QUALITY_THRESHOLD_FROZEN", "0.60")
        monkeypatch.delenv("BOB3_SPEC_QUALITY_THRESHOLD", raising=False)

        first = qs._resolve_threshold()
        # Even if live env changes, frozen value stays
        monkeypatch.setenv("BOB3_SPEC_QUALITY_THRESHOLD", "0.10")
        second = qs._resolve_threshold()
...
# ---------------------------------------------------------------------------
# AC: Function defined: bob3.spec_quality.quality_score.score_threshold
# ---------------------------------------------------------------------------
...
class TestScoreThresholdDefined:
    ...
    def test_is_not_hardcoded(self, monkeypatch):
        ...
        monkeypatch.setattr(tr, "_frozen_initialized", False)
        monkeypatch.setattr(tr, "_frozen_value", None)
        monkeypatch.setenv("BOB3_SPEC_QUALITY_THRESHOLD", "0.30")
        result = qs.score_threshold()
    ...
    def test_delegates_to_resolve_threshold(self, monkeypatch):
        ...
        monkeypatch.setattr(tr, "_frozen_initialized", False)
        monkeypatch.setattr(tr, "_frozen_value", None)
        monkeypatch.setenv("BOB3_SPEC_QUALITY_THRESHOLD", "0.72")
        assert qs.score_threshold() == qs._resolve_threshold()


# ---------------------------------------------------------------------------
# AC: integration: bob3.spec_quality
# ---------------------------------------------------------------------------

class TestIntegrationBob3SpecQuality:
    """Verify the bob3.spec_quality package is reachable and the gate is wired correctly."""

    def test_package_importable(self):
        pkg = importlib.import_module("bob3.spec_quality")
        assert pkg is not None

    def test_quality_score_module_importable(self):
        mod = importlib.import_module("bob3.spec_quality.quality_score")
        assert mod is not None

    def test_threshold_resolver_importable(self):
        mod = importlib.import_module("bob3.spec_quality.threshold_resolver")
        assert mod is not None
    ...
    def test_env_change_mid_run_takes_effect(self, monkeypatch):
        ...
        monkeypatch.setenv("BOB3_SPEC_QUALITY_THRESHOLD", "0.50")
