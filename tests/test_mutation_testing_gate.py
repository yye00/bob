"""Tests for bob3.mutation_testing_gate.
...
from bob3.mutation_testing_gate import (
...
def test_run_mutation_tests_returns_dict_on_success(tmp_path):
    ...
    with patch(
        "bob3.mutation_testing_gate.run_mutation_test", return_value=report
    ):
        result = run_mutation_tests(
            feature_id="feat-ok",
            src_files=["src/bob3/foo.py"],
...
def test_run_mutation_tests_fails_when_score_below_threshold(tmp_path):
    ...
    with patch(
        "bob3.mutation_testing_gate.run_mutation_test", return_value=report
    ), patch("bob3.mutation_testing_gate.persist_surviving_mutants") as mock_persist:
        result = run_mutation_tests(
            feature_id="feat-low",
            src_files=["src/bob3/foo.py"],
            test_dir=str(tmp_path),
...
def test_run_mutation_tests_returns_skipped_when_mutmut_missing(tmp_path):
    with patch(
        "bob3.mutation_testing_gate.run_mutation_test",
...
def test_run_mutation_tests_respects_custom_threshold(tmp_path):
    ...
    with patch(
        "bob3.mutation_testing_gate.run_mutation_test", return_value=report
    ), patch("bob3.mutation_testing_gate.persist_surviving_mutants"):
        result = run_mutation_tests(
            feature_id="feat-custom",
            src_files=["src/bob3/foo.py"],
            test_dir=str(tmp_path),
