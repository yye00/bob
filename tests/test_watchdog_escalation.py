...
Verifies escalate_stall_observation in bob.watchdog escalates repeated
spec_gate_stall_observed events to a needs_human_attention sentinel after N
consecutive observations (default 5, configurable via BOB_STALL_ESCALATION_COUNT).
...
from bob.watchdog import escalate_spec_gate_stall, escalate_stall_observation
...
def test_invalid_env_threshold_falls_back_to_default(tmp_path):
    marker = tmp_path / "STALL_ATTENTION.txt"
    with patch.dict(os.environ, {"BOB_STALL_ESCALATION_COUNT": "banana"}):
