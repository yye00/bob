"""Tests for AC: bob3.spawn.verify_project_metadata — stale project metadata after rsync.
...
def test_spawn_module_importable():
    """bob3.spawn must be importable as a module."""
    import bob3.spawn  # noqa: F401


def test_verify_project_metadata_importable():
    """bob3.spawn.verify_project_metadata must be a callable."""
...
def _make_db(tmp_path: pathlib.Path, name: str, spec_path: str = "") -> pathlib.Path:
    """Create a minimal bob3.db with a single projects row."""
...
class TestVerifyProjectMetadataRunLoopIntegration:
    """Verify bob3.run_loop also exposes verify_project_metadata (integration AC)."""

    def test_run_loop_exports_verify_project_metadata(self):
        """bob3.run_loop.verify_project_metadata must be callable."""
    ...
    def test_spawn_and_run_loop_return_same_type(self, tmp_path):
        ...
        from bob3.spawn import verify_project_metadata as spawn_vpm
        from bob3.run_loop import verify_project_metadata as run_loop_vpm
