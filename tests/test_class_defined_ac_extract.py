...
import pytest
from bob3.verification.class_defined_ac_check import extract_class_name_from_criterion


def test_extracts_simple_class_name():
    result = extract_class_name_from_criterion("Class defined: bob3.verification.mutation_gate.MutationReport")
...
def test_returns_none_for_non_matching_string():
    result = extract_class_name_from_criterion("Function defined: bob3.verification.mutation_gate.some_func")
...
def test_returns_none_for_file_exists_criterion():
    result = extract_class_name_from_criterion("File exists: src/bob3/verification/mutation_gate.py")
    assert result is None
