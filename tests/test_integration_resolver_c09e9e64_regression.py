...
The exact criterion that repeatedly blocked feature c09e9e64 across 3 attempts:
  "integration: all spec_findings.yaml writes in bob3.reviews route through
...
def test_c09e9e64_criterion_emits_prose_demoted_log(
    ...
    assert demoted_records, (
        "Expected at least one log record from 'bob3.verification.integration_demotion'"
    )
...
def test_c09e9e64_extract_finds_bob3_reviews() -> None:
    """extract_integration_targets must return 'bob3.reviews' for the regression criterion."""
    ...
    assert "bob3.reviews" in targets, (
        f"Expected 'bob3.reviews' in extracted targets, got {targets!r}"
    )
