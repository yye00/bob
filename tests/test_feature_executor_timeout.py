"""Tests for bob3.feature_executor.enforce_timeout (feature 86e81c64).
...
- BOB3_FEATURE_TIMEOUT_SECONDS env var controls the default timeout
...
from bob3.feature_executor import FeatureTimeoutError, enforce_timeout
...
class TestEnforceTimeoutImport:
    ...
    def test_importable(self):
        import bob3.feature_executor as m
...
class TestEnforceTimeoutEnvVar:
    """BOB3_FEATURE_TIMEOUT_SECONDS controls the default timeout."""
    ...
    async def test_explicit_timeout_overrides_env_var(self, monkeypatch):
        monkeypatch.setenv("BOB3_FEATURE_TIMEOUT_SECONDS", "0.001")
...
class TestEnforceTimeoutTelemetry:
    ...
    async def test_timeout_emits_warning_log(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="bob3.feature_executor"):
            with pytest.raises(FeatureTimeoutError):
...
class TestOrchestratorIntegration:
    """bob3.orchestrator imports and can use enforce_timeout from feature_executor."""

    def test_orchestrator_package_importable(self):
        import bob3.orchestrator
        assert bob3.orchestrator is not None

    def test_feature_executor_importable_from_bob3(self):
        import bob3.feature_executor
        assert hasattr(bob3.feature_executor, "enforce_timeout")

    def test_run_loop_has_resolve_timeout(self):
        from bob3.orchestrator.run_loop import _resolve_feature_timeout_seconds
