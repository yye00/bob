...
- Creates src/bob/mcp_lifecycle.py module
- Implements start_mcp_server() to start the bob-memory MCP
...
- Integrates atexit handler to stop MCP server on Bob exit
...
MODULE_PATH = SRC_DIR / "bob" / "mcp_lifecycle.py"
...
def _clear_active_processes_registry():
    from bob import mcp_lifecycle as _mod
...
class TestFileExists:
    """Step 1: src/bob/mcp_lifecycle.py must exist."""
...
class TestStartMCPServer:
    """Step 2: start_mcp_server() starts the bob-memory MCP as subprocess."""

    def test_start_mcp_server_function_exists(self):
        from bob.mcp_lifecycle import start_mcp_server

        assert callable(start_mcp_server)

    @patch("subprocess.Popen")
    def test_start_mcp_server_starts_subprocess(self, mock_popen):
        from bob.mcp_lifecycle import MCPLifecycleManager
    ...
    def test_start_mcp_server_uses_correct_command(self, mock_popen):
        ...
        expected_cmd = [sys.executable, "-m", "bob.memory_mcp"]
        assert cmd == expected_cmd

    @patch("subprocess.Popen")
    def test_start_mcp_server_raises_on_immediate_exit(self, mock_popen):
        from bob.mcp_lifecycle import MCPLifecycleManager, MCPStartupError
...
class TestStartupValidation:
    ...
    def test_mcp_startup_error_class_exists(self):
        from bob.mcp_lifecycle import MCPStartupError
...
class TestCLIIntegration:
    ...
    def test_stop_mcp_server_convenience_function(self):
        ...
        from bob.mcp_lifecycle import stop_mcp_server
    ...
    def test_get_mcp_manager_returns_singleton(self):
        ...
        from bob.mcp_lifecycle import get_mcp_manager
...
class TestAtexitHandler:
    """Step 7: atexit handler to stop MCP server on Bob exit."""
...
# ===================================================================
# Step 9: Simulate MCP startup failure - Bob exits with clear error
# ===================================================================
...
class TestConfigIntegration:
    ...
    def test_manager_uses_bob_memory_config(self):
        from bob.mcp_lifecycle import MCPLifecycleManager
        from bob.orchestrator.mcp_config import BOB_MEMORY_MCP

        manager = MCPLifecycleManager()
        assert manager.config.name == BOB_MEMORY_MCP.name
        assert manager.config.command == BOB_MEMORY_MCP.command
