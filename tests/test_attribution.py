"""Tests for bob3.test_attribution — regression-vs-baseline ownership attribution.
        ...
        features = [
            {
                "id": CURRENT_FEATURE,
                "acceptance_criteria": (
                    '["File exists: src/bob3/foo.py", "pytest: tests/test_foo.py"]'
                ),
            }
        ]
        result = build_test_to_feature_map(features)
        assert "tests/test_foo.py" in result
        assert "File exists: src/bob3/foo.py" not in result
        assert len(result) == 1
...
# ---------------------------------------------------------------------------
# Integration: both functions are accessible via bob3.enhanced_verification
# ---------------------------------------------------------------------------

class TestEnhancedVerificationIntegration:
    def test_build_test_to_feature_map_accessible_via_test_attribution(self):
        """build_test_to_feature_map is importable from bob3.test_attribution."""
        from bob3.test_attribution import build_test_to_feature_map as fn
        assert callable(fn)

    def test_attribute_regression_to_owner_accessible_via_test_attribution(self):
        """attribute_regression_to_owner is importable from bob3.test_attribution."""
        from bob3.test_attribution import attribute_regression_to_owner as fn
        assert callable(fn)

    def test_integration_with_verifier_namespace(self):
        """Key functions are re-exported through bob3.verification.verifier."""
