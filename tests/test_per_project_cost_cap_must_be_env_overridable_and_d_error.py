...
class TestResolveMaxCostUsdDoesNotSilentlyReturnZeroOnInvalid:
    ...
    def test_malformed_env_does_not_return_zero(self, monkeypatch):
        """Non-numeric BOB_MAX_COST_USD must NOT silently return 0.0."""
        monkeypatch.setenv("BOB_MAX_COST_USD", "completely_invalid")
        from bob.per_project_cost_cap_must_env_overridable_default import resolve_max_cost_usd

        result = resolve_max_cost_usd()
        assert result != 0.0, "Malformed BOB_MAX_COST_USD must not silently resolve to 0"

    def test_none_like_string_does_not_return_zero(self, monkeypatch):
        """'None' string in BOB_MAX_COST_USD must not silently return 0.0."""
    ...
    def test_nan_string_does_not_block_spawns(self, monkeypatch):
        """'NaN' in BOB_MAX_COST_USD must not produce a falsy or zero effective cap."""
