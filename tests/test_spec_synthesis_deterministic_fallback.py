"""Tests for bob3.spec_synthesis.ensure_boundary_and_error_coverage.

Verifies that the public API in bob3.spec_synthesis correctly delegates
...
from bob3.spec_synthesis import ensure_boundary_and_error_coverage
...
class TestEnsureBoundaryAndErrorCoverage:
    """Public API in bob3.spec_synthesis must match spec_synthesizer behaviour."""

    def test_importable_from_spec_synthesis(self):
        """Function must be importable from bob3.spec_synthesis (not just spec_synthesizer)."""
    ...
    def test_original_criteria_preserved(self):
        original = [
            "File exists: src/bob3/foo.py",
            "Function defined: bob3.foo.do_thing",
    ...
    def test_repeated_calls_are_idempotent(self):
        criteria = ["File exists: src/bob3/baz.py", "pytest: tests/test_baz.py"]
    ...
    def test_empty_title_does_not_raise(self):
        result = ensure_boundary_and_error_coverage(
            ["File exists: src/bob3/thing.py"], title=""
        )
        assert isinstance(result, list)
    ...
    def test_composite_would_pass_gate(self):
        ...
        criteria = [
            "File exists: src/bob3/widget.py",
            "Function defined: bob3.widget.process",
            "pytest: tests/test_widget.py",
            "integration: bob3.widget",
        ]
