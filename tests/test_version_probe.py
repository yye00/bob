"""Tests for bob3.version_probe — SDK version and model ID probing."""
...
from bob3.version_probe import get_model_id, get_sdk_version, preflight_version_check
...
class TestPreflightVersionCheck:
    def test_no_warning_when_versions_match(self):
        with patch("bob3.version_probe.get_sdk_version", return_value="0.0.25"):
            with patch("bob3.version_probe._get_pinned_sdk_version", return_value="0.0.25"):
    ...
    def test_emits_warning_when_versions_differ(self):
        with patch("bob3.version_probe.get_sdk_version", return_value="1.0.0"):
            with patch("bob3.version_probe._get_pinned_sdk_version", return_value="0.0.25"):
                with warnings.catch_warnings(record=True) as caught:
                    warnings.simplefilter("always")
                    preflight_version_check()
    ...
    def test_no_warning_when_pinned_version_unavailable(self):
        with patch("bob3.version_probe.get_sdk_version", return_value="0.0.25"):
            with patch("bob3.version_probe._get_pinned_sdk_version", return_value=None):
                with warnings.catch_warnings(record=True) as caught:
                    warnings.simplefilter("always")
                    preflight_version_check()
        sdk_warnings = [w for w in caught if issubclass(w.category, UserWarning)]
    ...
    def test_does_not_raise(self):
        ...
        with patch("bob3.version_probe.get_sdk_version", return_value="99.0.0"):
            with patch("bob3.version_probe._get_pinned_sdk_version", return_value="0.0.1"):
...
class TestTelemetryIntegration:
    def test_emit_telemetry_line_includes_sdk_version(self, tmp_path):
        ...
        with mpatch("bob3.telemetry.get_run_jsonl_path", return_value=run_jsonl):
    ...
    def test_sdk_version_in_telemetry_matches_probe(self, tmp_path):
        ...
        from unittest.mock import patch as mpatch
        from bob3.telemetry import emit_telemetry_line
        from bob3.version_probe import get_sdk_version
    ...
    def test_model_id_in_telemetry_matches_probe(self, tmp_path):
        ...
        from unittest.mock import patch as mpatch
        from bob3.telemetry import emit_telemetry_line
        from bob3.version_probe import get_model_id
