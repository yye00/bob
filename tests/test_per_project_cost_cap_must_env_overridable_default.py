...
def _clean_env(monkeypatch):
    """Remove BOB_MAX_COST_USD from env before each test."""
...
def test_per_project_cost_cap_must_env_overridable_default():
    ...
    from bob.per_project_cost_cap_must_env_overridable_default import (
...
def test_default_is_effectively_unlimited_no_env_var():
    """Without BOB_MAX_COST_USD, resolve_max_cost_usd() returns 1_000_000."""
    from bob.per_project_cost_cap_must_env_overridable_default import (
...
def test_env_var_overrides_default(monkeypatch):
    """When BOB_MAX_COST_USD=250, resolve_max_cost_usd() returns 250.0."""
...
def test_env_var_float_value(monkeypatch):
    """Decimal values in BOB_MAX_COST_USD are accepted correctly."""
    monkeypatch.setenv("BOB_MAX_COST_USD", "1500.75")
    from bob.per_project_cost_cap_must_env_overridable_default import (
        resolve_max_cost_usd,
    )
...
def test_empty_env_var_falls_back_to_unlimited(monkeypatch):
    """An empty BOB_MAX_COST_USD must fall back to unlimited, not zero."""
...
def test_malformed_env_var_falls_back_to_unlimited(monkeypatch):
    """A non-numeric BOB_MAX_COST_USD must fall back to unlimited, not raise."""
...
def test_zero_env_var_not_blocked(monkeypatch):
    """BOB_MAX_COST_USD=0 is valid (user explicitly set budget to 0), but
...
def test_models_project_default_uses_env(monkeypatch):
    """Project.max_cost_usd default_factory reads BOB_MAX_COST_USD."""
    monkeypatch.setenv("BOB_MAX_COST_USD", "999")
    # Re-import needed to pick up env change in default_factory
    import bob.models as _models
