...
class TestRejectNoCoverageUplift:
    ...
    def test_raises_when_ac_region_not_in_source(self, tmp_path):
        ...
        with pytest.raises(NoCoverageUpliftError, match="bob3.mymodule.my_function"):
            reject_no_coverage_uplift(good_heuristic, ac_region="bob3.mymodule.my_function")
    ...
    def test_does_not_raise_when_ac_region_present_in_source(self, tmp_path):
        ...
        good.write_text(
            "import pytest\n# Tests: bob3.mymodule.my_function\ndef test_x():\n    pytest.fail('not done')\n",
            encoding="utf-8",
        )
        # Should not raise
        reject_no_coverage_uplift(good, ac_region="bob3.mymodule.my_function")
