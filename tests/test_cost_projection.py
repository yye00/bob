...
work against a real schema (the same one bob ships) so a regression
...
from bob.orchestrator.cost_projection import (
...
# A minimal slice of the bob schema that's enough to exercise the
...
def test_run_loop_integration_blocks_spawn_and_marks_needs_human(
    ...
    monkeypatch.setenv("BOB_COST_PROJECTION_GATE", "1")
