...
Covers:
- get_promote_on_n reads BOB3_PROMOTE_ON_N env var (default 3)
...
from bob3.ablation import get_mode_config, AblationMode


class TestGetPromoteOnN:
    """Tests for bob3.learnings.get_promote_on_n."""

    def test_default_is_three(self):
        with patch.dict(os.environ, {}, clear=False):
            env = {k: v for k, v in os.environ.items() if k != "BOB3_PROMOTE_ON_N"}
            with patch.dict(os.environ, env, clear=True):
                assert get_promote_on_n() == 3

    def test_reads_env_var(self):
        with patch.dict(os.environ, {"BOB3_PROMOTE_ON_N": "5"}):
            assert get_promote_on_n() == 5

    def test_env_var_value_one(self):
        with patch.dict(os.environ, {"BOB3_PROMOTE_ON_N": "1"}):
            assert get_promote_on_n() == 1

    def test_env_var_value_ten(self):
        with patch.dict(os.environ, {"BOB3_PROMOTE_ON_N": "10"}):
            assert get_promote_on_n() == 10

    def test_returns_int(self):
        with patch.dict(os.environ, {"BOB3_PROMOTE_ON_N": "7"}):
            result = get_promote_on_n()
            assert isinstance(result, int)

    def test_invalid_value_raises(self):
        with patch.dict(os.environ, {"BOB3_PROMOTE_ON_N": "not_a_number"}):
            with pytest.raises(ValueError):
                get_promote_on_n()

    def test_zero_is_valid(self):
        with patch.dict(os.environ, {"BOB3_PROMOTE_ON_N": "0"}):
            assert get_promote_on_n() == 0

    def test_unset_env_returns_default(self):
        env_without = {k: v for k, v in os.environ.items() if k != "BOB3_PROMOTE_ON_N"}
...
class TestTelemetryPromoteOnN:
    ...
    def test_telemetry_schema_has_promote_on_n(self, tmp_path):
        ...
        from bob3.telemetry import emit_telemetry_line, _DEFAULT_RUN_JSONL_PATH
        run_jsonl = tmp_path / ".bob3" / "run.jsonl"
        with (
            patch("bob3.telemetry._DEFAULT_RUN_JSONL_PATH", run_jsonl),
            patch("bob3.telemetry.get_run_jsonl_path", return_value=run_jsonl),
            patch.dict(os.environ, {"BOB3_PROMOTE_ON_N": "4"}),
        ):
    ...
    def test_telemetry_promote_on_n_value_matches_env(self, tmp_path):
        ...
        with (
            patch("bob3.telemetry._DEFAULT_RUN_JSONL_PATH", run_jsonl),
            patch("bob3.telemetry.get_run_jsonl_path", return_value=run_jsonl),
            patch.dict(os.environ, {"BOB3_PROMOTE_ON_N": "6"}),
        ):
            run_jsonl.parent.mkdir(parents=True, exist_ok=True)
