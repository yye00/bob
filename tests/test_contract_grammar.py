...
from bob.spec_quality.contract_grammar_lambda_binder import ContractGrammarBindingError
...
def test_free_variable_validation_rejects_unbound():
    """validate_lambda_bindings must reject decorators with unbound free variables.

    AC: pytest: tests/test_contract_grammar.py::test_free_variable_validation_rejects_unbound

    A zero-arg lambda referencing free variables must raise ContractGrammarBindingError.
    A partially-bound lambda must also be rejected. A fully-bound lambda passes.
    """
    from bob.spec_quality.contract_grammar_lambda_binder import ContractGrammarBindingError

    # Zero-arg lambda with free variable must be rejected
    broken = "@icontract.require(lambda: (x > 0))"
    with pytest.raises(ContractGrammarBindingError):
        validate_lambda_bindings(broken)

    # Partially-bound lambda must be rejected
    partially_broken = "@icontract.require(lambda x: (x > 0 and y < 10))"
    with pytest.raises(ContractGrammarBindingError):
        validate_lambda_bindings(partially_broken)

    # Fully-bound lambda must pass without raising
    valid = "@icontract.require(lambda x, y: (x > 0 and y < 10))"
    assert validate_lambda_bindings(valid) is True


def test_invalid_free_variables_rejected():
    """validate_lambda_binding must reject decorators containing unbound free variables.

    AC: pytest: tests/test_contract_grammar.py::test_invalid_free_variables_rejected

    A zero-arg lambda whose body references ``x`` must raise
    ContractGrammarBindingError before the decorator is persisted.
    A partially-bound lambda (``x`` bound, ``y`` free) must also be rejected.
    A fully-bound lambda must pass without error.
    """
    from bob.spec_quality.contract_grammar_lambda_binder import ContractGrammarBindingError

    # Zero-arg lambda referencing free variable must be rejected
    zero_arg = "@icontract.require(lambda: (x > 0))"
    with pytest.raises(ContractGrammarBindingError):
        validate_lambda_binding(zero_arg)

    # Partially-bound lambda must also be rejected
    partial = "@icontract.require(lambda x: (x > 0 and y < 10))"
    with pytest.raises(ContractGrammarBindingError):
        validate_lambda_binding(partial)

    # Fully-bound lambda must pass
    valid = "@icontract.require(lambda x, y: (x > 0 and y < 10))"
    assert validate_lambda_binding(valid) is True

    # ensure variant with result binding must pass
    ensure_valid = "@icontract.ensure(lambda result: (result > 0))"
    assert validate_lambda_binding(ensure_valid) is True
