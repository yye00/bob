"""Error-path tests for bob3.dispatch worker-leverage functions.
...
class TestSpawnWorkerErrors:
    def test_none_feature_raises_value_error(self, tmp_path):
        with pytest.raises(ValueError):
            spawn_worker(None, "prompt", tmp_path, bob3_dir=tmp_path / ".bob3")
...
class TestWriteFeatureSettingsErrors:
    ...
    def test_none_extra_allow_does_not_raise(self, tmp_path):
        ...
        path = write_feature_settings(feature, bob3_dir=tmp_path, extra_allow=None)
        assert path.exists()
