"""Tests for bob.stuck_reaper — top-level module exposing detect_and_reset_stuck_features.

Acceptance criteria:
- File exists: src/bob/stuck_reaper.py
- Function defined: bob.stuck_reaper.detect_and_reset_stuck_features
...
class TestModuleImports:
    def test_module_importable(self):
        import bob.stuck_reaper  # noqa: F401

    def test_detect_and_reset_stuck_features_callable(self):
        from bob.stuck_reaper import detect_and_reset_stuck_features
...
class TestDetectAndResetStuckFeatures:
    def test_returns_empty_list_when_no_executing_features(self):
        from bob.stuck_reaper import detect_and_reset_stuck_features
        with patch("bob.stuck_reaper.db") as mock_db:
    ...
    def test_logs_reap_event(self, caplog):
        ...
        with patch("bob.stuck_reaper.db") as mock_db:
            mock_db.list_features.return_value = [feature]
            with caplog.at_level(logging.INFO, logger="bob.stuck_reaper"):
                detect_and_reset_stuck_features("proj-1")
...
# ---------------------------------------------------------------------------
# Orchestrator integration: bob.orchestrator exports sweep via stuck_reaper
# ---------------------------------------------------------------------------

class TestOrchestratorIntegration:
    def test_orchestrator_imports_stuck_executing_reaper(self):
        """bob.orchestrator must import from the stuck-reaper module."""
