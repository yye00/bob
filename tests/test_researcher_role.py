"""Tests for bob3.agents.roles.researcher (BF-2 — hide-the-ticket pattern).
...
Verifies the researcher role and its integration with the coordinator:
  ...
  - researcher() callable is importable from bob3.agents.roles
  - researcher() returns the RESEARCHER Role instance
  - research_notes_path() produces the canonical .bob3/features/<id>/research_notes.md path
...
# ---------------------------------------------------------------------------
# researcher() callable — AC: Function defined: bob3.agents.roles.researcher
# ---------------------------------------------------------------------------
...
class TestResearcherCallable:
    ...
    def test_researcher_with_path_glob_returns_researcher_role(self):
        from bob3.agents.roles import researcher, RESEARCHER
        role = researcher(path_glob="src/bob3/**")
        assert role is RESEARCHER
...
class TestOrchestratorIntegration:
    ...
    def test_roles_importable_from_agents(self):
        from bob3.agents import roles as roles_module
    ...
    def test_researcher_role_not_visible_to_implementer(self):
        ...
        from bob3.agents.roles import RESEARCHER, IMPLEMENTER
        assert RESEARCHER.hide_intent is True
