...
class TestScoreGateLoopBoundaryThreshold:
    ...
    def test_accepts_when_composite_equals_threshold(self):
        ...
        criteria = [
            "File exists: src/bob/boundary.py",
            "Function defined: bob.boundary.check",
        ...
        with patch("bob.spec_synthesizer.score_synthesized_acs", return_value=0.85):
    ...
    def test_rejects_when_composite_just_below_threshold(self):
        ...
        async def mock_synthesize(**kwargs):
            return ["File exists: src/bob/near.py"]

        with patch("bob.spec_synthesizer.score_synthesized_acs", return_value=0.8499):
            report = asyncio.get_event_loop().run_until_complete(
                score_gate_loop(
                    synthesize_fn=mock_synthesize,
