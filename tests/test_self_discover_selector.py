"""Tests for bob.self_discover_selector.select_spec_sections.
...
class TestSelectSpecSectionsCore:
    def test_returns_dict(self):
        result = select_spec_sections(
            ...
            acceptance_criteria=[
                "File exists: src/bob/self_discover_selector.py",
                "Function defined: bob.self_discover_selector.select_spec_sections",
            ],
        )
...
class TestSelectSpecSectionsIntegration:
    """Verify select_spec_sections integrates cleanly with bob.spec_critic."""

    def test_section_map_usable_with_spec_critic_import(self):
        """Spec critic can be imported alongside self_discover_selector."""
        from bob.spec_critic import SpecCritic  # noqa: F401 — import check only

        result = select_spec_sections(
            feature_id="integ-001",
            name="Integration test feature",
            description="Checks that selector and spec_critic coexist.",
            acceptance_criteria=["File exists: src/bob/self_discover_selector.py"],
        )
        assert isinstance(result, dict)
        assert result["functional"] == "REQUIRED"
