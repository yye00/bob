"""Tests for src/bob/coverage_based_hack_detector.py (feature 8c495c04).

Verifies the coverage-based hack detector:
- Rejects implementations whose passing tests cover less than BOB_MIN_COVERAGE
...
- Reads BOB_MIN_COVERAGE from the environment with a default of 70.
"""
...
from bob.coverage_based_hack_detector import (
    CoverageResult,
    FileCoverageInfo,
    check_coverage,
...
def _make_coverage_json(
    *,
    ...
    filename: str = "src/bob/example.py",
) -> dict:
...
class TestMeasureCoverage:
    ...
    def test_uses_env_threshold_by_default(self, tmp_path: Path):
        with patch.dict(os.environ, {"BOB_MIN_COVERAGE": "90"}):
    ...
    def test_default_threshold_is_70(self, tmp_path: Path):
        env_without_coverage = {k: v for k, v in os.environ.items() if k != "BOB_MIN_COVERAGE"}
