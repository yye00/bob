"""Tests for bob.verifier.feature_test_scoper — verifier pytest scoping.
...
from bob.verifier.feature_test_scoper import (
...
# ---------------------------------------------------------------------------
# Integration: bob.verifier imports scope_pytest_to_feature
# ---------------------------------------------------------------------------


class TestVerifierIntegration:
    def test_importable_from_bob_verifier(self):
        """scope_pytest_to_feature is importable from bob.verifier."""
        from bob.verifier import scope_pytest_to_feature as fn
        assert callable(fn)

    def test_importable_from_feature_test_scoper(self):
        """scope_pytest_to_feature is importable from bob.verifier.feature_test_scoper."""
        from bob.verifier.feature_test_scoper import scope_pytest_to_feature as fn
        assert callable(fn)

    def test_both_imports_are_same_function(self):
        """Both import paths resolve to the same underlying function."""
        from bob.verifier import scope_pytest_to_feature as fn1
        from bob.verifier.feature_test_scoper import scope_pytest_to_feature as fn2
        assert fn1 is fn2
