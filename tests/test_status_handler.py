"""Tests for bob3.status_handler — handle_pending_successor_verify (f77b0d51).

Acceptance criteria:
- File exists: src/bob3/status_handler.py
- Function defined: bob3.status_handler.handle_pending_successor_verify
...
# ---------------------------------------------------------------------------
# AC 2: Function defined — bob3.status_handler.handle_pending_successor_verify
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AC 3: Integration with bob3.run_loop
# ---------------------------------------------------------------------------
...
class TestRunLoopIntegration:
    def test_run_loop_exports_handle_pending_successor_verify(self):
        ...
        assert hasattr(bob3.run_loop, "handle_pending_successor_verify")
        assert callable(bob3.run_loop.handle_pending_successor_verify)

    def test_run_loop_function_delegates(self):
        from bob3.run_loop import handle_pending_successor_verify as rl_handle
        with patch(
...
class TestBehavior:
    ...
    def test_returns_true_when_both_conditions_met(self, tmp_path):
        ...
        src_bob3 = tmp_path / "src" / "bob3"
        src_bob3.mkdir(parents=True)
        (src_bob3 / "enhanced_verification.py").write_text("# verifier patched")
