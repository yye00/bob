...
from bob3.durable_orchestration_loop_temporal_integration import (
    ActivityResult,
...
class TestActivitySpawnSubAgent:
    ...
    async def test_spawn_sub_agent_success(self, feature_id: str) -> None:
        ...
        with patch(
            "bob3.durable_orchestration_loop_temporal_integration._call_spawn_sub_agent",
...
class TestActivityRunVerification:
    ...
    async def test_run_verification_success(self, feature_id: str) -> None:
        ...
        with patch(
            "bob3.durable_orchestration_loop_temporal_integration._call_run_verification",
            new_callable=AsyncMock,
        ) as mock_verify:
...
class TestActivityEmitTelemetry:
    ...
    async def test_emit_telemetry_success(self, feature_id: str) -> None:
        ...
        with patch(
            "bob3.durable_orchestration_loop_temporal_integration._call_emit_telemetry"
        ) as mock_emit:
...
class TestDurableOrchestrationLoop:
    ...
    async def test_run_feature_orchestrates_all_three_activities(
        ...
        with (
            patch("bob3.durable_orchestration_loop_temporal_integration.activity_spawn_sub_agent", mock_spawn),
            patch("bob3.durable_orchestration_loop_temporal_integration.activity_run_verification", mock_verify),
            patch("bob3.durable_orchestration_loop_temporal_integration.activity_emit_telemetry", mock_telemetry),
        ):
...
class TestRunDurableOrchestrationWorkflow:
    ...
    async def test_returns_workflow_state(self, project_id: str) -> None:
        ...
        with patch(
            "bob3.durable_orchestration_loop_temporal_integration.DurableOrchestrationLoop.run_feature",
            mock_run_feature,
        ):
            with patch(
                "bob3.durable_orchestration_loop_temporal_integration._get_ready_features",
                return_value=[],
            ):
