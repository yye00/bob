"""Tests for bob.orchestrator.resume_scan (87f0d6aa).

Verifies:
- resume_scan is importable from bob.orchestrator.resume_scan
...
- It is integrated into bob.orchestrator.run_loop
"""
...
from bob.orchestrator.resume_scan import resume_scan
...
class TestResumeScanImport:
    def test_function_is_importable(self):
        from bob.orchestrator.resume_scan import resume_scan as fn  # noqa: F401

        assert callable(fn)

    def test_accepts_project_id_returns_list(self):
        with patch("bob.orchestrator.resume_scan.db") as mock_db:
...
class TestResumeScanLogging:
    def test_logs_info_for_each_promoted_feature(self, caplog):
        ...
        with patch("bob.orchestrator.resume_scan.db") as mock_db:
            ...
            with caplog.at_level(
                logging.INFO, logger="bob.orchestrator.resume_scan"
            ):
    ...
    def test_logs_debug_on_list_features_failure(self, caplog):
        with patch("bob.orchestrator.resume_scan.db") as mock_db:
            ...
            with caplog.at_level(
                logging.DEBUG, logger="bob.orchestrator.resume_scan"
            ):
...
class TestRunLoopIntegration:
    ...
    def test_resume_scan_module_accessible_from_orchestrator(self):
        from bob.orchestrator import resume_scan as mod  # noqa: F401
