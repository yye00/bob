...
``sweep_orphans`` functions added to ``bob3.mcp_lifecycle``.
...
- Each test redirects the registry file to a tmp path via the
  ``BOB3_MCP_REGISTRY_PATH`` env var so test runs do not interfere
  with the operator's real ``~/.bob3/mcp_registry.json``.
- The orphan sweep test uses a Python child that ``exec``s itself with
  ``argv[0]`` containing ``bob3.memory_mcp`` so the cmdline-based
...
def registry_path(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("BOB3_MCP_REGISTRY_PATH", str(path))
    yield path
...
# A tiny Python child whose argv[0] contains 'bob3.memory_mcp' so the
...
# command line: ``python -c <code> bob3.memory_mcp``. The token then
...
def _spawn_mcp_mimic(
    ...
    """Spawn a process whose cmdline contains ``bob3.memory_mcp``.

    The trailing positional arg ``bob3.memory_mcp`` makes the token
    appear in ``/proc/<pid>/cmdline`` so ``_is_memory_mcp_pid``
    matches it without us having to start the real MCP server.
    """
    return subprocess.Popen(
        [sys.executable, "-c", _MCP_MIMIC_SCRIPT, "bob3.memory_mcp"],
    )
    ...
    def test_sweep_finds_orphan_mcp(self, registry_path):
        ...
        parent_code = textwrap.dedent(
            ...
            child = subprocess.Popen(
                [sys.executable, "-c", {_MCP_MIMIC_SCRIPT!r}, "bob3.memory_mcp"],
