...
class TestLintAcceptanceCriteriaStructuredForms:
    def test_file_exists_ac_passes(self):
        issues = lint_acceptance_criteria("F", ["File exists: src/bob3/foo.py"])
        assert issues == []

    def test_function_defined_ac_passes(self):
        issues = lint_acceptance_criteria("F", ["Function defined: bob3.foo.bar"])
        assert issues == []

    def test_class_defined_ac_passes(self):
        issues = lint_acceptance_criteria("F", ["Class defined: bob3.module.MyClass"])
        assert issues == []
    ...
    def test_integration_ac_passes(self):
        issues = lint_acceptance_criteria("F", ["integration: bob3.cli.plan"])
        assert issues == []
