"""Tests for bob3.behavior_criteria — EARS-style behavior AC sixth grammar.
...
Verifies that:
  ...
  - bob3.verifier exposes verify_behavior_ac (integration)
"""
...
def test_parse_behavior_criteria_returns_none_for_non_behavior():
    ...
    assert parse_behavior_criteria("integration: bob3.verifier") is None
...
def test_verifier_integration():
    """bob3.verifier exposes verify_behavior_ac (integration AC)."""
    from bob3 import verifier
    assert hasattr(verifier, "verify_behavior_ac"), (
        "bob3.verifier must expose verify_behavior_ac for integration"
    )
...
def test_key_example_parametrized(given_val, expected):
    ...
    from bob3.behavior_criteria import key_example
