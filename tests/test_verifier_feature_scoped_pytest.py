"""Tests for bob.verifier.scope_pytest_to_feature — scoped pytest invocation.
...
def test_scope_ignores_non_pytest_acs(tmp_path):
    ...
    acs = [
        "Function defined: bob.verifier.scope_pytest_to_feature",
        "File exists: src/bob/verifier/__init__.py",
        "integration: bob.verifier",
    ]
    result = scope_pytest_to_feature(FEATURE_ID, acs, tmp_path)
    assert result == []
...
def test_scope_pytest_to_feature_is_importable_from_verifier():
    """scope_pytest_to_feature is importable from bob.verifier (integration AC)."""
    from bob.verifier import scope_pytest_to_feature as spf
    assert callable(spf)
