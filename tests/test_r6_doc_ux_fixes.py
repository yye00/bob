...
Covers:
- R6-005: README "Commands" table now lists ``bob show-reviews``.
- R6-006: README env-var table now lists ``BOB_SNAPSHOT_TIMEOUT``;
  conftest no longer snapshots stale env-vars (BOB_WORKSPACE etc.)
...
- R6-008: ``bob run`` and ``bob init`` exit 1 with an actionable message
  when ``node`` or ``claude`` are missing.
- R6-009: README documents the resume-after-interruption flow.
- R6-010: ``using-bob-memory`` SKILL.md tools table includes
...
# ----------------------------------------------------------------
# R6-005: bob show-reviews appears in the README Commands table
# ----------------------------------------------------------------
...
class TestReadmeShowReviewsCommand:
    def test_show_reviews_listed_in_commands_table(self):
        ...
        assert "`bob show-reviews`" in text, (
    ...
    def test_show_reviews_in_commands_section(self):
        ...
        show_reviews_idx = text.find("`bob show-reviews`")
...
# ----------------------------------------------------------------
# R6-006: BOB_SNAPSHOT_TIMEOUT documented; conftest cleanup
# ----------------------------------------------------------------
...
class TestEnvVarDocs:
    def test_snapshot_timeout_documented(self):
        text = README.read_text()
        assert "`BOB_SNAPSHOT_TIMEOUT`" in text, (
            "README env-var table must mention BOB_SNAPSHOT_TIMEOUT (R6-006)"
        )

    def test_snapshot_timeout_doc_describes_fallback(self):
        text = README.read_text()
        assert "BOB_TEST_RUN_TIMEOUT" in text and "300" in text, (
            "BOB_SNAPSHOT_TIMEOUT row must document its fallback chain"
        )
...
class TestConftestEnvSnapshot:
    ...
    def test_conftest_does_not_snapshot_unread_vars(self):
        from tests.conftest import _BOB_ENV_VARS_TO_SNAPSHOT

        # These names appeared in the snapshot list but no module under
        # src/bob/ ever reads them — they were dead state. They must
        # NOT be re-introduced unless code starts honoring them.
        unread = {"BOB_WORKSPACE", "BOB_LOG_DIR", "BOB_LOG_LEVEL"}
        assert unread.isdisjoint(set(_BOB_ENV_VARS_TO_SNAPSHOT)), (
            f"conftest snapshots env-vars that no code reads: "
            f"{unread & set(_BOB_ENV_VARS_TO_SNAPSHOT)}"
        )
    ...
    def test_conftest_keeps_real_env_vars(self):
        ...
        for name in (
            ...
            assert name in _BOB_ENV_VARS_TO_SNAPSHOT, (
...
class TestBootstrapSpecWorkspace:
    ...
    def test_workspace_line_does_not_hardcode_author_path(self):
        ...
        # those are not the path that ``bob plan`` honors.)
...
class TestNodePreflightCheck:
    """``bob run`` / ``bob init`` must surface missing node/claude clearly."""
    ...
    def test_run_exits_when_node_missing(self, monkeypatch):
        ...
        monkeypatch.setattr("bob.cli.shutil.which", fake_which)
...
class TestResumeFlowDocs:
    ...
    def test_readme_describes_resume_command(self):
        ...
        assert "bob run --all" in text
        assert "--feature" in text and "--fresh" in text


# ----------------------------------------------------------------
# R6-010: using-bob-memory SKILL.md mentions delete/demote/candidates
# ----------------------------------------------------------------


class TestUsingMemorySkillToolsTable:
    SKILL = ROOT / "src" / "bob" / "skills" / "using-bob-memory" / "SKILL.md"
...
class TestNoStubsGrepPortability:
    SKILL = ROOT / "src" / "bob" / "skills" / "no-stubs-no-mocks" / "SKILL.md"
...
class TestExecutingFeatureLogIsInjectionSafe:
    ...
    def test_executing_log_message_has_no_raw_newlines(self):
        ...
        logger = logging.getLogger("bob.orchestrator.run_loop")
