"""Tests for worker cache telemetry in bob3.dispatch.
...
class TestSpawnWorkerWithCacheEnv:
    ...
    def test_cache_telemetry_parsed_from_stdout(self, tmp_path):
        ...
        emitted_events = []
        original_emit = __import__("bob3.dispatch", fromlist=["emit_worker_cache_event"]).emit_worker_cache_event
        ...
        with mock.patch("bob3.dispatch.subprocess.run", side_effect=fake_run):
            with mock.patch("bob3.dispatch.emit_worker_cache_event", side_effect=capturing_emit):
                spawn_worker_with_cache(
                    feature, "test prompt", tmp_path, bob3_dir=tmp_path / ".bob3"
                )
