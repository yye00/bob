"""Tests for bob.final_reaper_sweep — sweep_orphans_on_exit (feature 3b5fe995).

AC: File exists: src/bob/final_reaper_sweep.py
AC: Function defined: bob.final_reaper_sweep.sweep_orphans_on_exit
...
class TestModuleAndFunction(unittest.TestCase):
    ...
    def test_final_reaper_sweep_module_importable(self):
        """bob.final_reaper_sweep must be importable."""
        import bob.final_reaper_sweep  # noqa: F401

    def test_sweep_orphans_on_exit_is_callable(self):
        """sweep_orphans_on_exit must be a callable defined in bob.final_reaper_sweep."""
...
class TestSweepOrphansOnExitDelegation(unittest.TestCase):
    """Verify delegation to bob.final_reaper.sweep_orphans_on_exit."""

    def test_delegates_to_final_reaper(self):
        """sweep_orphans_on_exit must delegate to bob.final_reaper.sweep_orphans_on_exit."""
        from bob.final_reaper_sweep import sweep_orphans_on_exit

        with patch("bob.final_reaper_sweep.sweep_orphans_on_exit") as _mock:
...
class TestOrchestratorIntegration(unittest.TestCase):
    """Integration AC: sweep_orphans_on_exit integrates with bob.orchestrator."""

    def test_final_reaper_sweep_importable(self):
        """bob.final_reaper_sweep must be importable with sweep_orphans_on_exit."""
        import bob.final_reaper_sweep as frs
