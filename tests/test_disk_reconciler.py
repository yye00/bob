"""Tests for bob.orchestrator.disk_reconciler (feature 2f69b554).
...
from bob.orchestrator.disk_reconciler import evaluate_ac_against_disk, reconcile_from_disk
...
def _make_db(tmp_path: pathlib.Path) -> pathlib.Path:
    """Create a minimal bob SQLite database in tmp_path."""
...
def test_evaluate_function_defined_passes(tmp_path):
    ...
    passed, _ = evaluate_ac_against_disk(
        "Function defined: bob.mymod.my_function", tmp_path
    )
    assert passed is True
...
def test_evaluate_function_defined_fails_missing(tmp_path):
    ...
    passed, _ = evaluate_ac_against_disk(
        "Function defined: bob.nonexistent.missing_fn", tmp_path
    )
    assert passed is False
...
def test_evaluate_integration_wired(tmp_path):
    ...
    pkg = tmp_path / "src" / "bob" / "mypkg"
    pkg.mkdir(parents=True)
    (pkg / "mymod.py").write_text("def foo(): pass\n")
    caller = tmp_path / "src" / "bob" / "caller.py"
    caller.write_text("from bob.mypkg.mymod import foo\n")
    passed, _ = evaluate_ac_against_disk("integration: bob.mypkg.mymod", tmp_path)
    assert passed is True
...
def test_evaluate_integration_not_imported(tmp_path):
    ...
    """
    pkg = tmp_path / "src" / "bob" / "missing"
    pkg.mkdir(parents=True)
    (pkg / "pkg.py").write_text("x = 1\n")
    # No other file imports it — bare dotted path, no prose connectors
    passed, _ = evaluate_ac_against_disk("integration: bob.missing.pkg", tmp_path)
    assert passed is False
