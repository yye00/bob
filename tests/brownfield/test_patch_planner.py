...
class TestEmitDiffPlan:
    ...
    def test_plan_path_under_bob_features_dir(self, feature_id, scratch_dir, minimal_touches):
        plan_path = emit_diff_plan(feature_id, minimal_touches, workspace=scratch_dir)
        expected = scratch_dir / ".bob" / "features" / feature_id
...
# ---------------------------------------------------------------------------
# plan_diff (alias for AC: Function defined: bob.brownfield.patch_planner.plan_diff)
# ---------------------------------------------------------------------------
...
class TestApplyDiffPlan:
    ...
    def test_backup_created(self, feature_id, scratch_dir, sample_py_file, minimal_touches):
        plan_path = emit_diff_plan(feature_id, minimal_touches, workspace=scratch_dir)
        apply_diff_plan(plan_path, workspace=scratch_dir)
        backup = scratch_dir / ".bob" / "features" / feature_id / "orig" / "src" / "auth" / "login.py"
...
# ---------------------------------------------------------------------------
# Integration: bob.orchestrator references patch_planner
# ---------------------------------------------------------------------------
