"""Tests for bob.brownfield.repo_mapper.run_repomapper_mcp (F-R7-611 / BF-1).
...
class TestRepoMapperModuleExists(unittest.TestCase):
    ...
    def test_module_importable(self):
        import bob.brownfield.repo_mapper  # noqa: F401

    def test_function_run_repomapper_mcp_defined(self):
        from bob.brownfield import repo_mapper
    ...
    def test_repomapper_handle_importable(self):
        from bob.brownfield.repo_mapper import RepoMapperHandle
...
class TestRunRepomapperMcpReturnsHandle(unittest.TestCase):
    ...
    @patch("bob.brownfield.survey.subprocess.Popen")
    def test_returns_repomapper_handle(self, mock_popen):
        from bob.brownfield.repo_mapper import run_repomapper_mcp, RepoMapperHandle
    ...
    def test_handle_has_workspace(self, mock_popen):
        from bob.brownfield.repo_mapper import run_repomapper_mcp
        mock_popen.return_value = self._make_mock_proc()
        workspace = Path("/tmp/test_workspace")
