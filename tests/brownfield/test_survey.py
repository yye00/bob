"""Tests for bob3.brownfield.survey — BF-1 Brownfield Survey/Index."""
...
def test_build_survey_creates_db(repo: Path, tmp_path: Path) -> None:
    """build_survey must create survey.db under .bob3/."""
...
# ---------------------------------------------------------------------------
# CLI integration (AC: integration: bob3.cli)
# ---------------------------------------------------------------------------


def test_cli_survey_command_registered() -> None:
    """bob3 CLI must expose a 'survey' command."""
    from bob3.cli import main

    assert "survey" in main.commands, (
        f"'survey' command not found in bob3 CLI; available: {list(main.commands.keys())}"
    )


def test_cli_survey_build(repo: Path, tmp_path: Path) -> None:
    """bob3 survey <path> must build the index and exit 0."""
    from bob3.cli import main
...
def test_cli_survey_refresh(repo: Path, tmp_path: Path) -> None:
    """bob3 survey --refresh <path> must perform incremental update and exit 0."""
    from bob3.cli import main
