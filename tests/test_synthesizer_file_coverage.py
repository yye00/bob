"""Tests for bob3.synthesizer.emit_file_exists_acs — file-coverage post-synthesis step.

Verifies that:
1. emit_file_exists_acs is importable from bob3.synthesizer
...
from bob3.synthesizer import emit_file_exists_acs


def test_emit_file_exists_acs_is_callable():
    """AC: Function defined: bob3.synthesizer.emit_file_exists_acs"""
    assert callable(emit_file_exists_acs)


def test_emits_file_exists_for_named_py_path():
    """When description names src/bob3/brownfield/survey.py, a File exists: AC is added."""
    description = "This feature lives in src/bob3/brownfield/survey.py and extends the survey module."
    ...
    assert any("File exists: src/bob3/brownfield/survey.py" in ac for ac in result), (
        f"Expected 'File exists: src/bob3/brownfield/survey.py' in result, got {result}"
    )
...
def test_multiple_py_paths_emitted():
    ...
    description = (
        "This feature modifies src/bob3/synthesizer.py and "
        "also touches src/bob3/spec_synthesizer.py."
    )
    criteria = []
    result = emit_file_exists_acs(criteria, description)
    assert any("src/bob3/synthesizer.py" in ac for ac in result), (
        f"Expected File exists: for synthesizer.py in {result}"
    )
    assert any("src/bob3/spec_synthesizer.py" in ac for ac in result), (
        f"Expected File exists: for spec_synthesizer.py in {result}"
    )


def test_survey_py_path_use_case():
    """Regression: src/bob3/brownfield/survey.py is covered even if synthesis derived a different slug."""
    description = (
        "PEAS description. The implementation resides in "
        "src/bob3/brownfield/survey.py which provides the survey index."
    )
    # Simulate: synthesizer produced a different file AC, not the exact path
    criteria = ["File exists: src/bob3/brownfield/survey_index.py", "pytest: tests/test_survey.py"]
    result = emit_file_exists_acs(criteria, description)
