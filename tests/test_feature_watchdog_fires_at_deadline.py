...
async def test_watchdog_cancels_task_at_deadline_with_alive_pid():
    ...
    with (
        ...
        with patch("bob3.orchestrator.feature_watchdog.asyncio.sleep", side_effect=instant_sleep):
...
async def test_watchdog_cancelled_before_deadline_raises_cancelled_error():
    ...
    with patch("bob3.orchestrator.feature_watchdog.asyncio.sleep", side_effect=cancel_during_sleep):
...
async def test_watchdog_does_not_cancel_already_done_task():
    ...
    with (
        patch("bob3.orchestrator.feature_watchdog._pid_is_alive", return_value=True),
        patch("bob3.orchestrator.feature_watchdog.os.kill"),
        patch("bob3.orchestrator.feature_watchdog.asyncio.sleep", side_effect=instant_sleep),
    ):
        await cancel_subagent_at_deadline(
            pid=12345,
