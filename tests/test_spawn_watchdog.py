...
def test_spawn_watchdog_reads_timeout_from_env(tmp_path, monkeypatch):
    """BOB_CRITERION_EXEC_TIMEOUT overrides the default when not supplied."""
    monkeypatch.setenv("BOB_CRITERION_EXEC_TIMEOUT", "7200")
    proc = _FakeProc()
...
def test_spawn_watchdog_default_timeout_when_env_unset(monkeypatch):
    monkeypatch.delenv("BOB_CRITERION_EXEC_TIMEOUT", raising=False)
    proc = _FakeProc()
    w = SpawnWatchdog(proc=proc, feature_id="feat-1")
...
def test_heartbeat_written_to_progress_jsonl(tmp_path):
    ...
    progress_path = tmp_path / ".bob" / "progress.jsonl"
    proc = _FakeProc()
...
def test_default_timeout_integrates_with_env_var(tmp_path, monkeypatch):
    """When BOB_CRITERION_EXEC_TIMEOUT is set, SpawnWatchdog uses it as default timeout."""
    monkeypatch.setenv("BOB_CRITERION_EXEC_TIMEOUT", "9999")
    proc = _FakeProc()
