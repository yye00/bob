...
# Ensure this workspace's src is on PYTHONPATH so imports use the local bob.
...
RUN_LOOP="src/bob/orchestrator/run_loop.py"
DISK_RECONCILER="src/bob/orchestrator/disk_reconciler.py"
...
def test_disk_reconciler_promotes_avoids_failed():
    ...
    fake_feature.acceptance_criteria = '["file_exists: src/bob/run_loop.py"]'

    with patch("bob.orchestrator.run_loop.db") as mock_db, \
         patch("bob.orchestrator.run_loop.find_subagent_pid_for_feature") as mock_pid, \
...
def test_final_sweep_summary_emitted():
    ...
    run_loop_logger = logging.getLogger("bob.orchestrator.run_loop")
