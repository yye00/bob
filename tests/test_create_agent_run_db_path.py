...
Verifies that bob.database.create_agent_run writes to the explicitly
...
from bob.database import create_agent_run
...
def test_create_agent_run_importable_from_database():
    """bob.database.create_agent_run must be importable and callable."""
...
def test_create_agent_run_env_var_respected_when_no_db_path(monkeypatch):
    """When db_path is omitted, BOB_DATABASE_PATH env var must be honored."""
    with tempfile.TemporaryDirectory() as td:
        ...
        assert row is not None, "Row must land in the BOB_DATABASE_PATH db"


def test_create_agent_run_db_path_overrides_env_var(monkeypatch):
    """Explicit db_path must override BOB_DATABASE_PATH."""
    with tempfile.TemporaryDirectory() as td:
...
def test_synthesize_for_feature_importable_from_orchestrator():
    """bob.orchestrator.synthesize_for_feature must be importable."""
    import bob.orchestrator as orch
    assert hasattr(orch, "synthesize_for_feature"), (
        "synthesize_for_feature must be accessible via bob.orchestrator"
    )
