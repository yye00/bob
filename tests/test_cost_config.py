...
class TestResolveMaxCostUsdDefault:
    """Default behaviour when BOB3_MAX_COST_USD is not set."""

    def test_no_env_var_returns_unlimited(self):
        from bob3.cost_config import resolve_max_cost_usd
...
class TestResolveMaxCostUsdWithEnv:
    """BOB3_MAX_COST_USD is read and returned when set to a valid number."""

    def test_valid_integer_string(self, monkeypatch):
        monkeypatch.setenv("BOB3_MAX_COST_USD", "200")
        from bob3.cost_config import resolve_max_cost_usd

        assert resolve_max_cost_usd() == pytest.approx(200.0)

    def test_valid_float_string(self, monkeypatch):
        monkeypatch.setenv("BOB3_MAX_COST_USD", "750.50")
        from bob3.cost_config import resolve_max_cost_usd
    ...
    def test_large_value_is_honoured(self, monkeypatch):
        monkeypatch.setenv("BOB3_MAX_COST_USD", "5000000")
        from bob3.cost_config import resolve_max_cost_usd
...
class TestResolveMaxCostUsdMalformedEnv:
    """Malformed BOB3_MAX_COST_USD falls back to unlimited, never raises."""
    ...
    def test_malformed_never_returns_zero(self, monkeypatch):
        ...
        monkeypatch.setenv("BOB3_MAX_COST_USD", "garbage")
        from bob3.cost_config import resolve_max_cost_usd
