...
Verifies the two required functions:
  - bob3.run_loop.is_subagent_startup_crash
  - bob3.run_loop.get_exempt_count
...
from bob3.run_loop import get_exempt_count, is_subagent_startup_crash
...
class TestGetExemptCount:
    ...
    def test_env_var_used_when_no_explicit_dir(self, tmp_path: Path, monkeypatch) -> None:
        """BOB3_STARTUP_EXEMPT_DIR env var used when sidecar_dir is None."""
        feature_id = "test-env-feature"
        sidecar = tmp_path / f"{feature_id}.count"
        sidecar.write_text("5")
        monkeypatch.setenv("BOB3_STARTUP_EXEMPT_DIR", str(tmp_path))
