"""Tests for bob.synthesizer file-exists AC emission for .py paths in descriptions.
...
from bob.synthesizer import emit_file_exists_acs, should_emit_function_ac


class TestEmitFileExistsAcs:
    def test_emits_file_exists_for_py_path_with_directory(self):
        desc = "Implements src/bob/brownfield/survey.py with survey functionality."
        criteria, added = emit_file_exists_acs([], desc)
        assert any("src/bob/brownfield/survey.py" in ac for ac in criteria)
        assert "src/bob/brownfield/survey.py" in added

    def test_does_not_duplicate_already_covered_path(self):
        desc = "Implements src/bob/brownfield/survey.py."
    ...
    def test_emits_multiple_paths(self):
        desc = "Adds src/bob/scorer.py and tools/spec_quality_score.py."
        criteria, added = emit_file_exists_acs([], desc)
        paths = set(added)
        assert "src/bob/scorer.py" in paths
    ...
    def test_returns_augmented_criteria_list(self):
        desc = "Modifies src/bob/spec_synthesizer.py."
