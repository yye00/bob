"""Tests for per-worker settings.json generation in bob3.dispatch.
...
- Settings file is created under .bob3/features/<feature_id>/settings.json
"""
...
class TestWriteFeatureSettings:
    ...
    def test_extra_allow_appended(self, tmp_path):
        ...
        path = write_feature_settings(
            feature, bob3_dir=tmp_path, extra_allow=["Bash(npm:*)"]
        )
        data = json.loads(path.read_text())
    ...
    def test_different_features_get_different_paths(self, tmp_path):
        ...
        path1 = write_feature_settings(f1, bob3_dir=tmp_path)
        path2 = write_feature_settings(f2, bob3_dir=tmp_path)
        assert path1 != path2
...
class TestSpawnWorkerWithCacheSettingsFlag:
    def test_settings_flag_passed_to_subprocess(self, tmp_path):
        ...
        with mock.patch("bob3.dispatch.subprocess.run", side_effect=fake_run):
            spawn_worker_with_cache(
                feature, "test prompt", tmp_path, bob3_dir=tmp_path / ".bob3"
            )
    ...
    def test_worker_md_written_before_subprocess(self, tmp_path):
        ...
        def fake_run(cmd, *, cwd, env, capture_output, text, timeout):
            ...
            md_path = bob3_dir / "features" / "WORKER.md"
        ...
        with mock.patch("bob3.dispatch.subprocess.run", side_effect=fake_run):
            spawn_worker_with_cache(
                feature, "test prompt", tmp_path, bob3_dir=bob3_dir
            )
