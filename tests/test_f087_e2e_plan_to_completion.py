...
End-to-end integration test that exercises the full Bob workflow from
...
Step 2: Run bob plan test-spec.yaml --create
Step 3: Verify 3 features created in database
Step 4: Run bob run in loop until all features complete
...
def tmp_db(tmp_path):
    ...
    with patch("bob.db.get_database_path", return_value=db_path), \
         patch("bob.cli.get_database_path", return_value=db_path):
        yield db_path
...
class TestE2EPlanFromSpec:
    ...
    def test_plan_create_inserts_three_features(self, tmp_db, spec_file):
        """bob plan --create inserts 3 features into the database."""
    ...
    def test_plan_create_establishes_dependencies(self, tmp_db, spec_file):
        """bob plan --create creates correct dependency relationships."""
        with patch("bob.db.get_database_path", return_value=tmp_db):
            project = create_project(
                name="e2e-test-project",
                workspace_path="/tmp/e2e-project",
...
class TestE2ERunToCompletion:
    ...
    async def test_cascade_unlocks_dependent_features(self, tmp_db, workspace, spec_file):
        ...
        with patch("bob.db.get_database_path", return_value=tmp_db):
            ...
            from bob.orchestrator.run_loop import cascade_update_dependents
