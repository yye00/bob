"""Tests for AC-test generation via bob.test_writer.generate_failing_tests.
...
bob.test_writer (re-exported from bob.orchestrator.test_writer_agent).
"""
...
class TestGenerateFailingTestsACCoverage:
    def test_one_emitted_test_per_ac(self, tmp_path):
        ...
        acs = ["File exists: src/a.py", "Function defined: bob.mod.fn", "pytest: tests/t.py"]
    ...
    def test_emitted_test_files_exist_on_disk(self, tmp_path):
        ...
        acs = ["File exists: src/x.py", "Function defined: bob.m.f"]
    ...
    def test_bijection_is_satisfied_for_well_formed_acs(self, tmp_path):
        ...
        acs = ["File exists: src/a.py", "Function defined: bob.m.f"]
...
class TestGenerateFailingTestsTemplateContent:
    def test_emitted_test_contains_pytest_fail(self, tmp_path):
        ...
        Uses a non-structural AC (pytest:) to get the placeholder template which
        always emits pytest.fail. Structural ACs (File exists:, Function defined:)
        generate real assertion tests instead of pytest.fail placeholders.
        """
        result = generate_failing_tests("feat-content-check", ["pytest: tests/test_x.py"], workspace=tmp_path)
        for et in result["emitted"]:
    ...
    def test_emitted_test_imports_pytest(self, tmp_path):
        ...
        Uses a non-structural AC (pytest:) to get the placeholder template which
        always imports pytest. Structural ACs (File exists:, Function defined:)
        generate real assertion tests using importlib/pathlib instead.
        """
        result = generate_failing_tests("feat-pytest-import", ["pytest: tests/test_y.py"], workspace=tmp_path)
        for et in result["emitted"]:
            content = et.test_path.read_text(encoding="utf-8")
...
class TestGenerateFailingTestsAcIdNaming:
    ...
    def test_emitted_ac_text_matches_input(self, tmp_path):
        ...
        ac = "Function defined: bob.mymod.myfunc"
