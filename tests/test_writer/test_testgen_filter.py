"""Tests for the TestGen-LLM triple filter via bob3.test_writer.

Validates the triple filter (compile / fails-on-stub / coverage heuristic)
as exposed through bob3.test_writer.triple_filter_one and the underlying
bob3.orchestrator.test_writer_agent.triple_filter.
"""
...
class TestTripleFilterOne:
    ...
    def test_accepted_test_has_all_checks_true(self, tmp_path):
        ...
        et = emit_failing_test("feat-all-true", 0, "Function defined: bob3.m.f", workspace=tmp_path)
        result = triple_filter_one(et, workspace=tmp_path)
