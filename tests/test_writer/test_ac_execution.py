...
from bob3.test_writer import generate_failing_test


class TestGenerateFailingTestFunction:
    """Tests for bob3.test_writer.generate_failing_test (singular)."""
    ...
    def test_emitted_file_is_valid_python(self, tmp_path):
        result = generate_failing_test("feat-exec-syntax", 0, "Function defined: bob3.m.f", workspace=tmp_path)
...
class TestEmittedTestExecution:
    ...
    def test_generate_failing_tests_pipeline_runs(self, tmp_path):
        out = generate_failing_tests(
            "feat-exec-pipeline",
            ["File exists: src/a.py", "Function defined: bob3.m.g"],
            workspace=tmp_path,
        )
