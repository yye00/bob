...
criteria are satisfied:
  - src/bob/test_writer.py exists
  - bob.test_writer.TestWriter is importable and callable
  - integration with bob.orchestrator (run_loop imports emit_failing_tests)
"""
...
class TestFileExists:
    def test_test_writer_module_file_exists(self):
        """AC: File exists: src/bob/test_writer.py"""
        module_file = Path(__file__).parent.parent.parent / "src" / "bob" / "test_writer.py"
        assert module_file.exists(), f"Expected {module_file} to exist"


class TestWriterClass:
    def test_testwriter_class_importable(self):
        """AC: Function defined: bob.test_writer.TestWriter"""
        from bob.test_writer import TestWriter  # noqa: F401 — import is the assertion

    def test_testwriter_class_has_run_method(self):
        """TestWriter.run is the main entry point."""
        from bob.test_writer import TestWriter
        tw = TestWriter()
    ...
    def test_testwriter_run_emits_tests(self, tmp_path):
        ...
        result = tw.run(
            ...
            ["File exists: src/bob/test_writer.py"],
            workspace=tmp_path,
        )
...
class TestOrchestratorIntegration:
    def test_run_loop_imports_emit_failing_tests(self):
        """AC: integration: bob.orchestrator — run_loop imports emit_failing_tests."""
        import bob.orchestrator.run_loop as run_loop_module
    ...
    def test_test_writer_agent_module_importable(self):
        """bob.orchestrator.test_writer_agent must be importable."""
        mod = importlib.import_module("bob.orchestrator.test_writer_agent")
    ...
    def test_test_writer_module_exports_all_public_api(self):
        """bob.test_writer must re-export the full public API."""
        import bob.test_writer as tw_mod
        required = [
            "TestWriter",
            "emit_failing_test",
            "triple_filter_one",
        ...
        for name in required:
            assert hasattr(tw_mod, name), f"bob.test_writer missing: {name}"
