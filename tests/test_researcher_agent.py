...
Verifies:
  - src/bob3/agents/roles.py defines RESEARCHER with correct attributes
  - bob3.agents.roles.researcher function/constant is importable
  - src/bob3/agents/prompts/researcher.txt exists and contains required content
...
PROMPTS_DIR = Path(__file__).parent.parent / "src" / "bob3" / "agents" / "prompts"
RESEARCHER_PROMPT_FILE = PROMPTS_DIR / "researcher.txt"


class TestResearcherRoleFile:
    def test_roles_file_exists(self):
        roles_file = Path(__file__).parent.parent / "src" / "bob3" / "agents" / "roles.py"
