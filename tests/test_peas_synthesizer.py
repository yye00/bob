"""Tests for bob3.peas_synthesizer — the canonical PEAS pipeline module.

Verifies that parse_peas_markdown and synthesize_features are importable
from bob3.peas_synthesizer and behave correctly.
"""
...
from bob3.peas_synthesizer import (
    parse_peas_markdown,
    synthesize_features,
    emit_stub_features,
    TBD_PLACEHOLDER,
...
class TestParsePeasMarkdown:
    def test_importable_from_peas_synthesizer(self):
        from bob3.peas_synthesizer import parse_peas_markdown as fn
        assert callable(fn)
...
class TestSynthesizeFeatures:
    def test_importable_from_peas_synthesizer(self):
        from bob3.peas_synthesizer import synthesize_features as fn
        assert callable(fn)
    ...
    def test_existing_real_acs_unchanged(self):
        real_ac = ["File exists: src/bob3/mymodule.py", "pytest: tests/test_mymodule.py"]
        stubs = [
            {
                "key": "F-R7-002",
