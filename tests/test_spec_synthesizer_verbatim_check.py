...
Verifies that ``bob3.spec_synthesizer.emit_function_defined_ac`` only emits
...
from bob3.spec_synthesizer import emit_function_defined_ac
...
class TestEmitFunctionDefinedAcVerbatimCheck:
    ...
    def test_returns_ac_when_symbol_appears_verbatim_with_backticks(self):
        result = emit_function_defined_ac(
            "bob3.spec_synthesizer",
            "emit_function_defined_ac",
            "Callers use `emit_function_defined_ac` to gate AC emission.",
        )
        assert result == "Function defined: bob3.spec_synthesizer.emit_function_defined_ac"
    ...
    def test_returns_ac_for_exact_boundary_match(self):
        ...
        assert result == "Function defined: bob3.scorer.compute_score"
    ...
    def test_return_type_is_str_or_none(self):
        present = emit_function_defined_ac("bob3.mod", "foo", "foo is defined here")
        absent = emit_function_defined_ac("bob3.mod", "bar", "unrelated description")
    ...
    def test_module_and_symbol_in_returned_string(self):
        ...
        assert "bob3.mymodule" in result
...
class TestEmitFunctionDefinedAcErrorPaths:
    ...
    def test_empty_symbol_raises_value_error(self):
        with pytest.raises(ValueError):
            emit_function_defined_ac("bob3.mod", "", "prose")
    ...
    def test_none_symbol_raises_type_error(self):
        with pytest.raises(TypeError):
            emit_function_defined_ac("bob3.mod", None, "prose")  # type: ignore[arg-type]

    def test_none_description_raises_type_error(self):
        with pytest.raises(TypeError):
            emit_function_defined_ac("bob3.mod", "my_func", None)  # type: ignore[arg-type]
...
class TestEmitFunctionDefinedAcIntegration:
    """Integration: emit_function_defined_ac is importable from bob3.spec_synthesizer."""

    def test_function_importable(self):
        from bob3.spec_synthesizer import emit_function_defined_ac as fn
        assert callable(fn)
    ...
    def test_real_world_case_invented_name_returns_none(self):
        ...
        result = emit_function_defined_ac("bob3.reaper", "apply_exponential_backoff", prose)
        assert result is None, (
