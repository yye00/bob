...
Verifies that bob3.orchestrator exposes promote_gate_blocked_features and that
...
def test_orchestrator_exports_promote_gate_blocked_features():
    """bob3.orchestrator must expose promote_gate_blocked_features."""
    mod = importlib.import_module("bob3.orchestrator")
    assert hasattr(mod, "promote_gate_blocked_features"), (
        "bob3.orchestrator must export promote_gate_blocked_features"
    )
...
def test_promote_gate_blocked_features_raises_on_invalid_feature_id():
    ...
    from bob3.orchestrator import promote_gate_blocked_features
    import bob3.gate_resynth as gr
...
def test_promote_gate_blocked_features_records_attempt():
    ...
    import bob3.gate_resynth as gr
    from bob3.orchestrator import promote_gate_blocked_features, is_already_resynthesized
...
def test_is_already_resynthesized_exported():
    """bob3.orchestrator must also expose is_already_resynthesized."""
    mod = importlib.import_module("bob3.orchestrator")
    assert hasattr(mod, "is_already_resynthesized"), (
        "bob3.orchestrator must export is_already_resynthesized"
    )
    assert callable(mod.is_already_resynthesized)


def test_orchestrator_integration_imports():
    """All gate-blocking related names must be importable from bob3.orchestrator."""
    mod = importlib.import_module("bob3.orchestrator")
    for name in ("promote_gate_blocked_features", "is_already_resynthesized",
                 "resynthesize_gate_blocked_features"):
        assert hasattr(mod, name), f"bob3.orchestrator must export {name!r}"
