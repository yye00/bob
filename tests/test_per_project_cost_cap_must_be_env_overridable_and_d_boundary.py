...
Project.max_cost_usd with edge-case BOB_MAX_COST_USD values.
"""
...
class TestResolveMaxCostUsdBoundary:
    ...
    def test_no_env_var_returns_unlimited(self):
        """Absent BOB_MAX_COST_USD → 1_000_000.0, does not raise."""
        from bob.per_project_cost_cap_must_env_overridable_default import resolve_max_cost_usd

        result = resolve_max_cost_usd()
        assert result == 1_000_000.0

    def test_empty_string_env_var_returns_unlimited(self, monkeypatch):
        """Empty BOB_MAX_COST_USD → 1_000_000.0, does not raise."""
    ...
    def test_zero_env_var_returns_zero(self, monkeypatch):
        """BOB_MAX_COST_USD=0 → 0.0, does not raise (explicit user budget of 0)."""
    ...
    def test_minimum_positive_env_var_returns_that_value(self, monkeypatch):
        """Very small positive BOB_MAX_COST_USD (0.01) → 0.01, does not raise."""
    ...
    def test_negative_env_var_clamped_to_zero(self, monkeypatch):
        """Negative BOB_MAX_COST_USD is clamped to 0.0, does not raise."""
        monkeypatch.setenv("BOB_MAX_COST_USD", "-100")
        from bob.per_project_cost_cap_must_env_overridable_default import resolve_max_cost_usd
    ...
    def test_whitespace_only_env_var_returns_unlimited(self, monkeypatch):
        """Whitespace-only BOB_MAX_COST_USD → unlimited (treats as empty), does not raise."""
        monkeypatch.setenv("BOB_MAX_COST_USD", "   ")
        from bob.per_project_cost_cap_must_env_overridable_default import resolve_max_cost_usd
...
class TestPerProjectCostCapEntrypointBoundary:
    ...
    def test_entrypoint_empty_env_returns_ok(self, monkeypatch):
        """Entry-point with empty BOB_MAX_COST_USD returns ok=True, does not raise."""
        monkeypatch.setenv("BOB_MAX_COST_USD", "")
        from bob.per_project_cost_cap_must_env_overridable_default import (
            per_project_cost_cap_must_env_overridable_default,
        )
    ...
    def test_entrypoint_zero_env_returns_ok(self, monkeypatch):
        """Entry-point with BOB_MAX_COST_USD=0 returns ok=True, does not raise."""
        monkeypatch.setenv("BOB_MAX_COST_USD", "0")
        from bob.per_project_cost_cap_must_env_overridable_default import (
            per_project_cost_cap_must_env_overridable_default,
        )
