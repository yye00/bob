"""Tests for bob3.spec_findings_writer — atomic write and corruption recovery.
...
from bob3.spec_findings_writer import load_with_corruption_recovery, write_atomic
...
class TestWriteAtomic:
    ...
    def test_rename_failure_preserves_original(self, tmp_path):
        ...
        import bob3.spec_findings_writer as sfw
...
class TestLoadWithCorruptionRecovery:
    ...
    def test_corrupt_yaml_logs_structured_event(self, tmp_path, caplog):
        p = tmp_path / "spec_findings.yaml"
        p.write_text(": broken\n", encoding="utf-8")
        with caplog.at_level(logging.ERROR, logger="bob3.spec_findings_writer"):
            load_with_corruption_recovery(p)
