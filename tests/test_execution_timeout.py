"""Tests for bob.execution_timeout — per-feature hard wall-clock timeout.
...
import bob.execution_timeout as et
...
def test_module_importable():
    mod = importlib.import_module("bob.execution_timeout")
...
def test_resolve_reads_env_var(monkeypatch):
    monkeypatch.setenv("BOB_FEATURE_TIMEOUT_SECONDS", "3600")
...
# ---------------------------------------------------------------------------
# Integration: bob.orchestrator exposes enforce_feature_timeout
# ---------------------------------------------------------------------------

def test_orchestrator_integration():
    import bob.orchestrator as orch
    assert hasattr(orch, "enforce_feature_timeout"), (
        "bob.orchestrator must expose enforce_feature_timeout (integration AC)"
    )
...
async def test_timeout_emits_warning_log(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="bob.execution_timeout"):
