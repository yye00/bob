...
from bob3.container_runner import ContainerResult, run_in_container
...
class TestRunInContainerSignature:
    ...
    def test_function_exists(self):
        from bob3.container_runner import run_in_container
...
class TestDockerUnavailableFallback:
    ...
    def test_fallback_logs_warning(self, tmp_path, caplog):
        import logging
        with patch("bob3.container_runner._docker_available", return_value=False):
            with caplog.at_level(logging.WARNING, logger="bob3.container_runner"):
...
class TestDockerAvailableExecution:
    ...
    def test_runs_in_container_sets_flag(self, tmp_path):
        ...
        with patch("bob3.container_runner._docker_available", return_value=True):
            with patch("bob3.container_runner._run_docker", return_value=mock_result):
                result = run_in_container(["echo", "hi"], image="python:3.11-slim", workspace=tmp_path)
    ...
    def test_passes_image_to_docker(self, tmp_path):
        ...
        with patch("bob3.container_runner._docker_available", return_value=True):
            with patch("bob3.container_runner._run_docker", return_value=mock_result) as mock_docker:
                run_in_container(["echo", "x"], image="python:3.11-slim", workspace=tmp_path)
...
class TestDockerAvailableCheck:
    ...
    def test_docker_available_returns_bool(self):
        from bob3.container_runner import _docker_available
        result = _docker_available()
        assert isinstance(result, bool)

    def test_docker_unavailable_when_not_found(self):
        from bob3.container_runner import _docker_available
        with patch("bob3.container_runner.shutil.which", return_value=None):
            assert _docker_available() is False

    def test_docker_available_when_found(self):
        from bob3.container_runner import _docker_available
        with patch("bob3.container_runner.shutil.which", return_value="/usr/bin/docker"):
...
class TestRunDockerInternal:
    ...
    def test_uses_docker_run(self, tmp_path):
        from bob3.container_runner import _run_docker
        with patch("bob3.container_runner.subprocess.run") as mock_run:
