...
def _create_simple_pdf(path: pathlib.Path, text: str = "Hello, Bob3!") -> None:
...
class TestPdfUtilsFileExists:
    ...
    def test_pdf_utils_file_exists(self):
        path = WORKSPACE / "src" / "bob3" / "pdf_utils.py"
        assert path.exists(), "src/bob3/pdf_utils.py must exist"

    def test_pdf_utils_importable(self):
        import bob3.pdf_utils  # noqa: F401

    def test_pdf_utils_imports_fitz(self):
        import bob3.pdf_utils
        import inspect

        source = inspect.getsource(bob3.pdf_utils)
...
class TestPDFContent:
    ...
    def test_pdfcontent_importable(self):
        from bob3.pdf_utils import PDFContent
...
class TestValidatePdf:
    ...
    def test_validate_pdf_importable(self):
        from bob3.pdf_utils import validate_pdf
...
class TestExtractPdfText:
    ...
    def test_extract_returns_pdfcontent(self, tmp_path):
        from bob3.pdf_utils import PDFContent, extract_pdf_text
...
class TestChunkPdfForContext:
    ...
    def test_chunk_importable(self):
        from bob3.pdf_utils import chunk_pdf_for_context

        assert callable(chunk_pdf_for_context)

    def test_chunk_returns_list_of_strings(self, tmp_path):
        from bob3.pdf_utils import PDFContent, chunk_pdf_for_context
