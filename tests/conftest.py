...
_BOB3_ENV_VARS_TO_SNAPSHOT = (
    ...
    "BOB3_EVALUATOR_ENABLED",
    "BOB3_EVALUATOR_TIMEOUT_SECONDS",
    # Frozen-registry mode
    "BOB3_FROZEN_REGISTRY",
...
def _reset_module_state():
    ...
    # ``BOB3_EVALUATOR_ENABLED=0`` so pre-existing tests behave exactly
    # as they did before the evaluator was wired in. Tests that need
    # the evaluator to fire (test_evaluator_agent.py) mock
    # ``spawn_evaluator_agent`` directly OR set ``BOB3_EVALUATOR_ENABLED=1``.
    if "BOB3_EVALUATOR_ENABLED" not in os.environ:
        os.environ["BOB3_EVALUATOR_ENABLED"] = "0"
    ...
    if "BOB3_COST_PROJECTION_GATE" not in os.environ:
        os.environ["BOB3_COST_PROJECTION_GATE"] = "0"

    yield

    # Reset module-level state
    try:
        import bob3.registry_transfer as _rt
        _rt._FROZEN = False
    except ImportError:
        pass
