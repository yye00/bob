"""Tests for bob.synthesis.deterministic_fallback module.

AC: pytest: tests/test_deterministic_fallback.py
AC: integration: bob.synthesis

Verifies that:
- ensure_boundary_and_error_coverage is importable from bob.synthesis.deterministic_fallback
...
- The integration with bob.synthesis package is intact
"""
...
class TestEnsureBoundaryAndErrorCoverageImport:
    ...
    def test_importable_from_synthesis_deterministic_fallback(self):
        from bob.synthesis.deterministic_fallback import ensure_boundary_and_error_coverage as fn
        assert callable(fn)
...
class TestIntegrationBobSynthesis:
    """Integration: bob.synthesis package must expose the function."""

    def test_importable_from_bob_synthesis(self):
        """bob.synthesis.deterministic_fallback is a real sub-module."""
        import importlib
        mod = importlib.import_module("bob.synthesis.deterministic_fallback")
    ...
    def test_function_delegates_correctly(self):
        ...
        from bob.spec_synthesizer import _ensure_boundary_and_error_coverage as ref
        criteria = ["File exists: src/bob/thing.py"]
        title = "thing feature"
