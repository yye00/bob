"""Tests for bob3.mutation_verifier.run_mutation_testing.

Acceptance criteria:
- File exists: src/bob3/mutation_verifier.py
- Function defined: bob3.mutation_verifier.run_mutation_testing
...
import bob3.mutation_verifier as mod
from bob3.mutation_verifier import (
...
def test_module_importable():
    """Module bob3.mutation_verifier imports without error."""
...
def test_passes_when_score_above_threshold(tmp_path):
    ...
    with patch(
        "bob3.mutation_verifier.run_mutation_test",
...
def test_fails_when_score_below_threshold(tmp_path):
    ...
    with patch(
        ...
        "bob3.mutation_verifier.persist_surviving_mutants"
    ) as mock_persist:
        result = run_mutation_testing(
            feature_id="feat-fail",
            src_files=["src/bob3/foo.py"],
...
def test_custom_threshold_is_respected(tmp_path):
    ...
    with patch(
        "bob3.mutation_verifier.run_mutation_test",
        return_value=report,
    ), patch("bob3.mutation_verifier.persist_surviving_mutants"):
        result = run_mutation_testing(
            feature_id="feat-custom",
            src_files=["src/bob3/foo.py"],
            test_dir=str(tmp_path),
...
# ---------------------------------------------------------------------------
# Integration: bob3.orchestrator imports run_mutation_testing
# ---------------------------------------------------------------------------


def test_orchestrator_integration():
    """bob3.orchestrator exposes run_mutation_testing (integration AC)."""
    import bob3.orchestrator as orch

    assert hasattr(orch, "run_mutation_testing"), (
        "bob3.orchestrator must expose run_mutation_testing "
        "(add: from bob3.mutation_verifier import run_mutation_testing)"
    )
