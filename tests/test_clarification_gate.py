"""Tests for bob3.clarification_gate.
...
from bob3.clarification_gate import (
...
class TestGenerateCandidateStubs:
    ...
    def test_non_function_ac_returns_empty(self):
        result = generate_candidate_stubs(["File exists: src/bob3/clarification_gate.py"])
        assert result == []

    def test_function_ac_returns_n_stubs(self):
        acs = ["Function defined: bob3.clarification_gate.generate_candidate_stubs"]
...
class TestClarificationGatePipeline:
    def test_pipeline_ci_mode_spec_needs_human(self):
        ...
        acs = [
            "Function defined: bob3.clarification_gate.generate_candidate_stubs",
            "Function defined: bob3.clarification_gate.compute_disagreement_score",
            "Function defined: bob3.clarification_gate.build_clarification_questions",
        ]
    ...
    def test_pipeline_no_function_acs_all_clear(self):
        ...
        acs = ["File exists: src/bob3/clarification_gate.py"]
        stubs = generate_candidate_stubs(acs)
        slots = compute_disagreement_score(stubs)
