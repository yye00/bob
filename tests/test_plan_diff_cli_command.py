"""Tests: bob plan diff <feature_id> exits 0 and stdout contains unified-diff marker (F-9792cc6f).

Acceptance criterion:
    pytest: tests/test_plan_diff_cli_command.py asserts running
    "bob plan diff F-R7-463" exits 0 and stdout contains "---" unified-diff marker
"""
...
class TestPlanDiffCliCommand:
    """bob plan diff subcommand produces unified-diff output."""
    ...
    def test_diff_plan_vs_spec_returns_unified_diff_marker(self, tmp_path):
        ...
        from bob.orchestrator.plan_gate import write_plan_artifact, diff_plan_vs_spec
