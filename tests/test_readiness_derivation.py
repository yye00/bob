...
# ---------------------------------------------------------------------------
# Tests for bob3.readiness.derive_readiness_score
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Tests for bob3.run_loop.decay_confidence_components
# ---------------------------------------------------------------------------
...
class TestDecayConfidenceComponents:
    ...
    def test_function_exists_in_run_loop(self):
        ...
        assert hasattr(run_loop, "decay_confidence_components"), (
            "bob3.run_loop must export decay_confidence_components as a public function"
        )
    ...
    def test_decay_reduces_each_component(self, tmp_path, monkeypatch):
        ...
        import pathlib
        schema_path = pathlib.Path(__file__).parents[1] / "src" / "bob3" / "schema.sql"
        ...
        monkeypatch.setenv("BOB3_CONFIDENCE_DECAY_PER_FAILURE", "0.15")
        from bob3.orchestrator.run_loop import decay_confidence_components
