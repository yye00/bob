"""Error-path tests for bob3.synthesizer transient-upstream retry behavior.
...
class TestSynthesizeForFeatureErrorPaths:
    ...
    async def test_invalid_max_attempts_env_var_falls_back_to_default(self):
        """Invalid BOB3_SYNTH_MAX_ATTEMPTS value falls back gracefully, does not crash."""
...
class TestRetryDoesNotSilentlySucceed:
    ...
    async def test_unparseable_response_returns_none_not_empty_list(self):
        ...
        with patch("bob3.spec_synthesizer._llm_spawn_synthesizer", side_effect=unparseable_response):
