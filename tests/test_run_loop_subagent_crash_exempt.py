...
- check_worktree_artifacts counts Python files under .worktrees/hotfix-*/src/bob/
"""
...
class TestCheckWorktreeArtifacts:
    """check_worktree_artifacts counts .py files under .worktrees/hotfix-*/src/bob/."""
    ...
    def test_hotfix_worktree_with_py_files_counted(self, tmp_path: Path) -> None:
        bob_src = tmp_path / ".worktrees" / "hotfix-abc" / "src" / "bob"
        bob_src.mkdir(parents=True)
        (bob_src / "module.py").write_text("x = 1")
        (bob_src / "helper.py").write_text("y = 2")
        result = check_worktree_artifacts(bob_root=str(tmp_path))
        assert result == 2
    ...
    def test_non_hotfix_worktree_not_counted(self, tmp_path: Path) -> None:
        ...
        worktrees.mkdir()
        other = worktrees / "feature-branch" / "src" / "bob"
    ...
    def test_multiple_hotfix_worktrees_all_counted(self, tmp_path: Path) -> None:
        for wt in ("hotfix-1", "hotfix-2", "hotfix-abc"):
            d = tmp_path / ".worktrees" / wt / "src" / "bob"
            d.mkdir(parents=True)
            (d / "file.py").write_text("x = 1")
        result = check_worktree_artifacts(bob_root=str(tmp_path))
        assert result == 3

    def test_missing_src_bob_dir_returns_zero(self, tmp_path: Path) -> None:
    ...
    def test_non_py_files_not_counted(self, tmp_path: Path) -> None:
        d = tmp_path / ".worktrees" / "hotfix-abc" / "src" / "bob"
        d.mkdir(parents=True)
    ...
    def test_nested_py_files_in_worktree_counted(self, tmp_path: Path) -> None:
        d = tmp_path / ".worktrees" / "hotfix-nest" / "src" / "bob"
        subpkg = d / "subpkg"
