...
Verifies the AC-mandated public API:
  - bob3.liveness.probe_matches_orchestrator
  - bob3.liveness.check_lock_file_holder
  - bob3.liveness.verify_db_activity
  - bob3.liveness.check_orchestrator_running
  - bob3.liveness.validate_lock_file_holder
  - Integration with bob3.orchestrator via bob3.orchestrator.liveness_probe

This module tests the bob3.liveness public wrapper layer and the
...
# ---------------------------------------------------------------------------
# check_orchestrator_running tests (AC: bob3.liveness.check_orchestrator_running)
# ---------------------------------------------------------------------------
...
class TestCheckOrchestratorRunning:
    ...
    def test_returns_true_for_bob3_run(self):
        """Detects legacy 'bob3 run' process."""
        with patch(
            "bob3.orchestrator.liveness_probe._iter_candidate_pids",
            return_value=[(999_002, "bob3 run --all")],
        ):
            assert check_orchestrator_running() is True
...
# ---------------------------------------------------------------------------
# validate_lock_file_holder tests (AC: bob3.liveness.validate_lock_file_holder)
# ---------------------------------------------------------------------------
...
class TestValidateLockFileHolder:
    ...
    def test_returns_false_when_lock_absent(self, tmp_path):
        """Returns False when .bob3.lock does not exist."""
        lock = tmp_path / ".bob3.lock"
        assert validate_lock_file_holder(lock) is False
...
# ---------------------------------------------------------------------------
# Three-signal gate integration (AC: integration: bob3.orchestrator)
# ---------------------------------------------------------------------------

class TestThreeSignalGate:
    """Three-signal safe_to_remove_lock gate via bob3.orchestrator.liveness_probe."""
...
# ---------------------------------------------------------------------------
# Integration: verify bob3.liveness re-exports are the same functions
# ---------------------------------------------------------------------------

# ---------------------------------------------------------------------------
# probe_matches_orchestrator tests (AC: bob3.liveness.probe_matches_orchestrator)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# verify_db_activity tests (AC: bob3.liveness.verify_db_activity)
# ---------------------------------------------------------------------------
...
class TestLivenessModuleIntegration:
    """bob3.liveness wrappers delegate to bob3.orchestrator.liveness_probe."""
    ...
    def test_module_exports_all_ac_functions(self):
        """bob3.liveness exports all AC-mandated functions."""
        import bob3.liveness as m
    ...
    def test_module_exports_verify_lock_file_holder(self):
        """bob3.liveness exports verify_lock_file_holder (AC: Function defined)."""
        import bob3.liveness as m
