"""Tests for peas_pipeline_bob_extract_peas_prose_only_spec_features."""
...
from bob.peas_pipeline_bob_extract_peas_prose_only_spec_features import (
    peas_pipeline_bob_extract_peas_prose_only_spec_features,
)
...
class TestPeasPipelineFunction:
    def test_function_is_callable(self):
        assert callable(peas_pipeline_bob_extract_peas_prose_only_spec_features)

    def test_returns_dict(self, tmp_path):
        peas_path = _make_peas_file(tmp_path, SAMPLE_PEAS)
        result = peas_pipeline_bob_extract_peas_prose_only_spec_features(peas_path)
    ...
    def test_write_to_out_path(self, tmp_path):
        ...
        result = peas_pipeline_bob_extract_peas_prose_only_spec_features(peas_path, out_path=out_path)
    ...
    def test_nonexistent_path_raises_value_error(self, tmp_path):
        ...
        with pytest.raises(ValueError, match="does not exist"):
            peas_pipeline_bob_extract_peas_prose_only_spec_features(missing)

    def test_non_path_input_raises_value_error(self):
        """AC: invalid input (wrong type) raises ValueError."""
        with pytest.raises((ValueError, TypeError)):
            peas_pipeline_bob_extract_peas_prose_only_spec_features(12345)  # type: ignore[arg-type]


def test_peas_pipeline_bob_extract_peas_prose_only_spec_features(tmp_path):
    """AC test: function is importable, callable, returns correct summary dict."""
    peas_path = _make_peas_file(tmp_path, SAMPLE_PEAS)
    with patch("bob.spec_synthesizer.synthesize_for_feature", new_callable=AsyncMock, return_value=FAKE_ACS):
        result = peas_pipeline_bob_extract_peas_prose_only_spec_features(peas_path)
