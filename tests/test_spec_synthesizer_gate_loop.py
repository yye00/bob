"""Tests for bob3.spec_synthesizer.sanitize_spec_file_with_gate_loop.
...
Covers:
  ...
  - threshold default reads from BOB3_SPEC_QUALITY_THRESHOLD env var
...
class TestSanitizeSpecFileWithGateLoopInterface:
    def test_is_importable(self):
        from bob3.spec_synthesizer import sanitize_spec_file_with_gate_loop as fn
        assert callable(fn)
    ...
    def test_no_placeholders_returns_zero_totals(self, tmp_path):
        spec_file = _write_spec(tmp_path, {
            "feat_a": {
                ...
                "acceptance_criteria": ["File exists: src/bob3/done.py"],
            }
        })
        report = _run(
            sanitize_spec_file_with_gate_loop(spec_file, project_id="test")
...
class TestGatePassed:
    def test_gate_passed_increments_when_criteria_above_threshold(self, tmp_path):
        ...
        with patch("bob3.spec_synthesizer.synthesize_for_feature", side_effect=fake_synthesize):
            report = _run(
                sanitize_spec_file_with_gate_loop(
                    spec_file,
                    project_id="test",
    ...
    def test_written_is_false_when_no_changes(self, tmp_path):
        spec_file = _write_spec(tmp_path, {
            "feat_a": {
                ...
                "acceptance_criteria": ["File exists: src/bob3/nu.py"],
            }
        })
        report = _run(
            sanitize_spec_file_with_gate_loop(
...
class TestThresholdEnvVar:
    ...
    def test_env_var_overrides_threshold(self, monkeypatch, tmp_path):
        ...
        async def fake_synthesize(**kwargs):
            return ["File exists: src/bob3/xi.py", "pytest: tests/test_xi.py"]
...
class TestIntegration:
    ...
    def test_score_gate_report_dataclass_importable(self):
        import dataclasses
        from bob3.spec_synthesizer import ScoreGateReport
    ...
    def test_gate_loop_reuses_score_gate_loop_internally(self, tmp_path):
        ...
        async def fake_synthesize(**kwargs):
            return [
                "File exists: src/bob3/upsilon.py",
