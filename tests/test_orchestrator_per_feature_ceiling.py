...
Verifies that:
- apply_pessimistic_cost is importable from bob.orchestrator.run_loop
- apply_pessimistic_cost uses per_feature_ceiling (not project-level budget)
- BOB_PER_FEATURE_COST_CEILING env var controls the ceiling in run_loop
...
# --- Integration: importability from bob.orchestrator.run_loop ---
...
def test_apply_pessimistic_cost_module_path():
    """apply_pessimistic_cost must be accessible from bob.orchestrator.run_loop."""
    import bob.orchestrator.run_loop as rl
    fn = getattr(rl, "apply_pessimistic_cost", None)
    assert fn is not None, (
        "bob.orchestrator.run_loop.apply_pessimistic_cost must be publicly accessible"
    )
    assert callable(fn)
...
def test_apply_pessimistic_cost_uses_per_feature_ceiling_not_project_budget():
    ...
    The fix uses a sane per-feature ceiling (default $20 via BOB_PER_FEATURE_COST_CEILING).
    """
...
# --- Environment variable: BOB_PER_FEATURE_COST_CEILING ---
...
def test_env_var_default_ceiling_is_20(monkeypatch):
    ...
    default = float(os.environ.get("BOB_PER_FEATURE_COST_CEILING", "20.0"))
    assert default == pytest.approx(20.0)


def test_env_var_overrides_ceiling(monkeypatch):
    """BOB_PER_FEATURE_COST_CEILING env var overrides the default $20 ceiling."""
    monkeypatch.setenv("BOB_PER_FEATURE_COST_CEILING", "50.0")
...
def test_env_var_invalid_falls_back_to_20(monkeypatch):
    """Invalid BOB_PER_FEATURE_COST_CEILING falls back to $20 (no crash)."""
    monkeypatch.setenv("BOB_PER_FEATURE_COST_CEILING", "not-a-float")
    try:
        value = float(os.environ.get("BOB_PER_FEATURE_COST_CEILING", "20.0"))
...
def test_fix_prevents_budget_exceeded_on_telemetry_loss():
    """Simulate the exact scenario that caused BUDGET_EXCEEDED in bob v.17 r1.
...
# --- Integration: bob.orchestrator.run_loop module ---

def test_run_loop_module_integration():
    """Integration: bob.orchestrator.run_loop imports cleanly and exposes apply_pessimistic_cost."""
    import bob.orchestrator.run_loop as rl
