...
(the project's own DB) rather than the cwd-resolved or BOB_DATABASE_PATH-
...
def _make_test_db(tmp_path: pathlib.Path, suffix: str = "") -> pathlib.Path:
    ...
    db_path = tmp_path / f"bob{suffix}.db"
...
class TestCreateAgentRunDbPathThreading:
    ...
    def test_does_not_write_to_cwd_db(self, tmp_path, monkeypatch):
        ...
        cwd_db.rename(tmp_path / "bob.db")
        cwd_shadow = tmp_path / "bob.db"
    ...
    def test_does_not_write_to_env_db(self, tmp_path, monkeypatch):
        """When db_path is supplied, the BOB_DATABASE_PATH DB is untouched."""
        project_db = _make_test_db(tmp_path, "-project")
        ...
        assert after_env == before_env, (
            "BOB_DATABASE_PATH DB was written when explicit db_path was supplied"
        )
...
class TestGetDatabasePathResolution:
    """get_database_path() respects BOB_DATABASE_PATH, falls back to cwd."""

    def test_env_var_takes_precedence(self, tmp_path, monkeypatch):
        """BOB_DATABASE_PATH overrides cwd-relative default."""
        custom = tmp_path / "custom.db"
        monkeypatch.setenv("BOB_DATABASE_PATH", str(custom))
        assert get_database_path() == custom

    def test_cwd_fallback(self, tmp_path, monkeypatch):
        """Without env var, resolves to cwd/bob.db."""
        monkeypatch.delenv("BOB_DATABASE_PATH", raising=False)
        monkeypatch.chdir(tmp_path)
        assert get_database_path() == tmp_path / "bob.db"
