"""Tests for bob.acceptance_criteria.verify_artifacts — AC artifact-existence verifier.
...
from bob.acceptance_criteria import ArtifactMiss, verify_artifact_existence

_WORKSPACE = Path(__file__).resolve().parent.parent
_IMPL_REL = "src/bob/acceptance_criteria.py"
...
class TestVerifyArtifactExistenceFunctionDefined:
    def test_function_defined_ac_passes_for_real_function(self):
        ...
        ac = "Function defined: bob.acceptance_criteria.verify_artifact_existence"
        result = verify_artifact_existence([ac], _WORKSPACE)
    ...
    def test_function_defined_ac_fails_for_missing_function(self):
        ...
        ac = "Function defined: bob.acceptance_criteria.no_such_function_xyz"
        result = verify_artifact_existence([ac], _WORKSPACE)
        assert len(result) >= 1
    ...
    def test_function_defined_ac_fails_for_missing_module(self):
        ...
        ac = "Function defined: bob.no_module_xyz_abc.some_func"
        result = verify_artifact_existence([ac], _WORKSPACE)
        assert len(result) >= 1
...
class TestVerifyArtifactExistenceMultipleAcs:
    ...
    def test_all_present_returns_empty(self):
        ...
        acs = [
            ...
            "Function defined: bob.acceptance_criteria.verify_artifact_existence",
        ]
...
class TestVerifyArtifactExistenceOrchestratorIntegration:
    def test_verify_artifacts_callable_from_orchestrator_path(self):
        """bob.orchestrator can import and use verify_artifact_existence."""
        from bob.acceptance_criteria import verify_artifact_existence as fn
        assert callable(fn)

    def test_run_loop_exposes_pre_pytest_hook(self):
        """run_loop exposes verify_artifact_existence_pre_pytest."""
        import importlib
        mod = importlib.import_module("bob.run_loop")
        assert hasattr(mod, "verify_artifact_existence_pre_pytest"), (
            "bob.run_loop must expose verify_artifact_existence_pre_pytest"
        )
    ...
    def test_pre_pytest_hook_returns_empty_list_for_real_ac(self):
        ...
        from bob.run_loop import verify_artifact_existence_pre_pytest
        ac = f"File exists: {_IMPL_REL}"
