"""Tests for bob.zombie_reaper.reap_zombie_runs / reap_zombie_subruns (AC: 815f1875)."""
...
from bob.zombie_reaper import reap_zombie_runs, reap_zombie_subruns
...
class TestReapZombieSubrunsImport:
    def test_function_is_importable(self):
        from bob.zombie_reaper import reap_zombie_subruns as fn  # noqa: F401

        assert callable(fn)

    def test_accepts_project_id(self):
        with patch("bob.zombie_reaper.scan_and_reap") as mock_sar:
...
class TestReapZombieRun:
    def test_reap_sets_status_timeout(self):
        ...
        with patch("bob.orchestrator.zombie_run_reaper.db") as mock_db:
            from bob.orchestrator.zombie_run_reaper import reap_zombie_run
    ...
    def test_reap_logs_info(self, caplog):
        ...
        with patch("bob.orchestrator.zombie_run_reaper.db"):
            from bob.orchestrator.zombie_run_reaper import reap_zombie_run

            with caplog.at_level(logging.INFO, logger="bob.orchestrator.zombie_run_reaper"):
                reap_zombie_run(run)
...
class TestReapZombieRunsCanonicalName:
    ...
    def test_reap_zombie_runs_is_importable(self):
        from bob.zombie_reaper import reap_zombie_runs as fn  # noqa: F401
