"""Tests for bob3.pending_successor_detector.detect_pending_successor_verify (F-R7-596).
...
- Integration path: detect_pending_successor_verify is importable from bob3.run_loop
"""
...
from bob3.pending_successor_detector import detect_pending_successor_verify
...
def test_ac_body_with_full_path_token_returns_true():
    """AC body containing the full path 'src/bob3/enhanced_verification.py' returns True."""
    result = detect_pending_successor_verify(
        "my feature",
        ["File exists: src/bob3/enhanced_verification.py"],
    )
...
def test_ac_body_with_verifier_suffix_returns_true():
    ...
    result = detect_pending_successor_verify(
        "some feature",
        ["File exists: src/bob3/ac_verifier.py"],
    )
    assert result is True
...
def test_ac_body_without_tokens_returns_false():
    ...
    result = detect_pending_successor_verify(
        "my feature",
        ["File exists: src/bob3/dispatch.py", "Function defined: bob3.dispatch.run"],
    )
...
def test_ac_body_scan_short_circuits_on_first_match():
    ...
    result = detect_pending_successor_verify(
        ...
        [
            ...
            "File exists: src/bob3/unrelated.py",
        ],
    )
    assert result is True
...
def test_multiple_acs_none_match_returns_false():
    ...
    result = detect_pending_successor_verify(
        ...
        [
            ...
            "Function defined: bob3.run_loop.start",
...
def test_target_file_scan_detects_verifier_import(tmp_path):
    ...
    target_file = tmp_path / "src" / "bob3" / "some_handler.py"
    target_file.parent.mkdir(parents=True)
    target_file.write_text("from bob3 import enhanced_verification\n", encoding="utf-8")

    result = detect_pending_successor_verify(
        "some feature",
        ["File exists: src/bob3/some_handler.py"],
...
def test_target_file_scan_negative_when_no_token(tmp_path):
    ...
    target_file = tmp_path / "src" / "bob3" / "plain.py"
    target_file.parent.mkdir(parents=True)
    target_file.write_text("def run(): pass\n", encoding="utf-8")

    result = detect_pending_successor_verify(
        "some feature",
        ["File exists: src/bob3/plain.py"],
        workspace=str(tmp_path),
    )
    assert result is False
...
def test_target_file_scan_skipped_when_no_workspace():
    ...
    result = detect_pending_successor_verify(
        "some feature",
        ["File exists: src/bob3/nonexistent_module.py"],
        workspace=None,
    )
...
def test_target_file_missing_does_not_raise(tmp_path):
    ...
    result = detect_pending_successor_verify(
        "some feature",
        ["File exists: src/bob3/does_not_exist.py"],
        workspace=str(tmp_path),
    )
    assert result is False
...
def test_target_file_with_suffix_verifier_token(tmp_path):
    ...
    target_file = tmp_path / "src" / "bob3" / "some_module.py"
    target_file.parent.mkdir(parents=True)
...
def test_title_with_verifier_and_behavior_ac_returns_true():
    ...
    result = detect_pending_successor_verify(
        ...
        [
            "File exists: src/bob3/ac_artifact_verifier.py",
...
def test_title_with_verifier_no_behavior_acs_returns_false():
    ...
    result = detect_pending_successor_verify(
        "AC artifact-existence verifier",
        ["File exists: src/bob3/some_unrelated.py"],
    )
    assert result is False
...
def test_json_encoded_ac_list_with_token_returns_true():
    ...
    ac_json = json.dumps(["File exists: src/bob3/enhanced_verification.py"])
...
def test_json_encoded_ac_list_without_token_returns_false():
    ...
    ac_json = json.dumps(["File exists: src/bob3/run_loop.py"])
    result = detect_pending_successor_verify("my feature", ac_json)
    assert result is False
...
# ---------------------------------------------------------------------------
# Integration: callable from bob3.run_loop
# ---------------------------------------------------------------------------


def test_importable_from_run_loop():
    """detect_pending_successor_verify must be importable from bob3.run_loop."""
    from bob3.run_loop import detect_pending_successor_verify as run_loop_fn
    assert callable(run_loop_fn)


def test_run_loop_delegate_matches_detector():
    """bob3.run_loop.detect_pending_successor_verify delegates to the detector."""
    from bob3.run_loop import detect_pending_successor_verify as run_loop_fn
