"""Tests for bob3.orchestrator.verifier_sandbox.
...
from bob3.orchestrator.verifier_sandbox import (
    SandboxResult,
    SandboxViolation,
...
SAFE_DIFF = _make_diff("src/bob3/some_feature.py")
ENHANCED_VERIFICATION_DIFF = _make_diff("src/bob3/enhanced_verification.py")
SUPERPOWERS_DIFF = _make_diff("src/bob3/superpowers.py")
RUN_LOOP_DIFF = _make_diff("src/bob3/orchestrator/run_loop.py")
MULTI_PROTECTED_DIFF = _make_diff(
    "src/bob3/enhanced_verification.py",
    "src/bob3/superpowers.py",
)
MIXED_DIFF = _make_diff("src/bob3/new_feature.py", "src/bob3/enhanced_verification.py")
...
class TestExtractTouchedPaths:
    def test_single_file(self):
        ...
        assert "src/bob3/some_feature.py" in paths

    def test_multiple_files(self):
        paths = _extract_touched_paths(MULTI_PROTECTED_DIFF)
        assert "src/bob3/enhanced_verification.py" in paths
        assert "src/bob3/superpowers.py" in paths

    def test_deduplication(self):
        diff = SAFE_DIFF + SAFE_DIFF
        paths = _extract_touched_paths(diff)
        assert paths.count("src/bob3/some_feature.py") == 1
...
class TestIsProtected:
    def test_enhanced_verification(self):
        assert _is_protected("src/bob3/enhanced_verification.py") is True

    def test_superpowers(self):
        assert _is_protected("src/bob3/superpowers.py") is True

    def test_run_loop(self):
        assert _is_protected("src/bob3/orchestrator/run_loop.py") is True

    def test_evaluator_file(self):
        assert _is_protected("src/bob3/evaluator_agent.py") is True

    def test_safe_file(self):
        assert _is_protected("src/bob3/some_feature.py") is False
...
class TestCheckVerifierUntouchedAllowed:
    ...
    def test_verifier_sandbox_itself_allowed(self):
        diff = _make_diff("src/bob3/orchestrator/verifier_sandbox.py", "src/bob3/enhanced_verification.py")
        result = check_verifier_untouched(
            diff,
