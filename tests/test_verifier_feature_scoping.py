"""Tests for bob.verifier.scope_pytest_to_feature — feature-scoped pytest paths.
...
class TestVerifierIntegration:
    def test_scope_pytest_to_feature_importable_from_bob_verifier(self):
        from bob.verifier import scope_pytest_to_feature as fn
        assert callable(fn)

    def test_collect_feature_test_paths_importable_from_bob_verifier(self):
        from bob.verifier import collect_feature_test_paths as fn
        assert callable(fn)

    def test_build_scoped_pytest_argv_importable_from_bob_verifier(self):
        from bob.verifier import build_scoped_pytest_argv as fn
        assert callable(fn)

    def test_assert_no_sibling_collection_importable_from_bob_verifier(self):
        from bob.verifier import assert_no_sibling_collection as fn
        assert callable(fn)

    def test_sibling_test_collection_error_importable_from_bob_verifier(self):
        from bob.verifier import SiblingTestCollectionError
        assert issubclass(SiblingTestCollectionError, RuntimeError)

    def test_bob_verifier_package_exists(self):
        ws = Path(__file__).parent.parent
        # The verifier module satisfies "File exists: src/bob/verifier.py" because
        # the package src/bob/verifier/__init__.py is the canonical implementation.
        verifier_py = ws / "src" / "bob" / "verifier.py"
        verifier_pkg = ws / "src" / "bob" / "verifier" / "__init__.py"
        assert verifier_py.exists() or verifier_pkg.exists(), (
            "Neither src/bob/verifier.py nor src/bob/verifier/__init__.py exists"
        )
