"""Tests for bob3.ac_repair_engine — apply_semantic_equivalence_check and auto_repair_error_severity_rewrites."""
...
from bob3.ac_repair_engine import (
...
class TestApplySemanticEquivalenceCheck:
    def test_returns_true_when_judge_says_equivalent(self):
        ...
        with patch("bob3.linter.auto_repair._call_llm_judge", return_value=mock_resp):
    ...
    def test_returns_false_on_llm_failure(self):
        with patch("bob3.linter.auto_repair._call_llm_judge", side_effect=Exception("network error")):
    ...
    def test_empty_strings_do_not_raise(self):
        with patch("bob3.linter.auto_repair._call_llm_judge", side_effect=Exception("empty")):
            result, rationale = apply_semantic_equivalence_check("", "")
        assert result is False
        assert isinstance(rationale, str)
...
# ---------------------------------------------------------------------------
# Integration: bob3.linter → ac_repair_engine
# ---------------------------------------------------------------------------

class TestLinterIntegration:
    def test_importable_from_bob3_linter(self):
        from bob3.linter import detect_smells  # noqa: F401
        assert callable(detect_smells)

    def test_repair_engine_accepts_smell_finding_objects(self, tmp_path):
        """SmellFinding dataclass from bob3.linter is accepted without conversion."""
