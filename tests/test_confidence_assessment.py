"""Tests for bob.confidence.assess_feature_confidence.

Verifies:
- assess_feature_confidence is importable from bob.confidence
...
class TestImport:
    """bob.confidence.assess_feature_confidence must be importable."""

    def test_importable_from_bob_confidence(self):
        from bob.confidence import assess_feature_confidence

        assert callable(assess_feature_confidence)

    def test_module_path_is_bob_confidence(self):
        """The function must live under bob.confidence (per AC)."""
        import bob.confidence as mod

        assert hasattr(mod, "assess_feature_confidence"), (
            "bob.confidence must expose assess_feature_confidence"
        )
...
class TestReturnStructure:
    ...
    def test_returns_all_required_keys(self, tmp_path, monkeypatch):
        ...
        schema_path = "/home/yelkhamr/dark-factory/bob63/src/bob/schema.sql"
