"""Tests for spec_quality_score_gate_threshold_must_honor_bob3_spec.

Verifies that the spec quality gate threshold honors the
BOB3_SPEC_QUALITY_THRESHOLD env var and is computed lazily on each call.
"""
...
from bob3.spec_quality_score_gate_threshold_must_honor_bob3_spec import (
    spec_quality_score_gate_threshold_must_honor_bob3_spec,
)
...
_GOOD_ACS = [
    ...
    "Function defined: bob3.foo.my_function",
...
def _make_call(acs=None, env_threshold=None, frozen=None):
    ...
    if env_threshold is not None:
        env_patches["BOB3_SPEC_QUALITY_THRESHOLD"] = str(env_threshold)
    if frozen is not None:
        env_patches["BOB3_SPEC_QUALITY_THRESHOLD_FROZEN"] = str(frozen)
    ...
    with patch.dict(os.environ, env_patches, clear=False):
        ...
        return spec_quality_score_gate_threshold_must_honor_bob3_spec(
            name="test-feature",
            description=None,
...
class TestSpecQualityScoreGateThresholdMustHonorBob3Spec:
    """AC: spec_quality_score gate threshold MUST honor BOB3_SPEC_QUALITY_THRESHOLD."""

    def test_function_is_callable(self):
        """The required function exists and is callable."""
        assert callable(spec_quality_score_gate_threshold_must_honor_bob3_spec)
    ...
    def test_default_threshold_is_085(self):
        """Default threshold is 0.85 when BOB3_SPEC_QUALITY_THRESHOLD not set."""
        env = {k: v for k, v in os.environ.items()
               if k not in ("BOB3_SPEC_QUALITY_THRESHOLD", "BOB3_SPEC_QUALITY_THRESHOLD_FROZEN")}
        import bob3.spec_quality.threshold_resolver as _tr
        _tr._frozen_initialized = False
        _tr._frozen_value = None
    ...
    def test_env_var_lowered_threshold_promotes_borderline_feature(self):
        """Lowering BOB3_SPEC_QUALITY_THRESHOLD via env var allows lower-scoring features to pass."""
        ...
        with patch.dict(os.environ,
                        {"BOB3_SPEC_QUALITY_THRESHOLD": "0.85"},
                        clear=False):
            _tr._frozen_initialized = False
            _tr._frozen_value = None
        ...
        with patch.dict(os.environ,
                        {"BOB3_SPEC_QUALITY_THRESHOLD": "0.10"},
                        clear=False):
            _tr._frozen_initialized = False
            _tr._frozen_value = None
    ...
    def test_gate_uses_env_var_threshold_not_hardcoded_085(self):
        ...
        env_no_override = {k: v for k, v in os.environ.items()
                           if k not in ("BOB3_SPEC_QUALITY_THRESHOLD",
                                        "BOB3_SPEC_QUALITY_THRESHOLD_FROZEN")}
        ...
        _tr._frozen_value = None
        with patch.dict(os.environ,
                        {"BOB3_SPEC_QUALITY_THRESHOLD": "0.40"},
                        clear=False):
            _tr._frozen_initialized = False
            _tr._frozen_value = None
    ...
    def test_env_var_threshold_clamped_above_1(self):
        ...
        _tr._frozen_value = None
        with patch.dict(os.environ,
                        {"BOB3_SPEC_QUALITY_THRESHOLD": "1.5"},
                        clear=False):
            _tr._frozen_initialized = False
            _tr._frozen_value = None
            threshold = resolve_spec_quality_threshold()
    ...
    def test_env_var_threshold_clamped_below_0(self):
        ...
        _tr._frozen_value = None
        with patch.dict(os.environ,
                        {"BOB3_SPEC_QUALITY_THRESHOLD": "-0.5"},
                        clear=False):
            _tr._frozen_initialized = False
            _tr._frozen_value = None
            threshold = resolve_spec_quality_threshold()
    ...
    def test_unparseable_env_var_falls_back_to_default(self):
        """Unparseable BOB3_SPEC_QUALITY_THRESHOLD falls back to 0.85."""
        import bob3.spec_quality.threshold_resolver as _tr
        _tr._frozen_initialized = False
        _tr._frozen_value = None
        with patch.dict(os.environ,
                        {"BOB3_SPEC_QUALITY_THRESHOLD": "not-a-float"},
                        clear=False):
            _tr._frozen_initialized = False
            _tr._frozen_value = None
            threshold = resolve_spec_quality_threshold()
    ...
    def test_frozen_env_var_pins_threshold(self):
        """BOB3_SPEC_QUALITY_THRESHOLD_FROZEN pins threshold regardless of BOB3_SPEC_QUALITY_THRESHOLD."""
        import bob3.spec_quality.threshold_resolver as _tr
        _tr._frozen_initialized = False
        _tr._frozen_value = None
        with patch.dict(os.environ,
                        {
                            "BOB3_SPEC_QUALITY_THRESHOLD": "0.10",
                            "BOB3_SPEC_QUALITY_THRESHOLD_FROZEN": "0.70",
                        },
                        clear=False):
            _tr._frozen_initialized = False
            _tr._frozen_value = None
        ...
        assert threshold == pytest.approx(0.70), (
            "FROZEN var should override BOB3_SPEC_QUALITY_THRESHOLD"
        )
    ...
    def test_lazy_evaluation_env_var_change_takes_effect(self):
        ...
        with patch.dict(os.environ, {"BOB3_SPEC_QUALITY_THRESHOLD": "0.80"}, clear=False):
            _tr._frozen_initialized = False
            _tr._frozen_value = None
        ...
        with patch.dict(os.environ, {"BOB3_SPEC_QUALITY_THRESHOLD": "0.50"}, clear=False):
            _tr._frozen_initialized = False
            _tr._frozen_value = None
    ...
    def test_facade_passes_score_and_gate_result_through(self):
        ...
        import bob3.spec_quality.threshold_resolver as _tr
        from bob3.spec_quality.quality_score import gate_for_ready, QualityReport, ScoreComponents, compute_score
        ...
        with patch.dict(os.environ, env, clear=True):
            _tr._frozen_initialized = False
            _tr._frozen_value = None
            score, passed, remediation = spec_quality_score_gate_threshold_must_honor_bob3_spec(
                name="facade-test",
                description=None,
    ...
    def test_threshold_in_remediation_message_matches_env_var(self):
        ...
        _tr._frozen_value = None
        with patch.dict(os.environ,
                        {"BOB3_SPEC_QUALITY_THRESHOLD": "0.60"},
                        clear=False):
            _tr._frozen_initialized = False
            _tr._frozen_value = None
...
def test_spec_quality_score_gate_threshold_must_honor_bob3_spec():
    ...
    result = spec_quality_score_gate_threshold_must_honor_bob3_spec(
        name="ac-master-test",
        description=None,
        acceptance_criteria=_GOOD_ACS,
    )
