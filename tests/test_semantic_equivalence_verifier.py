"""Tests for bob3.semantic_equivalence_verifier."""
...
from bob3.semantic_equivalence_verifier import auto_repair_ac, verify_semantic_equivalence
...
class TestVerifySemanticEquivalence:
    ...
    def test_llm_failure_returns_false(self):
        with patch(
            "bob3.linter_ac_repair._call_llm_judge", side_effect=Exception("LLM down")
        ):
    ...
    def test_empty_strings_return_false_on_llm_error(self):
        with patch(
            "bob3.linter_ac_repair._call_llm_judge", side_effect=Exception("empty")
        ):
