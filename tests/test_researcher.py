...
Verifies:
  - File exists: src/bob3/agents/roles.py
  - Function defined: bob3.agents.roles.researcher_prompt_template
  - File exists: src/bob3/agents/researcher.py
  - Function defined: bob3.agents.researcher.research_subsystem
  - integration: bob3.coordinator (merge_research_and_intent)
"""
...
_ROLES_FILE = Path(__file__).parent.parent / "src" / "bob3" / "agents" / "roles.py"
_RESEARCHER_FILE = Path(__file__).parent.parent / "src" / "bob3" / "agents" / "researcher.py"
...
class TestResearcherPromptTemplate:
    """bob3.agents.roles.researcher_prompt_template must be callable."""

    def test_function_importable(self):
        from bob3.agents.roles import researcher_prompt_template  # noqa: F401

    def test_returns_string(self):
        from bob3.agents.roles import researcher_prompt_template
    ...
    def test_prompt_contains_path_glob(self):
        from bob3.agents.roles import researcher_prompt_template
        result = researcher_prompt_template(
            path_glob="src/bob3/coordinator/**",
            symbol_shortlist=["merge"],
        )
        assert "src/bob3/coordinator/**" in result
...
class TestResearchSubsystem:
    """bob3.agents.researcher.research_subsystem must be callable and correct."""

    def test_function_importable(self):
        from bob3.agents.researcher import research_subsystem  # noqa: F401

    def test_returns_dict(self, tmp_path):
        from bob3.agents.researcher import research_subsystem
    ...
    def test_result_has_prompt(self, tmp_path):
        ...
        assert "src/bob3/coordinator/**" in result["prompt"]
    ...
    def test_cache_hit_true_when_notes_present(self, tmp_path):
        ...
        notes_dir = tmp_path / ".bob3" / "features" / feature_id
...
class TestCoordinatorIntegration:
    ...
    def test_coordinator_importable(self):
        from bob3.coordinator import merge_research_and_intent  # noqa: F401
