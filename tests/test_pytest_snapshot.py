"""Tests for bob3.pytest_snapshot.enforce_maxfail_zero.

Acceptance criteria:
- File exists: src/bob3/pytest_snapshot.py
- Function defined: bob3.pytest_snapshot.enforce_maxfail_zero
- pytest: tests/test_pytest_snapshot.py
- integration: bob3.orchestrator (run_loop imports enforce_maxfail_zero)
"""
...
from bob3.pytest_snapshot import enforce_maxfail_zero, MAXFAIL_ZERO
...
class TestOrchestratorIntegration:
    """run_loop.capture_pytest_snapshot must import enforce_maxfail_zero from bob3.pytest_snapshot."""

    def test_run_loop_imports_enforce_maxfail_zero(self):
        import importlib
        import bob3.orchestrator.run_loop as run_loop_mod
        src = importlib.util.find_spec("bob3.orchestrator.run_loop")
        assert src is not None, "bob3.orchestrator.run_loop must be importable"

    def test_enforce_maxfail_zero_importable_from_bob3_pytest_snapshot(self):
        from bob3.pytest_snapshot import enforce_maxfail_zero as fn
        assert callable(fn)
    ...
    def test_run_loop_source_references_pytest_snapshot(self):
        ...
        assert "bob3.snapshot_pytest" in src or "bob3.pytest_snapshot" in src, (
            "run_loop must import enforce_maxfail_zero from a bob3 snapshot module"
        )
