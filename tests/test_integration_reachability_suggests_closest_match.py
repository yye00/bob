...
from bob3.spec_quality.integration_reachability import suggest_closest_match


def test_suggests_close_file_match(tmp_path):
    # Create bob3/cli/plan.py - a typo "pln" should suggest "plan"
    plan = tmp_path / "src" / "bob3" / "cli" / "plan.py"
    plan.parent.mkdir(parents=True)
    plan.write_text("# plan")

    match = suggest_closest_match("bob3.cli.pln", workspace=tmp_path)
