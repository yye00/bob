...
captured "all" (first word) from "integration: all ... bob.reviews ..." and
called _integration_wired(workspace, "all") — which obviously failed. The
actual module reference bob.reviews was never tried.
"""
...
def test_enhanced_verification_pattern_8_integration_must_scan_all():
    ...
    from bob.enhanced_verification_pattern_8_integration_must_scan_all import (
...
class TestPatternScanAllDottedTokens:
    ...
    def test_extracts_dotted_tokens_not_first_word(self):
        """Regression: 'integration: all ... bob.reviews ...' must try bob.reviews, not 'all'."""
        from bob.enhanced_verification_pattern_8_integration_must_scan_all import (
        ...
        # Must find bob.reviews (the actual dotted reference)
        assert "bob.reviews" in targets, (
            f"Expected 'bob.reviews' in extracted targets, got: {targets}"
        )
...
class TestScanAllWithWorkspace:
    ...
    def real_workspace(self) -> pathlib.Path:
        ...
        # The workspace is bob71 itself — src/bob is there.
    ...
    def test_returns_true_for_wired_module(self, real_workspace):
        ...
        # bob.enhanced_verification is a real module in the workspace
        criterion = "integration: bob.enhanced_verification module is imported"
    ...
    def test_c09e9e64_regression_form_does_not_hard_fail(self, real_workspace):
        ...
        # Either bob.reviews resolves (True, "") OR prose demotion fires (True, warning).
        assert passed, (
    ...
    def test_bad_single_dotted_token_hard_fails(self, tmp_path):
        ...
        criterion = "integration: bob.totally.nonexistent.module.path"
        passed, reason = enhanced_verification_pattern_8_integration_must_scan_all(
            criterion, tmp_path
        )
