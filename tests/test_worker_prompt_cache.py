"""Tests for worker prompt-cache enablement in bob3.dispatch.
...
class TestSpawnWorkerWithCacheEnv:
    def test_sets_anthropic_prompt_caching_env(self, tmp_path):
        ...
        with mock.patch("bob3.dispatch.subprocess.run", side_effect=fake_run):
            spawn_worker_with_cache(
                feature, "do stuff", tmp_path, bob3_dir=tmp_path / ".bob3"
            )
    ...
    def test_returns_dict_with_returncode(self, tmp_path):
        ...
        with mock.patch("bob3.dispatch.subprocess.run", side_effect=_fake_run_factory(0)):
    ...
    def test_emits_cache_event_after_worker(self, tmp_path):
        ...
        with mock.patch("bob3.dispatch.subprocess.run", side_effect=_fake_run_factory(0, stdout=cache_stdout)):
