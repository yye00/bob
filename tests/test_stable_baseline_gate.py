"""Tests for bob3.stable_baseline_gate.enforce_stable_baseline_gate.

Acceptance criteria:
  - File exists: src/bob3/stable_baseline_gate.py
  - Function defined: bob3.stable_baseline_gate.enforce_stable_baseline_gate
...
from bob3.stable_baseline_gate import (
...
# ---------------------------------------------------------------------------
# Integration: bob3.verifier exposes the gate
# ---------------------------------------------------------------------------

def test_bob3_verifier_exposes_baseline_unstable_error():
    """bob3.verifier must re-export BaselineUnstableError."""
    from bob3 import verifier
    assert hasattr(verifier, "BaselineUnstableError")


def test_bob3_verifier_exposes_abort_on_collection_failure():
    """bob3.verifier must re-export abort_on_collection_failure."""
    from bob3 import verifier
    assert hasattr(verifier, "abort_on_collection_failure")


def test_bob3_verifier_exposes_should_abort_on_collection_failure():
    """bob3.verifier must re-export should_abort_on_collection_failure."""
    from bob3 import verifier
...
def test_enforce_stable_baseline_gate_importable_from_stable_baseline_gate():
    """enforce_stable_baseline_gate must be importable from bob3.stable_baseline_gate."""
    from bob3.stable_baseline_gate import enforce_stable_baseline_gate as fn
    assert callable(fn)


# ---------------------------------------------------------------------------
# Integration via bob3.verifier — gate is callable from verifier namespace
# ---------------------------------------------------------------------------

def test_abort_on_collection_failure_via_verifier_raises_on_failure(tmp_path):
    """abort_on_collection_failure via bob3.verifier raises BaselineUnstableError."""
    from bob3.verifier import abort_on_collection_failure, BaselineUnstableError as BUE
...
def test_should_abort_on_collection_failure_via_verifier_returns_true(tmp_path):
    ...
    from bob3.verifier import should_abort_on_collection_failure
