"""Tests for bob.reward_hacking_detector — Reward-hacking detector (LLM judge)."""
...
from bob.reward_hacking_detector import (
    HackingVerdict,
    AttackVectorScore,
...
class TestDetectRewardHacking:
    ...
    async def test_returns_hacking_verdict_instance(self):
        with patch("bob.reward_hacking_detector._run_llm_judge", new_callable=AsyncMock) as mock_run:
    ...
    async def test_records_verdict_in_db(self, tmp_path):
        """Verdict should be persisted to bob.db."""
        import sqlite3
        ...
        with patch("bob.reward_hacking_detector._run_llm_judge", new_callable=AsyncMock) as mock_run, \
             patch("bob.reward_hacking_detector._get_db_path", return_value=db_path):
            mock_run.return_value = self._make_clean_llm_response()
            result = await detect_reward_hacking(
