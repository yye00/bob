...
from bob.skills_installer import log_skill_activation
...
def test_log_skill_activation_writes_event(tmp_path):
    ...
    progress_file = tmp_path / ".bob" / "progress.jsonl"

    with patch("bob.skills_installer.get_progress_path", return_value=progress_file):
...
def test_log_skill_activation_creates_parent_dir(tmp_path):
    """log_skill_activation creates .bob/ if it doesn't exist."""
    progress_file = tmp_path / "deep" / "nested" / ".bob" / "progress.jsonl"
...
def test_skill_activation_report_basic(tmp_path):
    ...
    runner = CliRunner()
    with patch("bob.cli.get_progress_path", return_value=progress_file):
...
def test_skill_activation_report_no_file(tmp_path):
    ...
    missing = tmp_path / ".bob" / "progress.jsonl"

    runner = CliRunner()
    with patch("bob.cli.get_progress_path", return_value=missing):
        result = runner.invoke(main, ["skill-activation-report"])
