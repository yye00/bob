...
- Configures BOB_MEMORY_MCP with managed_by_bob=True
- Configures PERPLEXITY_MCP with managed_by_bob=False
- Configures PUPPETEER_MCP with managed_by_bob=False
- Provides get_mcp_config() helper
- Provides get_bob_managed_servers() helper
...
MODULE_PATH = SRC_DIR / "bob" / "orchestrator" / "mcp_config.py"
...
class TestFileExists:
    """Step 1: src/bob/orchestrator/mcp_config.py must exist."""
...
class TestMCPServerConfigDataclass:
    ...
    def test_is_dataclass(self):
        import dataclasses
        from bob.orchestrator.mcp_config import MCPServerConfig
    ...
    def test_has_managed_by_bob_field(self):
        from bob.orchestrator.mcp_config import MCPServerConfig

        field_names = {f.name for f in dataclass_fields(MCPServerConfig)}
        assert "managed_by_bob" in field_names
    ...
    def test_can_create_config(self):
        ...
        assert config.managed_by_bob is True


# ===================================================================
# Step 3: BOB_MEMORY_MCP config (formerly TITANS_MEMORY_MCP)
# ===================================================================


class TestBobMemoryMCPConfig:
    """Step 3: BOB_MEMORY_MCP config with managed_by_bob=True."""

    def test_memory_config_exists(self):
        from bob.orchestrator.mcp_config import BOB_MEMORY_MCP

        assert BOB_MEMORY_MCP is not None

    def test_memory_is_mcp_server_config(self):
        from bob.orchestrator.mcp_config import BOB_MEMORY_MCP, MCPServerConfig

        assert isinstance(BOB_MEMORY_MCP, MCPServerConfig)

    def test_memory_name(self):
        from bob.orchestrator.mcp_config import BOB_MEMORY_MCP

        assert BOB_MEMORY_MCP.name == "bob-memory"
    ...
    def test_memory_command(self):
        ...
        assert BOB_MEMORY_MCP.command == [sys.executable, "-m", "bob.memory_mcp"]

    def test_memory_managed_by_bob(self):
        from bob.orchestrator.mcp_config import BOB_MEMORY_MCP

        assert BOB_MEMORY_MCP.managed_by_bob is True

    def test_memory_env_vars_empty(self):
        """Bob Memory no longer requires OPENAI_API_KEY — everything is local."""
        from bob.orchestrator.mcp_config import BOB_MEMORY_MCP

        assert BOB_MEMORY_MCP.env_vars == []

    def test_memory_is_required(self):
        from bob.orchestrator.mcp_config import BOB_MEMORY_MCP

        assert BOB_MEMORY_MCP.required is True
...
class TestPerplexityMCPConfig:
    """Step 4: PERPLEXITY_MCP config with managed_by_bob=False."""

    def test_perplexity_config_exists(self):
        from bob.orchestrator.mcp_config import PERPLEXITY_MCP

        assert PERPLEXITY_MCP is not None

    def test_perplexity_is_mcp_server_config(self):
        from bob.orchestrator.mcp_config import PERPLEXITY_MCP, MCPServerConfig

        assert isinstance(PERPLEXITY_MCP, MCPServerConfig)

    def test_perplexity_not_managed_by_bob(self):
        from bob.orchestrator.mcp_config import PERPLEXITY_MCP

        assert PERPLEXITY_MCP.managed_by_bob is False
...
class TestPuppeteerMCPConfig:
    """Step 5: PUPPETEER_MCP config with managed_by_bob=False."""

    def test_puppeteer_config_exists(self):
        from bob.orchestrator.mcp_config import PUPPETEER_MCP

        assert PUPPETEER_MCP is not None

    def test_puppeteer_is_mcp_server_config(self):
        from bob.orchestrator.mcp_config import PUPPETEER_MCP, MCPServerConfig

        assert isinstance(PUPPETEER_MCP, MCPServerConfig)

    def test_puppeteer_not_managed_by_bob(self):
        from bob.orchestrator.mcp_config import PUPPETEER_MCP

        assert PUPPETEER_MCP.managed_by_bob is False
...
class TestGetMCPConfig:
    ...
    def test_get_mcp_config_returns_dict(self):
        from bob.orchestrator.mcp_config import get_mcp_config

        result = get_mcp_config()
        assert isinstance(result, dict)

    def test_get_mcp_config_contains_bob_memory(self):
        from bob.orchestrator.mcp_config import get_mcp_config

        result = get_mcp_config()
        names = {config.name for config in result.values()}
        assert "bob-memory" in names
    ...
    def test_get_mcp_config_values_are_mcp_server_configs(self):
        from bob.orchestrator.mcp_config import get_mcp_config, MCPServerConfig
...
# ===================================================================
# Step 7: get_bob_managed_servers() helper
# ===================================================================


class TestGetBobManagedServers:
    """Step 7: get_bob_managed_servers() returns only servers Bob manages."""

    def test_returns_list(self):
        from bob.orchestrator.mcp_config import get_bob_managed_servers

        result = get_bob_managed_servers()
        assert isinstance(result, list)

    def test_contains_bob_memory(self):
        from bob.orchestrator.mcp_config import get_bob_managed_servers
...
class TestMCPConfigIntegration:
    ...
    def test_memory_config_produces_valid_mcp_servers_dict(self):
        """BOB_MEMORY_MCP can be converted to an mcp_servers dict for the SDK."""
        from bob.orchestrator.mcp_config import BOB_MEMORY_MCP

        mcp_dict = {
            BOB_MEMORY_MCP.name: {
                "type": "stdio",
                "command": BOB_MEMORY_MCP.command[0],
                "args": BOB_MEMORY_MCP.command[1:],
            }
        }
        assert BOB_MEMORY_MCP.name in mcp_dict
        assert mcp_dict[BOB_MEMORY_MCP.name]["type"] == "stdio"
        import sys

        assert mcp_dict[BOB_MEMORY_MCP.name]["command"] == sys.executable

    def test_get_allowed_tools_returns_list(self):
        from bob.orchestrator.mcp_config import get_allowed_tools
...
class TestPerplexityApiKeyValidation:
    ...
    def test_warns_when_perplexity_api_key_missing(self, monkeypatch, caplog):
        ...
        caplog.clear()
        with caplog.at_level(logging.WARNING, logger="bob.orchestrator.mcp_config"):
    ...
    def test_validate_perplexity_available_missing(self, monkeypatch):
        from bob.orchestrator.mcp_config import validate_perplexity_available
