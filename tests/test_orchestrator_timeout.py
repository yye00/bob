...
Verifies that bob3.orchestrator.enforce_feature_timeout enforces a hard
...
import bob3.timeout as _timeout_mod


# ---------------------------------------------------------------------------
# Module-level contract: enforce_feature_timeout accessible via bob3.orchestrator
# ---------------------------------------------------------------------------

def test_enforce_feature_timeout_accessible_from_orchestrator():
    assert hasattr(bob3.orchestrator, "enforce_feature_timeout"), (
        "bob3.orchestrator must expose enforce_feature_timeout"
    )
    fn = bob3.orchestrator.enforce_feature_timeout
    assert callable(fn)
...
async def test_enforce_feature_timeout_returns_result_on_fast_coroutine():
    ...
    result = await bob3.orchestrator.enforce_feature_timeout(
...
async def test_enforce_feature_timeout_raises_on_slow_coroutine():
    ...
    with pytest.raises(_timeout_mod.FeatureTimeoutError) as exc_info:
        await bob3.orchestrator.enforce_feature_timeout(
...
async def test_enforce_feature_timeout_raises_valueerror_on_empty_feature_id():
    ...
    with pytest.raises(ValueError):
        await bob3.orchestrator.enforce_feature_timeout("", noop(), timeout_seconds=10.0)
...
async def test_enforce_feature_timeout_raises_valueerror_on_non_positive_timeout():
    ...
    with pytest.raises(ValueError):
        await bob3.orchestrator.enforce_feature_timeout("feat-x", noop(), timeout_seconds=0)

    with pytest.raises(ValueError):
        await bob3.orchestrator.enforce_feature_timeout("feat-x", noop(), timeout_seconds=-1.0)


# ---------------------------------------------------------------------------
# resolve_timeout_seconds — reads BOB3_FEATURE_TIMEOUT_SECONDS from env
# ---------------------------------------------------------------------------

def test_resolve_timeout_seconds_default():
    env_backup = os.environ.pop("BOB3_FEATURE_TIMEOUT_SECONDS", None)
    try:
        result = _timeout_mod.resolve_timeout_seconds()
        assert result == pytest.approx(1800.0)
    finally:
        if env_backup is not None:
            os.environ["BOB3_FEATURE_TIMEOUT_SECONDS"] = env_backup
