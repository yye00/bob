"""Tests for bob3.dispatch.spawn_worker_with_cache and related worker leverage helpers.
...
ACs tested:
  - Function defined: bob3.dispatch.spawn_worker_with_cache
  - File exists: .bob3/features/WORKER.md
  - File exists: .bob3/features/<id>/settings.json
...
class TestEmitWorkerCacheEvent:
    ...
    def test_logs_the_event(self, caplog):
        import logging
        with caplog.at_level(logging.INFO, logger="bob3.dispatch"):
...
class TestBuildWorkerMd:
    ...
    def test_contains_localization_shortlist_when_provided(self):
        feature = _make_feature(localization_shortlist=["src/bob3/foo.py", "src/bob3/bar.py"])
        md = build_worker_md(feature, workspace="/some/path")
        assert "src/bob3/foo.py" in md
        assert "src/bob3/bar.py" in md
...
class TestWriteFeatureSettings:
    def test_creates_settings_json(self, tmp_path):
        ...
        settings_path = write_feature_settings(feature, bob3_dir=tmp_path)
...
class TestSpawnWorkerWithCache:
    ...
    def _make_call(self, tmp_path, feature=None, **kwargs):
        ...
        with mock.patch("bob3.dispatch.subprocess.run") as mock_run:
    ...
    def test_settings_file_created_before_spawn(self, tmp_path):
        ...
        settings_path = tmp_path / ".bob3" / "features" / "feat-spawn-02" / "settings.json"
        assert settings_path.exists()
...
# ── Integration: bob3.dispatch module surface ─────────────────────────────────

class TestBob3DispatchIntegration:
    """Verify the integration AC: bob3.dispatch exports all new symbols."""

    def test_spawn_worker_with_cache_importable(self):
        from bob3.dispatch import spawn_worker_with_cache  # noqa: PLC0415
        assert callable(spawn_worker_with_cache)

    def test_build_worker_md_importable(self):
        from bob3.dispatch import build_worker_md  # noqa: PLC0415
        assert callable(build_worker_md)

    def test_write_feature_settings_importable(self):
        from bob3.dispatch import write_feature_settings  # noqa: PLC0415
        assert callable(write_feature_settings)

    def test_emit_worker_cache_event_importable(self):
        from bob3.dispatch import emit_worker_cache_event  # noqa: PLC0415
        assert callable(emit_worker_cache_event)

    def test_spawn_worker_with_cache_in_all(self):
        import bob3.dispatch as d  # noqa: PLC0415
