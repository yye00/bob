...
Covers:
- bob.cost_enforcement.validate_reported_cost
- bob.cost_enforcement.log_cost_telemetry_lost
- integration: bob.orchestrator exposes validate_reported_cost and log_cost_telemetry_lost
...
class TestImportability:
    """Both functions must be importable from bob.cost_enforcement."""

    def test_validate_reported_cost_importable(self):
        mod = importlib.import_module("bob.cost_enforcement")
    ...
    def test_result_type_importable(self):
        from bob.cost_enforcement import CostValidationResult
        assert CostValidationResult is not None


# ---------------------------------------------------------------------------
# AC 5 — integration: bob.orchestrator exposes both functions
# ---------------------------------------------------------------------------


class TestOrchestratorIntegration:
    """AC5: both functions must be accessible from bob.orchestrator."""

    def test_validate_reported_cost_in_orchestrator(self):
        mod = importlib.import_module("bob.orchestrator")
        assert callable(getattr(mod, "validate_reported_cost", None)), (
            "validate_reported_cost not found in bob.orchestrator"
        )

    def test_log_cost_telemetry_lost_in_orchestrator(self):
        mod = importlib.import_module("bob.orchestrator")
        assert callable(getattr(mod, "log_cost_telemetry_lost", None)), (
            "log_cost_telemetry_lost not found in bob.orchestrator"
        )
...
class TestValidateReportedCostThresholdEnvVar:
    """BOB_COST_TELEMETRY_LOST_WORK_THRESHOLD overrides default threshold."""
...
class TestLogCostTelemetryLost:
    ...
    def test_emits_warning_log(self, caplog):
        with caplog.at_level(logging.WARNING, logger="bob.cost_enforcement"):
            log_cost_telemetry_lost(
                feature_id="9b2e1060-0000-0000-0000-000000000000",
                work_events=176217,
                exit_code=1,
