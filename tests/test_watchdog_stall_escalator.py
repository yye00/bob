...
Verifies the AC-required function bob.watchdog_stall_escalator.escalate_spec_gate_stall
...
from bob.watchdog_stall_escalator import escalate_spec_gate_stall
...
class TestEscalateSpecGateStallEnvOverride:
    """BOB_STALL_ESCALATION_COUNT env var overrides default threshold."""
    ...
    def test_invalid_env_falls_back_to_default(self, tmp_path):
        marker = tmp_path / "STALL_ATTENTION.txt"
        with patch.dict(os.environ, {"BOB_STALL_ESCALATION_COUNT": "notanumber"}):
            result = escalate_spec_gate_stall(observation_count=0, marker_path=marker)
        assert result["threshold"] == 5
