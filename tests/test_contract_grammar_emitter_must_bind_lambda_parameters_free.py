"""Tests for contract_grammar_emitter_must_bind_lambda_parameters_free.

Verifies that the named entry-point function correctly emits bound lambda
decorators for icontract.require, fixing the zero-arg lambda bug (feature
73879589) that caused NH-demotion of Design-by-Contract features.
"""

import pytest
import icontract

from bob3.contract_grammar_emitter_must_bind_lambda_parameters_free import (
    contract_grammar_emitter_must_bind_lambda_parameters_free as emit_bound_require,
    ContractGrammarBindingError,
)


def test_contract_grammar_emitter_must_bind_lambda_parameters_free():
...
def test_zero_arg_lambda_form_is_broken():
    ...
    from bob3.spec_quality.contract_grammar_lambda_binder import validate_lambda_binding
