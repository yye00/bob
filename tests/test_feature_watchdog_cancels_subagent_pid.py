...
async def test_os_kill_process_lookup_error_is_swallowed():
    ...
    with (
        patch("bob.orchestrator.feature_watchdog._pid_is_alive", side_effect=alive_side_effect),
        patch("bob.orchestrator.feature_watchdog.os.kill", side_effect=kill_side_effect),
        patch("bob.orchestrator.feature_watchdog.asyncio.sleep", side_effect=instant_sleep),
    ):
