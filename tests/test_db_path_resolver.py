"""Tests for bob.db_path_resolver.get_absolute_db_path.

Verifies that get_absolute_db_path:
  - is importable from bob.db_path_resolver
  - returns an absolute path when given an explicit path argument
  - returns an absolute path when falling back to BOB_DATABASE_PATH env var
  - returns an absolute path when falling back to cwd/bob.db
...
def test_importable():
    from bob.db_path_resolver import get_absolute_db_path
...
def test_env_var_used_when_no_explicit_path(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        monkeypatch.setenv("BOB_DATABASE_PATH", env_path)
