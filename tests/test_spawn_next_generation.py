"""Tests for spawn_next_generation.sh bob init re-run behavior.

Verifies that tools/spawn_next_generation.sh invokes bob init after rsync
...
class TestSpawnScriptContainsBobInit:
    def test_contains_bob_init_invocation(self):
...
def test_projects_name_matches_workspace_after_spawn(tmp_path):
    ...
    Simulates a child generation workspace whose bob.db still has the parent's
...
class TestSpawnScriptInvokesParentGenInheritance:
    ...
    def test_inheritance_section_appears_after_init(self):
        """Inheritance stamping must happen after bob init (so columns exist)."""
        text = SPAWN_SCRIPT.read_text()
        init_pos = text.find("bob init")
        if init_pos == -1:
            # Try regex variant
            import re as _re
        ...
        assert init_pos != -1, "spawn script must invoke bob init"
        ...
        assert inherit_pos > init_pos, (
            "parent-gen inheritance must appear after bob init in spawn script"
        )


# ---------------------------------------------------------------------------
# AC test: test_bob_init_rerun_after_spawn
# ---------------------------------------------------------------------------

def test_bob_init_rerun_after_spawn():
    """spawn_next_generation.sh must invoke bob init after rsync to fix stale metadata.

    Verifies that:
    1. The spawn script file exists.
    2. It contains a bob init invocation with --name flag.
    ...
    # Must invoke bob init (literal or via variable expansion)
    has_init = (
        "bob init" in text
        or bool(re.search(r'bob\$?NEXT_NUM["\s]+init', text))
        or bool(re.search(r'bin/bob[^"]*"\s+init', text))
    )
    assert has_init, "spawn_next_generation.sh must invoke bob init after rsync"

    # Must pass --name flag to init
    assert "--name" in text, "bob init call must include --name flag"
    ...
    assert init_pos != -1, "spawn script must invoke bob init"
    assert init_pos > rsync_pos, "bob init must appear after rsync in spawn script"
    ...
    assert "--spec" in text or "SPEC_FOR_NEXT" in text, (
        "spawn script must conditionally handle --spec flag for bob init"
    )
