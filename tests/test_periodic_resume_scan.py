"""Tests for bob.orchestrator.periodic_resume_scan (AC: f9f35288).

Verifies:
- periodic_resume_scan is importable from bob.orchestrator.periodic_resume_scan
...
from bob.orchestrator.periodic_resume_scan import periodic_resume_scan
...
class TestPeriodicResumeScanImport:
    def test_function_is_importable(self):
        from bob.orchestrator.periodic_resume_scan import periodic_resume_scan as fn  # noqa: F401
...
class TestOrchestratorIntegration:
    ...
    def test_periodic_resume_scan_module_importable(self):
        from bob.orchestrator import periodic_resume_scan as mod  # noqa: F401

        assert hasattr(mod, "periodic_resume_scan")

    def test_periodic_resume_scan_is_callable_from_orchestrator_package(self):
        from bob.orchestrator.periodic_resume_scan import periodic_resume_scan as fn
...
# ---------------------------------------------------------------------------
# Tests for bob.periodic_resume_scan.promote_interrupted_rows (6abe05be)
# ---------------------------------------------------------------------------


from bob.periodic_resume_scan import promote_interrupted_rows  # noqa: E402


class TestPromoteInterruptedRowsImport:
    def test_function_is_importable(self):
        from bob.periodic_resume_scan import promote_interrupted_rows as fn  # noqa: F401

        assert callable(fn)

    def test_accepts_project_id_returns_list(self):
        with patch("bob.periodic_resume_scan.db") as mock_db:
            mock_db.list_features.return_value = []
...
class TestPromoteInterruptedRowsLogging:
    def test_logs_info_for_each_promoted_feature(self, caplog):
        ...
        with patch("bob.periodic_resume_scan.db") as mock_db:
            mock_db.list_features.return_value = [feat]
            mock_db.update_feature.return_value = None
            with caplog.at_level(logging.INFO, logger="bob.periodic_resume_scan"):
    ...
    def test_logs_debug_on_list_features_failure(self, caplog):
        with patch("bob.periodic_resume_scan.db") as mock_db:
            mock_db.list_features.side_effect = Exception("DB error")
            with caplog.at_level(logging.DEBUG, logger="bob.periodic_resume_scan"):
