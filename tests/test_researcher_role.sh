...
#
# Verifies:
#   1. src/bob3/agents/roles.py exists and contains required markers.
#   2. The researcher role has hide_intent=True.
#   3. research_notes_path() returns a path under .bob3/features/<id>/.
...
ROLES_PY="$WORKSPACE/src/bob3/agents/roles.py"
...
from bob3.agents.roles import RESEARCHER, get_role
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    expected = pathlib.Path(tmp) / ".bob3" / "features" / "test-feat-id" / "research_notes.md"
...
import sys, tempfile, pathlib
sys.path.insert(0, "src")
from bob3.agents.roles import research_notes_path, should_skip_research
with tempfile.TemporaryDirectory() as tmp:
...
prompt = build_researcher_prompt("src/bob3/orchestrator/**", ["run_loop", "disk_reconciler"])
...
assert "src/bob3/orchestrator/**" in prompt, "Path glob must appear in researcher prompt"
