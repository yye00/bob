...
class TestWritePlanArtifact:
    ...
    def test_plan_ready_logged(self, tmp_path, caplog):
        ...
        with caplog.at_level(logging.INFO, logger="bob3.orchestrator.plan_gate"):
            write_plan_artifact(
                feature_id=feature_id,
                name="Feature F",
                description=None,
...
class TestIsApproved:
    ...
    def test_returns_true_after_approve(self, tmp_path):
        from bob3.orchestrator.plan_gate import write_plan_artifact, approve_plan, is_approved
