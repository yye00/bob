...
def test_disk_promote_on_tests_pass_failure():
    ...
    feature = _make_feature(feature_id, '["structural: src/bob/run_loop.py contains F-R7-612"]')
    ...
    with patch("bob.orchestrator.run_loop.db") as mock_db, \
         ...
         patch("bob.orchestrator.run_loop._check_executing_feature_acs") as mock_check_acs:
...
def test_needs_human_when_disk_check_fails():
    ...
    with patch("bob.orchestrator.run_loop.db") as mock_db, \
         patch("bob.orchestrator.run_loop._may_demote", return_value=True), \
         patch("bob.orchestrator.run_loop._check_executing_feature_acs") as mock_check_acs, \
         patch("bob.orchestrator.run_loop._rca_auto_reset_if_infra", return_value=False):
