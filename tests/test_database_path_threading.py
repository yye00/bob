...
Verifies that:
- set_database_path sets BOB3_DATABASE_PATH and returns the absolute path
...
from bob3 import db
from bob3.orchestrator import set_database_path
...
def test_set_database_path_sets_env_var(tmp_path, monkeypatch):
    """set_database_path stores the resolved absolute path in BOB3_DATABASE_PATH."""
    monkeypatch.delenv("BOB3_DATABASE_PATH", raising=False)
    target = tmp_path / "mydb.db"
    result = set_database_path(target)
    assert result == target.resolve()
    assert os.environ["BOB3_DATABASE_PATH"] == str(target.resolve())
...
def test_create_agent_run_uses_explicit_db_path(project_db, tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("BOB3_DATABASE_PATH", str(shadow))
