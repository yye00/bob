"""Tests for bob3.spec_synthesizer — score-gate loop and sanitize_spec_file.
...
Covers:
  ...
  - score_gate_threshold_from_env reads BOB3_SPEC_QUALITY_THRESHOLD
...
class TestScoreGateThresholdFromEnv:
    def test_reads_env_var(self, monkeypatch):
        monkeypatch.setenv("BOB3_SPEC_QUALITY_THRESHOLD", "0.9")
...
class TestScorerImportResilience:
    ...
    def test_scorer_import_resilient_to_cwd(self, tmp_path, monkeypatch):
        ...
        result = compute(
            ...
            acceptance_criteria=["File exists: src/bob3/test_feature.py"],
        )
...
def test_score_gate_loop_import_resilience(tmp_path, monkeypatch):
    ...
    from bob3.spec_synthesizer import _load_compute, score_gate_loop, ScoreGateReport
...
def test_score_gate_loop_imports_scorer_with_cwd_independence(tmp_path, monkeypatch):
    ...
    good_criteria = [
        "File exists: src/bob3/feature.py",
        "pytest: tests/test_feature.py",
        "Function defined: bob3.feature.process",
...
def test_score_gate_loop_retries_with_feedback():
    ...
    async def _track_feedback(**kwargs):
        ...
        return [
            "File exists: src/bob3/feedback_feature.py",
            "Function defined: bob3.feedback_feature.process",
            ...
            "integration: bob3.feedback_feature",
        ]

    report = _run(score_gate_loop(
        synthesize_fn=_track_feedback,
        title="feedback feature",
        description="Adds process function to bob3.feedback_feature that processes valid inputs",
        project_id="test",
...
def test_score_gate_loop_fallback_on_exhaustion():
    ...
    report = _run(score_gate_loop(
        ...
        description="Adds exhausted_fn to bob3.exhausted_feature module that handles requests",
        project_id="test",
...
def test_score_gate_loop_passes_above_threshold():
    ...
    good_criteria = [
        "File exists: src/bob3/feature.py",
        "Function defined: bob3.feature.feature_fn",
    ...
    report = _run(score_gate_loop(
        ...
        description="Adds feature_fn to bob3.feature that processes input dicts and raises ValueError on bad input",
        project_id="test",
...
def test_score_gate_loop_retries_below_threshold():
    ...
    good_criteria = [
        "File exists: src/bob3/retry_feature.py",
        "Function defined: bob3.retry_feature.process",
        ...
        "integration: bob3.retry_feature",
    ]
    ...
    report = _run(score_gate_loop(
        synthesize_fn=_synth,
        title="retry feature",
        description="Adds process function to bob3.retry_feature that computes results from valid inputs",
        project_id="test",
        threshold=threshold,
        max_retries=3,
        use_fallback=True,
...
def test_score_gate_loop_exhausts_retries():
    ...
    report = _run(score_gate_loop(
        ...
        description="Adds exhausted_fn to bob3.exhausted module",
        project_id="test",
...
def test_score_gate_loop_imports_scorer_robustly():
    ...
    from bob3.spec_synthesizer import _load_compute
    compute = _load_compute()
    ...
    good_criteria = [
        "File exists: src/bob3/robust_import.py",
        "pytest: tests/test_robust_import.py",
        "Function defined: bob3.robust_import.robust_import",
    ...
    report = _run(score_gate_loop(
        ...
        title="robust import",
        description="Provides robust_import function in bob3.robust_import module",
        project_id="test",
...
def test_score_gate_loop_handles_missing_scorer_loudly():
    ...
    async def _dummy_synth(**kwargs):
        return [
            "File exists: src/bob3/loud_fail.py",
...
def test_scorer_import_fallback_on_modulenotfound(monkeypatch):
    ...
    try:
        ...
        result = compute(
            ...
            acceptance_criteria=["File exists: src/bob3/fallback_test.py"],
        )
        assert hasattr(result, "composite"), (
...
def test_scorer_import_fallback_adds_gen_root(monkeypatch):
    ...
    from __file__ (<gen>/src/bob3/spec_synthesizer.py → parents[2]) and inserts it into
    ...
    try:
        ...
        result = compute(
            ...
            acceptance_criteria=["File exists: src/bob3/gen_root_test.py"],
        )
        assert hasattr(result, "composite"), (
...
def test_scorer_import_fallback_adds_sys_path(monkeypatch):
    ...
    gen root from __file__ (<gen>/src/bob3/spec_synthesizer.py → parents[2]) and inserts
...
from bob3.spec_synthesizer import derive_canonical_slug  # noqa: E402
...
def test_scorer_import_failure_raises_loudly() -> None:
    ...
    # Save the original module so we can restore it after the reload, preventing
    # the reloaded module's ScoreGateReport class from leaking into later tests.
    original_spec_synthesizer = sys.modules.get("bob3.spec_synthesizer")
    ...
    try:
        ...
        # Restore the original module object (not a fresh reload) so that
        # already-imported ScoreGateReport references in other test modules
        # remain valid after this test completes.
        if original_spec_synthesizer is not None:
            sys.modules["bob3.spec_synthesizer"] = original_spec_synthesizer
        else:
            importlib.reload(__import__("bob3.spec_synthesizer", fromlist=["_"]))
...
def test_score_gate_loop_with_mismatched_cwd(tmp_path, monkeypatch):
    ...
    criteria = [
        "File exists: src/bob3/mismatch_feature.py",
        "pytest: tests/test_mismatch_feature.py",
        "Function defined: bob3.mismatch_feature.run",
...
def test_score_gate_loop_retries_on_low_score():
    ...
    good_criteria = [
        "File exists: src/bob3/retry_loop_feature.py",
        "Function defined: bob3.retry_loop_feature.compute",
        ...
        "integration: bob3.retry_loop_feature",
    ]
    ...
    report = _run(score_gate_loop(
        ...
        description="Adds compute function to bob3.retry_loop_feature that raises ValueError on empty input",
        project_id="test",
...
def test_score_gate_loop_exhaustion_fallback():
    ...
    report = _run(score_gate_loop(
        ...
        description="Adds exhaustion_fn to bob3.exhaustion that raises ValueError on bad input",
        project_id="test",
