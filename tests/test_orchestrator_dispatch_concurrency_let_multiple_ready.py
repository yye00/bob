...
in parallel instead of strict single-flight, using BOB3_MAX_CONCURRENT_FEATURES
...
def test_orchestrator_dispatch_concurrency_let_multiple_ready():
    ...
    5. BOB3_MAX_CONCURRENT_FEATURES env var is honoured.
    """
    # 1. Importable from the canonical module path
    from bob3.orchestrator_dispatch_concurrency_let_multiple_ready import (
    ...
    async def run():
        with mock.patch(
            "bob3.orchestrator_dispatch_concurrency_let_multiple_ready.db"
        ) as mock_db:
...
def test_module_exposes_correct_function_name():
    ...
    import bob3.orchestrator_dispatch_concurrency_let_multiple_ready as m
    assert hasattr(m, "orchestrator_dispatch_concurrency_let_multiple_ready")


def test_env_var_controls_concurrency(monkeypatch):
    """BOB3_MAX_CONCURRENT_FEATURES=1 limits dispatch to 1 feature at a time."""
