...
'integration: bob.does_not_exist_module' is a non-prose form — there are no
...
BAD_DOTTED_CRITERION = "integration: bob.does_not_exist_module"


@pytest.fixture()
def empty_workspace(tmp_path: pathlib.Path) -> pathlib.Path:
    """Workspace where bob.does_not_exist_module does not exist."""
...
def test_bad_dotted_target_reason_mentions_body(empty_workspace: pathlib.Path) -> None:
    ...
    assert "bob.does_not_exist_module" in reason or "no wired" in reason, (
