"""Tests for bob3.prompt_reload.reload_if_modified (feature 2ce973a8).
...
def test_module_importable():
    """bob3.prompt_reload is importable."""
    import bob3.prompt_reload  # noqa: F401 — import test


def test_reload_if_modified_importable():
    """bob3.prompt_reload.reload_if_modified is importable and callable."""
    from bob3.prompt_reload import reload_if_modified
...
def test_reload_if_modified_returns_bool_for_real_module():
    ...
    result = reload_if_modified("bob3.prompt_reload")
...
def test_reload_if_modified_returns_false_first_call():
    ...
    mod_name = "bob3.__test_first_call_prompt_reload__"
...
def test_reload_if_modified_returns_false_when_unchanged(tmp_path):
    ...
    mod_name = "bob3.__dummy_unchanged__"
...
def test_reload_if_modified_triggers_reload_on_mtime_change(tmp_path):
    ...
    mod_name = "bob3.__dummy_changed__"
    spec = importlib.util.spec_from_file_location(mod_name, src)
    mod = importlib.util.module_from_spec(spec)
    sys.modules[mod_name] = mod
    spec.loader.exec_module(mod)
...
def test_reload_if_modified_missing_module_returns_false():
    ...
    result = reload_if_modified("bob3.__totally_nonexistent_xyz_abc__")
    assert result is False
...
def test_reload_if_modified_raises_value_error_on_list():
    ...
    with pytest.raises(ValueError):
        reload_if_modified(["bob3.superpowers"])  # type: ignore[arg-type]
...
def test_reload_if_modified_delegates_to_reloader():
    ...
    with patch.object(_reloader, "reload_if_stale", return_value=True) as mock_fn:
        result = reload_if_modified("bob3.superpowers")
...
def test_reload_if_modified_default_module_is_superpowers():
    """Default module_name argument is 'bob3.superpowers'."""
    import bob3.orchestrator.prompt_source_reloader as _reloader
    from bob3.prompt_reload import reload_if_modified
