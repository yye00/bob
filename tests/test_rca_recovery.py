"""Tests for bob.rca_recovery — second-line defense against false NH escalation.
...
class TestIsInfraOnly:
    """bob.rca_recovery.is_infra_only: classification predicate."""

    def test_returns_true_when_verdict_infra_only(self) -> None:
        from bob.rca_recovery import is_infra_only
...
class TestLearnSignature:
    """bob.rca_recovery.learn_signature: persist novel infra signatures."""

    def test_appends_pattern_to_spawn_retry_yaml(self, tmp_path: pathlib.Path) -> None:
        import yaml

        from bob.rca_recovery import learn_signature
