"""Boundary tests for bob.dispatch worker-leverage functions.
...
class TestWriteFeatureSettingsBoundary:
    ...
    def test_empty_extra_allow(self, tmp_path):
        ...
        path = write_feature_settings(feature, bob_dir=tmp_path, extra_allow=[])
        data = json.loads(path.read_text())
        assert "allow" in data["permissions"]

    def test_single_extra_allow(self, tmp_path):
        feature = _make_feature(id="feat-boundary-s3")
        path = write_feature_settings(feature, bob_dir=tmp_path, extra_allow=["Read(*)"])
        data = json.loads(path.read_text())
...
class TestSpawnWorkerBoundary:
    def test_empty_prompt_does_not_raise(self, tmp_path):
        ...
        with mock.patch("bob.dispatch.subprocess.run", side_effect=fake_run):
            result = spawn_worker(
                feature, "", tmp_path, bob_dir=tmp_path / ".bob"
            )
    ...
    def test_minimum_timeout_does_not_raise(self, tmp_path):
        ...
        with mock.patch("bob.dispatch.subprocess.run", side_effect=fake_run):
            result = spawn_worker(
                feature, "prompt", tmp_path, bob_dir=tmp_path / ".bob", timeout=1
            )
        assert result["returncode"] == 0
