...
assert_file_exists "brownfield/__init__.py exists" \
    "src/bob3/brownfield/__init__.py"

assert_file_exists "brownfield/survey.py exists" \
    "src/bob3/brownfield/survey.py"

assert_file_exists "brownfield/resurrection.py exists" \
    "src/bob3/brownfield/resurrection.py"

assert_file_exists "brownfield/elicit.py exists" \
    "src/bob3/brownfield/elicit.py"

# ── survey.py structural checks ───────────────────────────────────────────────

SURVEY_CONTENT=$(cat src/bob3/brownfield/survey.py)
...
MARKER_LINE=$(grep -n "F-R7-611" src/bob3/brownfield/survey.py | head -1 | cut -d: -f1)
REPOMAPPER_LINE=$(grep -n "RepoMapper" src/bob3/brownfield/survey.py | head -1 | cut -d: -f1)
...
RESURRECTION_CONTENT=$(cat src/bob3/brownfield/resurrection.py)
...
R_MARKER_LINE=$(grep -n "F-R7-611" src/bob3/brownfield/resurrection.py | head -1 | cut -d: -f1)
R_GATE_LINE=$(grep -n "deep_resurrection_scan" src/bob3/brownfield/resurrection.py | head -1 | cut -d: -f1)
...
ELICIT_CONTENT=$(cat src/bob3/brownfield/elicit.py)
...
E_MARKER_LINE=$(grep -n "F-R7-611" src/bob3/brownfield/elicit.py | head -1 | cut -d: -f1)
E_AUQ_LINE=$(grep -n "AskUserQuestion" src/bob3/brownfield/elicit.py | head -1 | cut -d: -f1)
...
E_FM_LINE=$(grep -n "feature.mode" src/bob3/brownfield/elicit.py | head -1 | cut -d: -f1)
...
assert_python_import "bob3.brownfield imports cleanly" \
    "bob3.brownfield"

assert_python_import "bob3.brownfield.survey imports cleanly" \
    "bob3.brownfield.survey"

assert_python_import "bob3.brownfield.resurrection imports cleanly" \
    "bob3.brownfield.resurrection"

assert_python_import "bob3.brownfield.elicit imports cleanly" \
    "bob3.brownfield.elicit"
...
assert_python_eval "ResurrectionConfig.deep_resurrection_scan defaults to False" \
    "from bob3.brownfield.resurrection import ResurrectionConfig; c = ResurrectionConfig(); assert c.deep_resurrection_scan == False"

# deep_resurrection_scan can be set to True
assert_python_eval "ResurrectionConfig.deep_resurrection_scan can be True" \
    "from bob3.brownfield.resurrection import ResurrectionConfig; c = ResurrectionConfig(deep_resurrection_scan=True); assert c.deep_resurrection_scan == True"
...
assert_python_eval "Signal-B and Signal-C empty when deep_resurrection_scan=False" \
    "from bob3.brownfield.resurrection import run_resurrection_scan, ResurrectionConfig; r = run_resurrection_scan('/tmp', config=ResurrectionConfig(deep_resurrection_scan=False)); assert r.signal_b_exports_without_impl == []; assert r.signal_c_todo_clusters == []"

# elicit in interactive mode emits AskUserQuestion
assert_python_eval "elicit interactive mode emits AskUserQuestion" \
    "from bob3.brownfield.elicit import elicit, ElicitationRequest, MODE_INTERACTIVE; r = elicit(ElicitationRequest(intent_stub='test'), feature_mode=MODE_INTERACTIVE); assert r.ask_user_question_emitted == True"

# elicit in headless mode returns candidates
assert_python_eval "elicit headless mode returns candidates" \
    "from bob3.brownfield.elicit import elicit, ElicitationRequest, MODE_HEADLESS; r = elicit(ElicitationRequest(intent_stub='test'), feature_mode=MODE_HEADLESS); assert len(r.candidates) > 0"

# elicit_from_feature dispatches on feature.mode
assert_python_eval "elicit_from_feature dispatches on feature.mode" \
    "from bob3.brownfield.elicit import elicit_from_feature; class F: mode='headless'; description='test'; research_notes=''; r = elicit_from_feature(F()); assert r.mode == 'headless'"

# _cache_key is deterministic
assert_python_eval "survey cache key is deterministic" \
    "from bob3.brownfield.survey import _cache_key; from pathlib import Path; k1 = _cache_key(Path('/tmp'), '**/*.py', ['a', 'b']); k2 = _cache_key(Path('/tmp'), '**/*.py', ['b', 'a']); assert k1 == k2, 'cache key not deterministic across symbol order'"
