"""Tests for bob3.cost_control (bb0c4b7a).
...
import bob3.cost_control as cost_control_mod
from bob3.cost_control import (
...
class TestModuleExists:
    ...
    def test_module_importable(self):
        """bob3.cost_control must be importable without error."""
        import bob3.cost_control  # noqa: F401

    def test_enforce_per_attempt_cost_cap_defined(self):
        """bob3.cost_control.enforce_per_attempt_cost_cap must be a callable."""
        assert callable(enforce_per_attempt_cost_cap)

    def test_terminate_subagent_on_cap_defined(self):
        """bob3.cost_control.terminate_subagent_on_cap must be a callable."""
...
class TestEnforcePerAttemptCostCapTriggersTermination:
    ...
    def test_cost_above_default_cap_returns_true(self, monkeypatch):
        ...
        with patch("bob3.cost_control.terminate_subagent_on_cost_cap") as mock_term:
            result = enforce_per_attempt_cost_cap(
                feature_id=_FEATURE_ID,
                pid=_SAFE_PID,
                reported_cost=10.01,
    ...
    def test_custom_cap_via_env_var(self, monkeypatch):
        """Custom cap via BOB3_PER_ATTEMPT_COST_CAP is respected."""
        monkeypatch.setenv("BOB3_PER_ATTEMPT_COST_CAP", "5.0")
...
class TestTerminateSubagentOnCap:
    ...
    def test_returns_none(self, monkeypatch):
        ...
        with patch("bob3.cost_control.terminate_subagent_on_cost_cap"):
...
class TestOrchestratorIntegration:
    """AC: integration: bob3.orchestrator — cost_control works alongside orchestrator modules."""
    ...
    def test_orchestrator_per_attempt_cost_cap_importable_alongside(self):
        ...
        from bob3.cost_control import enforce_per_attempt_cost_cap as ctrl_enforce
    ...
    def test_cost_control_and_cost_cap_consistent_default(self, monkeypatch):
        ...
        monkeypatch.delenv("BOB3_PER_ATTEMPT_COST_CAP", raising=False)
        from bob3.orchestrator.per_attempt_cost_cap import get_per_attempt_cap
        cap = get_per_attempt_cap()
        assert cap == 10.0

    def test_env_override_propagates_to_cost_control(self, monkeypatch):
        """BOB3_PER_ATTEMPT_COST_CAP env var override is visible from cost_control."""
        monkeypatch.setenv("BOB3_PER_ATTEMPT_COST_CAP", "7.5")
        from bob3.orchestrator.per_attempt_cost_cap import get_per_attempt_cap
        cap = get_per_attempt_cap()
        assert cap == 7.5
