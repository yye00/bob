...
- Function defined: bob3.brownfield.resurrection.detect_resurrection_signals
- Function defined: bob3.brownfield.resurrection.write_resurrection_report
...
def test_write_resurrection_report_creates_parent_dirs(tmp_path: pathlib.Path):
    ...
    # Use a deeply nested bob3_root to test mkdir -p behavior
    deep_root = tmp_path / "a" / "b" / "c"
    report_path = write_resurrection_report(
        feature_id="deep-test",
        signals=signals,
        bob3_root=str(deep_root),
    )
    assert pathlib.Path(report_path).exists()


# ---------------------------------------------------------------------------
# integration: bob3.orchestrator imports from bob3.brownfield.resurrection
# ---------------------------------------------------------------------------
