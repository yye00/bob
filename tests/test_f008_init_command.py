"""Tests for F008: Implement 'bob init <project>' command to create new project."""
...
class TestDatabaseInitialization:
    ...
    def test_database_file_created(self, tmp_path):
        ...
        assert db_path.exists(), "bob.db must be created in project workspace"
...
# ============================================================
# Step 6: Run 'bob init test-project' and verify database created
# ============================================================
...
# ============================================================
# R9-008: bob init must honor BOB_DATABASE_PATH
# ============================================================


class TestInitRespectsBobDatabasePath:
    """Regression test for R9-008.

    When BOB_DATABASE_PATH is set (the recommended hardened-deployment
    pattern: DB outside the workspace so sub-agents can't tamper with
    budget data), ``bob init <project>`` must create the database at
    that path — NOT at ``<project>/bob.db``. Otherwise subsequent
    ``bob plan`` / ``bob run`` / ``bob status`` commands (which all
    ...
    def test_init_uses_env_path_when_set(self, tmp_path, monkeypatch):
        """With BOB_DATABASE_PATH set, the DB lands at the env path,
        not inside the workspace."""
        from bob.cli import main

        secure_dir = tmp_path / "secure"
        env_db_path = secure_dir / "bob.db"
        ...
        monkeypatch.setenv("BOB_DATABASE_PATH", str(env_db_path))
        ...
        assert env_db_path.exists(), (
            f"Database must be created at BOB_DATABASE_PATH={env_db_path}, "
            f"got: {result.output}"
        )

        # The DB did NOT land inside the workspace.
        workspace_db = project_path / "bob.db"
        assert not workspace_db.exists(), (
            f"Database must NOT be created at workspace path "
            f"{workspace_db} when BOB_DATABASE_PATH is set"
        )

    def test_init_falls_back_to_workspace_when_env_unset(
        self, tmp_path, monkeypatch
    ):
        """Without BOB_DATABASE_PATH, init keeps the legacy behavior:
        the DB lives inside the workspace at <project>/bob.db."""
        from bob.cli import main
        ...
        assert workspace_db.exists(), (
            "Without BOB_DATABASE_PATH, init must create "
            "<project>/bob.db (legacy behavior)"
        )
    ...
    def test_subsequent_status_finds_project_via_env_path(
        ...
        """End-to-end sanity: after init with BOB_DATABASE_PATH set,
        running ``bob status`` (which goes through get_database_path
