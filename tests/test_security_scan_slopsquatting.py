...
def test_whitelist_includes_src_bob3_module(tmp_path: Path) -> None:
    """Modules in src/bob3/*.py are whitelisted."""
...
def test_spec_quality_score_local_module_whitelisted(tmp_path: Path) -> None:
    """spec_quality_score local module is whitelisted when in src/bob3/."""
    src = tmp_path / "src" / "bob3"
    src.mkdir(parents=True)
    (src / "__init__.py").write_text("")
...
def test_slopsquatting_check_skips_whitelisted_local_module(tmp_path: Path) -> None:
    ...
    When spec_quality_score.py exists in src/bob3/, it is local and must
    not be probed against PyPI even if the diff adds 'import spec_quality_score'.
    """
    from bob3.security_checks import _run_slopsquatting
...
def test_whitelist_nested_package_modules(tmp_path: Path) -> None:
    """Modules in nested packages (src/bob3/subpkg/*.py) are whitelisted."""
