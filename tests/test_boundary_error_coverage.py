...
class TestGetProseAcs:
    ...
    def test_filters_function_defined(self):
        result = get_prose_acs(["Function defined: bob.foo.bar"])
        assert result == []
    ...
    def test_filters_integration(self):
        result = get_prose_acs(["integration: bob.memory"])
        assert result == []

    def test_filters_class_defined(self):
        result = get_prose_acs(["Class defined: bob.foo.MyClass"])
        assert result == []
    ...
    def test_filters_file_modified(self):
        result = get_prose_acs(["file modified: src/bob/foo.py"])
        assert result == []
...
class TestEnsureBoundaryAndErrorCoverage:
    ...
    def test_slug_with_failing_does_not_satisfy_error_coverage(self):
        ...
        criteria = [
            "File exists: src/bob/failing_tests_handler.py",
            "Function defined: bob.failing_tests_handler.process",
    ...
    def test_slug_with_length_capped_does_not_satisfy_boundary_coverage(self):
        ...
        criteria = [
            "File exists: src/bob/derived_module_slug_length_capped.py",
            "Function defined: bob.derived_module_slug_length_capped.cap",
...
# ---------------------------------------------------------------------------
# Integration: dark_factory.boundary_error_coverage ↔ bob.reward_hacking_detector
# ---------------------------------------------------------------------------


class TestIntegrationWithRewardHackingDetector:
    """Verify dark_factory.boundary_error_coverage is importable and usable
    from the same Python environment as bob.reward_hacking_detector."""

    def test_reward_hacking_detector_importable(self):
        import bob.reward_hacking_detector  # noqa: F401

    def test_dark_factory_importable_alongside_bob(self):
    ...
    def test_combined_detection_on_real_feature_acs(self):
        ...
        criteria = [
            "File exists: src/bob/derived_module_slug.py",
            "Function defined: bob.derived_module_slug.cap_slug",
            "pytest: tests/test_derived_module_slug_length_capped.py",
