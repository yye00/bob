...
from bob.skill_library.registry import similarity_threshold
...
def test_load_from_env_returns_default_when_env_not_set(monkeypatch):
    monkeypatch.delenv("BOB_SKILL_LIBRARY_EMBED_MODEL", raising=False)
    assert load_from_env() == "sentence-transformers/all-MiniLM-L6-v2"


def test_load_from_env_honours_env_var(monkeypatch):
    monkeypatch.setenv("BOB_SKILL_LIBRARY_EMBED_MODEL", "custom/model-name")
