"""Tests for bob.run_loop.handle_pending_successor_verify (7525d70f).

Verifies that run_loop exposes handle_pending_successor_verify as a public
function that correctly delegates to bob.status_handler.
"""
...
class TestHandlePendingSuccessorVerifyImport:
    """handle_pending_successor_verify must be importable from bob.run_loop."""
...
class TestHandlePendingSuccessorVerifyDelegation:
    ...
    def test_nonexistent_workspace_does_not_raise(self):
        from bob.run_loop import handle_pending_successor_verify
        nonexistent = Path("/tmp/bob_test_nonexistent_run_loop_9z9z9")
        result = handle_pending_successor_verify(
...
class TestPendingSuccessorVerifyStatus:
    ...
    def test_detect_pending_successor_verify_accessible(self):
        from bob.run_loop import detect_pending_successor_verify
