...
from bob.spec_quality.spec_extractor import ACFilterResult

_VERIFIER_TARGET = "src/bob/enhanced_verification.py"
_NORMAL_TARGET = "src/bob/some_normal_module.py"
...
class TestIntegrationWithSpecExtractor:
    """Tests that verifier.spec_extractor integrates with bob.spec_quality.spec_extractor."""

    def test_same_verifier_extension_modules(self):
        """verifier.spec_extractor uses same VERIFIER_EXTENSION_MODULES as bob."""
        from bob.spec_quality.spec_extractor import (
            VERIFIER_EXTENSION_MODULES as BOB_MODULES,
        )
        from verifier.spec_extractor import (
            VERIFIER_EXTENSION_MODULES as VERIFIER_MODULES,
        )
        assert VERIFIER_MODULES == BOB_MODULES

    def test_consistent_demotion_behavior(self):
        """verifier.spec_extractor and bob.spec_extractor agree on demotion."""
        from bob.spec_quality.spec_extractor import (
        ...
        result_bob = filter_behavior_acs_for_verifier_extension(acs, _VERIFIER_TARGET)
        assert len(result_verifier.demoted) == len(result_bob.demoted)
        assert result_verifier.is_verifier_extension == result_bob.is_verifier_extension
