...
- CLI command quality-gate mutmut is registered on the bob3 CLI
...
def test_quality_gate_mutmut_cli_registered():
    """The bob3 CLI must expose 'quality-gate mutmut' as a registered command."""
    ...
    assert result.exit_code == 0, (
        f"'bob3 quality-gate --help' failed with exit code {result.exit_code}:\n"
        f"{result.output}"
    )
...
def test_quality_gate_mutmut_help():
    """'bob3 quality-gate mutmut --help' must succeed and describe the command."""
    from click.testing import CliRunner
    from bob3.cli import main
    ...
    assert result.exit_code == 0, (
        f"'bob3 quality-gate mutmut --help' failed:\n{result.output}"
    )
