...
Asserts tools.spec_quality_score._is_code_identifier is importable and behaves deterministically on empty/zero/
...
def test_boundary_importable_and_defined():
    ...
    assert hasattr(mod, 'is_code_shaped_token'), 'is_code_shaped_token' + " must be defined in " + 'tools.spec_quality_score'
    obj = getattr(mod, 'is_code_shaped_token')
    assert callable(obj), 'is_code_shaped_token' + " must be callable"
