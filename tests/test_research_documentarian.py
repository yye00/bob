"""Tests for bob3.research_documentarian.document_subsystem (BF-2).
...
class TestDocumentSubsystemBasic:
    ...
    def test_returns_dict(self):
        from bob3.research_documentarian import document_subsystem
...
class TestDocumentSubsystemPrompt:
    ...
    def test_prompt_contains_path_glob(self):
        from bob3.research_documentarian import document_subsystem
        result = document_subsystem(path_glob="src/bob3/orchestrator/**")
        assert "src/bob3/orchestrator/**" in result["researcher_prompt"]
...
class TestDocumentSubsystemCache:
    ...
    def test_cache_hit_when_notes_match(self, tmp_path):
        ...
        notes_dir = tmp_path / ".bob3" / "features" / "feat-cached"
    ...
    def test_cache_miss_when_sha_differs(self, tmp_path):
        from bob3.research_documentarian import document_subsystem
        notes_dir = tmp_path / ".bob3" / "features" / "feat-stale"
        notes_dir.mkdir(parents=True)
        notes_file = notes_dir / "research_notes.md"
        notes_file.write_text(
            "---\nsurvey_sha: old_sha\npath_glob: src/**\n---\nStale.\n"
