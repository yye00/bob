"""Tests for bob3.ac_repairer — repair_smelly_acs and verify_semantic_equivalence."""
...
from bob3.ac_repairer import repair_smelly_acs, verify_semantic_equivalence
...
class TestVerifySemanticEquivalence:
    def test_returns_true_when_judge_says_equivalent(self):
        ...
        with patch("bob3.ac_repairer._call_llm_judge", return_value=mock_response):
    ...
    def test_returns_false_on_llm_failure(self):
        with patch("bob3.ac_repairer._call_llm_judge", side_effect=Exception("timeout")):
...
class TestRepairSmellAcs:
    ...
    def test_spec_critic_integration_import(self):
        """Verify the module can be imported from bob3.ac_repairer (spec_critic integration)."""
        from bob3.ac_repairer import repair_smelly_acs as rsa
        from bob3.ac_repairer import verify_semantic_equivalence as vse
        assert callable(rsa)
        assert callable(vse)
