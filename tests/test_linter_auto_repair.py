"""Tests for bob3.linter.auto_repair — apply_semantic_equivalence_check and should_auto_repair."""
...
class TestApplySemanticEquivalenceCheckHappyPath:
    ...
    def test_equivalent_true_returned_when_judge_says_true(self):
        ...
        with patch("bob3.linter.auto_repair._call_llm_judge", return_value=resp):
    ...
    def test_llm_failure_returns_false_with_error_message(self):
        with patch("bob3.linter.auto_repair._call_llm_judge", side_effect=RuntimeError("network")):
    ...
    def test_empty_strings_return_false_on_llm_failure(self):
        with patch("bob3.linter.auto_repair._call_llm_judge", side_effect=Exception("empty input")):
            is_equiv, rationale = apply_semantic_equivalence_check("", "")
        assert is_equiv is False
        assert isinstance(rationale, str)
...
class TestShouldAutoRepairWithSmellFinding:
    """Tests using real SmellFinding objects from bob3.linter."""
