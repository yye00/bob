...
from bob.security.slopsquatting_scan import (
    find_local_modules,
...
def workspace_with_local_module(tmp_path: Path) -> Path:
    """Fixture with a src/bob/ tree that includes spec_quality_score as a local module."""
    src_bob = tmp_path / "src" / "bob"
    src_bob.mkdir(parents=True)
    # local module with same name as a real local file
    (src_bob / "spec_quality_score.py").write_text("# spec quality scoring\n")
    return tmp_path
...
def test_local_module_same_name_as_real_pypi_prefers_local(tmp_path: Path) -> None:
    ...
    (src_bob / "requests.py").write_text("# local override of requests\n")
