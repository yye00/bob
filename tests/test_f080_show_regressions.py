...
class TestShowRegressionsCommandRegistered:
    ...
    def test_show_regressions_uses_rich_table(self):
        ...
        from bob.cli import show_regressions_cmd
...
class TestShowRegressionsQueryView:
    ...
    def test_calls_query_active_regressions(self):
        ...
        runner = CliRunner()
        with patch("bob.cli.query_active_regressions", return_value=[]) as mock_query, \
...
class TestShowRegressionsDisplay:
    ...
    def test_displays_affected_feature_name(self):
        ...
        runner = CliRunner()
        with patch("bob.cli.query_active_regressions", return_value=entries), \
             patch("bob.cli._get_current_project_id", return_value="proj-1"):
            result = runner.invoke(main, ["show-regressions"])
        assert result.exit_code == 0
    ...
    def test_no_regressions_message(self):
        ...
        runner = CliRunner()
        with patch("bob.cli.query_active_regressions", return_value=[]), \
             patch("bob.cli._get_current_project_id", return_value="proj-1"):
            result = runner.invoke(main, ["show-regressions"])
        assert result.exit_code == 0
        output_lower = result.output.lower()
...
class TestShowRegressionsWithRealData:
    ...
    def test_show_regressions_with_real_db(self, db_with_regressions):
        ...
        with patch("bob.db.get_database_path", return_value=db_path), \
             ...
             patch("bob.cli.query_active_regressions", wraps=query_active_regressions) as mock_query:
            result = runner.invoke(main, ["show-regressions"])
