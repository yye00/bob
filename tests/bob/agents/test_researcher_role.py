"""Tests for bob.agents.roles — researcher role and related helpers.
...
class TestResearcherPromptFunction:
    ...
    def test_empty_path_glob_returns_empty(self):
        from bob.agents.roles import researcher_prompt
        result = researcher_prompt()
        assert result == ""

    def test_with_path_glob_returns_nonempty(self):
        from bob.agents.roles import researcher_prompt
        result = researcher_prompt(path_glob="src/bob/**")
        assert len(result) > 0
...
class TestBuildResearcherPrompt:
    ...
    def test_path_glob_in_output(self):
        ...
        prompt = build_researcher_prompt("src/bob/**", [])
...
class TestResearchNotesPath:
    ...
    def test_dot_bob_in_path(self, tmp_path):
        from bob.agents.roles import research_notes_path
        result = research_notes_path("feat-abc", tmp_path)
...
class TestRoleRegistry:
    ...
    def test_all_roles_includes_researcher(self):
        from bob.agents.roles import all_roles, RESEARCHER
        assert RESEARCHER in all_roles()


class TestOrchestratorIntegration:
    """Verify bob.agents.roles integrates with orchestrator coordinator."""
