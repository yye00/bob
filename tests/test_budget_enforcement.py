...
- bob.orchestrator.enforce_budget_on_zero_cost is importable and callable
...
from bob.orchestrator import enforce_budget_on_zero_cost
from bob.orchestrator.cost_telemetry_guard import EnforceBudgetResult
...
def test_zero_cost_with_high_work_events_enforces_ceiling():
    ...
    Replicates the failure observed in bob v.15 r12 (2026-05-29):
...
# --- Integration AC: enforce_budget_on_zero_cost in bob.orchestrator ---

def test_enforce_budget_on_zero_cost_importable_from_orchestrator():
    """AC: Function defined: bob.orchestrator.enforce_budget_on_zero_cost."""
    fn = getattr(bob.orchestrator, "enforce_budget_on_zero_cost", None)
    assert fn is not None, "enforce_budget_on_zero_cost not found in bob.orchestrator"
    assert callable(fn)


def test_enforce_budget_on_zero_cost_callable_from_orchestrator_namespace():
    """Function accessible via bob.orchestrator namespace works end-to-end."""
    fn = bob.orchestrator.enforce_budget_on_zero_cost
    result = fn(
        reported_cost=0.0,
        work_events=500,
...
def test_invalid_input_negative_ceiling_raises_value_error():
    ...
    named entry point in bob.cost_enforcement.enforce_zero_cost_policy is the
    canonical ValueError raiser. Here we verify via orchestrator proxy.
    """
    from bob.orchestrator import enforce_zero_cost_policy
