...
Covers:
  ...
  - emit_telemetry: CTX_BUDGET_KILL events to .bob/events.jsonl
...
def test_main_block_when_over_budget(tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setenv("BOB_FEATURE_ID", "feat-xyz")
    payload = json.dumps({
        "session_id": "sess-abc",
...
def test_main_emits_telemetry_on_block(tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setenv("BOB_WORKSPACE", str(tmp_path))
    monkeypatch.setenv("BOB_FEATURE_ID", "feat-telemetry-test")
    payload = json.dumps({
        "session_id": "sess-tel",
        "transcript_path": str(transcript),
    })
