"""Tests for bob.linter_ac_repair — semantic_equivalence_check and auto_repair_ac."""
...
from bob.linter_ac_repair import auto_repair_ac, semantic_equivalence_check
...
class TestSemanticEquivalenceCheck:
    ...
    def test_llm_failure_returns_false(self):
        with patch(
            "bob.linter_ac_repair._call_llm_judge",
...
class TestAutoRepairAc:
    ...
    def test_opt_out_prevents_repair(self, tmp_path):
        finding = _make_finding()
        resp = _make_response("EQUIVALENT: true\nRATIONALE: Same.")
        with patch("bob.linter_ac_repair._call_llm_judge", return_value=resp) as mock_judge:
            result = auto_repair_ac(
                feature_id="feat-004",
                findings=[finding],
                original_acs=["The system should process requests."],
...
# ---------------------------------------------------------------------------
# Integration: bob.linter exports auto_repair_ac and semantic_equivalence_check
# ---------------------------------------------------------------------------


class TestLinterIntegration:
    def test_auto_repair_ac_importable_from_bob_linter(self):
        from bob.linter import auto_repair_ac as _auto_repair_ac
        from bob.linter import semantic_equivalence_check as _sem_check

        assert callable(_auto_repair_ac)
        assert callable(_sem_check)

    def test_auto_repair_ac_from_linter_is_same_function(self):
        from bob.linter import auto_repair_ac as linter_auto_repair
        from bob.linter_ac_repair import auto_repair_ac as direct_auto_repair

        assert linter_auto_repair is direct_auto_repair

    def test_semantic_equivalence_check_from_linter_is_same_function(self):
        from bob.linter import semantic_equivalence_check as linter_sem_check
        from bob.linter_ac_repair import semantic_equivalence_check as direct_sem_check
