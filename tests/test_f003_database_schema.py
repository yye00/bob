"""Tests for F003: Database schema file (schema.sql) with complete Bob v2.1 schema."""
...
SCHEMA_PATH = WORKSPACE / "src" / "bob" / "schema.sql"


class TestSchemaFileExists:
    """Step 1: src/bob/schema.sql must exist."""

    def test_schema_file_exists(self):
        assert SCHEMA_PATH.is_file(), "src/bob/schema.sql must exist"
...
class TestBobSpecificTables:
    """Step 3: Bob-specific tables must be present."""
    ...
    @pytest.mark.parametrize("table_name", BOB_TABLES)
    def test_bob_table_exists(self, table_name):
        tables = self._get_tables()
        assert table_name in tables, f"Bob table '{table_name}' must exist in schema"
...
class TestTitansMemoryExclusion:
    ...
    def test_schema_has_titans_memory_comments(self):
        """Schema should have comments about bob-memory handling project memory and lessons.

        Originally checked for "TITANS Memory MCP" wording; bob now uses
        bob-memory (the legacy column name `titans_memory_id` is intentionally
        retained for backwards compatibility).
        """
        content = SCHEMA_PATH.read_text()
        assert "bob-memory" in content, (
            "Schema should mention bob-memory (the external memory backend)"
        )
...
class TestKeyTableColumns:
    ...
    def test_sub_agent_runs_has_mcp_enabled(self):
        ...
        assert "mcp_enabled" in cols, "sub_agent_runs should have mcp_enabled column for Bob"
