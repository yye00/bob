"""Tests for bob3.db.create_agent_run — direct database layer.

Verifies that bob3.db.create_agent_run:
  - is importable and callable from bob3.db
  ...
  - respects BOB3_DATABASE_PATH when db_path is not supplied
"""
...
def test_create_agent_run_importable_from_bob3_db():
    """create_agent_run must be directly importable from bob3.db."""
...
def test_env_var_honored_when_no_db_path(monkeypatch):
    """When db_path is omitted, BOB3_DATABASE_PATH env var must be used."""
    with tempfile.TemporaryDirectory() as td:
...
def test_explicit_db_path_overrides_env_var(monkeypatch):
    """Explicit db_path must take precedence over BOB3_DATABASE_PATH."""
    with tempfile.TemporaryDirectory() as td:
...
def test_synthesize_for_feature_importable():
    """bob3.orchestrator must expose synthesize_for_feature."""
...
def test_create_agent_run_importable_from_db_module():
    """bob3.db.create_agent_run must be the same callable as the one used above."""
