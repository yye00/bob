"""Tests for bob.brownfield.patch_planner — BF-7 CodeT patch-mode + reviewable diff-plan artifact."""
...
class TestEmitDiffPlan:
    ...
    def test_plan_path_under_bob_features_dir(self, feature_id, scratch_dir, minimal_diff_plan):
        plan_path = emit_diff_plan(feature_id, minimal_diff_plan["touches"], workspace=scratch_dir)

        expected_dir = scratch_dir / ".bob" / "features" / feature_id
...
class TestApplyDiffPlan:
    ...
    def test_backup_created_before_apply(
        ...
        orig_path = scratch_dir / ".bob" / "features" / feature_id / "orig" / "src" / "auth" / "login.py"
...
class TestRollbackChanges:
    ...
    def test_rollback_clears_orig_dir(
        ...
        orig_dir = scratch_dir / ".bob" / "features" / feature_id / "orig"
...
# ---------------------------------------------------------------------------
# Integration: bob.coordinator import
# ---------------------------------------------------------------------------


class TestCoordinatorIntegration:
    def test_coordinator_module_imports_patch_planner(self):
        """AC: integration: bob.coordinator — patch_planner must be importable from coordinator path."""
        ...
        from bob.brownfield import patch_planner  # noqa: F401
    ...
    def test_patch_planner_exported_in_brownfield_init(self):
        import bob.brownfield as bf
        # patch_planner should be importable via the package
        from bob.brownfield.patch_planner import emit_diff_plan, apply_diff_plan, rollback_changes  # noqa: F401

    def test_patch_planner_class_importable(self):
        """AC: Function defined: bob.brownfield.patch_planner.PatchPlanner."""
        from bob.brownfield.patch_planner import PatchPlanner
