"""Tests for bob3.synthesizer_must_emit_file_exists_acs_py_paths_named.
...
from bob3.synthesizer_must_emit_file_exists_acs_py_paths_named import (
...
class TestIsCodeShapedToken:
    ...
    def test_dot_is_code_shaped(self):
        assert is_code_shaped_token("bob3.spec_synthesizer") is True
...
class TestFilterApiSurfaces:
    ...
    def test_keeps_code_shaped_tokens(self):
        tokens = ["my_func", "bob3.module", "survey.py", "RetryCounter", "defined"]
        result = filter_api_surfaces(tokens)
        assert "my_func" in result
        assert "bob3.module" in result
...
class TestExtractPyPaths:
    def test_extracts_path_with_directory(self):
        desc = "See src/bob3/brownfield/survey.py for details."
    ...
    def test_deduplicates(self):
        desc = "See src/bob3/foo.py and also src/bob3/foo.py again."
        paths = extract_py_paths(desc)
    ...
    def test_extract_concrete_py_paths_is_alias(self):
        desc = "File at src/bob3/brownfield/survey.py."
...
class TestEmitFileExistsAcsForPyPaths:
    def test_adds_file_exists_ac_for_uncovered_path(self):
        desc = "This feature creates src/bob3/brownfield/survey.py."
        criteria = ["Function defined: bob3.brownfield.survey.run"]
        augmented, added = emit_file_exists_acs_for_py_paths(criteria, desc)
        assert "File exists: src/bob3/brownfield/survey.py" in augmented
        assert "src/bob3/brownfield/survey.py" in added

    def test_does_not_duplicate_already_covered_path(self):
        desc = "This feature creates src/bob3/brownfield/survey.py."
        criteria = ["File exists: src/bob3/brownfield/survey.py"]
        augmented, added = emit_file_exists_acs_for_py_paths(criteria, desc)
        file_exists_count = sum(
            1 for ac in augmented if "src/bob3/brownfield/survey.py" in ac
        )
    ...
    def test_no_py_paths_in_description_returns_unchanged(self):
        ...
        criteria = ["Function defined: bob3.foo.bar"]
        augmented, added = emit_file_exists_acs_for_py_paths(criteria, desc)
    ...
    def test_multiple_paths_all_added(self):
        desc = "Creates src/bob3/foo.py and src/bob3/bar.py."
        criteria = []
        augmented, added = emit_file_exists_acs_for_py_paths(criteria, desc)
        assert "File exists: src/bob3/foo.py" in augmented
        assert "File exists: src/bob3/bar.py" in augmented
        assert len(added) == 2
...
class TestExtractCodeShapedApiSurfaces:
    ...
    def test_extracts_py_paths(self):
        desc = "See src/bob3/brownfield/survey.py."
        surfaces = extract_code_shaped_api_surfaces(desc)
        assert "src/bob3/brownfield/survey.py" in surfaces
...
def test_synthesizer_must_emit_file_exists_acs_py_paths_named():
    ...
    description = (
        ...
        "src/bob3/brownfield/survey.py explicitly but synthesis derived a "
        "different slug, the path was uncovered. "
        "Fix: emit File-exists AC for src/bob3/brownfield/survey.py and "
        "only treat code-shaped tokens as API surfaces."
    )
    existing_criteria = [
        "Function defined: bob3.brownfield.survey.some_function",
    ...
    assert "File exists: src/bob3/brownfield/survey.py" in result["criteria"], (
        "Expected File-exists AC for src/bob3/brownfield/survey.py"
    )
    assert "src/bob3/brownfield/survey.py" in result["added_paths"]
...
def test_synthesizer_none_criteria_defaults_to_empty():
    ...
    result = synthesizer_must_emit_file_exists_acs_py_paths_named(
        description="Feature at src/bob3/foo.py.",
        acceptance_criteria=None,
    )
    assert isinstance(result["criteria"], list)
    assert "File exists: src/bob3/foo.py" in result["criteria"]
...
def test_synthesizer_no_duplicate_file_exists_acs():
    ...
    desc = "Creates src/bob3/brownfield/survey.py."
    criteria = ["File exists: src/bob3/brownfield/survey.py"]
    result = synthesizer_must_emit_file_exists_acs_py_paths_named(
        description=desc,
        acceptance_criteria=criteria,
