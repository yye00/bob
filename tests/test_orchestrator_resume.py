"""Tests for bob.orchestrator.resume_interrupted_work (feature 2d9615ff).

Verifies:
- resume_interrupted_work is defined in bob.orchestrator
- It promotes 'interrupted' features to 'ready' on every loop tick
- Integration with bob.orchestrator.run_loop
"""
...
# ---------------------------------------------------------------------------
# AC: Function defined: bob.orchestrator.resume_interrupted_work
# ---------------------------------------------------------------------------


class TestResumeInterruptedWorkDefined:
    def test_function_exists_in_orchestrator(self):
        orchestrator = importlib.import_module("bob.orchestrator")
        assert hasattr(orchestrator, "resume_interrupted_work"), (
            "bob.orchestrator must expose 'resume_interrupted_work' as a "
    ...
    def test_importable_directly(self):
        from bob.orchestrator import resume_interrupted_work as fn  # noqa: F401
