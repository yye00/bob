"""Boundary-case tests for bob.synthesizer transient-upstream retry behavior.
...
from bob.synthesizer import synthesize_for_feature, score_gate_loop, ScoreGateReport
...
class TestSynthesizeForFeatureBoundary:
    ...
    async def test_empty_description_does_not_raise(self):
        ...
        with patch("bob.spec_synthesizer._llm_spawn_synthesizer", side_effect=empty_response):
    ...
    async def test_max_attempts_one_does_not_hang(self):
        """BOB_SYNTH_MAX_ATTEMPTS=1 is minimum: exactly one attempt, returns promptly."""
        call_count = 0
    ...
    async def test_empty_project_context_does_not_raise(self):
        ...
        with patch("bob.spec_synthesizer._llm_spawn_synthesizer", side_effect=valid_response):
            with patch("asyncio.sleep", new_callable=AsyncMock):
                result = await synthesize_for_feature(
                    project_id="proj-1",
