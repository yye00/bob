...
from bob3.skill_library.embeddings import (
    default_backend,
    load_from_env,
...
def test_get_embedding_model_raises_import_error_when_fastembed_missing(monkeypatch):
    ...
    import bob3.skill_library.embeddings as emb_mod
...
def test_load_from_env_invalid_model_name_not_validated_at_load_time(monkeypatch):
    ...
    monkeypatch.setenv("BOB3_SKILL_LIBRARY_EMBED_MODEL", "invalid/model-name-that-does-not-exist")
...
def test_embed_texts_raises_when_fastembed_missing(monkeypatch):
    ...
    try:
        with pytest.raises(ImportError):
            from bob3.skill_library.embeddings import embed_texts
