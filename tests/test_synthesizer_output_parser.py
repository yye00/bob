"""Tests for bob.synthesizer_output_parser.
...
from bob.synthesizer_output_parser import (
...
class TestParseCriteriaResponseObjectFormat:
    ...
    def test_objects_with_text_key(self):
        text = '```json\n[{"text": "Function defined: bob.mymodule.my_func"}]\n```'
        result = parse_criteria_response(text)
        assert result == ["Function defined: bob.mymodule.my_func"]
...
class TestInjectMissingBoundaryAndErrorAcs:
    ...
    def test_slug_references_feature_in_injected_acs(self):
        criteria = ["File exists: src/bob/mymodule.py"]
