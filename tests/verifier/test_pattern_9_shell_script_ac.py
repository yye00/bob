"""Tests for bob3.verifier.pattern_9_shell_script_ac (F-R7-594).
...
from bob3.verifier.pattern_9_shell_script_ac import (
...
def test_is_shell_script_integration_ac_false_for_python_module() -> None:
    assert is_shell_script_integration_ac("integration: bob3.verifier") is False
...
def test_demote_false_for_non_shell_integration_ac(tmp_path: pathlib.Path) -> None:
    assert should_demote_to_pass_with_warning("integration: bob3.verifier", tmp_path) is False
...
def test_demote_emits_warning_log_on_pass(
    ...
    with caplog.at_level(logging.WARNING, logger="bob3.verifier.pattern_9_shell_script_ac"):
