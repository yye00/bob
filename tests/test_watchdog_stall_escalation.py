...
class TestStallEscalationThreshold:
    ...
    def test_env_override_respected(self, tmp_path):
        result = _run_watchdog_script(
            ...
            env={"BOB_STALL_ESCALATION_COUNT": "10"},
            tmpdir=tmp_path,
        )
        assert result.returncode == 0, result.stderr
    ...
    def test_env_clamped_minimum_to_2(self, tmp_path):
        result = _run_watchdog_script(
            "_stall_escalation_count",
            env={"BOB_STALL_ESCALATION_COUNT": "1"},
            tmpdir=tmp_path,
        )
        assert result.returncode == 0, result.stderr
    ...
    def test_env_clamped_maximum_to_60(self, tmp_path):
        result = _run_watchdog_script(
            "_stall_escalation_count",
            env={"BOB_STALL_ESCALATION_COUNT": "99"},
            tmpdir=tmp_path,
        )
        assert result.returncode == 0, result.stderr
    ...
    def test_non_numeric_env_falls_back_to_default(self, tmp_path):
        result = _run_watchdog_script(
            "_stall_escalation_count",
            env={"BOB_STALL_ESCALATION_COUNT": "banana"},
            tmpdir=tmp_path,
        )
        assert result.returncode == 0, result.stderr
        assert result.stdout.strip() == "5"
    ...
    def test_custom_threshold_env_boundary(self, tmp_path):
        """With BOB_STALL_ESCALATION_COUNT=3, sentinel appears at 3rd stall, not 5th."""
        sentinel = _sentinel_path(tmp_path, gen=4)
        sentinel.parent.mkdir(parents=True, exist_ok=True)
        chain_log = tmp_path / "chain.log"
        ...
        script_no_sentinel = textwrap.dedent(f"""
            ...
            CHAIN_LOG="{chain_log}"
            STATE_FILE="{tmp_path}/state.json"
            export BOB_STALL_ESCALATION_COUNT=3
        ...
        result = _run_watchdog_script(
            script_no_sentinel,
            env={"BOB_STALL_ESCALATION_COUNT": "3"},
            tmpdir=tmp_path,
        )
        assert result.returncode == 0, result.stderr
        assert not sentinel.exists(), \
        ...
        assert result.returncode == 0, result.stderr
        assert sentinel.exists(), \
            "Sentinel not written at Nth=3 stall with BOB_STALL_ESCALATION_COUNT=3"
...
class TestWatchdogFileExists:
    ...
    def test_bob_stall_escalation_count_env_referenced(self):
        content = WATCHDOG_SH.read_text()
        assert "BOB_STALL_ESCALATION_COUNT" in content
