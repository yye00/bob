"""Tests for disk_reconciler_verify_fail_check in bob3.run_loop.
...
def test_function_is_importable():
    """disk_reconciler_verify_fail_check must be importable from bob3.run_loop."""
    assert callable(disk_reconciler_verify_fail_check)


def test_function_in_all():
    """disk_reconciler_verify_fail_check must appear in bob3.run_loop.__all__."""
...
def test_returns_false_when_failed_gate_is_none():
    ...
    result = disk_reconciler_verify_fail_check(
        ...
        feature_id="feat-1",
        feature_name="Test",
        acceptance_criteria_json='["File exists: src/bob3/foo.py"]',
        failed_gate=None,
    )
...
def test_returns_false_when_only_integration_acs():
    ...
    result = disk_reconciler_verify_fail_check(
        ...
        feature_id="feat-1",
        feature_name="Test",
        acceptance_criteria_json='["integration: bob3.some_module"]',
        failed_gate="tests_pass",
    )
    assert result is False
...
@patch("bob3.orchestrator.disk_reconciler.check_executing_feature_acs", return_value=True)
...
@patch("bob3.orchestrator.disk_reconciler.check_executing_feature_acs", return_value=False)
...
def test_function_defined_ac_is_recognized_as_structural(mock_check):
    ...
    disk_reconciler_verify_fail_check(
        ...
        feature_id="feat-1",
        feature_name="Test",
        acceptance_criteria_json='["Function defined: bob3.run_loop.disk_reconciler_verify_fail_check"]',
        failed_gate="tests_pass",
    )
    mock_check.assert_called_once()
...
@patch("bob3.orchestrator.disk_reconciler.check_executing_feature_acs")
...
def test_empty_passed_gates_list_is_valid():
    ...
    with patch("bob3.orchestrator.disk_reconciler.check_executing_feature_acs", return_value=False):
        result = disk_reconciler_verify_fail_check(
            project_id="proj-1",
            feature_id="feat-1",
            feature_name="Test",
