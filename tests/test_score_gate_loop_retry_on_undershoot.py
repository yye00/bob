...
class TestScoreGateLoopRetryOnUndershoot:
    ...
    def test_retries_when_first_attempt_undershoots(self):
        ...
        call_count = {"n": 0}
        low_quality = ["File exists: src/bob3/x.py"]  # will score low
        high_quality = [
            "File exists: src/bob3/x.py",
            "Function defined: bob3.x.run",
    ...
    def test_gate_passed_when_retry_succeeds(self):
        ...
        call_count = {"n": 0}
        low_quality = ["File exists: src/bob3/x.py"]
        high_quality = [
            "File exists: src/bob3/x.py",
            "Function defined: bob3.x.run",
            "pytest: tests/test_x.py",
    ...
    def test_avg_attempts_reflects_retry_count(self):
        ...
        async def mock_synthesize(**kwargs):
            call_count["n"] += 1
            if call_count["n"] < 3:
                return ["File exists: src/bob3/y.py"]  # low quality
            return [
                "File exists: src/bob3/y.py",
                "pytest: tests/test_y.py",
                "Function defined: bob3.y.compute",
    ...
    def test_feedback_prompt_included_on_retry(self):
        ...
        async def mock_synthesize(retry_feedback=None, **kwargs):
            ...
            if len(received_feedback["values"]) == 1:
                return ["File exists: src/bob3/z.py"]  # undershoots
            return [
                "File exists: src/bob3/z.py",
