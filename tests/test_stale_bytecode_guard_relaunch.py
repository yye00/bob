...
from bob.stale_bytecode_guard_relaunch import stale_bytecode_guard_relaunch


@pytest.fixture()
def workspace(tmp_path):
    """Create a minimal workspace with src/bob/orchestrator/ directory."""
...
def test_stale_bytecode_guard_relaunch_missing_lock_file(workspace):
    """Returns True (conservative) when .bob.lock does not exist."""
    tmp_path, orch_dir, lock_file = workspace
...
def test_stale_bytecode_guard_relaunch_multiple_bob_dirs(tmp_path):
    """Checks all src/bob*/orchestrator/ dirs, not just bob."""
    for gen in ["bob", "bob4", "bob12"]:
