"""Tests for bob.criteria_parser — parse_criteria_response and inject_missing_boundary_error_acs."""
import pytest
from bob.criteria_parser import parse_criteria_response, inject_missing_boundary_error_acs
...
class TestParseCriteriaResponseObjectFormat:
    ...
    def test_objects_with_text_key(self):
        text = '```json\n[{"text": "integration: bob.synthesizer"}]\n```'
        result = parse_criteria_response(text)
        assert result == ["integration: bob.synthesizer"]
