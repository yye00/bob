"""Tests for the coverage heuristic in the triple-filter.
...
class TestCoverageFilter:
    def test_emitted_test_raises_coverage_heuristic(self, tmp_path):
        ...
        emitted = emit_failing_tests("feat-cov-filter-check", acs, workspace=tmp_path)
    ...
    def test_triple_filter_rejects_no_coverage_uplift(self, tmp_path):
        """triple_filter must reject a test that has no coverage uplift."""
        acs = ["File exists: src/bob/placeholder.py"]
        emitted = emit_failing_tests("feat-cov-filter-rej", acs, workspace=tmp_path)
