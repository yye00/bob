"""Tests for the stub-pass filter in the triple-filter (AC: test_ac_pass_filter).
...
class TestAcPassFilter:
    ...
    def test_test_that_always_passes_is_rejected(self, tmp_path):
        ...
        # _check_fails_on_stub runs pytest; assert True returns exit 0 (stub passes)
    ...
    def test_triple_filter_rejects_mysteriously_green_test(self, tmp_path):
        ...
        emitted = emit_failing_tests("feat-pass-filter", acs, workspace=tmp_path)
    ...
    def test_all_emitted_tests_fail_on_stub_for_multi_ac(self, tmp_path):
        ...
        acs = [
            ...
            "Function defined: bob.a.fn",
        ]
        emitted = emit_failing_tests("feat-multi-stub", acs, workspace=tmp_path)
        for et in emitted:
            assert _check_fails_on_stub(et.test_path), (
