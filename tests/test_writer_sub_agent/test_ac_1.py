...
from bob3.orchestrator.test_writer_agent import EmittedTest, BijectionReport
...
class TestTestWriterAgentGenerate:
    ...
    def test_generate_emitted_tests_fail_on_stub(self, tmp_path):
        ...
        from bob3.orchestrator.test_writer_agent import _check_fails_on_stub
        agent = TestWriterAgent(workspace=tmp_path)
        result = agent.generate("feat-ac1-stub", ["Function defined: bob3.x.fn"])
        for et in result["emitted"]:
