"""Tests for the compilation check in the triple-filter.
...
class TestCompilationFilter:
    def test_emitted_test_file_compiles(self, tmp_path):
        ...
        acs = ["File exists: src/bob3/sample.py"]
        emitted = emit_failing_tests("feat-compile-filter-check", acs, workspace=tmp_path)
    ...
    def test_triple_filter_rejects_uncompilable(self, tmp_path):
        ...
        acs = ["File exists: src/bob3/sample.py"]
        emitted = emit_failing_tests("feat-compile-filter-bad", acs, workspace=tmp_path)
        # Corrupt the file
    ...
    def test_empty_file_compiles(self, tmp_path):
        """An empty Python file is valid syntax and must pass _check_compiles."""
        empty = tmp_path / "empty.py"
        empty.write_text("", encoding="utf-8")
        assert _check_compiles(empty)
