"""Tests for disk_reconciler_verify_fail_promotion in bob.run_loop.
...
bob.run_loop and behaves correctly as the primary entry point for the
...
def test_importable_from_run_loop() -> None:
    """disk_reconciler_verify_fail_promotion must be importable from bob.run_loop."""
    assert callable(disk_reconciler_verify_fail_promotion)


def test_integration_bob_run_loop() -> None:
    """Integration: bob.run_loop exports disk_reconciler_verify_fail_promotion."""
...
def test_returns_bool_on_guard_bypass() -> None:
    ...
    result = disk_reconciler_verify_fail_promotion(
        ...
        acceptance_criteria_json=json.dumps(["File exists: src/bob/run_loop.py"]),
