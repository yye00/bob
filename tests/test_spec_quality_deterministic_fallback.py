"""Tests for ensure_boundary_and_error_coverage via bob3.spec_quality.

AC: pytest: tests/test_spec_quality_deterministic_fallback.py
AC: integration: bob3.spec_synthesis

Verifies that:
- ensure_boundary_and_error_coverage is importable from bob3.spec_quality
- The function injects boundary and error ACs when missing
- The integration with bob3.spec_synthesis is intact
...
from bob3.spec_quality import ensure_boundary_and_error_coverage
...
class TestSpecQualityImport:
    """ensure_boundary_and_error_coverage must be importable from bob3.spec_quality."""

    def test_importable_from_spec_quality(self):
        from bob3.spec_quality import ensure_boundary_and_error_coverage as fn
        assert callable(fn)
...
class TestSpecSynthesisIntegration:
    """Integration: bob3.spec_synthesis exposes the same guarantee."""

    def test_spec_synthesis_has_ensure_boundary_and_error_coverage(self):
        import bob3.spec_synthesis as ss
        assert hasattr(ss, "ensure_boundary_and_error_coverage"), (
            "bob3.spec_synthesis must export ensure_boundary_and_error_coverage"
        )
    ...
    def test_spec_synthesis_deterministic_fallback_carries_boundary_ac(self):
        from bob3.spec_synthesis import deterministic_fallback
...
class TestEnsureBoundaryAndErrorCoverage:
    """Core behavior of ensure_boundary_and_error_coverage from bob3.spec_quality."""
...
class TestDeterministicFallbackCompositeGuarantee:
    ...
    def test_fallback_result_has_boundary_and_error_acs(self):
        ...
        result = ensure_boundary_and_error_coverage(
            [
                "File exists: src/bob3/rate_limited.py",
                "pytest: tests/test_rate_limited.py",
                "Function defined: bob3.rate_limited.process",
            ],
