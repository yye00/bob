"""Tests for bob.subagent_reaper — feature terminal-state process cleanup.

AC: pytest: tests/test_subagent_reaper.py
    Verifies that bob.subagent_reaper exposes the required public functions
...
class TestSubagentReaperPublicInterface(unittest.TestCase):
    """Verify the public API of bob.subagent_reaper."""

    def test_reap_subagent_on_terminal_state_importable(self):
        """reap_subagent_on_terminal_state is importable from bob.subagent_reaper."""
        from bob.subagent_reaper import reap_subagent_on_terminal_state  # noqa: F401

        self.assertTrue(callable(reap_subagent_on_terminal_state))

    def test_sweep_orphan_subagents_importable(self):
        """sweep_orphan_subagents is importable from bob.subagent_reaper."""
        from bob.subagent_reaper import sweep_orphan_subagents  # noqa: F401

        self.assertTrue(callable(sweep_orphan_subagents))

    def test_reap_subagent_for_feature_importable(self):
        """reap_subagent_for_feature is importable from bob.subagent_reaper."""
        from bob.subagent_reaper import reap_subagent_for_feature  # noqa: F401

        self.assertTrue(callable(reap_subagent_for_feature))

    def test_find_subagent_pid_for_feature_importable(self):
        """find_subagent_pid_for_feature is importable from bob.subagent_reaper."""
        from bob.subagent_reaper import find_subagent_pid_for_feature  # noqa: F401
...
class TestReapSubagentOnTerminalState(unittest.TestCase):
    ...
    def test_empty_feature_id_returns_empty_list(self):
        ...
        from bob.subagent_reaper import reap_subagent_on_terminal_state
    ...
    def test_valid_feature_id_no_matching_process_returns_empty(self):
        ...
        with patch(
            "bob.subagent_reaper.reap_subagent_for_feature"
        ) as mock_reap:
...
class TestSweepOrphanSubagents(unittest.TestCase):
    ...
    def test_returns_list(self):
        ...
        from bob.subagent_reaper import sweep_orphan_subagents
    ...
    def test_no_orphans_returns_empty_list(self):
        ...
        with patch(
            "bob.subagent_reaper.sweep_orphan_subagents"
        ) as mock_sweep:
            mock_sweep.return_value = []
            result = sweep_orphan_subagents()
    ...
    def test_orphans_found_returns_tuples(self):
        ...
        import bob.orchestrator.subagent_reaper as _orch_reaper
...
class TestReapOnTerminalTransition(unittest.TestCase):
    ...
    def test_importable(self):
        """reap_on_terminal_transition is importable from bob.subagent_reaper."""
        from bob.subagent_reaper import reap_on_terminal_transition  # noqa: F401

        self.assertTrue(callable(reap_on_terminal_transition))

    def test_empty_feature_id_returns_empty_list(self):
        """Empty string feature_id returns [] without raising."""
        from bob.subagent_reaper import reap_on_terminal_transition
...
class TestReapStaleOrphans(unittest.TestCase):
    ...
    def test_importable(self):
        """reap_stale_orphans is importable from bob.subagent_reaper."""
        from bob.subagent_reaper import reap_stale_orphans  # noqa: F401

        self.assertTrue(callable(reap_stale_orphans))

    def test_returns_list(self):
        """reap_stale_orphans returns a list."""
        from bob.subagent_reaper import reap_stale_orphans
