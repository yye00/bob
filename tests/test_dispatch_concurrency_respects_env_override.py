"""Tests that BOB_MAX_CONCURRENT_FEATURES env var overrides the default.
...
def test_env_override_invalid_value_falls_back_to_default(monkeypatch):
    ...
    monkeypatch.setenv("BOB_MAX_CONCURRENT_FEATURES", "not_a_number")
    resolve = _reload_resolver()
    assert resolve() == 3
...
def test_env_override_one_is_sequential(monkeypatch):
    """BOB_MAX_CONCURRENT_FEATURES=1 gives sequential mode (cap=1)."""
...
def test_env_override_large_value_accepted(monkeypatch):
    ...
    monkeypatch.setenv("BOB_MAX_CONCURRENT_FEATURES", "20")
    resolve = _reload_resolver()
    assert resolve() == 20
