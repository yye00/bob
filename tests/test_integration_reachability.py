"""Tests for bob.spec_quality.integration_reachability.check_spec.
...
class TestReachabilityResult:
    ...
    def test_format_report_failed_includes_suggestion(self):
        issue = ReachabilityIssue(
            feature_name="F1",
            ac_index=0,
            criterion="integration: bob.cli.pln",
            missing_module="bob.cli.pln",
            closest_match="bob.cli.plan",
        )
        r = ReachabilityResult(issues=[issue])
        report = r.format_report()
        assert "bob.cli.plan" in report
...
class TestWorkspaceFileReachability:
    def test_module_exists_as_src_file_passes(self, tmp_path):
        # Create src/bob/cli/plan.py
        plan_file = tmp_path / "src" / "bob" / "cli" / "plan.py"
        plan_file.parent.mkdir(parents=True)
        plan_file.write_text("# plan module\n")

        features = [_make_feature("F1", "integration: bob.cli.plan")]
        result = check_spec(features, workspace=tmp_path)
        assert result.passed is True
...
class TestClosestMatchSuggestion:
    def test_typo_in_module_name_suggests_correction(self, tmp_path):
        # Create bob/cli/plan.py in the workspace
        plan_file = tmp_path / "src" / "bob" / "cli" / "plan.py"
        plan_file.parent.mkdir(parents=True)
        plan_file.write_text("# plan")

        # Reference a typo: "bob.cli.pln" instead of "bob.cli.plan"
        features = [_make_feature("F1", "integration: bob.cli.pln")]
        result = check_spec(features, workspace=tmp_path)
        assert result.passed is False
        assert len(result.issues) == 1
        issue = result.issues[0]
        assert issue.missing_module == "bob.cli.pln"
        # The suggestion should be non-None and close to "bob.cli.plan"
...
class TestMultipleFeaturesAndACs:
    def test_multiple_integration_acs_in_one_feature(self, tmp_path):
        ...
        (tmp_path / "src" / "bob" / "exists.py").write_text("")

        features = [
            _make_feature(
                "F1",
                "integration: bob.exists",
                "integration: bob.missing_module",
            )
        ]
        result = check_spec(features, workspace=tmp_path)
        assert result.passed is False
        assert len(result.issues) == 1
        assert result.issues[0].missing_module == "bob.missing_module"
