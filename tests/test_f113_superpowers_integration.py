...
def workspace_with_files(tmp_path):
    ...
    bob verifier's pytest invocation anyway). This way the verifier's
...
class TestTddMode:
    ...
    def test_tdd_prompt_contains_red_green_refactor(self):
        ...
        from bob.superpowers import get_tdd_prompt
    ...
    def test_should_use_tdd_with_acceptance_criteria(self):
        ...
        from bob.superpowers import should_use_tdd
...
class TestSubagentDrivenDevelopment:
    ...
    def test_subagent_prompt_contains_guidelines(self):
        ...
        from bob.superpowers import get_subagent_prompt
    ...
    def test_should_use_subagents_with_many_criteria(self):
        ...
        from bob.superpowers import should_use_subagents
...
class TestOrientationDocumentation:
    ...
    def test_orientation_documents_systematic_debugging(self):
        ...
        from bob.superpowers import get_superpowers_orientation
...
class TestVerificationOnCompletion:
    ...
    async def test_verification_evidence_created_on_success(self, project):
        ...
        with tempfile.TemporaryDirectory() as ws:
            src_dir = pathlib.Path(ws) / "src" / "bob"
            src_dir.mkdir(parents=True)
...
class TestBuildSuperpowersPrompt:
    ...
    def test_all_skills_combined(self):
        ...
        from bob.superpowers import build_superpowers_prompt
...
class TestTestsPassCheck:
    ...
    def test_timeout_fails_without_hanging_runner(self, tmp_path, monkeypatch):
        ...
        # BOB_TEST_RUN_TIMEOUT env override path.
        monkeypatch.setenv("BOB_TEST_RUN_TIMEOUT", "10")
    ...
    def test_check_returns_warning_when_workspace_missing(self, tmp_path):
        ...
        from bob.superpowers import _check_tests_pass
...
class TestGrandchildKillAndAnsiOutput:
    ...
    def test_subprocess_grandchild_does_not_hang_verifier(
        ...
        # grandchild exits.
        monkeypatch.setenv("BOB_TEST_RUN_TIMEOUT", "3")
...
# ===================================================================
# Recursion guard: workspace == bob itself
# ===================================================================


class TestSelfTestRecursionGuard:
    """Bug 3: ``_check_tests_pass`` must skip when the workspace IS bob."""

    def test_workspace_equal_to_bob_src_is_skipped(self):
        """Running the verifier on bob's own src tree returns a warning skip."""
        import bob
        bob_src_root = pathlib.Path(bob.__file__).resolve().parents[1]

        from bob.superpowers import _check_tests_pass

        result = _check_tests_pass(bob_src_root, "bob", "tests")
        assert result["name"] == "tests_pass"
        assert result["passed"] is True
        assert result.get("severity") == "warning"
        assert "recursion guard" in result["details"].lower()

    def test_workspace_inside_bob_src_is_skipped(self):
        """A path inside bob's src tree also triggers the guard."""
        import bob
        bob_pkg_dir = pathlib.Path(bob.__file__).resolve().parent

        from bob.superpowers import _check_tests_pass

        result = _check_tests_pass(bob_pkg_dir, "src", "tests")
        assert result["name"] == "tests_pass"
        assert result["passed"] is True
        assert result.get("severity") == "warning"
        assert "recursion guard" in result["details"].lower()

    def test_workspace_at_bob_repo_root_fires_guard(self):
        """Bug 4 (off-by-one): workspace at bob repo root must trip the guard.

        Previously the guard was anchored at ``parents[1]`` (``src/``), so
        passing the bob *repo root* (``parents[2]``) — which is NOT under
        ``src/`` — would NOT trigger the guard and the verifier would
        recursively run bob's own pytest suite. Anchoring the guard at the
        repo root fixes that.
        """
        import bob
        bob_repo_root = pathlib.Path(bob.__file__).resolve().parents[2]

        from bob.superpowers import _check_tests_pass

        result = _check_tests_pass(bob_repo_root, "src", "tests")
        assert result["name"] == "tests_pass"
        assert result["passed"] is True
        assert result.get("severity") == "warning"
        assert "recursion guard" in result["details"].lower()

    def test_workspace_at_bob_src_subdirectory_fires_guard(self):
        """A real subdirectory of the bob repo (``<bob>/src``) triggers the guard."""
        import bob
        bob_src_dir = pathlib.Path(bob.__file__).resolve().parents[1]
        # Sanity: this is a real path on disk inside the repo.
        assert bob_src_dir.is_dir()
        assert bob_src_dir.name == "src"

        from bob.superpowers import _check_tests_pass

        result = _check_tests_pass(bob_src_dir, "bob", "tests")
        assert result["name"] == "tests_pass"
        assert result["passed"] is True
        assert result.get("severity") == "warning"
        assert "recursion guard" in result["details"].lower()
    ...
    def test_unrelated_project_src_does_not_fire_guard(self, tmp_path):
        ...
        Previous off-by-one anchored the guard at bob's ``src/``. Any
        workspace whose resolved path was an unrelated ``.../src`` would
        compare unequal to bob's specific ``src/`` and not trip — that's
        fine — but the regression case it allowed was a workspace like
        ``<bob>/src/another_thing/`` (a child of bob's ``src/``) being
        treated as bob itself even when it was an unrelated nested
        project. We assert here that an unrelated workspace at
        ``<unrelated>/src/`` outside the bob repo does NOT fire the
...
class TestNoForbiddenPatterns:
    """Ensure superpowers module follows Bob conventions."""
    ...
    def test_subprocess_use_in_superpowers_is_scoped(self):
        ...
        own.
        """
        source_path = SRC_DIR / "bob" / "superpowers.py"
    ...
    def test_run_loop_imports_superpowers(self):
        ...
        run_loop_path = SRC_DIR / "bob" / "orchestrator" / "run_loop.py"
        source = run_loop_path.read_text()
        assert "from bob.superpowers import" in source
...
class TestR10020ScaledTestRunTimeout:
    ...
    def test_count_tests_in_counts_def_test_lines(self, tmp_path):
        from bob.superpowers import _count_tests_in
    ...
    def test_floor_when_target_dir_is_none(self, monkeypatch):
        from bob.superpowers import _test_run_timeout, DEFAULT_TEST_RUN_TIMEOUT_S

        monkeypatch.delenv("BOB_TEST_RUN_TIMEOUT", raising=False)
        monkeypatch.delenv("BOB_TEST_RUN_PER_TEST_S", raising=False)
        monkeypatch.delenv("BOB_TEST_RUN_CAP", raising=False)
        assert _test_run_timeout() == DEFAULT_TEST_RUN_TIMEOUT_S

    def test_explicit_env_override_wins(self, tmp_path, monkeypatch):
        from bob.superpowers import _test_run_timeout
        ...
        monkeypatch.setenv("BOB_TEST_RUN_TIMEOUT", "42")
    ...
    def test_small_project_uses_floor(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("BOB_TEST_RUN_TIMEOUT", raising=False)
        monkeypatch.setenv("BOB_TEST_RUN_PER_TEST_S", "60")
        monkeypatch.delenv("BOB_TEST_RUN_CAP", raising=False)
    ...
    def test_medium_project_scales_above_floor(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("BOB_TEST_RUN_TIMEOUT", raising=False)
        monkeypatch.setenv("BOB_TEST_RUN_PER_TEST_S", "60")
        monkeypatch.setenv("BOB_TEST_RUN_CAP", "3600")
    ...
    def test_huge_project_clamped_to_cap(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("BOB_TEST_RUN_TIMEOUT", raising=False)
        monkeypatch.setenv("BOB_TEST_RUN_PER_TEST_S", "60")
        monkeypatch.setenv("BOB_TEST_RUN_CAP", "1800")
    ...
    def test_invalid_env_override_falls_back_to_scaling(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("BOB_TEST_RUN_TIMEOUT", "not-a-number")
        monkeypatch.setenv("BOB_TEST_RUN_PER_TEST_S", "negative-please")
        monkeypatch.delenv("BOB_TEST_RUN_CAP", raising=False)
