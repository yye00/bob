"""Tests for gate-blocked feature re-synthesis in bob3.orchestrator (feature 797ae88c).

Verifies that:
- score_gate_loop and synthesize_for_feature are accessible from bob3.orchestrator
...
def test_score_gate_loop_defined_in_orchestrator():
    """AC: Function defined: bob3.orchestrator.score_gate_loop"""
    mod = importlib.import_module("bob3.orchestrator")
    assert hasattr(mod, "score_gate_loop"), "score_gate_loop must be defined in bob3.orchestrator"
    assert callable(mod.score_gate_loop)


def test_synthesize_for_feature_defined_in_orchestrator():
    """AC: Function defined: bob3.orchestrator.synthesize_for_feature"""
    mod = importlib.import_module("bob3.orchestrator")
    assert hasattr(mod, "synthesize_for_feature"), (
        "synthesize_for_feature must be defined in bob3.orchestrator"
    )
...
def test_score_gate_loop_is_async():
    ...
    import inspect
    from bob3.orchestrator import score_gate_loop
...
def test_synthesize_for_feature_is_async():
    ...
    import inspect
    from bob3.orchestrator import synthesize_for_feature
    assert inspect.iscoroutinefunction(synthesize_for_feature)


def test_resynthesize_gate_blocked_features_available():
    """resynthesize_gate_blocked_features must be accessible from bob3.orchestrator."""
    from bob3.orchestrator import resynthesize_gate_blocked_features
    assert callable(resynthesize_gate_blocked_features)


def test_is_already_resynthesized_available():
    """is_already_resynthesized must be accessible from bob3.orchestrator."""
    from bob3.orchestrator import is_already_resynthesized
...
def test_orchestrator_integration_score_gate_loop_importable():
    """Integration: bob3.orchestrator exports score_gate_loop from bob3.spec_synthesizer."""
    from bob3.orchestrator import score_gate_loop
    from bob3.spec_synthesizer import score_gate_loop as _impl
    assert score_gate_loop is _impl, (
        "bob3.orchestrator.score_gate_loop must be the same object as "
        "bob3.spec_synthesizer.score_gate_loop"
    )


def test_orchestrator_integration_synthesize_for_feature_importable():
    """Integration: bob3.orchestrator exports synthesize_for_feature from bob3.spec_synthesizer."""
    from bob3.orchestrator import synthesize_for_feature
    from bob3.spec_synthesizer import synthesize_for_feature as _impl
    assert synthesize_for_feature is _impl, (
        "bob3.orchestrator.synthesize_for_feature must be the same object as "
        "bob3.spec_synthesizer.synthesize_for_feature"
    )
