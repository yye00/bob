"""Tests for bob.dispatch_backoff — exponential backoff after reaper-reset.

AC: pytest: tests/test_dispatch_backoff.py
AC: integration: bob.dispatch

Verifies:
- bob.dispatch_backoff.should_refuse_recent_reap exists and delegates to reaper
- bob.dispatch_backoff.stamp_reap_metadata exists and delegates to reaper
- bob.dispatch.should_refuse_recent_reap is available (integration AC)
...
from bob.dispatch_backoff import should_refuse_recent_reap, stamp_reap_metadata
...
class TestStampReapMetadata:
    ...
    def test_valid_call_delegates_to_reaper(self):
        with patch("bob.dispatch_backoff.bob.reaper" if False else "bob.reaper.db") as mock_db:
            with patch("bob.reaper.stamp_reap_metadata") as mock_stamp:
...
# ── Integration: bob.dispatch exposes should_refuse_recent_reap ──────────────


class TestDispatchIntegration:
    def test_should_refuse_recent_reap_importable_from_dispatch(self):
        import bob.dispatch as dispatch_mod
    ...
    def test_dispatch_backoff_module_importable(self):
        import bob.dispatch_backoff
        assert hasattr(bob.dispatch_backoff, "should_refuse_recent_reap")
        assert hasattr(bob.dispatch_backoff, "stamp_reap_metadata")
