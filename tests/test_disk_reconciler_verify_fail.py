...
bob checks disk state via disk_reconciler. If all ACs satisfy on disk with
...
from bob.disk_reconciler import reconcile_from_disk
...
# ---------------------------------------------------------------------------
# reconcile_from_disk is importable from bob.disk_reconciler (AC check)
# ---------------------------------------------------------------------------


def test_reconcile_from_disk_importable_and_callable() -> None:
    """reconcile_from_disk is importable from bob.disk_reconciler and callable."""
...
def test_disk_reconciler_promotion_check_is_callable() -> None:
    """disk_reconciler_promotion_check is importable and callable from bob.run_loop."""
...
def test_function_defined_ac_satisfies_structural_guard() -> None:
    ...
    acs = json.dumps(["Function defined: bob.run_loop.disk_reconciler_verify_fail_check"])
    with patch(
        "bob.orchestrator.disk_reconciler.check_executing_feature_acs",
        return_value=True,
    ):
...
def test_guard_rejects_integration_only_acs() -> None:
    ...
    acs = json.dumps(["integration: bob.run_loop"])
    result = disk_reconciler_verify_fail_check(
        project_id="proj-guard",
...
# ---------------------------------------------------------------------------
# integration: bob.run_loop — all required functions accessible
# ---------------------------------------------------------------------------


def test_run_loop_exports_disk_reconciler_verify_fail_functions() -> None:
    """All required disk_reconciler verify-fail functions accessible from bob.run_loop."""
...
def test_disk_reconciler_module_exports_reconcile_from_disk() -> None:
    """bob.disk_reconciler exports reconcile_from_disk as required by AC."""
