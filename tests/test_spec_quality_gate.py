...
def test_load_allowlist_patterns_returns_defaults():
    ...
    import os
    env_backup = os.environ.pop("BOB3_ALLOWLIST_PATTERNS", None)
    try:
        ...
        if env_backup is not None:
            os.environ["BOB3_ALLOWLIST_PATTERNS"] = env_backup


def test_load_allowlist_patterns_env_override(monkeypatch):
    """BOB3_ALLOWLIST_PATTERNS env var overrides defaults."""
    monkeypatch.setenv("BOB3_ALLOWLIST_PATTERNS", "F-R7-999,F-R7-888")
    patterns = load_allowlist_patterns()
    assert patterns == ["F-R7-999", "F-R7-888"]


def test_integration_bob3_spec_synthesizer_importable():
    """bob3.spec_synthesizer must be importable alongside bob73.spec_quality_gate."""
    import bob3.spec_synthesizer  # noqa: F401
...
from bob3.spec_quality_gate import check_quality_gate_with_allowlist
