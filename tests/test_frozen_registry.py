...
- activate_frozen_registry_if_configured() reads BOB_FROZEN_REGISTRY env var
...
import bob.registry_transfer as registry_transfer_module
from bob.registry_transfer import (
...
def test_load_transfer_registry_skipped_when_frozen(tmp_path, monkeypatch):
    ...
    export_path.write_text(json.dumps(export_data), encoding="utf-8")
    monkeypatch.setenv("BOB_REGISTRY_TRANSFER_PATH", str(export_path))
...
def test_activate_frozen_registry_env_var_1(monkeypatch):
    """BOB_FROZEN_REGISTRY=1 activates frozen mode."""
    monkeypatch.setenv("BOB_FROZEN_REGISTRY", "1")
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", UserWarning)
...
def test_activate_frozen_registry_env_var_true(monkeypatch):
    """BOB_FROZEN_REGISTRY=true activates frozen mode."""
    monkeypatch.setenv("BOB_FROZEN_REGISTRY", "true")
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", UserWarning)
        activated = activate_frozen_registry_if_configured()
    assert activated is True
...
def test_activate_frozen_registry_env_var_yes(monkeypatch):
    """BOB_FROZEN_REGISTRY=yes activates frozen mode."""
    monkeypatch.setenv("BOB_FROZEN_REGISTRY", "yes")
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", UserWarning)
        activated = activate_frozen_registry_if_configured()
    assert activated is True
...
def test_activate_frozen_registry_env_var_unset(monkeypatch):
    """Unset BOB_FROZEN_REGISTRY does not activate frozen mode."""
    monkeypatch.delenv("BOB_FROZEN_REGISTRY", raising=False)
    activated = activate_frozen_registry_if_configured()
...
def test_activate_frozen_registry_env_var_zero(monkeypatch):
    """BOB_FROZEN_REGISTRY=0 does not activate frozen mode."""
    monkeypatch.setenv("BOB_FROZEN_REGISTRY", "0")
    activated = activate_frozen_registry_if_configured()
    assert activated is False
    assert not is_registry_frozen()
