...
from bob import spec_synthesizer
...
class TestIntegrationCriterion:
    def test_integrate_into_named_module_extracts_target(self):
        criteria = deterministic_fallback(
            "audit hook",
            "Integrate the audit hook into bob.orchestrator.run_loop.",
        )
    ...
    def test_default_target_for_cli_intent(self):
        ...
        # When no concrete module is named, default routes to bob.cli for CLI
...
class TestLLMParseFailureFlowsToFallback:
    ...
    async def test_malformed_llm_response_triggers_fallback(
        ...
        import bob.orchestrator.claude_executor as ce
    ...
    async def test_well_formed_llm_response_skips_fallback(
        ...
        well_formed = (
            '```json\n'
            '["File exists: src/bob/result_cache.py", '
            '"Function defined: bob.result_cache.ResultCache", '
...
#   3. Integration targets are always bob.* qualified.
...
class TestReservedKeywordAndDigitTitles:
    ...
    criteria like ``bob.class.class`` or ``bob.123.123``."""
...
class TestIntegrationTargetModuleQualified:
    ...
    qualified under ``bob.*`` so a bare-noun target can't be trivially
    ...
    def test_integration_target_module_qualified(self, title: str, desc: str) -> None:
        ...
        assert body.startswith("bob."), (
            f"integration target {body!r} not qualified under bob.*: {criteria}"
        )


class TestFileAndFunctionPathsAgree:
    """Adversarial review #2: ``File exists: src/bob/<module>.py`` and
    ``Function defined: bob.<module>.<symbol>`` must agree on ``<module>``,
    ...
    def test_file_and_function_paths_agree(self, title: str) -> None:
        ...
        # Extract <module> from "File exists: src/bob/<module>.py"
        file_body = file_acs[0].split(":", 1)[1].strip()
        assert file_body.startswith("src/bob/") and file_body.endswith(".py"), file_body
        file_module = file_body[len("src/bob/"):-len(".py")]
        # Extract <module> from "Function defined: bob.<module>.<symbol>"
        fn_body = fn_acs[0].split(":", 1)[1].strip()
        assert fn_body.startswith("bob."), fn_body
        # The module is everything between "bob." and the final ".<symbol>".
        without_pkg = fn_body[len("bob."):]
...
class TestDefaultIntegrationTargetCLIBranchLive:
    ...
    must not be dead — it has to return ``bob.cli`` when CLI keywords
    appear and the regex parser found no explicit target."""

    def test_cli_intent_with_no_parsed_target_routes_to_bob_cli(self) -> None:
        # "Hook into the CLI" has no words between hook & into so the
        # structured regex doesn't fire; falls through to
        # _default_integration_target, which must route to bob.cli
        # (not bob.orchestrator.run_loop).
        criteria = deterministic_fallback(
            "diag subcommand",
            "Hook into the CLI.",
        )
        ...
        body = integ[0].split(":", 1)[1].strip()
        assert body == "bob.cli", (
            f"CLI-hint branch did not route to bob.cli: {body!r} in {criteria}"
        )
