...
from bob3.enhanced_verification import _check_criterion, _search_for_function
...
class TestIntegrationWithOrchestrator:
    """Integration check: bob3.orchestrator modules referenced in ACs are present."""

    def test_cost_telemetry_guard_importable(self):
        import importlib
        mod = importlib.import_module("bob3.orchestrator.cost_telemetry_guard")
    ...
    def test_check_criterion_integration_orchestrator_pass(self):
        # integration: bob3.orchestrator — should resolve to True (module exists)
        result = _check("integration: bob3.orchestrator")
        assert result is True
