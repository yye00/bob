"""Tests for persistent_spec_critic_findings_registry_regression (F-R7-450).
...
from bob3.persistent_spec_critic_findings_registry_regression import (
    persistent_spec_critic_findings_registry_regression,
)


def test_persistent_spec_critic_findings_registry_regression(tmp_path):
    ...
    result1 = persistent_spec_critic_findings_registry_regression(
    ...
    # Second run: same defect at same slot → regression, severity escalated
    result2 = persistent_spec_critic_findings_registry_regression(
        spec_hash="abc123def456abcd",
        slot_id="AC-0",
        defect_type="ambiguity",
        feature_id="feat-001",
    ...
    # Verify findings file written and keyed correctly
    ...
    # Push critic_repeat_rate > 0.30 to fire halt-gate
    for i in range(5):
        persistent_spec_critic_findings_registry_regression(
            spec_hash="abc123def456abcd",
            slot_id=f"AC-{i}",
...
def test_first_occurrence_not_regression(tmp_path):
    ...
    result = persistent_spec_critic_findings_registry_regression(
        spec_hash="hash0001",
        slot_id="FEATURE",
...
def test_severity_escalation_chain(tmp_path):
    """info → warning → error → critical on repeated regression."""
    findings_path = tmp_path / "spec_findings.yaml"
    metrics_path = tmp_path / "metrics.yaml"
    ...
    r1 = persistent_spec_critic_findings_registry_regression(**kwargs, severity="info", run_id="r1")
    assert r1["severity"] == "info"

    r2 = persistent_spec_critic_findings_registry_regression(**kwargs, severity="info", run_id="r2")
    assert r2["severity"] == "warning"
    assert r2["is_regression"] is True

    r3 = persistent_spec_critic_findings_registry_regression(**kwargs, severity="info", run_id="r3")
    assert r3["severity"] == "error"

    r4 = persistent_spec_critic_findings_registry_regression(**kwargs, severity="info", run_id="r4")
    assert r4["severity"] == "critical"

    # stays at critical
    r5 = persistent_spec_critic_findings_registry_regression(**kwargs, severity="info", run_id="r5")
    assert r5["severity"] == "critical"


def test_different_slot_independent(tmp_path):
    ...
    r1 = persistent_spec_critic_findings_registry_regression(slot_id="AC-0", run_id="x1", **base)
    r2 = persistent_spec_critic_findings_registry_regression(slot_id="AC-1", run_id="x2", **base)
...
def test_findings_keyed_by_composite_key(tmp_path):
