"""Tests for bob.ac_handler Pattern 9 — shell-script integration AC handler (F-R7-594).

Exercises ``demote_shell_script_integration`` and its alias
``demote_shell_script_integration_ac`` from ``bob.ac_handler``.
"""
...
def test_demote_shell_script_integration_returns_none_for_python_path(
    ...
    result = demote_shell_script_integration(
        "integration: bob.verifier", tmp_path
    )
    assert result is None
