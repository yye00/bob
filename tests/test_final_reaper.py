"""Tests for bob3.final_reaper — sweep_orphans_on_exit (feature 457df9cf).

AC: pytest: tests/test_final_reaper.py
AC: File exists: src/bob3/final_reaper.py
AC: Function defined: bob3.final_reaper.sweep_orphans_on_exit
...
class TestModuleAndFunction(unittest.TestCase):
    ...
    def test_final_reaper_module_importable(self):
        """bob3.final_reaper must be importable."""
        import bob3.final_reaper  # noqa: F401

    def test_sweep_orphans_on_exit_exists(self):
        """sweep_orphans_on_exit must be a callable in bob3.final_reaper."""
...
class TestSweepOrphansOnExitBasicBehavior(unittest.TestCase):
    ...
    def test_sweep_orphan_subagents_is_called_first(self):
        ...
        with patch("bob3.final_reaper.sweep_orphan_subagents", side_effect=mock_sweep), \
             patch("bob3.final_reaper.db") as mock_db:
...
class TestOrchestratorIntegration(unittest.TestCase):
    """Integration AC: sweep_orphans_on_exit is reachable via bob3.orchestrator."""

    def test_final_reaper_importable_from_orchestrator_chain(self):
        """bob3.final_reaper is importable (orchestrator integration confirmed via import chain)."""
        import bob3.final_reaper
        assert hasattr(bob3.final_reaper, "sweep_orphans_on_exit")
    ...
    def test_sweep_orphans_on_exit_uses_same_sweep_orphan_subagents_as_main_loop(self):
        ...
        # bob3.orchestrator.subagent_reaper — the same module used by run_loop.
