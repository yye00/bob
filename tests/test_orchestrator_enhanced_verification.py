"""Tests for bob3.orchestrator.enhanced_verification.verify_structural_log_line.

AC: "pytest: tests/test_orchestrator_enhanced_verification.py"
Feature: 0a898414-1b37-4a43-b65b-a2f7a447abd0

Covers:
- verify_structural_log_line is importable from bob3.orchestrator.enhanced_verification
...
from bob3.orchestrator.enhanced_verification import verify_structural_log_line


def _write_src(tmp_path: pathlib.Path, content: str, rel: str = "src/bob3/orchestrator/run_loop.py") -> pathlib.Path:
    target = tmp_path / rel
...
class TestExactMatch:
    ...
    def test_exact_match_returns_true(self, tmp_path):
        ...
        result = verify_structural_log_line(
            criterion_body="src/bob3/orchestrator/run_loop.py emits a 'Run finished: termination=%s' log line",
            workspace=workspace,
        )
    ...
    def test_exact_match_double_quoted_criterion(self, tmp_path):
        ...
        result = verify_structural_log_line(
            criterion_body='src/bob3/orchestrator/run_loop.py emits a "Queue drained" log line',
            workspace=workspace,
        )
        assert result is True
...
class TestAdjacentLiteralConcat:
    ...
    def test_multiline_adjacent_literal_returns_true(self, tmp_path):
        ...
        workspace = _write_src(tmp_path, src)
        result = verify_structural_log_line(
            criterion_body="src/bob3/orchestrator/run_loop.py emits a 'Queue drained — all features' log line",
            workspace=workspace,
        )
        assert result is True
...
class TestMissAndFallThrough:
    ...
    def test_non_emits_criterion_returns_none(self, tmp_path):
        ...
        result = verify_structural_log_line(
            criterion_body="src/bob3/orchestrator/run_loop.py defines function foo",
            workspace=workspace,
        )
        assert result is None
    ...
    def test_file_does_not_exist_returns_none(self, tmp_path):
        result = verify_structural_log_line(
            criterion_body="src/bob3/orchestrator/nonexistent.py emits a 'Some log' log line",
            workspace=tmp_path,
        )
        assert result is None
