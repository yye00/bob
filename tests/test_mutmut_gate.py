"""Tests for bob3.mutmut_gate — mutmut 3.x post-impl quality gate.

Acceptance criteria:
- File exists: src/bob3/mutmut_gate.py
- Function defined: bob3.mutmut_gate.run_mutation_tests
...
import bob3.mutmut_gate as mod
from bob3.mutmut_gate import MUTATION_SCORE_THRESHOLD, run_mutation_tests
from bob3.verification.mutation_gate import MutationReport, MutmutMissingError
...
def test_module_importable():
    """bob3.mutmut_gate must be importable."""
    assert mod is not None


def test_run_mutation_tests_callable():
    """bob3.mutmut_gate.run_mutation_tests must be a callable."""
...
# ---------------------------------------------------------------------------
# AC 5: Integration with bob3.verifier
# ---------------------------------------------------------------------------


def test_verifier_imports_run_mutation_tests():
    """bob3.verifier must import run_mutation_tests from bob3.mutmut_gate."""
    import bob3.verifier as verifier
    ...
    ), "bob3.verifier must expose run_mutation_tests or check_mutation_score"


def test_verifier_imports_check_mutation_score():
    """bob3.verifier re-exports check_mutation_score from mutmut_gate."""
    import bob3.verifier as verifier

    assert hasattr(verifier, "check_mutation_score"), (
        "bob3.verifier must re-export check_mutation_score from bob3.mutmut_gate"
    )
...
def test_verify_mutmut_cli_command_exists():
    """bob3 verify mutmut CLI command must exist."""
    ...
    assert "verify" in commands, (
        f"'verify' command/group missing from bob3 CLI; available: {commands}"
    )


def test_verify_mutmut_subcommand_exists():
    """bob3 verify mutmut subcommand must be registered."""
    from bob3.cli import main

    verify_group = main.commands.get("verify")
    assert verify_group is not None, "'verify' group not registered in bob3 CLI"
...
def test_skips_gracefully_when_mutmut_missing(tmp_path):
    ...
    with patch(
        "bob3.mutmut_gate.run_mutation_test",
...
def test_passes_on_high_mutation_score(tmp_path):
    ...
    with patch("bob3.mutmut_gate.run_mutation_test", return_value=good_report):
        result = run_mutation_tests(
            feature_id="feat-pass",
            src_files=["src/bob3/foo.py"],
            test_dir="tests",
...
def test_rejects_on_low_mutation_score(tmp_path):
    ...
    with patch("bob3.mutmut_gate.run_mutation_test", return_value=bad_report):
        with patch("bob3.mutmut_gate.persist_surviving_mutants") as mock_persist:
            result = run_mutation_tests(
                feature_id="feat-fail",
                src_files=["src/bob3/foo.py"],
                test_dir="tests",
...
def test_custom_threshold_respected(tmp_path):
    ...
    with patch("bob3.mutmut_gate.run_mutation_test", return_value=report):
        with patch("bob3.mutmut_gate.persist_surviving_mutants"):
