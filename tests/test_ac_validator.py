"""Tests for bob.ac_validator.validate_acceptance_criteria.

Verifies the public API at bob.ac_validator is reachable and behaves
identically to the underlying bob.validators.ac_form implementation.
"""
...
from bob.ac_validator import validate_acceptance_criteria, MalformedACError
...
class TestValidCriteria:
    ...
    def test_file_exists_ac_passes(self):
        assert validate_acceptance_criteria(["File exists: src/bob/ac_validator.py"]) == []

    def test_function_defined_ac_passes(self):
        assert validate_acceptance_criteria(
            ["Function defined: bob.ac_validator.validate_acceptance_criteria"]
        ) == []

    def test_integration_ac_passes(self):
        assert validate_acceptance_criteria(["integration: bob.plan_reviewer"]) == []

    def test_class_defined_ac_passes(self):
        assert validate_acceptance_criteria(["Class defined: bob.validators.ac_form.MalformedACError"]) == []
    ...
    def test_multiple_valid_acs_pass(self):
        acs = [
            "File exists: src/bob/ac_validator.py",
            "Function defined: bob.ac_validator.validate_acceptance_criteria",
            "pytest: tests/test_ac_validator.py",
            "integration: bob.plan_reviewer",
        ]
...
class TestMalformedCriteria:
    ...
    def test_v13_function_defined_parens_raises(self):
        bad = "Function defined: bob.module.fn (validates input)"
        with pytest.raises(ValueError):
            validate_acceptance_criteria([bad])
