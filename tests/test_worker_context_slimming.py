...
- CLAUDE_OPERATOR.md and WORKER.md template files exist in src/bob/
...
from bob.dispatch import build_worker_md, write_feature_settings
...
def _make_feature(**kwargs) -> SimpleNamespace:
    defaults = dict(
        ...
        localization_shortlist=["src/bob/dispatch.py", "tests/"],
    )
...
class TestBuildWorkerMdContent:
    ...
    def test_no_operator_loop_bullets_in_content(self):
        ...
        assert "## Bob Operator Loop" not in content
...
class TestWorkerContextFiles:
    def test_claude_operator_md_exists(self):
        """CLAUDE_OPERATOR.md must exist in src/bob/ (operator context split)."""
        path = Path("src/bob/CLAUDE_OPERATOR.md")
        assert path.exists(), f"Expected {path} to exist"

    def test_worker_md_exists(self):
        """WORKER.md template must exist in src/bob/."""
        path = Path("src/bob/WORKER.md")
        assert path.exists(), f"Expected {path} to exist"
...
class TestWritePerFeatureWorkerMd:
    def test_write_feature_settings_creates_file(self, tmp_path):
        """write_feature_settings creates settings.json under .bob/features/<id>/."""
        feature = _make_feature(id="test-feat-slim-write")
        settings_path = write_feature_settings(feature, bob_dir=tmp_path / ".bob")
