...
Covers:
  ...
  - integration: bob.orchestrator (emit_plan_ready_event importable)
"""
...
import bob.implementers as implementer
...
def test_orchestrator_emit_plan_ready_event_importable():
    """emit_plan_ready_event must be importable from bob.orchestrator."""
    fn = getattr(orchestrator, "emit_plan_ready_event", None)
    assert callable(fn), "bob.orchestrator.emit_plan_ready_event must be a callable"
...
def test_implementer_refuse_start_unless_approved_importable():
    """refuse_start_unless_approved must be importable from bob.implementers."""
    fn = getattr(implementer, "refuse_start_unless_approved", None)
    assert callable(fn), "bob.implementers.refuse_start_unless_approved must be callable"
...
# ---------------------------------------------------------------------------
# integration: bob.orchestrator — all plan_gate symbols reachable
# ---------------------------------------------------------------------------

def test_orchestrator_integration_symbols():
    """Key plan_gate symbols must be accessible from bob.orchestrator."""
