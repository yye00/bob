"""Tests for bob.feature_timeout_executor — per-feature wall-clock timeout.
...
import bob.feature_timeout_executor as fte
...
def test_module_importable():
    mod = importlib.import_module("bob.feature_timeout_executor")
...
def test_resolve_reads_env_var(monkeypatch):
    monkeypatch.setenv("BOB_FEATURE_TIMEOUT_SECONDS", "900")
...
def test_resolve_accepts_fractional_seconds(monkeypatch):
    monkeypatch.setenv("BOB_FEATURE_TIMEOUT_SECONDS", "1800.5")
    result = fte.resolve_timeout_seconds()
...
async def test_execute_uses_env_timeout(monkeypatch):
    monkeypatch.setenv("BOB_FEATURE_TIMEOUT_SECONDS", "5")
...
async def test_timeout_telemetry_emitted(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="bob.feature_timeout_executor"):
...
def test_orchestrator_imports_execute_with_timeout():
    ...
    assert hasattr(mod, "_execute_with_timeout"), (
        "bob.orchestrator.run_loop must import execute_with_timeout "
        "from bob.feature_timeout_executor for integration AC 9bdba8e1"
    )
