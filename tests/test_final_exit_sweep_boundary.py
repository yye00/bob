...
class TestFinalExitSweepBoundary:
    ...
    def test_single_feature_already_promoted(self) -> None:
        ...
        with (
            patch("bob3.orchestrator.run_loop.db") as mock_db,
            patch("bob3.orchestrator.run_loop.find_subagent_pid_for_feature", return_value=[]),
            patch("bob3.orchestrator.run_loop._check_executing_feature_acs", return_value=True),
        ):
    ...
    def test_feature_with_empty_acs_list(self) -> None:
        ...
        with (
            patch("bob3.orchestrator.run_loop.db") as mock_db,
            patch("bob3.orchestrator.run_loop.find_subagent_pid_for_feature", return_value=[]),
            patch("bob3.orchestrator.run_loop._check_executing_feature_acs", return_value=False),
        ):
            mock_db.list_features.return_value = [feature]
    ...
    def test_feature_with_live_pid_not_touched(self) -> None:
        ...
        with (
            patch("bob3.orchestrator.run_loop.db") as mock_db,
            patch("bob3.orchestrator.run_loop.find_subagent_pid_for_feature", return_value=[99999]),
