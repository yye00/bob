...
Asserts bob.database.create_agent_run is importable and behaves deterministically on empty/zero/
...
def test_boundary_importable_and_defined():
    mod = importlib.import_module('bob.database')
    assert hasattr(mod, 'create_agent_run'), 'create_agent_run' + " must be defined in " + 'bob.database'
    obj = getattr(mod, 'create_agent_run')
    assert callable(obj), 'create_agent_run' + " must be callable"
