...
Asserts that 'integration: bob3.real.module' (a clean dotted reference) still
...
class TestPolicyVerbDoesNotShadowExecutable:
    ...
    def test_clean_dotted_reference_routes_to_wiring_lookup(self, tmp_path):
        """'integration: bob3.real.module' must call _integration_wired, not demote."""
        criterion = "integration: bob3.real.module"
        mock_wired = MagicMock(return_value=True)
        with patch("bob3.enhanced_verification._integration_wired", mock_wired):
    ...
    def test_clean_dotted_reference_fails_when_not_wired(self, tmp_path):
        ...
        criterion = "integration: bob3.not_wired.module"
    ...
    def test_dotted_with_must_still_calls_wiring_first(self, tmp_path):
        ...
        criterion = "integration: bob3.gate.check MUST pass before promotion"
        mock_wired = MagicMock(return_value=True)
        with patch("bob3.enhanced_verification._integration_wired", mock_wired):
            ok, reason = resolve_integration_ac(criterion, tmp_path)
    ...
    def test_bare_dotted_no_prose_hard_fails(self, tmp_path):
        ...
        # Body has no spaces or prose tokens: 'bob3.module' alone → not prose
        criterion = "integration: bob3.module"
        mock_wired = MagicMock(return_value=False)
        with patch("bob3.enhanced_verification._integration_wired", mock_wired):
            ok, reason = resolve_integration_ac(criterion, tmp_path)
