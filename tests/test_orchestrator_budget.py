...
ceiling (BOB_PER_FEATURE_COST_CEILING, default $20), NOT the entire project
...
class TestApplyPessimisticCostExportedFromRunLoop:
    """apply_pessimistic_cost is accessible from bob.orchestrator.run_loop."""

    def test_importable_from_run_loop(self):
        """AC: Function defined: bob.orchestrator.run_loop.apply_pessimistic_cost."""
        from bob.orchestrator.run_loop import apply_pessimistic_cost as fn
        assert callable(fn)

    def test_same_function_as_cost_telemetry_guard(self):
        """The export in run_loop IS the same function from cost_telemetry_guard."""
        from bob.orchestrator.cost_telemetry_guard import apply_pessimistic_cost as direct
        from bob.orchestrator.run_loop import apply_pessimistic_cost as via_loop
...
class TestPerFeatureCeilingNotProjectBudget:
    ...
    def test_env_override_bob_per_feature_cost_ceiling(self, monkeypatch):
        """BOB_PER_FEATURE_COST_CEILING env var sets the ceiling used in run_loop."""
    ...
    def test_env_default_is_twenty_dollars(self, monkeypatch):
        """Default when BOB_PER_FEATURE_COST_CEILING is unset is $20."""
...
class TestRunLoopEnvCeilingLogic:
    ...
    def test_ceiling_defaults_to_20_when_env_unset(self, monkeypatch):
        """run_loop logic: BOB_PER_FEATURE_COST_CEILING absent → 20.0."""
        monkeypatch.delenv("BOB_PER_FEATURE_COST_CEILING", raising=False)
        try:
            pf_default = float(os.environ.get("BOB_PER_FEATURE_COST_CEILING", "20.0"))
        except (TypeError, ValueError):
            pf_default = 20.0
        assert pf_default == pytest.approx(20.0)

    def test_ceiling_reads_env_override(self, monkeypatch):
        """run_loop logic: BOB_PER_FEATURE_COST_CEILING=30 → 30.0."""
        monkeypatch.setenv("BOB_PER_FEATURE_COST_CEILING", "30.0")
        try:
            pf_default = float(os.environ.get("BOB_PER_FEATURE_COST_CEILING", "20.0"))
        except (TypeError, ValueError):
            pf_default = 20.0
    ...
    def test_ceiling_falls_back_on_invalid_env(self, monkeypatch):
        """run_loop logic: invalid BOB_PER_FEATURE_COST_CEILING → fallback 20.0."""
        monkeypatch.setenv("BOB_PER_FEATURE_COST_CEILING", "not-a-number")
        try:
            pf_default = float(os.environ.get("BOB_PER_FEATURE_COST_CEILING", "20.0"))
        except (TypeError, ValueError):
            pf_default = 20.0
