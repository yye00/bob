"""Tests for bob.readiness_gate — stuck-readiness decomposition trigger.
...
from bob.readiness_gate import mark_pending_decomposition, should_trigger_decomposition
...
class TestRunLoopIntegration:
    """Verify that bob.run_loop uses the same decomposition logic."""

    def test_run_loop_has_decompose_threshold(self):
        from bob.orchestrator.run_loop import _DECOMPOSE_READINESS_THRESHOLD
        assert _DECOMPOSE_READINESS_THRESHOLD == 0.80

    def test_run_loop_has_min_attempts(self):
        from bob.orchestrator.run_loop import _DECOMPOSE_MIN_ATTEMPTS
    ...
    def test_run_loop_transition_sets_pending_decomposition(self):
        ...
        from bob import db
        from bob.orchestrator.run_loop import _transition_to_pending_decomposition
