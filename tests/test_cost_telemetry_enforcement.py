...
AC1: Function defined: bob3.orchestrator.enforce_budget_on_zero_cost_with_work_events
AC2: File exists: src/bob3/cost_telemetry.py
AC3: pytest: tests/test_cost_telemetry_enforcement.py (this file)
AC4: integration: bob3.orchestrator (function importable from the package)
...
# ---------------------------------------------------------------------------
# AC 1 & 4 — importability from both bob3.cost_telemetry and bob3.orchestrator
# ---------------------------------------------------------------------------


class TestImportability:
    """The function must be importable from bob3.orchestrator (integration AC)."""

    def test_importable_from_bob3_orchestrator(self):
        """AC4: bob3.orchestrator exposes enforce_budget_on_zero_cost_with_work_events."""
        mod = importlib.import_module("bob3.orchestrator")
        func = getattr(mod, "enforce_budget_on_zero_cost_with_work_events", None)
        assert func is not None, (
            "enforce_budget_on_zero_cost_with_work_events not found in bob3.orchestrator"
        )

    def test_importable_from_bob3_cost_telemetry(self):
        """AC2: bob3.cost_telemetry module exists and exposes the function."""
        mod = importlib.import_module("bob3.cost_telemetry")
        func = getattr(mod, "enforce_budget_on_zero_cost_with_work_events", None)
        assert func is not None
...
class TestThresholdEnvVar:
    """BOB3_COST_TELEMETRY_LOST_WORK_THRESHOLD overrides the default threshold."""
