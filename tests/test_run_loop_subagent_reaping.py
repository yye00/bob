"""Tests for bob3.run_loop subagent reaping functions (8b02d5ec / 0143c5c4).

Acceptance criteria:
  - Function defined: bob3.run_loop.sigterm_subagent_on_terminal_transition
  - Function defined: bob3.run_loop.sweep_orphan_subagents
  - Function defined: bob3.run_loop.reap_subagent_on_terminal_transition
  - Function defined: bob3.run_loop.orphan_subagent_sweeper
  - integration: bob3.run_loop
  - integration: bob3.orchestrator

Verifies that both function pairs are importable from the public bob3.run_loop
module and correctly delegate to bob3.orchestrator.subagent_reaper.
"""
...
class TestSigtermSubagentOnTerminalTransition(unittest.TestCase):
    """Tests for bob3.run_loop.sigterm_subagent_on_terminal_transition."""

    def test_function_is_importable_from_bob3_run_loop(self):
        """sigterm_subagent_on_terminal_transition is accessible at bob3.run_loop."""
    ...
    def test_returns_empty_list_when_no_subagent_running(self):
        ...
        from bob3.run_loop import sigterm_subagent_on_terminal_transition
...
class TestSweepOrphanSubagentsRunLoop(unittest.TestCase):
    """Tests for bob3.run_loop.sweep_orphan_subagents."""

    def test_function_is_importable_from_bob3_run_loop(self):
        """sweep_orphan_subagents is accessible at bob3.run_loop."""
        import bob3.run_loop as rl
...
class TestRunLoopModuleExports(unittest.TestCase):
    """Verifies the public __all__ of bob3.run_loop includes the new functions."""

    def test_sigterm_in_all(self):
        """sigterm_subagent_on_terminal_transition appears in bob3.run_loop.__all__."""
        import bob3.run_loop as rl
        self.assertIn("sigterm_subagent_on_terminal_transition", rl.__all__)

    def test_sweep_orphans_in_all(self):
        """sweep_orphan_subagents appears in bob3.run_loop.__all__."""
        import bob3.run_loop as rl
...
class TestIntegrationRunLoop(unittest.TestCase):
    """Integration-level tests verifying bob3.run_loop wires correctly."""
...
class TestReapSubagentOnTerminalTransition(unittest.TestCase):
    """Tests for bob3.run_loop.reap_subagent_on_terminal_transition (AC: 0143c5c4)."""

    def test_function_is_importable_from_bob3_run_loop(self):
        """reap_subagent_on_terminal_transition is accessible at bob3.run_loop."""
        import bob3.run_loop as rl
        self.assertTrue(callable(rl.reap_subagent_on_terminal_transition))

    def test_returns_empty_list_when_no_subagent_running(self):
        """Returns [] when no claude process is tagged with the feature id."""
        from bob3.run_loop import reap_subagent_on_terminal_transition
    ...
    def test_in_all(self):
        """reap_subagent_on_terminal_transition appears in bob3.run_loop.__all__."""
        import bob3.run_loop as rl
        self.assertIn("reap_subagent_on_terminal_transition", rl.__all__)


class TestOrphanSubagentSweeper(unittest.TestCase):
    """Tests for bob3.run_loop.orphan_subagent_sweeper (AC: 0143c5c4)."""

    def test_function_is_importable_from_bob3_run_loop(self):
        """orphan_subagent_sweeper is accessible at bob3.run_loop."""
        import bob3.run_loop as rl
        self.assertTrue(callable(rl.orphan_subagent_sweeper))

    def test_returns_empty_list_when_no_stale_features(self):
        """Returns [] when no terminal features have exceeded 5min dwell."""
        from bob3.run_loop import orphan_subagent_sweeper
    ...
    def test_in_all(self):
        """orphan_subagent_sweeper appears in bob3.run_loop.__all__."""
        import bob3.run_loop as rl
        self.assertIn("orphan_subagent_sweeper", rl.__all__)


class TestIntegrationRunLoopOrchestrator(unittest.TestCase):
    """Integration: verifies bob3.run_loop wires to bob3.orchestrator correctly."""
    ...
    def test_bob3_orchestrator_subagent_reaper_importable(self):
        """bob3.orchestrator.subagent_reaper is importable (integration AC)."""
        import bob3.orchestrator.subagent_reaper as reaper
...
class TestBackstopReapOrphanSubagents(unittest.TestCase):
    """Tests for bob3.run_loop.backstop_reap_orphan_subagents (AC: b7561f93)."""

    def test_function_is_importable_from_bob3_run_loop(self):
        """backstop_reap_orphan_subagents is accessible at bob3.run_loop."""
        import bob3.run_loop as rl
        self.assertTrue(callable(rl.backstop_reap_orphan_subagents))

    def test_in_all(self):
        """backstop_reap_orphan_subagents appears in bob3.run_loop.__all__."""
        import bob3.run_loop as rl
        self.assertIn("backstop_reap_orphan_subagents", rl.__all__)

    def test_returns_empty_list_when_no_stale_features(self):
        """Returns [] when no terminal features have exceeded 5min dwell."""
        from bob3.run_loop import backstop_reap_orphan_subagents
...
class TestReapSubagentOnTerminalState(unittest.TestCase):
    """Tests for bob3.run_loop.reap_subagent_on_terminal_state (AC: 7933f525)."""

    def test_function_is_importable_from_bob3_run_loop(self):
        """reap_subagent_on_terminal_state is accessible at bob3.run_loop."""
        import bob3.run_loop as rl
        self.assertTrue(callable(rl.reap_subagent_on_terminal_state))

    def test_in_all(self):
        """reap_subagent_on_terminal_state appears in bob3.run_loop.__all__."""
        import bob3.run_loop as rl
        self.assertIn("reap_subagent_on_terminal_state", rl.__all__)

    def test_returns_empty_list_when_no_subagent_running(self):
        """Returns [] when no claude process is tagged with the feature id."""
        from bob3.run_loop import reap_subagent_on_terminal_state
...
class TestReapSubagentOnTerminal(unittest.TestCase):
    """Tests for bob3.run_loop.reap_subagent_on_terminal (AC: c493ccc8)."""

    def test_function_is_importable_from_bob3_run_loop(self):
        """reap_subagent_on_terminal is accessible at bob3.run_loop."""
        import bob3.run_loop as rl
        self.assertTrue(callable(rl.reap_subagent_on_terminal))

    def test_in_all(self):
        """reap_subagent_on_terminal is in bob3.run_loop.__all__."""
        import bob3.run_loop as rl
    ...
    def test_returns_empty_list_when_no_subagent_running(self):
        """Returns [] when no claude process is tagged with the feature id."""
        from bob3.run_loop import reap_subagent_on_terminal
...
class TestSubagentReaperModule(unittest.TestCase):
    """Tests for src/bob3/subagent_reaper.py (AC: File exists: src/bob3/subagent_reaper.py)."""

    def test_module_is_importable(self):
        """bob3.subagent_reaper is importable."""
        import bob3.subagent_reaper as sr
        self.assertIsNotNone(sr)

    def test_find_subagent_pid_for_feature_exported(self):
        """bob3.subagent_reaper exports find_subagent_pid_for_feature."""
        import bob3.subagent_reaper as sr
        self.assertTrue(callable(sr.find_subagent_pid_for_feature))

    def test_reap_subagent_for_feature_exported(self):
        """bob3.subagent_reaper exports reap_subagent_for_feature."""
        import bob3.subagent_reaper as sr
        self.assertTrue(callable(sr.reap_subagent_for_feature))

    def test_sweep_orphan_subagents_exported(self):
        """bob3.subagent_reaper exports sweep_orphan_subagents."""
        import bob3.subagent_reaper as sr
        self.assertTrue(callable(sr.sweep_orphan_subagents))

    def test_reap_delegates_to_orchestrator(self):
        """reap_subagent_for_feature in bob3.subagent_reaper delegates to orchestrator."""
        import bob3.subagent_reaper as sr
    ...
    def test_sweep_delegates_to_orchestrator(self):
        """sweep_orphan_subagents in bob3.subagent_reaper delegates to orchestrator."""
        import bob3.subagent_reaper as sr
    ...
    def test_in_all(self):
        """All public symbols are in bob3.subagent_reaper.__all__."""
        import bob3.subagent_reaper as sr
        for name in [
...
class TestBackstopReaperBehavior(unittest.TestCase):
    ...
    def test_no_args_returns_list(self):
        ...
        from bob3.run_loop import backstop_reaper
        with patch(
            "bob3.orchestrator.subagent_reaper._query_stale_terminal_features",
            return_value=[],
        ):
