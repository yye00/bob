"""Tests for bob.spec_emission — schema-constrained spec emission.

Covers emit_constrained_spec and validate_spec_against_schema.
Integration: bob.critic is imported as a side-effect of importing spec_emission.
"""
...
class TestEmitConstrainedSpec:
    def test_valid_spec_returns_same_dict(self):
        from bob.spec_emission import emit_constrained_spec
...
class TestValidateSpecAgainstSchema:
    def test_valid_spec_returns_true(self):
        from bob.spec_emission import validate_spec_against_schema
...
class TestIntegrationWithCritic:
    def test_import_triggers_bob_critic_integration(self):
        """Importing spec_emission must also import bob.spec_critic (integration AC)."""
        import bob.spec_emission  # noqa: F401
        import sys
        assert "bob.spec_critic" in sys.modules

    def test_emit_constrained_spec_reachable_from_bob(self):
        from bob.spec_emission import emit_constrained_spec
        assert callable(emit_constrained_spec)

    def test_validate_spec_against_schema_reachable_from_bob(self):
        from bob.spec_emission import validate_spec_against_schema
