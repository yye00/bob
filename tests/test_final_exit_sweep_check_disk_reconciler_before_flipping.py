"""Tests for bob3.final_exit_sweep_check_disk_reconciler_before_flipping (F-R7-598).

Verifies that the canonical entry point
``bob3.final_exit_sweep_check_disk_reconciler_before_flipping``
...
from bob3.final_exit_sweep_check_disk_reconciler_before_flipping import (
...
def test_final_exit_sweep_check_disk_reconciler_before_flipping(tmp_path: pathlib.Path) -> None:
    ...
    feat = _feature(
        ...
        acceptance_criteria=["File exists: src/bob3/my_module.py"],
    )
    features = [feat]

    with patch(
        "bob3.final_exit_sweep_check_disk_reconciler_before_flipping.promote_if_acs_satisfied",
        return_value=True,
    ) as mock_promote:
...
def test_disk_not_satisfied_falls_through_to_failed(tmp_path: pathlib.Path) -> None:
    ...
    feat = _feature(acceptance_criteria=["File exists: src/bob3/missing.py"])
