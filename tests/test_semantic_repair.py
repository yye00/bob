"""Tests for bob.semantic_repair — verify_semantic_equivalence and apply_auto_repair."""
...
from bob.semantic_repair import verify_semantic_equivalence, apply_auto_repair
from bob.linter import SmellFinding
...
class TestVerifySemanticEquivalence:
    def test_returns_true_when_llm_says_equivalent(self):
        with patch("bob.spec_quality.ac_auto_repair._call_llm_judge", return_value=_equiv_true()):
    ...
    def test_returns_false_when_llm_says_not_equivalent(self):
        with patch("bob.spec_quality.ac_auto_repair._call_llm_judge", return_value=_equiv_false()):
    ...
    def test_returns_false_on_llm_exception(self):
        with patch("bob.spec_quality.ac_auto_repair._call_llm_judge", side_effect=Exception("network error")):
    ...
    def test_empty_strings_on_llm_error_returns_false(self):
        with patch("bob.spec_quality.ac_auto_repair._call_llm_judge", side_effect=Exception("empty")):
...
class TestLinterIntegration:
    ...
    def test_linter_findings_with_error_severity_can_be_repaired(self, tmp_path: Path):
        from bob.linter import detect_smells, filter_by_severity
