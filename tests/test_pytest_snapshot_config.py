"""Tests for bob.pytest_snapshot_config.

Acceptance criteria:
- File exists: src/bob/pytest_snapshot_config.py
- Function defined: bob.pytest_snapshot_config.enforce_maxfail_zero
...
import bob.pytest_snapshot_config as mod
from bob.pytest_snapshot_config import (
    MAXFAIL_ZERO,
...
class TestOrchestratorIntegration:
    """bob.pytest_snapshot_config integrates with bob.orchestrator."""
    ...
    def test_module_importable_from_orchestrator_import_chain(self):
        from bob.pytest_snapshot_config import enforce_maxfail_zero as fn
        assert callable(fn)
    ...
    def test_enforce_maxfail_zero_used_by_orchestrator(self):
        ...
        source = run_loop_path.read_text()
        assert "bob.pytest_snapshot_config" in source
        assert "enforce_maxfail_zero" in source
