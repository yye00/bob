"""Tests for bob3.synthesize retry behavior on transient upstream API 400/empty responses.
...
retry loop (default 40 attempts, env-tunable BOB3_SYNTH_MAX_ATTEMPTS) with exponential
...
class TestRetryOnException:
    ...
    async def test_retries_on_connection_error(self):
        ...
        with patch("bob3.spec_synthesizer._llm_spawn_synthesizer", side_effect=network_error):
...
class TestMaxAttemptsEnvVar:
    """BOB3_SYNTH_MAX_ATTEMPTS is read from env and controls retry count."""

    @pytest.mark.asyncio
    async def test_env_var_controls_max_attempts(self):
        """BOB3_SYNTH_MAX_ATTEMPTS env var is respected."""
        call_count = 0
        ...
        with patch("bob3.spec_synthesizer._llm_spawn_synthesizer", side_effect=always_empty):
            with patch.dict(os.environ, {"BOB3_SYNTH_MAX_ATTEMPTS": "7"}):
                with patch("asyncio.sleep", new_callable=AsyncMock):
                    await synthesize_for_feature(
                        project_id="proj-1",
    ...
    async def test_invalid_env_var_falls_back_to_default(self):
        """Non-integer BOB3_SYNTH_MAX_ATTEMPTS falls back to default (40), does not crash."""
        call_count = 0
...
class TestExponentialBackoff:
    ...
    async def test_backoff_grows_between_retries(self):
        ...
        with patch("bob3.spec_synthesizer._llm_spawn_synthesizer", side_effect=always_empty):
            with patch.dict(os.environ, {"BOB3_SYNTH_MAX_ATTEMPTS": "4"}):
...
class TestPublicAPIAccessible:
    """The retry logic is accessible via the public bob3.synthesize module."""

    def test_synthesize_for_feature_importable(self):
        """synthesize_for_feature is importable from bob3.synthesize."""
        from bob3.synthesize import synthesize_for_feature as sfr
        assert callable(sfr)

    def test_score_gate_loop_importable(self):
        """score_gate_loop is importable from bob3.synthesize."""
        from bob3.synthesize import score_gate_loop as sgl
        assert callable(sgl)

    def test_score_gate_report_importable(self):
        """ScoreGateReport is importable from bob3.synthesize."""
        from bob3.synthesize import ScoreGateReport as SGR
        assert SGR is not None

    def test_deterministic_fallback_importable(self):
        """deterministic_fallback is importable from bob3.synthesize."""
        from bob3.synthesize import deterministic_fallback as df
        assert callable(df)

    def test_synthesizer_module_accessible(self):
        """synthesize_for_feature and score_gate_loop accessible via bob3.synthesizer."""
        from bob3.synthesizer import synthesize_for_feature, score_gate_loop
