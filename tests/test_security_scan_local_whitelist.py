...
class TestWhitelistLocalModules:
    ...
    def test_src_bob3_module_whitelisted(self, tmp_path: Path) -> None:
        """A .py file under src/bob3/ is whitelisted by its stem."""
...
class TestSlopsquattingCheckIntegration:
    def test_local_module_not_flagged(self, tmp_path: Path) -> None:
        """Integration: local module in src/bob3/ is not flagged as missing PyPI dist."""
        pkg = tmp_path / "src" / "bob3"
        pkg.mkdir(parents=True)
        (pkg / "__init__.py").write_text("")
        (pkg / "spec_quality_score.py").write_text("# local module\n")
