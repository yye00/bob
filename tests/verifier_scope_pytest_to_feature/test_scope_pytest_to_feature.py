...
- Function defined: bob.verifier.scope_pytest_to_feature_subtree
...
- File exists: test_ac_13_integration_bob_orchestrator_run_loop.py
...
class TestVerifierPyExists:
    ...
    def test_src_bob_verifier_py_exists(self):
        verifier_py = WORKSPACE / "src" / "bob" / "verifier.py"
...
class TestFileExistsTestAc13:
    ...
    def test_test_ac_13_integration_bob_orchestrator_run_loop_exists(self):
        target = WORKSPACE / "tests" / "test_ac_13_integration_bob_orchestrator_run_loop.py"
        assert target.exists(), f"Expected {target} to exist"
        assert target.is_file()
        assert target.name == "test_ac_13_integration_bob_orchestrator_run_loop.py"
