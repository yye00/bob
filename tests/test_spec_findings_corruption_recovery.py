"""Recovery tests for bob.spec_findings corruption handling.

Feature 280eba15-9818-41d0-a392-f61067a112ce

Verifies that bob boot-path survives a corrupt spec_findings.yaml via
...
class TestQuarantineCorruptFile:
    ...
    def test_function_is_importable(self):
        from bob.spec_findings import quarantine_corrupt_file
...
class TestCorruptionRecovery:
    ...
    def test_empty_findings_is_recoverable_not_fatal(self, tmp_path):
        """Empty findings dict (post-quarantine) allows bob to continue."""
        p = tmp_path / "spec_findings.yaml"
        p.write_text("garbage: [unclosed bracket", encoding="utf-8")
        findings = load_safe(p)
        # An empty dict is acceptable — bob continues with no findings
        assert findings == {}
