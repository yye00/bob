"""Tests for bob3.synthesizer retry behavior on transient upstream API errors.
...
- Retry count is capped by BOB3_SYNTH_MAX_ATTEMPTS env var
...
class TestSynthesizeForFeatureRetry:
    ...
    async def test_retries_on_empty_text(self, caplog):
        ...
        with patch("bob3.spec_synthesizer._llm_spawn_synthesizer", side_effect=fake_llm):
    ...
    async def test_max_attempts_capped_by_env_var(self):
        """BOB3_SYNTH_MAX_ATTEMPTS limits the number of retry attempts."""
        call_count = 0
    ...
    async def test_retry_is_logged_not_silent(self, caplog):
        ...
        with patch("bob3.spec_synthesizer._llm_spawn_synthesizer", side_effect=empty_then_valid):
            with patch.dict(os.environ, {"BOB3_SYNTH_MAX_ATTEMPTS": "5"}):
                with patch("asyncio.sleep", new_callable=AsyncMock):
    ...
    async def test_immediate_success_no_retry(self):
        ...
        with patch("bob3.spec_synthesizer._llm_spawn_synthesizer", side_effect=immediate_success):
            with patch.dict(os.environ, {"BOB3_SYNTH_MAX_ATTEMPTS": "40"}):
    ...
    async def test_recovery_logged_when_retry_succeeds(self, caplog):
        ...
        with patch("bob3.spec_synthesizer._llm_spawn_synthesizer", side_effect=fail_then_succeed):
            with patch.dict(os.environ, {"BOB3_SYNTH_MAX_ATTEMPTS": "5"}):
                with patch("asyncio.sleep", new_callable=AsyncMock):
...
class TestIntegrationWithOrchestrator:
    """Integration: bob3.orchestrator uses bob3.synthesizer functions."""

    def test_synthesizer_importable_from_orchestrator_context(self):
        """bob3.synthesizer functions are importable (integration check)."""
        from bob3.synthesizer import synthesize_for_feature, score_gate_loop
    ...
    def test_synthesize_for_feature_is_coroutine(self):
        ...
        import asyncio
        from bob3.synthesizer import synthesize_for_feature
    ...
    def test_score_gate_loop_is_coroutine(self):
        ...
        import asyncio
        from bob3.synthesizer import score_gate_loop
