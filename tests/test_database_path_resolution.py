...
rather than silently resolving from cwd or BOB3_DATABASE_PATH.
...
def test_create_agent_run_falls_back_to_env_when_no_db_path(project_in_db, monkeypatch):
    """When db_path is omitted, create_agent_run resolves from BOB3_DATABASE_PATH."""
    project, project_db = project_in_db
    monkeypatch.setenv("BOB3_DATABASE_PATH", str(project_db))
    ...
    row = conn.execute("SELECT id FROM sub_agent_runs WHERE id = ?", (run.id,)).fetchone()
    conn.close()
    assert row is not None, "agent_run must be written to the BOB3_DATABASE_PATH db"


def test_get_database_path_prefers_env_over_cwd(tmp_path, monkeypatch):
    """get_database_path() returns the env-var path when BOB3_DATABASE_PATH is set."""
...
def test_get_database_path_falls_back_to_cwd(tmp_path, monkeypatch):
    """get_database_path() returns cwd/bob3.db when env var is not set."""
    monkeypatch.delenv("BOB3_DATABASE_PATH", raising=False)
    monkeypatch.chdir(tmp_path)
    resolved = db.get_database_path()
    assert resolved == tmp_path / "bob3.db"
