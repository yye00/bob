"""Tests for prompt-cache enablement in bob3.dispatch.spawn_worker.
...
class TestSpawnWorkerPromptCache:
    ...
    def test_spawn_worker_emits_cache_event(self, tmp_path):
        ...
        with mock.patch("subprocess.run", side_effect=fake_run):
            with mock.patch("bob3.dispatch.emit_worker_cache_event") as mock_emit:
    ...
    def test_spawn_worker_raises_on_none_feature(self, tmp_path):
        with pytest.raises(ValueError):
            spawn_worker(None, "prompt", str(tmp_path), bob3_dir=tmp_path / ".bob3")
