...
Covers:
- bob.orchestrator.probe_ancestry.is_self_or_ancestor (documented predicate)
- bob.orchestrator.probe_ancestry.is_shell_wrapper (documented predicate)
- Integration: bob.orchestrator.liveness_probe.is_orchestrator_alive delegates
...
class TestIsShellWrapper:
    ...
    def test_timeout_variant_with_suffix(self):
        ...
        assert is_shell_wrapper("timeout60 bob run") is True
...
class TestIsOrchestratorAliveIntegration:
    ...
    def test_does_not_fire_for_sh_quoting_bob(self):
        """sh quoting 'bob run' does not trigger False positive."""
        with patch(
            "bob.orchestrator.liveness_probe._iter_candidate_pids",
            return_value=[(77002, "sh -c 'bob run --all'")],
        ):
            assert is_orchestrator_alive() is False
    ...
    def test_does_not_fire_for_ancestor_pid(self):
        ...
        with (
            ...
            patch(
                "bob.orchestrator.liveness_probe.collect_ancestor_pids",
                return_value=fake_ancestry,
            ),
        ):
            assert is_orchestrator_alive() is False
...
class TestModuleIntegration:
    ...
    def test_probe_ancestry_module_importable(self):
        """bob.orchestrator.probe_ancestry imports without error."""
        import bob.orchestrator.probe_ancestry as mod
    ...
    def test_liveness_probe_imports_probe_ancestry(self):
        """bob.orchestrator.liveness_probe imports from probe_ancestry."""
        import bob.orchestrator.liveness_probe as lp
