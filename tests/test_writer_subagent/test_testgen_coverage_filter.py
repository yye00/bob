"""Tests verifying the Coverage check of the TestGen-LLM triple filter.

The triple filter's third leg rejects tests that fail to raise coverage of
...
class TestTestgenCoverageFilter:
    ...
    def test_pass_body_fails_coverage_check(self, tmp_path):
        """A test body with only pass must fail the coverage heuristic."""
        f = tmp_path / "test_pass.py"
        f.write_text("def test_nothing():\n    pass\n")
        assert _check_raises_coverage(f) is False

    def test_pytest_fail_passes_coverage_check(self, tmp_path):
        """A test containing pytest.fail satisfies the coverage heuristic."""
    ...
    def test_reject_with_missing_ac_region_raises(self, tmp_path):
    ...
    def test_reject_silent_when_ac_region_present(self, tmp_path):
        ...
        reject_no_coverage_uplift(f, ac_region="my_target_symbol")
    ...
    def test_generated_structural_tests_pass_coverage_filter(self, tmp_path):
        ...
        acs = ["File exists: src/bob3/testgen_cov_target.py"]
        result = generate_failing_tests("feat-testgen-cov-filter", acs, workspace=tmp_path)
    ...
    def test_filter_result_raises_coverage_field_is_bool(self, tmp_path):
        """Each FilterResult must have raises_coverage as a bool."""
        result = generate_failing_tests(
            "feat-testgen-cov-bool",
            ["File exists: src/bob3/cov_bool.py"],
            workspace=tmp_path,
        )
        for fr in result["filter_results"]:
