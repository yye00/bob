"""Tests for per-worker settings isolation in bob3.dispatch.
...
class TestWriteFeatureSettingsIsolation:
    ...
    def test_settings_path_under_bob3_features_dir(self, tmp_path):
    ...
    def test_extra_allow_isolated_to_feature(self, tmp_path):
        ...
        write_feature_settings(feat_a, bob3_dir=tmp_path, extra_allow=["Bash(special-cmd*)"])
        write_feature_settings(feat_b, bob3_dir=tmp_path)
    ...
    def test_overwrite_existing_settings(self, tmp_path):
        ...
        path1 = write_feature_settings(feature, bob3_dir=tmp_path)
        path2 = write_feature_settings(
            feature, bob3_dir=tmp_path, extra_allow=["Bash(new-tool*)"]
        )
        assert path1 == path2
    ...
    def test_creates_parent_directories(self, tmp_path):
        """bob3_dir does not need to pre-exist."""
        deep_bob3_dir = tmp_path / "nested" / "deep" / ".bob3"
        feature = _make_feature(id="feat-iso-deep")
        path = write_feature_settings(feature, bob3_dir=deep_bob3_dir)
        assert path.exists()
...
class TestGenerateWorkerSettingsAlias:
    ...
    def test_returns_path(self, tmp_path):
        ...
        result = generate_worker_settings(feature, bob3_dir=tmp_path)
        assert isinstance(result, Path)

    def test_file_exists(self, tmp_path):
        feature = _make_feature(id="feat-gen-02")
        path = generate_worker_settings(feature, bob3_dir=tmp_path)
        assert path.exists()

    def test_same_path_as_write_feature_settings(self, tmp_path):
        feature = _make_feature(id="feat-gen-same")
        path_gen = generate_worker_settings(feature, bob3_dir=tmp_path / "gen")
        path_write = write_feature_settings(feature, bob3_dir=tmp_path / "write")
    ...
    def test_extra_allow_passed_through(self, tmp_path):
        ...
        path = generate_worker_settings(
            feature, bob3_dir=tmp_path, extra_allow=["Bash(custom*)"]
        )
        data = json.loads(path.read_text())
