...
class TestParseCriteriaResponseObjectFormat:
    ...
    def test_object_array_with_ac_key(self):
        ...
        payload = json.dumps([
            {"ac": "Function defined: bob3.synthesizer.parse_criteria_response"},
        ])
        result = parse_criteria_response(f"```json\n{payload}\n```")
        assert isinstance(result, list)
        assert result[0] == "Function defined: bob3.synthesizer.parse_criteria_response"
    ...
    def test_object_array_with_acceptance_criterion_key(self):
        ...
        result = parse_criteria_response(f"```json\n{payload}\n```")
        assert isinstance(result, list)
        assert result[0] == "integration: bob3.orchestrator"
