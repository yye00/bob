"""Tests for src/bob/sweep_orchestrator.py

Covers: SweepPlan, SweepResult, SweepRun, run_sweep
- Parallel dispatch with BOB_SWEEP_PARALLELISM cap
- Cost-budget enforcement via BOB_SWEEP_BUDGET_USD
- Resumable checkpoints written to .bob/sweep_checkpoint.json
- YAML plan loading
"""
...
from bob.sweep_orchestrator import (
    SweepPlan,
    SweepResult,
    SweepRun,
    load_sweep_plan,
...
class TestRunSweepParallelism:
    ...
    async def test_parallelism_env_limits_concurrency(self, tmp_path: Path):
        """With BOB_SWEEP_PARALLELISM=1, runs execute sequentially."""
        ...
        with patch.dict(os.environ, {"BOB_SWEEP_PARALLELISM": "1"}):
            await run_sweep(
                plan,
                run_one=fake_run_one,
                checkpoint_path=tmp_path / "ckpt.json",
    ...
    async def test_parallelism_default_allows_concurrent_runs(self, tmp_path: Path):
        ...
        with patch.dict(os.environ, {"BOB_SWEEP_PARALLELISM": "4"}, clear=False):
            await run_sweep(
                plan,
                run_one=fake_run_one,
                checkpoint_path=tmp_path / "ckpt.json",
...
class TestRunSweepBudget:
    ...
    async def test_budget_env_var_used_when_no_param(self, tmp_path: Path):
        """BOB_SWEEP_BUDGET_USD is respected when budget_usd is not passed."""
        plan = make_plan(runs=[{"variant": "V0", "spec": "s", "seed": i} for i in range(5)])
        executed: list[str] = []
        ...
        with patch.dict(os.environ, {"BOB_SWEEP_BUDGET_USD": "0.0"}):
            result = await run_sweep(
                plan,
                run_one=fake_run_one,
                checkpoint_path=tmp_path / "ckpt.json",
...
class TestRunSweepCheckpoints:
    ...
    @pytest.mark.asyncio
    async def test_checkpoint_default_path_is_bob_dir(self, tmp_path: Path):
        """When checkpoint_path is None, defaults to .bob/sweep_checkpoint.json."""
        plan = make_plan(runs=[{"variant": "V0", "spec": "s", "seed": 1}])
        ...
        try:
            os.chdir(tmp_path)
            (tmp_path / ".bob").mkdir(exist_ok=True)
            await run_sweep(plan, run_one=fake_run_one)
        finally:
            os.chdir(original_dir)

        assert (tmp_path / ".bob" / "sweep_checkpoint.json").exists()
...
class TestResolveSweepParallelism:
    def test_default_parallelism(self):
        from bob.sweep_orchestrator import _resolve_sweep_parallelism

        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("BOB_SWEEP_PARALLELISM", None)
            assert _resolve_sweep_parallelism() >= 1

    def test_env_override(self):
        from bob.sweep_orchestrator import _resolve_sweep_parallelism

        with patch.dict(os.environ, {"BOB_SWEEP_PARALLELISM": "6"}):
            assert _resolve_sweep_parallelism() == 6

    def test_invalid_env_falls_back_to_default(self):
        from bob.sweep_orchestrator import _resolve_sweep_parallelism

        with patch.dict(os.environ, {"BOB_SWEEP_PARALLELISM": "bad"}):
            assert _resolve_sweep_parallelism() >= 1

    def test_zero_env_falls_back_to_default(self):
        from bob.sweep_orchestrator import _resolve_sweep_parallelism

        with patch.dict(os.environ, {"BOB_SWEEP_PARALLELISM": "0"}):
            assert _resolve_sweep_parallelism() >= 1
...
class TestResolveSweepBudget:
    def test_default_budget_is_positive(self):
        from bob.sweep_orchestrator import _resolve_sweep_budget

        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("BOB_SWEEP_BUDGET_USD", None)
            assert _resolve_sweep_budget() > 0

    def test_env_override(self):
        from bob.sweep_orchestrator import _resolve_sweep_budget

        with patch.dict(os.environ, {"BOB_SWEEP_BUDGET_USD": "25.0"}):
            assert _resolve_sweep_budget() == pytest.approx(25.0)

    def test_invalid_env_falls_back_to_default(self):
        from bob.sweep_orchestrator import _resolve_sweep_budget

        with patch.dict(os.environ, {"BOB_SWEEP_BUDGET_USD": "not_a_float"}):
            assert _resolve_sweep_budget() > 0
