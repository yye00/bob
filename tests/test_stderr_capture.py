...
- Integration: bob.orchestrator.claude_executor exports all required symbols
"""
...
class TestPersistFailureArtifacts:
    """_persist_failure_artifacts writes stderr to .bob/agent_logs/ and returns path."""
    ...
    def test_also_writes_response_when_non_empty(self, tmp_path, monkeypatch):
        ...
        log_dir = tmp_path / ".bob" / "agent_logs"
        response_files = list(log_dir.glob("*.response.txt"))
        assert len(response_files) >= 1

    def test_returns_none_on_io_error(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        with patch("bob.orchestrator.claude_executor.Path.mkdir", side_effect=OSError("disk full")):
            result = _persist_failure_artifacts(
                agent_run_id="run1",
                captured_stderr="content",
                response_text="",
...
class TestModuleIntegration:
    """Integration check: bob.orchestrator.claude_executor exports required symbols."""
    ...
    def test_module_has_captured_stderr_log_string(self):
        import bob.orchestrator.claude_executor as mod
