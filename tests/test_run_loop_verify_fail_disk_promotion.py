...
AC: pytest: tests/test_run_loop_verify_fail_disk_promotion.py
    ...
    Function defined: bob.run_loop.disk_reconciler_verify_fail_check

Verifies that disk_reconciler_verify_fail_check:
  - is importable from bob.run_loop
...
def test_function_importable() -> None:
    """disk_reconciler_verify_fail_check is importable from bob.run_loop."""
...
def test_returns_false_when_no_structural_acs() -> None:
    ...
    acs = json.dumps(["pytest: tests/test_foo.py", "integration: bob.run_loop"])
    result = disk_reconciler_verify_fail_check(
        project_id="proj-1",
        feature_id="feat-1",
        feature_name="Test Feature",
...
def test_behavior_ac_counts_as_structural_prefix() -> None:
    ...
    acs = json.dumps(["Function defined: bob.run_loop.some_function"])
    with patch(
        "bob.orchestrator.disk_reconciler.check_executing_feature_acs",
        return_value=True,
    ):
