"""Tests for F069: 'bob run' with continuous orchestration loop.
...
class TestBudgetCheck:
    ...
    def test_budget_exceeded_when_project_cost_over_limit(self, tmp_db, project):
        ...
        with patch("bob.db.get_database_path", return_value=tmp_db):
            from bob.db import update_project
...
class TestCascadeUpdateDependents:
    ...
    def test_cascade_updates_dependent_status(self, tmp_db, project):
        ...
        with patch("bob.db.get_database_path", return_value=tmp_db):
            from bob.db import add_feature_dependency
...
class TestCLIRunCommand:
    """Test the CLI 'bob run' command integration."""

    def test_run_all_invokes_loop(self, tmp_db, project, ready_features):
        """Step 1: 'bob run --all' starts the orchestration loop."""
        runner = CliRunner()
        with patch("bob.db.get_database_path", return_value=tmp_db):
            with patch("bob.cli.start_mcp_server"):
                with patch(
                    "bob.cli._run_orchestration_loop",
    ...
    def test_run_with_max_cost(self, tmp_db, project):
        """Step 2: 'bob run --all --max-cost 50' passes budget to loop."""
        runner = CliRunner()
        with patch("bob.db.get_database_path", return_value=tmp_db):
            with patch("bob.cli.start_mcp_server"):
                with patch(
...
class TestRunFeatureScoping:
    """'bob run --feature <id>' must run ONLY that feature."""
    ...
    def test_run_feature_only_runs_target(self, tmp_db, project, ready_features):
        ...
        'bob run --feature <A.id>' via CliRunner. Assert that A's status
        ...
        with patch("bob.db.get_database_path", return_value=tmp_db):
            with patch("bob.cli.start_mcp_server"):
                with patch(
                    "bob.orchestrator.run_loop.spawn_sub_agent", spawn_mock
                ):
        ...
        with patch("bob.db.get_database_path", return_value=tmp_db):
            with patch("bob.cli.start_mcp_server"):
                with patch(
                    ...
                    # so 'bob run && deploy.sh' does not deploy on a
        ...
        with patch("bob.db.get_database_path", return_value=tmp_db):
            ...
            with patch("bob.cli.start_mcp_server"):
                with patch(
                    ...
                    # CI scripts (``bob run && deploy.sh``) do not
...
# bob run invocations from racing on the same project.
# ============================================================


class TestRunLockConcurrency:
    """Step: only one ``bob run`` at a time per project.
    ...
    ``<workspace>/.bob.lock``; the second invocation must fail fast
    ...
    def test_acquire_run_lock_returns_handle(self, tmp_path):
        ...
        try:
            ...
            assert (tmp_path / ".bob.lock").exists()
        finally:
    ...
    def test_lock_released_after_run_finishes(self, tmp_path):
        ...
        Simulates: first ``bob run`` finishes (lock released), then a
        second ``bob run`` can proceed.
        """
        from bob.orchestrator.run_loop import (
            acquire_run_lock,
            release_run_lock,
    ...
    def test_lock_released_via_subprocess_after_exit(self, tmp_path):
        ...
        script = textwrap.dedent(f"""
            ...
            from bob.orchestrator.run_loop import acquire_run_lock
    ...
    async def test_orchestration_loop_run_acquires_and_releases(
        ...
        assert (workspace / ".bob.lock").exists()
    ...
    async def test_concurrent_run_rejected_when_lock_held(
        ...
        # Pre-acquire the lock to simulate "another bob run already
...
class TestRunLockSymlinkHardening:
    """``acquire_run_lock`` must refuse a tampered ``.bob.lock``.

    A sub-agent with workspace write access can replace ``.bob.lock``
    with a symlink to ``/dev/null`` (or any non-regular file) before the
    next ``bob run``. ``flock`` on a non-regular file's fd succeeds
    ...
    def test_lock_path_as_symlink_to_dev_null_is_refused(self, tmp_path):
        """``.bob.lock`` -> /dev/null must raise AlreadyRunningError.
        ...
        # this isn't an ordinary "another bob run" collision.
    ...
    def test_lock_path_as_fifo_is_refused(self, tmp_path):
        """A fifo at .bob.lock must be refused by the S_ISREG check.
...
class TestCostTamperDetection:
    """Sub-agents have FS access to ``bob.db``; if they ``UPDATE projects
    ...
    def test_db_total_dropped_to_zero_is_clamped(self, tmp_db, project, caplog):
        ...
        """
        from bob.db import connect, update_project_cost
...
class TestSubAgentWallClockTimeout:
    ...
    with ``asyncio.wait_for`` using ``BOB_FEATURE_TIMEOUT_SECONDS``
    (default 3600s).
    """
    ...
    async def test_timeout_marks_feature_interrupted(
        ...
        This puts it on the F116 auto-resume path on the next ``bob run``
    ...
    async def test_timeout_does_not_cascade_dependents(
        ...
        """
        from bob.db import add_feature_dependency, create_feature
...
class TestSubAgentTimeoutCleanup:
    ...
    async def test_timeout_emits_security_warning_with_pgrep_guidance(
        ...
        with patch("bob.db.get_database_path", return_value=tmp_db):
            ...
            with patch(
                ...
                with caplog.at_level(logging.WARNING, logger="bob.orchestrator.run_loop"):
                    await loop.execute_feature(ready_features[0])
    ...
    async def test_spawn_sub_agent_closes_sdk_stream_on_cancellation(
        ...
        with patch("bob.db.get_database_path", return_value=tmp_db), patch.object(
...
# ============================================================
# R5-008: bob run exit codes reflect LoopTermination reason
# ============================================================


class TestRunExitCodes:
    """R5-008: ``bob run`` must exit non-zero on non-success terminations.

    Without these codes, ``bob run --all && deploy.sh`` would deploy after
    ...
    def test_run_all_exit_code_matches_termination(
        ...
        """``bob run --all`` exit code must match the termination reason."""
        runner = CliRunner()
        with patch("bob.db.get_database_path", return_value=tmp_db):
            with patch("bob.cli.start_mcp_server"):
                with patch(
    ...
    def test_run_feature_exit_code_matches_termination(
        ...
        """``bob run --feature`` exit code must match the termination reason."""
...
class TestPerFeatureSummaryLog:
    ...
    async def test_feature_summary_log_after_successful_completion(
        ...
        with patch("bob.db.get_database_path", return_value=tmp_db):
            ...
            caplog.set_level(_logging.INFO, logger="bob.orchestrator.run_loop")
            with patch(
                "bob.orchestrator.run_loop.spawn_sub_agent",
                new_callable=AsyncMock,
                side_effect=mock_spawn,
...
class TestInterruptionCheckpointRecordsProjectCost:
    ...
    async def test_checkpoint_records_actual_project_cost(
        ...
        from bob.db import list_checkpoints, update_project
    ...
    async def test_below_threshold_with_research_marks_needs_human(
        ...
        with patch("bob.db.get_database_path", return_value=tmp_db):
            ...
            with patch(
                ...
                "bob.orchestrator.run_loop.acquire_run_lock",
                return_value=None,
            ), patch(
                "bob.orchestrator.run_loop.release_run_lock",
                return_value=None,
            ):
                termination = await loop.run()
...
# ============================================================
# R10-006: Stale .bob.lock recovery
# ============================================================


class TestR10006StaleLockRecovery:
    """R10-006: a SIGKILL/OOM-killed previous run can leave a stale
    .bob.lock that the kernel didn't release because a still-running
    ...
    def test_pid_written_to_lock_file_on_acquire(self, tmp_path):
        ...
        try:
            pid = _read_lock_pid(tmp_path / ".bob.lock")
    ...
    def test_stale_lock_with_dead_pid_gives_actionable_error(self, tmp_path):
        ...
        try:
            ...
            (tmp_path / ".bob.lock").write_text(f"{dead_pid}\n")
    ...
    async def test_failed_feature_returns_all_blocked(
        ...
        with patch("bob.db.get_database_path", return_value=tmp_db):
            ...
            with patch(
                ...
                "bob.orchestrator.run_loop.acquire_run_lock", return_value=None
            ), patch(
                "bob.orchestrator.run_loop.release_run_lock", return_value=None
            ), patch(
...
# ============================================================
# R10-008: bob run --feature exits non-zero when feature failed
# ============================================================
...
class TestR10008CliExitCodeOnFailure:
    ...
    def test_cli_exits_nonzero_when_feature_failed(
        ...
        """End-to-end: `bob run --feature <id>` with a failing
...
# ============================================================
# R10-014: ``bob verify-feature`` CLI command
# ============================================================
...
class TestVerifyFeatureCommand:
    ...
    def test_verify_feature_passes_marks_completed(self, tmp_db, project, tmp_path):
        ...
        with patch("bob.db.get_database_path", return_value=tmp_db):
            ...
            runner = CliRunner()
            with patch(
                "bob.cli.run_verification_checklist",
                return_value={
                    "passed": True,
