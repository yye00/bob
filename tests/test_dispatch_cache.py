"""Tests for prompt-cache, slim-context, and per-worker-settings functions in bob3.dispatch.
...
ACs tested:
  ...
  - File exists: src/bob3/worker.md.template
...
class TestSpawnWorkerWithCachePromptCacheEnv:
    def test_anthropic_prompt_caching_env_set_to_1(self, tmp_path):
        ...
        with mock.patch("bob3.dispatch.subprocess.run", side_effect=fake_run):
            spawn_worker_with_cache(
                feature, "prompt", tmp_path, bob3_dir=tmp_path / ".bob3"
            )
...
class TestBuildWorkerMd:
    ...
    def test_worker_md_written_on_spawn(self, tmp_path):
        ...
        with mock.patch("bob3.dispatch.subprocess.run", side_effect=_fake_run_success):
            spawn_worker_with_cache(
                feature, "prompt", tmp_path, bob3_dir=tmp_path / ".bob3"
            )

        written = (tmp_path / ".bob3" / "features" / "WORKER.md")
...
class TestWorkerMdTemplate:
    def test_worker_md_template_file_exists(self):
        template_path = Path(__file__).parent.parent / "src" / "bob3" / "worker.md.template"
...
class TestWriteFeatureSettings:
    ...
    def test_settings_extra_allow_merged(self, tmp_path):
        ...
        path = write_feature_settings(
            feature, bob3_dir=tmp_path, extra_allow=["Bash(make*)"]
        )
        data = json.loads(path.read_text())
        assert "Bash(make*)" in data["permissions"]["allow"]

    def test_settings_written_under_bob3_features_dir(self, tmp_path):
...
class TestSpawnWorkerWithCacheIntegration:
    def test_returns_dict_with_returncode(self, tmp_path):
        ...
        with mock.patch("bob3.dispatch.subprocess.run", side_effect=_fake_run_success):
            result = spawn_worker_with_cache(
                feature, "do the thing", tmp_path, bob3_dir=tmp_path / ".bob3"
            )
    ...
    def test_cache_event_emitted_on_exit(self, tmp_path):
        ...
        with mock.patch("bob3.dispatch.subprocess.run", side_effect=_fake_run_success):
            with mock.patch("bob3.dispatch.emit_worker_cache_event", side_effect=fake_emit):
                spawn_worker_with_cache(
                    feature, "prompt", tmp_path, bob3_dir=tmp_path / ".bob3"
                )
