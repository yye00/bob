"""Tests for bob3.watchdog_escalator.escalate_spec_gate_stalls.
...
from bob3.watchdog_escalator import escalate_spec_gate_stalls
...
class TestModuleImport:
    ...
    def test_function_is_exported(self):
        import bob3.watchdog_escalator as mod
...
class TestEnvOverride:
    """BOB3_STALL_ESCALATION_COUNT overrides the default threshold."""
    ...
    def test_invalid_env_falls_back_to_default(self, tmp_path):
        marker = tmp_path / "STALL_ATTENTION.txt"
        with patch.dict(os.environ, {"BOB3_STALL_ESCALATION_COUNT": "bad"}):
            result = escalate_spec_gate_stalls(observation_count=0, marker_path=marker)
        assert result["threshold"] == 5
