    ...
    def test_loop_run_executes_feature_when_budget_remains(self, tmp_db, project):
        ...
        with patch("bob.db.get_database_path", return_value=tmp_db):
            ...
            with (
                ...
                patch("bob.orchestrator.run_loop.acquire_run_lock", return_value=MagicMock()),
                patch("bob.orchestrator.run_loop.release_run_lock"),
                patch("bob.orchestrator.run_loop.stop_mcp_server"),
                patch("bob.orchestrator.run_loop.sweep_orphans"),
            ):
