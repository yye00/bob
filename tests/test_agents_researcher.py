"""Tests for src/bob/agents/researcher.py (BF-2 hide-the-ticket pattern).
...
_SRC_FILE = Path(__file__).parent.parent / "src" / "bob" / "agents" / "researcher.py"
...
class TestFileExists:
    ...
    def test_module_importable(self):
        from bob.agents import researcher  # noqa: F401


class TestGetRole:
    def test_returns_researcher_role(self):
        from bob.agents.researcher import get_role
        role = get_role()
...
class TestDispatch:
    def test_returns_dict(self, tmp_path):
        from bob.agents.researcher import dispatch
        result = dispatch(
            feature_id="feat-001",
            path_glob="src/bob/**",
    ...
    def test_prompt_contains_path_glob(self, tmp_path):
        ...
        assert "src/bob/orchestrator/**" in result["prompt"]
...
class TestOrchestratorIntegration:
    """Verify that bob.orchestrator can reference researcher dispatch."""

    def test_coordinator_importable(self):
        from bob import coordinator  # noqa: F401

    def test_researcher_importable_from_agents(self):
        from bob.agents import researcher
