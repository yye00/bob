"""Tests for bob3.orchestrator.run_loop.periodic_resume_scan (92c96882).

Verifies the AC set for feature 92c96882:
- Function defined: bob3.orchestrator.run_loop.periodic_resume_scan
...
# ---------------------------------------------------------------------------
# AC: Function defined: bob3.orchestrator.run_loop.periodic_resume_scan
# ---------------------------------------------------------------------------
...
class TestPeriodicResumeScanDefinedInRunLoop:
    def test_function_exists_in_run_loop_module(self):
        ...
        assert hasattr(run_loop, "periodic_resume_scan"), (
            "bob3.orchestrator.run_loop must expose 'periodic_resume_scan' as a "
            "public module-level function"
        )

    def test_function_is_callable(self):
        from bob3.orchestrator.run_loop import periodic_resume_scan
    ...
    def test_importable_from_run_loop_directly(self):
        from bob3.orchestrator.run_loop import periodic_resume_scan as fn  # noqa: F401
...
class TestRunLoopIntegration:
    ...
    def test_public_and_private_are_consistent(self):
        ...
        from bob3.orchestrator.run_loop import periodic_resume_scan
        from bob3.orchestrator.run_loop import _periodic_resume_scan
        with patch("bob3.orchestrator.periodic_resume_scan.db") as mock_db:
            mock_db.list_features.return_value = []
    ...
    def test_run_loop_module_imports_orchestrator_periodic_resume_scan(self):
        run_loop = importlib.import_module("bob3.orchestrator.run_loop")
        prs_mod = importlib.import_module("bob3.orchestrator.periodic_resume_scan")
        assert hasattr(prs_mod, "periodic_resume_scan"), (
            "bob3.orchestrator.periodic_resume_scan must define periodic_resume_scan"
        )
