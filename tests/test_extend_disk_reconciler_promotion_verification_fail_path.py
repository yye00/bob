...
from bob3.extend_disk_reconciler_promotion_verification_fail_path import (
...
def _make_feature(
    ...
    acceptance_criteria: str = '["File exists: src/bob3/foo.py"]',
) -> MagicMock:
    f = MagicMock()
    f.id = feature_id
    f.name = name
...
@patch("bob3.extend_disk_reconciler_promotion_verification_fail_path.check_executing_feature_acs")
...
def test_structural_ac_file_exists_counts_as_structural():
    ...
    with patch(
        "bob3.extend_disk_reconciler_promotion_verification_fail_path.check_executing_feature_acs",
        return_value=False,
    ) as mock_check:
        extend_disk_reconciler_promotion_verification_fail_path(
            project_id="proj-001",
