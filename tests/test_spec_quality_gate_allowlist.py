...
def test_env_var_overrides_defaults(monkeypatch):
    """BOB_ALLOWLIST_PATTERNS env var replaces the default allowlist."""
    monkeypatch.setenv("BOB_ALLOWLIST_PATTERNS", "F-R7-999,F-R7-777")
    patterns = load_allowlist_patterns()
...
def test_env_var_deduplicates(monkeypatch):
    """Duplicate entries in BOB_ALLOWLIST_PATTERNS are deduplicated."""
    monkeypatch.setenv("BOB_ALLOWLIST_PATTERNS", "F-R7-478,F-R7-478,F-R7-479")
    patterns = load_allowlist_patterns()
...
def test_custom_pattern_via_env_exempts_feature(monkeypatch):
    ...
    monkeypatch.setenv("BOB_ALLOWLIST_PATTERNS", "CUSTOM-INFRA-FEATURE")
