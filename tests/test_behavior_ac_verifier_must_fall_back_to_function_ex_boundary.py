...
Asserts bob.enhanced_verification._search_for_function is importable and behaves deterministically on empty/zero/
...
def test_boundary_importable_and_defined():
    mod = importlib.import_module('bob.enhanced_verification')
    assert hasattr(mod, '_search_for_function'), '_search_for_function' + " must be defined in " + 'bob.enhanced_verification'
    obj = getattr(mod, '_search_for_function')
    assert callable(obj), '_search_for_function' + " must be callable"
