"""Tests for ``bob.disk_quota`` (F-R6-303).
...
def test_check_pre_spawn_allows_when_disk_ample(tmp_path: Path) -> None:
    ...
    with patch("bob.disk_quota.shutil.disk_usage", return_value=fake):
...
def test_check_pre_spawn_walks_to_existing_ancestor(tmp_path: Path) -> None:
    ...
    with patch(
        "bob.disk_quota.shutil.disk_usage", return_value=fake,
    ) as du:
...
def _isolated_db(tmp_path, monkeypatch):
    """Provide an isolated bob DB so the regression test does not
...
async def test_spawn_sub_agent_aborts_when_disk_low(
    ...
    monkeypatch.setenv("BOB_SESSIONS_ROOT", str(sessions_root))
    ...
    with patch("bob.disk_quota.shutil.disk_usage", return_value=fake), \
         patch(
            "bob.orchestrator.claude_executor.stream_query",
            _should_not_run,
        ):
