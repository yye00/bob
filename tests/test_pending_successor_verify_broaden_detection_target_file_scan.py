"""Tests for bob.pending_successor_verify_broaden_detection_target_file_scan.

Acceptance criteria:
- File exists: src/bob/pending_successor_verify_broaden_detection_target_file_scan.py
...
- Function defined: bob.pending_successor_verify_broaden_detection_target_file_scan.pending_successor_verify_broaden_detection_target_file_scan
...
class TestFileExists:
    def test_module_file_exists(self):
        import bob.pending_successor_verify_broaden_detection_target_file_scan as m
    ...
    def test_module_importable(self):
        import bob.pending_successor_verify_broaden_detection_target_file_scan  # noqa: F401
...
def test_pending_successor_verify_broaden_detection_target_file_scan():
    ...
    acs_no_verifier = [
        ...
        "Function defined: bob.my_feature.my_func",
...
class TestBroadenDetectionAcBodyScan:
    ...
    def test_detects_verification_py_suffix_path(self):
        ...
        acs = ["File exists: src/bob/some_other_verification.py"]
        assert pending_successor_verify_broaden_detection_target_file_scan("feat", acs) is True
    ...
    def test_target_file_scan_detects_path_in_function_defined_ac(self):
        ...
        acs = ["Function defined: bob.enhanced_verification.check_function_defined"]
        assert pending_successor_verify_broaden_detection_target_file_scan("feat", acs) is True
...
class TestBroadenDetectionTargetFileScan:
    ...
    def test_scans_referenced_file_when_reachable(self, tmp_path):
        ...
        target = tmp_path / "src" / "bob" / "some_handler.py"
        target.parent.mkdir(parents=True)
        target.write_text("from bob.enhanced_verification import check_function_defined\n")

        acs = [f"File exists: src/bob/some_handler.py"]
    ...
    def test_does_not_defer_when_referenced_file_is_unrelated(self, tmp_path):
        ...
        target = tmp_path / "src" / "bob" / "my_module.py"
        target.parent.mkdir(parents=True)
        target.write_text("def my_func(): return 42\n")

        acs = [f"File exists: src/bob/my_module.py"]
        result = pending_successor_verify_broaden_detection_target_file_scan(
    ...
    def test_safe_when_referenced_file_does_not_exist(self, tmp_path):
        ...
        acs = ["File exists: src/bob/nonexistent_module.py"]
        result = pending_successor_verify_broaden_detection_target_file_scan(
    ...
    def test_safe_when_workspace_is_none(self):
        ...
        acs = ["File exists: src/bob/some_handler.py"]
        result = pending_successor_verify_broaden_detection_target_file_scan(
            feature_name="some feature",
            acceptance_criteria=acs,
            workspace=None,
    ...
    def test_target_file_containing_verifier_extension_import(self, tmp_path):
        ...
        target = tmp_path / "src" / "bob" / "my_handler.py"
        target.parent.mkdir(parents=True)
        target.write_text(
    ...
    def test_target_file_scan_uses_path_tokens_in_content(self, tmp_path):
        ...
        target = tmp_path / "src" / "bob" / "my_glue.py"
        target.parent.mkdir(parents=True)
        target.write_text(
            "# This module delegates to artifact_verifier.py\n"
            "from bob.artifact_verifier import check\n"
        )
        acs = ["File exists: src/bob/my_glue.py"]
        result = pending_successor_verify_broaden_detection_target_file_scan(
