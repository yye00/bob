"""Tests for src/bob/external_baselines.py — external baseline runners.
...
from bob.external_baselines import (
    AIDER_VARIANT,
    CLAUDE_CODE_VARIANT,
    BaselineResult,
    _make_run_id,
...
class TestRunAiderBaselineNotInstalled:
    def test_not_installed_completion_status(self, tmp_path):
        ...
        with (
            patch("bob.external_baselines.shutil.which", return_value=None),
...
class TestRunAiderBaselineInstalled:
    def test_success_completion_status(self, tmp_path):
        run_jsonl = tmp_path / ".bob" / "run.jsonl"
        with (
            patch("bob.external_baselines.shutil.which", return_value="/usr/bin/aider"),
            patch("bob.external_baselines.subprocess.run", return_value=_make_completed_process(0)),
            patch("bob.telemetry.get_run_jsonl_path", return_value=run_jsonl),
        ):
    ...
    def test_failure_completion_status(self, tmp_path):
        ...
        with (
            patch("bob.external_baselines.shutil.which", return_value="/usr/bin/aider"),
            patch("bob.external_baselines.subprocess.run", return_value=_make_completed_process(1)),
            patch("bob.telemetry.get_run_jsonl_path", return_value=run_jsonl),
        ):
            result = run_aider_baseline("add feature", spec_id="s1", seed=1)
    ...
    def test_returncode_captured(self, tmp_path):
        ...
        with (
            patch("bob.external_baselines.shutil.which", return_value="/usr/bin/aider"),
            patch("bob.external_baselines.subprocess.run", return_value=_make_completed_process(42)),
            patch("bob.telemetry.get_run_jsonl_path", return_value=run_jsonl),
        ):
            result = run_aider_baseline("prompt", spec_id="s", seed=0)
    ...
    def test_stdout_captured(self, tmp_path):
        ...
        with (
            patch("bob.external_baselines.shutil.which", return_value="/usr/bin/aider"),
            patch("bob.external_baselines.subprocess.run", return_value=_make_completed_process(0, stdout="hello aider")),
            patch("bob.telemetry.get_run_jsonl_path", return_value=run_jsonl),
        ):
            result = run_aider_baseline("prompt", spec_id="s", seed=0)
    ...
    def test_stderr_captured(self, tmp_path):
        ...
        with (
            patch("bob.external_baselines.shutil.which", return_value="/usr/bin/aider"),
            patch("bob.external_baselines.subprocess.run", return_value=_make_completed_process(0, stderr="warn!")),
            patch("bob.telemetry.get_run_jsonl_path", return_value=run_jsonl),
        ):
            result = run_aider_baseline("prompt", spec_id="s", seed=0)
...
class TestRunClaudeCodeBaselineInstalled:
    def test_success_completion_status(self, tmp_path):
        run_jsonl = tmp_path / ".bob" / "run.jsonl"
        with (
            patch("bob.external_baselines.shutil.which", return_value="/usr/local/bin/claude"),
            patch("bob.external_baselines.subprocess.run", return_value=_make_completed_process(0)),
            patch("bob.telemetry.get_run_jsonl_path", return_value=run_jsonl),
        ):
    ...
    def test_stdout_captured(self, tmp_path):
        ...
        with (
            patch("bob.external_baselines.shutil.which", return_value="/usr/local/bin/claude"),
            patch("bob.external_baselines.subprocess.run", return_value=_make_completed_process(0, stdout="claude output")),
            patch("bob.telemetry.get_run_jsonl_path", return_value=run_jsonl),
        ):
