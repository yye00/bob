"""Tests for bob.per_attempt_cost_cap (feature 29bde5f2).

Verifies that enforce_per_attempt_cost_cap is importable from
bob.per_attempt_cost_cap and behaves correctly.
"""
...
from bob.per_attempt_cost_cap import enforce_per_attempt_cost_cap
...
class TestEnforcePerAttemptCostCapImport:
    """enforce_per_attempt_cost_cap is importable from bob.per_attempt_cost_cap."""
...
class TestEnforcePerAttemptCostCapBelowCap:
    ...
    def test_cost_below_cap_returns_false(self, monkeypatch):
        ...
        with patch("bob.cost_cap.terminate_subagent_on_cost_cap") as mock_term:
...
class TestEnforcePerAttemptCostCapAboveCap:
    ...
    def test_custom_cap_env_respected(self, monkeypatch):
        """Custom cap from BOB_PER_ATTEMPT_COST_CAP is enforced."""
...
class TestDefaultCap:
    ...
    def test_env_overrides_default(self, monkeypatch):
        monkeypatch.setenv("BOB_PER_ATTEMPT_COST_CAP", "20")
        cap = get_per_attempt_cap()
        assert cap == 20.0
    ...
    def test_env_clamped_above_maximum(self, monkeypatch):
        monkeypatch.setenv("BOB_PER_ATTEMPT_COST_CAP", "999")
        cap = get_per_attempt_cap()
        assert cap == 100.0


class TestOrchestratorIntegration:
    """Verify the module is importable and wired into bob.orchestrator."""

    def test_import_from_bob_per_attempt_cost_cap(self):
        """Primary AC: module importable at bob.per_attempt_cost_cap."""
        from bob.per_attempt_cost_cap import enforce_per_attempt_cost_cap as fn
        assert callable(fn)

    def test_orchestrator_has_per_attempt_cost_cap(self):
        """bob.orchestrator.per_attempt_cost_cap module is importable."""
        from bob.orchestrator.per_attempt_cost_cap import (
            get_per_attempt_cap,
            should_terminate_subagent,
