#!/usr/bin/env bash
# self_heal.sh — weekend watchdog self-healing routine for bob orchestrator.
...
ZOMBIE_PIDS="$(pgrep -P 1 -fa "bob.memory_mcp" 2>/dev/null | awk '{print $1}' || true)"
...
if [[ $PYOUT -eq 3 ]]; then
    log "HARD-BLOCK: bob.db is corrupt or unreadable"
    exit 1
fi
