...
#
# Monitors bob3 run --all in the current generation, detects round completion,
...
# ---------------------------------------------------------------------------
# PATH: ensure node + claude CLI are visible to subprocesses. bob3 cli.py
# does shutil.which("claude") at every entry point; without nvm sourced
# the check fails and `bob3 init` errors out, halting the chain at gen
...
Action required: Review the chain log and bob3.db in
...
find_bob3_pid() {
  ...
  # (matches both legacy `bob3 run` and gen-named `bob6 run`/`bob7 run`)
  local pids
...
round_is_complete() {
  local gen_dir="$1"
  local db="$gen_dir/bob3.db"

  # bob3 process must be gone
  local pid
  pid="$(find_bob3_pid "$gen_dir")"
  if [[ -n "$pid" ]]; then
...
# ---------------------------------------------------------------------------
# Detect: bob3 died but work remains (crash/ALL_BLOCKED). Caller decides whether
...
# ---------------------------------------------------------------------------
# Resolve which bob3 binary BUILDS bob<N>. Convention:
#   bob4 is built by /home/yelkhamr/dark-factory/bob/.venv/bin/bob3 (parent gen 3)
#   bob<N> for N>=5 is built by /home/yelkhamr/dark-factory/bob<N-1>/.venv/bin/bob3
# ---------------------------------------------------------------------------
parent_bob3_for_gen() {
  local gen="$1"
  if [[ "$gen" -eq 4 ]]; then
    echo "$HOME/dark-factory/bob/.venv/bin/bob3"
  else
    local prev=$(( gen - 1 ))
    # Prefer gen-named binary (bobN) so `ps`/`pgrep` confirms the right
    # generation is doing the building. Fall back to legacy `bob3` if the
    # parent venv predates the per-gen entry_point.
    local gen_named="$HOME/dark-factory/bob${prev}/.venv/bin/bob${prev}"
    local legacy="$HOME/dark-factory/bob${prev}/.venv/bin/bob3"
    if [[ -x "$gen_named" ]]; then
      echo "$gen_named"
    else
      echo "$legacy"
...
# ---------------------------------------------------------------------------
# Relaunch bob3 run --all in the CURRENT gen (post-crash recovery).
# Caller is responsible for calling self_heal.sh first.
# ---------------------------------------------------------------------------
relaunch_bob3_current_gen() {
  local gen_dir="$1"
  local gen="$2"
  local round="$3"

  local bob3_bin
  bob3_bin="$(parent_bob3_for_gen "$gen")"
  if [[ ! -x "$bob3_bin" ]]; then
    log_error "Parent bob3 binary not found at $bob3_bin"
    return 1
  fi

  local log_file="$gen_dir/round${round}_run_relaunch.log"
  log_event "Relaunching bob3 in $gen_dir (binary: $bob3_bin)"

  # setsid -f forks into a new session so bob3 reparents to PID 1, not
  # back to the watchdog (the prior "( nohup ... & )" pattern left bob3
  # parented to the watchdog, which then blocked in wait() and stopped
  # logging status / running halt-gate checks).
  (
    cd "$gen_dir" && \
    BOB3_SNAPSHOT_TIMEOUT=1800 BOB3_TEST_RUN_TIMEOUT=1800 \
    setsid -f "$bob3_bin" run --all --max-cost 1000 < /dev/null >> "$log_file" 2>&1
  )

  log_event "bob3 relaunched in $gen_dir, log: $log_file"
  return 0
}
...
# ---------------------------------------------------------------------------
# Run a round: init + plan + nohup bob3 run --all
# ---------------------------------------------------------------------------
launch_round() {
  local gen_dir="$1"   # bob(N+1) directory
  local bob3_bin="$2"  # path to bob3 binary from *current* gen's venv
  ...
  if [[ ! -f "$gen_dir/bob3.db" ]]; then
    (cd "$gen_dir" && "$bob3_bin" init .) >> "$log_file" 2>&1 || {
      log_error "bob3 init failed in $gen_dir"
      return 1
    }
  else
    log_info "bob3.db already exists in $gen_dir — skipping init"
  fi

  # plan (--create persists features into the DB; without it, plan is a no-op preview)
  (cd "$gen_dir" && "$bob3_bin" plan --create "$spec") >> "$log_file" 2>&1 || {
    log_error "bob3 plan --create $spec failed in $gen_dir"
    return 1
  }

  # setsid -f puts bob3 in its own session so it reparents to PID 1
  # (not the watchdog). nohup-in-subshell on its own left bob3 parented
  # to the watchdog, which then blocked in wait().
  (cd "$gen_dir" && setsid -f "$bob3_bin" run --all --max-cost 1000 < /dev/null >> "$log_file" 2>&1)

  log_event "bob3 run --all launched in $gen_dir, log: $log_file"
  return 0
}

# ---------------------------------------------------------------------------
# Convergence detector: compare two bob3 databases by spec_slot set.
#
...
#
# Uses get_completed_spec_slots() from bob3.migrations.add_spec_slot so the
# comparison is stable across `bob3 init` runs that mint fresh UUIDs.
# ---------------------------------------------------------------------------
check_convergence() {
  local db_a="$1"
  local db_b="$2"
...
# Locate the bob3 package (works both in-venv and with editable install)
import importlib.util

def _find_bob3_src():
    """Return the src/ directory that contains the bob3 package."""
...
_stall_escalation_count() {
  local raw="${BOB3_STALL_ESCALATION_COUNT:-5}"
  python3 -c "
import sys
try:
    n = int('$raw')
...
main() {
  ...
  # current_gen is the gen whose bob3 process we are watching right now
...
PYEOF
    ...
    # ----- Check: bob3 crashed but work remains → self-heal + relaunch -----
    if bob3_died_with_work "$gen_dir"; then
      local relaunch_count
      relaunch_count="$(state_get bob3_relaunch_count_gen_${current_gen})"
      [[ -z "$relaunch_count" || "$relaunch_count" == "None" ]] && relaunch_count=0

      if [[ "$relaunch_count" -ge 5 ]]; then
        halt "bob3 in gen=$current_gen relaunched 5 times — chain broken" \
          "Repeated bob3 crashes with features still ready. Manual review needed."
      fi

      log_event "bob3 died with work remaining in gen=$current_gen (relaunch attempt $((relaunch_count + 1))/5)"
      ...
      if relaunch_bob3_current_gen "$gen_dir" "$current_gen" "$(state_get current_round)"; then
        state_set "bob3_relaunch_count_gen_${current_gen}" "$((relaunch_count + 1))"
        state_set "last_event" "relaunched_bob3_gen_${current_gen}_attempt_$((relaunch_count + 1))"
        sleep 30  # let bob3 acquire lock and start
      else
    ...
    if round_is_complete "$gen_dir"; then
      ...
      # ----- Launch next round -----
      local bob3_bin="$gen_dir/.venv/bin/bob3"
      ...
      if ! launch_round "$next_gen_dir" "$bob3_bin" "$next_spec" "$next_gen" "$next_round"; then
      ...
      # Give bob3 a few seconds to write its lock file
      sleep 10
      continue
    fi

    # ----- Not complete yet: detect spec_gate_stall and escalate if needed -----
    # A stall tick is: bob3 has exited cleanly (no PID, no work remaining per
    # bob3_died_with_work) AND the round is not yet complete (has executing or
    # needs_human features blocking advance). This is the ALL_BLOCKED condition.
    local bob3_pid
    bob3_pid="$(find_bob3_pid "$gen_dir")"
    if [[ -z "$bob3_pid" ]]; then
      # bob3 not running and not "died with work" (checked above) and round
