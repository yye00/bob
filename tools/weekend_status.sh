...
# Safe to run while bob3 is active (read-only db access with timeout).
#
# Usage:
...
#   GEN_DIR    — the bob_N currently being built. Has bob3.db, .bob3.lock,
...
#   3. Highest-numbered bob<N>/ sibling with a bob3.db
...
" 2>/dev/null)"
    ...
    if [[ -n "$pid" ]]; then
        ...
        if [[ -n "$cwd" && -f "$cwd/bob3.db" ]]; then
            echo "$cwd"; return
        fi
    fi

    # 3. highest-numbered bob<N>/ with a bob3.db
    local best="" best_n=-1
    for d in "$PARENT_DIR"/bob[0-9]*/; do
        [[ -f "$d/bob3.db" ]] || continue
        local n="${d##*/bob}"; n="${n%/}"
        [[ "$n" =~ ^[0-9]+$ ]] || continue
...
echo "$HR"
echo "  BOB3 WEEKEND STATUS  —  $(date '+%Y-%m-%d %H:%M:%S %Z')"
echo "$HR"
...
MCP_PIDS=""
for pid in $(pgrep -f 'bob3\.memory_mcp' 2>/dev/null); do
    cwd="$(readlink -f /proc/$pid/cwd 2>/dev/null || true)"
    if [[ "$cwd" == "$GEN_DIR" ]]; then
        MCP_PIDS+="$pid "
    fi
...
MCP_PIDS="${MCP_PIDS% }"
MCP_OTHER_COUNT="$(pgrep -cf 'bob3\.memory_mcp' 2>/dev/null || echo 0)"
if [[ -n "$MCP_PIDS" ]]; then
...
if [[ -f "$LOCK_FILE" ]]; then
    ...
    if [[ "$LOCK_PID" =~ ^[0-9]+$ ]] && kill -0 "$LOCK_PID" 2>/dev/null; then
        echo "  .bob3.lock      : held by pid $LOCK_PID (alive)"
    else
        echo "  .bob3.lock      : STALE (pid $LOCK_PID gone) — needs self_heal"
    fi
else
    echo "  .bob3.lock      : free"
fi
...
echo ""
echo "FEATURE STATUS ($GEN_NAME/bob3.db)"
