...
#   bob_N's .venv/bin/bob is what builds bob_(N+1).
...
t = p.read_text()
key = f'bob{n} = "bob.cli:main"'
if key in t:
    sys.exit(0)
# Append the new key after the bob line, without disturbing siblings.
# The old regex stripped EVERY bobN= line including bob itself, which
# erased the anchor and wiped [project.scripts] (observed bob9 seed).
if 'bob = "bob.cli:main"' in t:
    t = t.replace('bob = "bob.cli:main"',
                  f'bob = "bob.cli:main"\n{key}')
else:
    # If bob anchor missing, recreate the whole [project.scripts] block.
...
echo "==> Installing bob (editable) + dev extras into bob$NEXT_NUM venv"
...
# parent's name and spec_path.  Running bob init with the correct --name and
# --spec rewrites those project-row fields so bob{NEXT_NUM}/bob.db reflects
...
"$NEXT_DIR/.venv/bin/python" - "$CURRENT_DIR/bob.db" "$NEXT_DIR/bob.db" <<'PYEOF'
import sys
from pathlib import Path
...
if not child_db.exists():
    print(f"  Child DB {child_db} not found; skipping inheritance (bob init not yet run?)")
    sys.exit(0)
...
import os
os.environ["BOB_DATABASE_PATH"] = str(child_db)
from bob.migrations.add_parent_gen_inheritance_fields import upgrade
upgrade(db_path=child_db)

from bob.orchestrator.parent_gen_inheritance import inherit_from_parent_db
...
Next steps to drive bob$((NEXT_NUM + 1)) from bob$NEXT_NUM:
  ...
  bob init                      # initialize the workspace
  bob plan examples/<spec>.yaml # add features for this round
  bob run --all --max-cost 1000
