"""specs.devin_style_editable_plan.plan_gate — plan approval gate for the Devin-style feature.

Implementer sub-agents call ``check_plan_approved`` to enforce the plan gate.
If plan.yaml is absent or ``approved: false``, the function raises
``ImplementerBlockedError`` so work cannot begin without human sign-off.

Public API::

    from specs.devin_style_editable_plan.plan_gate import check_plan_approved
